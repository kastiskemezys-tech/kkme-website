...
### Session 46 — 2026-05-07 — Phase 18.1.1 — Mobile map redesign + per-component reflow + double-band-aid removal (Claude Code)

**Headline:** Phase 18.1's two band-aids (`html { overflow-x: hidden }` page-level clip + retained `body { overflow-x: hidden }` defensive trim) both removed cleanly. Direction A simplified mobile map ships to replace the dotted-grid map below 900px (which was rendering as a noise blur at 328px wide). Three reflow fixes (DSCR triple panel + AssumptionsPanel grid covering RTE+cycles rows + S3 BreakdownBar spec P1-4) + three Pause-A-discovered scope additions (rv-analytics chart-canvas track `min-width: 0`, Sensitivity tornado SVG viewBox+responsive width, MonthlyHeatmap `1fr` → `minmax(0, 1fr)`) closed every site that the band-aid was masking. Pause B 5-viewport scrollWidth=clientWidth check passes at 360 / 414 / 768 / 1024 / 1440 with **both** clipping rules removed. All gates green: tsc 0, vitest 925/925, lint 127 (40e/87w; +1 warning vs 126 baseline, no new errors), no-raw-spacing exit 0, no-editorial-chips exit 0, build 7 routes. Bundle delta: production CSS chunk 42,421 B → 43,470 B (+1,049 B / +2.5%). 5 files modified, +319 / −39 lines (no new files committed). Frontend-only, no `model_version` bump, no worker deploy. 16 visual-audit PNGs in `docs/visual-audit/phase-18-1-1/`.

**Branch:** `phase-18-1-1-mobile-map-redesign` off `origin/main` at `2914235` (post-Phase-18.1-merge + phase-18-1-1 setup commit). 3 commits.

**Pause A — re-verification triage (rule #1):**

Playwright probe at 360 viewport with band-aid disabled. Body scrollWidth: **556** (196px overflow).

| # | Spec claim | Status | Evidence |
|---|---|---|---|
| 1 | DSCR triple panel | ✅ VERIFIED — primary overflow source | el=282px in 246px parent (`overflowsParent: true`); right edge x=556. Accounts for entire 196px excess. |
| 2 | Cycles_breakdown chart (own overflow) | ❌ **NOT REPRODUCIBLE in itself** | Chart already has `minWidth: 0; width: 100%` (`RevenueCard.tsx:494`). Sized 79=79 to its parent. Off-screen `elRight=459` is downstream of parent grid blow-up, not chart's own. |
| 3 | RTE assumption rows | ✅ VERIFIED via parent (AssumptionsPanel grid) | The grid at `RevenueCard.tsx:721` resolves `gridTemplateColumns: 'auto 1fr'` to 401px wide content inside a 246px box. `whiteSpace: nowrap` on label cells + missing `overflow-wrap` on italic notes. **One fix here covers RTE row + v7.3 cycles row + v7.2 fallback + 3 standard rows.** |
| 4 | S3Card BreakdownBar (spec P1-4) | ✅ VERIFIED collapse | `S3Card.tsx:80-96`. el=286 / scrollWidth=329. 130 + 85 + flex + 90 + 24 (3 gaps) = 329 fixed at desktop; flex bar squashes to ~0px. |
| 5 | Hero map / hero section | ✅ NOT overflowing | Phase 18.1 already correct here. |
| 6 | rv-analytics chart canvases | ⚠️ **NEW — discovered at Pause A** | DegradationChart + Sensitivity tornado + RevenueBacktest grid items resolve to 360px because chart.js's intrinsic 360px first-paint becomes the grid track's `auto` minimum. `min-width: 0` on `.rv-main > * / .rv-analytics > *` lets canvases shrink to grid track width. |
| 7 | Sensitivity tornado SVG | ⚠️ **NEW — discovered at Pause A** | Component had `<svg width={360}>` literal attribute — non-responsive. Switched to `viewBox` + `width: 100%` + `maxWidth: 360`. |
| 8 | MonthlyHeatmap grid | ⚠️ **NEW — discovered at Pause A** | `gridTemplateColumns: '60px repeat(13, 1fr)'` — `1fr` resolved to max-content (~22px per cell × 13 = 286 + 60 label = 346px) escaping 248 parent. Switched to `minmax(0, 1fr)`. |

**Mobile-simplified SVG design picks (operator-approved):** A1=(a) hand-coded simplified country paths · A2=(b) CSS `@media` visibility toggle · A3 = static arrowheads + threshold color (amber default / brick rose >500 MW) + floating MW labels.
...
| `npm run lint` (eslint) | 126 (40e/86w) | 127 (40e/87w) | ⚠️ +1 warning, 0 new errors |
...
**Pause B — 5-viewport `scrollWidth === clientWidth` check (Playwright headless, both band-aids removed):**

| Viewport | clientWidth | htmlScrollWidth | bodyScrollWidth | Pass |
|---|---|---|---|---|
| 360 mobile | 360 | 360 | 360 | ✅ |
| 414 mobile | 414 | 414 | 414 | ✅ |
| 768 tablet | 768 | 768 | 768 | ✅ |
| 1024 tablet | 1024 | 1024 | 1024 | ✅ |
| 1440 desktop | 1440 | 1440 | 1440 | ✅ |

**Files touched (5 modified):**

| File | Lines | Change |
|---|---|---|
| `app/components/HeroBalticMap.tsx` | +200 / -1 | New `MobileBalticMap` sub-component (`MOBILE_VIEWBOX 400×360` + 3 hand-coded country paths + 3 SVG marker definitions for amber/rose/muted arrowheads + cable-end coords keyed by cable id with country attribution + threshold color logic at MW>500). `.hero-map-desktop` className on existing center-map block (display moved to CSS class). New `.hero-map-mobile` sibling block holding `<MobileBalticMap resolved={resolved} />`. GSAP particle init gated on `window.innerWidth < 900`. |
| `app/components/RevenueCard.tsx` | +35 / -22 | DSCR cell `min-width: 0` + `overflow-wrap: anywhere` on label/sublabel; `.assumptions-grid` className replacing inline `gridTemplateColumns: 'auto 1fr'` + nowrap removal + `overflow-wrap: anywhere` on italic notes; `.rv-dur-dscr` className wrapping the DurationOptimizer + DSCRPanel 2-col grid (was `1fr 1fr` resolving to max-content under content pressure); inline `<style>` block adds `.rv-main > *, .rv-analytics > * { min-width: 0 }` for chart-canvas track shrinkability; MonthlyHeatmap grid `1fr` → `minmax(0, 1fr)`. |
| `app/components/S3Card.tsx` | +5 / -4 | `.breakdown-bar` family classNames added (`__label`, `__range`, `__bar`, `__scope`, `__pcs-note`). |
| `app/components/RevenueSensitivityTornado.tsx` | +4 / -2 | `<svg width={width}>` → `viewBox` + `width: '100%'` + `maxWidth: width`. Chart now scales to parent track. |
| `app/globals.css` | +75 / -10 | New `.assumptions-grid` rules + `<720px` stack `@media`. New `.rv-dur-dscr` rules + `<720px` stack `@media`. Six split `.breakdown-bar*` `@media (max-width: 720px)` blocks (one per selector — Tailwind v4 silent-drop avoidance, with `!important` on `display`/`flex`/`width` to beat inline styles). New `.hero-map-desktop` / `.hero-map-mobile` CSS-class display rules + two `<900px` `@media` toggle blocks. **Removed `html { overflow-x: hidden }`** (Phase 18.1 page-level band-aid) AND **removed `body { overflow-x: hidden }`** (defensive trim that turned out also removable). `body { overscroll-behavior-y: contain }` retained — it's not a clip rule. |

**Visual audit:** 16 PNGs in `docs/visual-audit/phase-18-1-1/`:

| Viewport | Hero | Detail captures (mobile only) |
|---|---|---|
| 360-mobile | dark + light | DSCR panel, Assumptions panel, cycles chart, duration optimizer, sensitivity tornado, S3 BreakdownBar |
| 414-mobile | dark + light | — |
| 768-tablet | dark + light | — |
| 1024-tablet | dark + light | — |
| 1440-desktop | dark + light | — |

**Bundle size delta:** Production CSS chunk 42,421 B → 43,470 B (+1,049 B / +2.5%). All from new mobile SVG paths + reflow CSS classes + map-toggle rules. Build output stable at 7 routes. JS bundle delta near-zero (mobile SVG is JSX rendering pre-existing data).

**Worker deploy:** Not needed. `model_version` not bumped. No engine fields touched.

**Surprises during implementation (worth flagging):**

1. **Tailwind v4 / Turbopack silent-drop pattern reproduced again, two new failure modes.** (1) Multiple selectors stacked inside a single `@media` block — confirmed in initial `.breakdown-bar` mobile reflow (had to split into 6 separate single-selector `@media (max-width: 720px) { ... }` blocks). (2) Inline `style={{ display: 'flex' }}` on a JSX element clobbers CSS class `display` rules so `@media` overrides silently drop — discovered when `.hero-map-desktop` / `.hero-map-mobile` swap didn't fire (both blocks remained `display: flex` at mobile). Resolved by removing inline `display` and declaring it in CSS class. Memory entry `feedback_tailwind_v4_silent_drop.md` should be updated to cover both modes.

2. **DSCR panel overflow source was upstream, not its own internal grid.** Pause A hypothesis was that the 3-cell DSCR grid at `RevenueCard.tsx:1088` blew past parent. The 1-line `min-width: 0` cell fix shrank the panel from 282 → 157px, but body scrollWidth only dropped 556 → 459. Trace revealed the `1fr 1fr` grid wrapping DurationOptimizer + DSCRPanel at line 1725 was the actual ancestor that refused to shrink — DurationOptimizer's 2H/4H pill row pushed past 1fr's track, positioning DSCRPanel at x=274 in a 246-wide parent. Fix needed at the wrapper: new `.rv-dur-dscr` class with `minmax(0, 1fr) minmax(0, 1fr)` desktop / single-column stack at `<720px`. Discipline rule #1 working as intended — the obvious site wasn't the load-bearing site.

3. **Three additional overflow sources NOT in the prompt's 4-item list** surfaced via Playwright `find-overflow` enumeration: chart-canvas intrinsic-width fallback (rv-analytics children), Sensitivity tornado SVG explicit `width=360`, MonthlyHeatmap `1fr` resolving to max-content. None show on a casual visual scan; all are the difference between "page renders" and "page geometrically fits viewport". The band-aid was masking all five overflow sources, not just the four named in the prompt.

4. **Both `html` and `body` `overflow-x: hidden` removable** — the operator's Pause-B-end ask to attempt `body` removal too came back green. The defensive trim turned out unnecessary once per-component fixes restored clean overflow architecture. Documented in commit message.
...
- Restoring the dotted-grid map's eastern off-map labels (Direction A creates a fresh viewBox; off-map labels were intended-invisible at the dotted-grid map's east edge)
- Interactive features on the mobile map (no tap-to-drill, no fullscreen modal)
- Animations on the mobile map (static arrowheads only)

**Tier 1 sequence after Phase 18.1.1:**
...
- ✅ Phase 18.1.1 SHIPPED 2026-05-07 (this session — mobile map redesign + per-component reflow + double-band-aid removal)
- 🟡 Phase 18.1.2 — Mobile map threshold tuning + Tailwind v4 memory refinement (~30 min Cowork) — operator hard-refresh validates 4-amber/2-brick split; if visually noisy, adjust threshold to 600 MW; tighten memory entry text to cover both silent-drop modes
...
**Phase 18.1.2 candidate scope (filed):**

1. **Mobile map threshold validation** — operator hard-refresh on phone confirms the 4-amber / 2-brick split at MW>500 reads cleanly. If visually noisy (too much rose), bump threshold to 600 MW (only Fenno-Skan 2 in brick). One-line edit at `MOBILE_HIGH_FLOW_THRESHOLD_MW` constant.
2. **Tailwind v4 memory refinement** — current `feedback_tailwind_v4_silent_drop.md` covers only the "stacked inside an existing @media block" case. Add coverage for (a) multiple selectors per `@media` block and (b) inline `style={{ display: ... }}` clobbering `@media`-class overrides. Cowork-side update.

**Operator action items post-merge:**
1. Open PR via GitHub web UI from `phase-18-1-1-mobile-map-redesign` → `dev` (or `main`)
2. After merge, hard-refresh kkme.eu post-Pages-auto-deploy + open on **mobile + desktop dark + light** to confirm the simplified map renders cleanly + threshold split feels right + sub-card reflows hold across all 5 viewports.
3. Apply roadmap delta to `docs/phases/_post-12-8-roadmap.md` (operator/Cowork-owned per discipline rule #5):
   - Phase 18.1.1 → Shipped appendix with full description (3 reflows + 3 scope additions + mobile SVG + double-band-aid removal)
   - Phase 18.1.2 added as new candidate (threshold tuning + memory refinement)
   - Currently-active update: Phase 18.1.1 SHIPPED; next CC across (18.1.2 / 18.2 / 18.3 / 12.12 #1+#2 / 7.7g-a-3 / 7.7g-b reduced / 19 / 20)
