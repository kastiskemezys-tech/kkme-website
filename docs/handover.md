...
### Session 49 — 2026-05-07 — Phase 18.2.3 — Tooltip dismiss-on-scroll mobile fix (Claude Code)

**Headline:** Operator hard-refresh feedback after Phase 18.2.2 merge: tooltip sticks visually on mobile after the chart is scrolled out of view. Operator screenshot showed S1 lower section + S2 top in viewport with S2's "MAR 26 / €6.12/MW/h" tooltip floating over a non-chart area — the chart canvas it was annotating had scrolled off. **Cause:** chart.js's `external` callback dismisses tooltips on mouseleave (desktop) but has no equivalent on `touchend` or `scroll`. Without listener wiring at the React-portal layer, the React state stays `visible: true` after the canvas scrolls away, painting the tooltip card at its last viewport-fixed position. **Fix:** single-file change to `useChartTooltipState` adds two passive listeners — `window` scroll (immediate dismiss when visible) and `document` touchend (250ms-debounced dismiss, races scroll-dismiss on tap-then-pan, gives 250ms grace on tap-and-read). Closure gates on `prev.visible` so non-visible hook instances are no-ops; React's value-equality dedupe (Phase 7.7e Session 25 loop-breaker) skips re-render when prev is already HIDDEN. Cross-site coverage is structural — every chart consumer uses `useChartTooltipState()`, so the propagation is automatic across all 19+ tooltip consumers (9 chart.js sites + 10 inline-SVG sites). **Universal across desktop + mobile** — additive on desktop where mouseleave already dismisses; primary fix on mobile where no equivalent existed. Single commit, 1 file, +28 / -0 lines. All 6 gates baseline-exact (tsc 0, vitest 925/925, lint 127 (40e/87w), no-raw-spacing exit 0, no-editorial-chips exit 0, build 7 routes). Bundle delta below measurement noise (4,620 KB total, exact match Phase 18.2.2 closeout reading). Frontend-only, no `model_version` bump, no worker deploy. **Visual-audit gap (recurring 3rd consecutive session)**: chrome-devtools-mcp browser locked by parallel CC sessions; operator approved curl-only verification path again (build clean + 20/20 chunks 200 + `touchend` symbol present in 5 chunks confirming new listener wiring bundled).

**Branch:** `phase-18-2-3-tooltip-dismiss-mobile` off `main` at `26f6e2e` (post-Phase-18.2.2-merge — operator merged PRs #69 + #70 between Sessions 48 + 49). 2 commits.

**Pause A — discovery:**

| Artifact | File:line | Role |
|---|---|---|
| `useChartTooltipState` hook | `ChartTooltip.tsx:284-310` | Per-instance state owner; one per chart consumer |
| `setStateRaw` (deduped via `tooltipStateEqual`) | `ChartTooltip.tsx:300-302` | Load-bearing loop-breaker from Phase 7.7e Session 25 (React error #185 fix). Reused by dismiss path so non-visible instances are no-ops. |
| `hide()` | `ChartTooltip.tsx:306-308` | Existing idempotent dismiss; pattern reused via direct `setStateRaw` in dismiss closure to avoid useCallback-dep churn |
| `<ChartTooltipPortal tt={tt} />` | `ChartTooltip.tsx:317-332` | Reads `state.visible`; portal-mounted by every consumer |
| `buildExternalTooltipHandler` | `chartTooltip.ts:170-204` | chart.js → React state bridge. Dismisses on `tooltip.opacity === 0` (desktop mouseleave path). |

**Why mobile sticks (root cause):** chart.js fires `external` on mouse + touchstart/touchmove. Desktop dismiss path: cursor leaves canvas → opacity=0 → external fires invisible → setState. Mobile dismiss path: doesn't exist — `touchend` doesn't reliably trigger chart.js's mouseleave-equivalent dismiss, and `scroll` is not a chart.js-listened event at all. Tooltip state stays `visible: true` after canvas scrolls out of view.

**Pause A interference pre-check (operator-requested):**

`grep -rn "stopPropagation" app/components app/lib` — 6 hits, all on `onClick` events (intel feed cards, asset detail panel, page interactions); zero on touch events; zero on chart-canvas DOM elements. Document-level `touchend` listener will fire cleanly on bubble.

**Implementation (single file, ~28 lines added inside `useChartTooltipState`):**
...
**Listener-cost arithmetic (operator-verified):**

- N=19 hook instances on the homepage → 19 passive scroll + 19 document-touchend listeners
- Per scroll-tick at 60Hz: 18 instances run `prev => prev.visible ? HIDDEN : prev` returning `prev` reference unchanged → React skips re-render via setState bailout. 1 instance (visible tooltip) returns `HIDDEN` → re-renders to invisible.
- ~1,140 function calls/sec during active scroll, each a single property check + ternary. Well below 60fps budget. Passive flag gives browser scroll preemption priority (no JS gating of scroll commit).
- Touchend at document level: shared event handler propagates from any touch on the page. 250ms debounce keeps it from interfering with rapid taps; tap-then-pan loses race to scroll-dismiss when finger moves.
- Refactor-to-singleton pub/sub stays as a Phase 7.7g-b consideration if chart count ever pushes past ~50; not needed at current scale.
...
| Symbol verification: `touchend` listener bundled | ✅ present in 5 chunks (confirms new listener wiring not tree-shaken) |
| Bundle dir size | 4,620 KB — exact-match Phase 18.2.2 closeout reading; delta below KB-rounding noise floor (~+28 source lines) |
| In-browser scroll / touchend dismiss / theme toggle | ⚠️ **SKIPPED** — chrome MCP lock (3rd consecutive session); operator owns post-merge visual confirmation |

**Visual-audit gap — chrome MCP lock now on 3rd consecutive session (47 + 48 + 49):**

Cross-session friction observation upgraded from "noted" (Session 48) to "structural pattern" (Session 49). Three consecutive CC sessions hit `The browser is already running for /Users/Kastis/.cache/chrome-devtools-mcp/chrome-profile. Use --isolated to run multiple browser instances.` Curl-only path accepted each time under operator approval. Cowork-side investigation candidate (when current Tier 1 sequence has slack):
- Test `--isolated` userDataDir flag for chrome MCP (would allow parallel CC sessions to each spawn isolated browser instances)
- OR: codify browser-ownership coordination (lock-file or session-handoff signal) so CC sessions don't race
- Filed as cross-session friction observation; not a Phase 18.2.3 blocker.

**Files touched (1 modified):**

| File | Change |
|---|---|
| `app/components/primitives/ChartTooltip.tsx` | `useChartTooltipState` hook gains a `useEffect` registering window-scroll + document-touchend dismiss listeners. ~+28 lines source. No new imports (`useEffect` was already imported for `mounted` flag in `<ChartTooltip>`). |

**Bundle size delta:** ~+28 lines source. Bundle dir size unchanged at 4,620 KB (KB-rounding noise floor). No new dependencies. CSS unchanged.

**Worker deploy:** Not needed. `model_version` not bumped. No engine fields touched.

**Universality note (operator-flagged):**

scroll-dismiss applies on desktop too, not just mobile. On desktop, user already had mouseleave dismiss for cursor-leaves-canvas; adding scroll-dismiss is **additive and desirable** — scrolling the page should dismiss any open tooltip regardless of input modality. Both desktop + mobile paths covered cleanly with one set of listeners. No `@media (max-width:...)` or `(pointer: coarse)` gating needed.

**Out of scope (per prompt):**
- M2 mobile tooltip pinning to fixed chart corner (was the "filed as 18.2.3" candidate at end of Session 48; reframed as Phase 18.2.4 if scroll-dismiss proves insufficient post-merge)
- Brighten/thicken crosshair variant (Phase 18.2.5 if needed)
- Tooltip layout / typography / position changes (already done in 18.2.2 α)
- IntersectionObserver-based dismiss (filed as Phase 18.2.5 only if a real edge-case surfaces — chart canvas leaving viewport via non-scroll mechanism)
- Engine / worker / chart redesign

**Visual-confirmation contract (operator post-merge):**
1. Mobile: tap any chart → tooltip appears → scroll the page → tooltip dismisses immediately as scroll begins
2. Mobile: tap any chart → release finger → tooltip stays visible for ~250ms → auto-dismisses (tap-and-read pattern)
3. Mobile: tap chart then immediately swipe to scroll → tooltip dismisses on scroll-start (faster than 250ms touchend timer)
4. Desktop: scrolling the page with a tooltip open → tooltip dismisses (additive vs prior mouseleave-only behavior; should feel like an improvement, not a regression)
5. Desktop: existing mouseleave dismiss path unchanged
6. Light + Dark theme switch — chart colors + tooltip colors still re-resolve via existing `useChartColors()` MutationObserver path (untouched)
7. Mobile (360px) + desktop (1440px) hard-refresh; verify no console warnings (especially "Maximum update depth exceeded" or "ResizeObserver loop limit exceeded" — would indicate dedupe-loop regression vs Phase 7.7e Session 25 risk class)

**Tier 1 sequence after Phase 18.2.3:**

- ✅ Phase 18.2 SHIPPED 2026-05-07 (Session 47)
- ✅ Phase 18.2.2 SHIPPED 2026-05-07 (Session 48 — crosshair "+" + α tooltip-tighten + β small-chart height bump)
- ✅ Phase 18.2.3 SHIPPED 2026-05-07 (this session — tooltip dismiss-on-scroll mobile fix)
- 🟡 Phase 18.2.4 — M2 mobile tooltip pinning to fixed corner (conditional — only if dismiss-on-scroll proves insufficient post-merge); ~30-45 min CC, scope-isolated to `ChartTooltip.tsx` portal positioning
- 🟡 Phase 18.2.5 — visibility-lift fallback for crosshair (brighten to `textSecondary` or thicken to 1.2px) OR IntersectionObserver-based tooltip dismiss for non-scroll edge cases; conditional, scope-isolated
...
1. Open PR via GitHub web UI from `phase-18-2-3-tooltip-dismiss-mobile` → `dev` (or `main`); per `feedback_pr_workflow_minimal.md`: no PR body draft, no branch delete after merge
2. After merge, hard-refresh kkme.eu post-Pages-auto-deploy on **mobile + desktop dark + light**: tap chart → tooltip appears; scroll → dismisses; tap-and-release → 250ms grace then dismisses
3. Apply roadmap delta to `docs/phases/_post-12-8-roadmap.md` (operator/Cowork-owned per discipline rule #5):
   - Phase 18.2.3 → Shipped appendix (tooltip dismiss-on-scroll mobile fix)
   - Phase 18.2.4 added as conditional candidate (M2 mobile tooltip pinning if dismiss-on-scroll insufficient)
   - Phase 18.2.5 added as conditional candidate (crosshair visibility-lift OR IntersectionObserver dismiss for non-scroll edge cases)
   - Currently-active update: Phase 18.2.3 SHIPPED; next CC across (18.2.4 conditional / 18.2.5 conditional / 18.1.1.1 / 18.2.1 / 18.3 / 12.12 #1+#2 / 7.7g-a-3 / 7.7g-b reduced / 19 / 20)
4. Cowork-side: when current Tier 1 sequence has slack, investigate chrome MCP `--isolated` userDataDir flag to unblock parallel CC visual-audit capability (recurring 3-session friction observation)
