...
| B-076 | bug | **P1** | `s2_activation` has been frozen 105 days, and it feeds the compression rate applied to every projection year | 2026-08-04 Phase 52 | open — mechanism established, direction measured, magnitude NOT measured | `/health` reads `2537.2h · stale: true`; last written ~2026-04-21. ONE writer: the 09:30 watchdog, which calls `computeS2Activation()` and writes only if it returns non-null. It returns null when every BTD fetch fails, and the branch then logs `BTD unavailable — keeping cached data` and writes nothing — true on any single tick, a permanent stall for 105 days. **BTD is not down:** laptop HTTP 200 in 0.98s (2,880 timeseries), VPS HTTP 200 in 0.15s, CF edge unreachable — the same long-documented condition `s2` works around with a VPS leg, and **`s2_activation` has none** (verified: nothing under /opt/kkme posts to `/s2/activation`). **Reach:** `deriveCompression` -> `effective_compression_rate` 0.15/yr multiplies revenue in EVERY year of the 20-year projection, so it reaches `gross_revenue_y1`, `net_revenue_y1`, `project_irr`, `equity_irr`, `npv_at_wacc`, `lcos_eur_mwh` and the scenario matrix. **Direction, on a controlled comparison** (one consistent aggregation, four months, same dataset and columns): LT aFRR procured-reserve median 2026-03 6.92, 2026-04 **3.46**, 2026-06 5.00, 2026-07 5.00 — the decline STOPPED, April was the bottom. The frozen series ends there and extrapolates its 66.9->13.5 decline forward at 15 %/yr. **So the published projections are UNDERSTATED.** **Magnitude deliberately NOT claimed** — my medians are not on the same basis as the payload's series (~3x scale difference, unreconciled), and comparing them would be the uncontrolled-sample error. Needs `computeS2Activation`'s own parse over fresh data plus a 54-config engine run as a signed delta. Fix shape is B-072's: a VPS relay to the existing authed `/s2/activation`; would become caller #13 in the rotation runbook. Full write-up: `docs/investigations/2026-08-04-s2-activation-frozen.md`. |
