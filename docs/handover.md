...
|----|------|-----|-------|------------|--------|-------|
| B-001 | bug | P1 | Arrow direction inversion on live hero interconnectors | 2026-04-13 audit | closed — 2026-04-15 | Fixed phase2b-1: flipped positiveFlowReceives in lib/baltic-places.ts |
...
| B-004 | bug | P2 | "9 SIGNALS · 4H UPDATES" tagline stale | 2026-04-13 audit | closed — 2026-04-15 | Replaced with "LIVE · ENTSO-E · LITGRID · AST · ELERING" in phase2b-1 |
| B-005 | bug | P2 | Light mode cable particle visibility | 2026-04-13 audit | closed — 2026-04-15 | Added --cable-particle token (amber dark / charcoal light) in phase2b-1 |
...
| B-007 | tech-debt | P2 | CLAUDE.md model version inconsistency | 2026-04-13 audit | closed — CLAUDE.md rewrite, 2026-04-13 | Resolved by the CLAUDE.md rewrite in the session that filed it. "This session" was unresolvable a day later; the date is the fact. |
...
| B-015 | tech-debt | - | GSD evaluated and rejected | 2026-04-13 decision | wontfix | See ADR-006. Re-evaluate 2026-10-13 or if workflow gaps emerge. |
| B-016 | tech-debt | - | UI UX Pro Max evaluated and rejected | 2026-04-13 decision | wontfix | See ADR-006 and ADR-005 (design system). Would conflict with three-font rule and halftone map identity. |
...
| B-036 | enhancement | P2 | No settled German activation prices — blocks E2 | 2026-07-30 Phase 36.E | open | **Tabled 2026-08-03 Phase 46: this row did not exist.** B-036 was filed in a session-log paragraph (handover ~:1069) and never entered the backlog table, so every `grep '^| B-'` sweep since — including the tooling this backlog is meant to feed — has been blind to it. The content is unchanged from the original filing; only its machine-readability is new. |
| B-037 | tech-debt | P3 | Fingrid API key unregistered | 2026-07-30 Phase 36.E | closed — **decision, not a gap** | **Tabled 2026-08-03 Phase 46: this row did not exist** (filed in prose at ~:1069). Re-read at execution time: FI is not in the mature-market comparator set and no published figure depends on it, so an unregistered key is a scoping decision that was taken, not an outstanding task. Recorded as closed-by-decision rather than left reading as open work. |
| B-038 | tech-debt | P3 | ENTSO-E `A15/B95/A01` DOES serve LT procured balancing capacity — corrects 36.C's "serves nobody" | 2026-07-30 Phase 36.E | open — belongs to 36.C's arc | **Tabled 2026-08-03 Phase 46: this row did not exist** (filed in prose at ~:1069). Corrects a reason recorded in 36.C. Not scoped to any current phase; carried here so the correction is findable. |
| B-039 | enhancement | P3 | No installed-storage-MW series, so E4 cannot fit a per-GW compression coefficient | 2026-07-30 Phase 36.E | open — sourcing, not code | **Tabled 2026-08-03 Phase 46: this row did not exist** (filed in prose at ~:1073). See the Phase 46 schedulable-phase paragraph. |
| B-040 | enhancement | P3 | E5 has no evidence base at all | 2026-07-30 Phase 36.E | open — sourcing, not code | **Tabled 2026-08-03 Phase 46: this row did not exist** (filed in prose at ~:1074). See the Phase 46 schedulable-phase paragraph. |
| B-066 | tech-debt | P2 | Backlog IDs filed in prose never reach the table, and one referenced ID does not exist | 2026-08-03 Phase 46 | open | The B-050/B-053 "malformed rows" item is bigger than filed. Five IDs (B-036..B-040) existed only as sentences in session-log paragraphs and were invisible to every `^\| B-` sweep; they are tabled above. Separately, **B-044 — named in the Phase 46 prompt as "TAM / APVA citability" — does not exist anywhere in this repo** (`grep -c B-044 docs/handover.md` → 0). Either it was filed somewhere untracked or the ID is wrong. Root fix: filing must go through the table, and a test should assert that every B-id mentioned anywhere in handover.md has a table row. |
| B-067 | bug | ~~P1~~ | `calcIRR` returns its own bracket bound as a value — 2 for anything above a 200 % return, -0.99 for anything ruinous | 2026-08-04 Phase 49 | closed — fixed 2026-08-04 Phase 49 (`a95dcba`) | The bisection never established that its bracket contained a root, so with no root in it the loop walked to the end it started from and published that bound. `calcIRR([-100, 10000, 10000])` -> 2; `calcIRR([100, 10, 10])` -> 2 for a stream with no IRR at all. **Not theoretical: all 47 of the 54 configurations that `/revenue` labelled `irr_status: 'uneconomic'` on the v6 fallback path were -0.99 escaping and being laundered into `null` by a `< -0.50` sentinel, with NPV at that "root" of -1.2e46.** 'Uneconomic' meant the solver gave up. `solveIRR` enumerates roots in a bounded domain and returns `{value, reason, npv_at_root, bound}`; null now means undefined and nothing else, at both edges. Two further bare bisections found and repaired (`computeRevenue_legacy`, EU-ranking `computeRevenueWorker`, the latter returning exactly 0 % for any market with a negative IRR); `sizeDebt` audited and cleared. 54/54 byte-identical. |
| B-068 | bug | ~~P2~~ | The `/revenue` v6 fallback emits 19 fewer top-level keys than the primary path, with a 200 status | 2026-08-04 Phase 49 | closed — fixed 2026-08-04 Phase 49 (`c6c659e`) | Measured on the frozen KV: 78 keys -> 59. Absent were `moic`, `lcos_eur_mwh`, `debt_sizing`, `warranty_status`, `cycles_breakdown`, `assumptions_panel`, `roundtrip_efficiency_curve`, `per_product_at_cod` and eleven more — every consumer of one got `undefined` from a healthy-looking response, on a path exercised rarely and reviewed never. `conformToPublicShape` declares every key the primary declares, null for what v6 cannot compute, plus a `degraded` block naming engine, reason and unavailable fields. `REVENUE_PAYLOAD_KEYS` is asserted against a live v7 payload across all 54, so adding a v7 field without declaring it fails a test (A9). |
| B-069 | bug | P2 | `computeBaseYear` still multiplies `da_mwh_per_mw_day` by `trading_fraction` — the B-065 unit error, on the base-year path only | 2026-08-04 Phase 49 | open — re-scoped from B-065, whose primary-path fix shipped in `aaac252` | `fetch-s1.js` `computeBaseYear`: `trd_monthly = capture * rte * trd_real * da_mwh_per_mw_day * y1_mix.trading_fraction * days`. `trading_fraction` is EUR/EUR (`workers/lib/units.js` declares it `SHARE_OF_VALUE`); `da_mwh_per_mw_day` is MWh/MW/day. **Reach, established 2026-08-04 rather than assumed:** it feeds `base_year.annual_totals`, which is published verbatim on every request, and previously fed the `signal_inputs.s1_capture` back-derivation (removed, B-070). It does NOT feed `gross_revenue_y1`, IRR, DSCR or NPV — those come from the partitioned primary path. So this is a wrong published diagnostic, not a wrong headline. Fixing it moves `base_year.annual_totals.*` and needs its own signed delta. |
| B-070 | bug | ~~P2~~ | `signal_inputs.s1_capture` back-derived a capture 8.03x too high and published it as an observed signal input | 2026-08-04 Phase 49 | closed — fixed 2026-08-04 Phase 49 (`c6c659e`) | When today's `capture_2h`/`capture_4h` were missing but monthly history was intact, the field reconstructed a capture from `base_year.annual_totals.trading` by dividing out `effective_arb_pct` — inverting a forward pass that had multiplied by `trading_fraction`. It was never the inverse of anything. Measured on the frozen KV with only the day captures removed: **EUR 101.91 observed -> EUR 818.59 published**, a Baltic day-ahead capture of EUR 818/MWh. A reconstruction of an input is not the input; the field is now null when there is no signal, and the substitution is recorded in `degraded`. |
| B-071 | bug | ~~P2~~ | S3's FX leg died with its scrape leg — one `Promise.all`, two lost signals | 2026-08-04 Phase 49 | closed — fixed 2026-08-04 Phase 49 (`e57d30e`) | `fetchFxRates()` and the TradingEconomics scrape shared a `Promise.all`, so the TE abort rejected the whole function and the catch returned a payload with no `fx_rates` at all — while Frankfurter had answered in under a second. Observed live 2026-08-04T08:00:20Z: `/s3` carrying no `fx_rates` while `data_freshness.ecb_euribor` updated normally at 08:01:06Z. This is 39.2's own finding recurring one signal over. Legs now settle independently; whatever survives is published. |
| B-072 | bug | P1 | The TradingEconomics scrape times out from Cloudflare Workers only — every other network gets a fast 200 | 2026-08-04 Phase 49 | open — root cause narrowed, mechanism NOT established | `/health` reports `s3 · degraded · AbortError` continuously since 2026-08-03T08:01:11Z (last successful lithium scrape). `_scrape_status`, `_scrape_ctype` and `_scrape_bytes` are all null, so no response headers ever arrived — a hang, not an HTTP rejection. **Controlled probe, same URL and the same three headers the worker sends:** laptop (residential) HTTP 200 ttfb 0.14s 408,928 B · VPS 89.167.124.42 (Hetzner datacenter) HTTP 200 ttfb 0.10s 408,914 B · Cloudflare Worker hangs 20s. So not the upstream, not the headers, and not datacenter-IP blocking in general. tradingeconomics.com is fronted by CloudFront (`x-amz-cf-pop: WAW51-P6`). Whether that is a CloudFront/WAF rule against Cloudflare egress or something in the path between them is **not established and deliberately not guessed**. Options for the fix phase: route the scrape via the VPS cron (proven to work), or find a second lithium source. The alert is already wired and fired correctly at onset (39.2). |
| B-073 | bug | ~~P2~~ | Five scheduled KV writers had neither a staleness threshold nor an alert | 2026-08-04 Phase 49 | closed — fixed 2026-08-04 Phase 49 (`e57d30e`) | The class guard behind item 4: enumerate every key the `scheduled()` handler writes and cross-reference the threshold table. `genload`, `s_wind`, `s_solar`, `s_load` (hourly cron; they back the hero map's live overlays, i.e. the homepage) and `s2_activation` (09:30 daily watchdog; feeds `deriveCompression`, so a silent stop ages the compression trajectory the whole revenue projection rests on). Each could have stopped for a week with nothing noticing. Thresholds derived from each key's own cadence. New gate `scheduled-writer-coverage`, registered with its injection. |
| B-074 | tech-debt | P3 | 12 monitored keys have a staleness threshold but no alert wired to them by name | 2026-08-04 Phase 49 | open — reported, operator call | `da_tomorrow`, `da_tomorrow:lastgood`, `euribor`, `genload`, `s2_activation`, `s6`, `s7`, `s8`, `s9`, `s_load`, `s_solar`, `s_wind`. They surface in `/health` and nothing pages on them. The coverage gate reports rather than fails, because `/health` IS a surface and whether each needs a push alert is a judgement about noise, not a defect. Run `node scripts/gates/scheduled-writer-coverage.mjs` for the current list. |
| B-075 | bug | P2 | `computeHistorical` pairs LT against SE4 by ARRAY INDEX across 30-day ranges | 2026-08-04 Phase 49 | closed — fixed 2026-08-04 Phase 52, operator-signed | `fetchBznRange` returns a flat `extractPrices` scrape, and `computeHistorical` then computes `spread = lt30[i] - se430[i]` positionally. Under curveType A03 the two zones omit repeated positions independently, so one zone dropping a position shifts its whole remaining series against the other. Every value of `rsi_30d`, `trend_vs_90d` and `pct_hours_above_20` is computed from that pairing. The correct fix is to align by timestamp, which is what `parseA44Periods` already makes possible — same shape as item 1 and deliberately not folded into it, so the two deltas stay attributable (C3). |
| B-076 | bug | ~~P1~~ | `s2_activation` frozen 105 days — and FOUR of this row's own claims were falsified by measuring them | 2026-08-04 Phase 52 | **partly closed 2026-08-06 Phase 53** — the base-year defect it exposed is SHIPPED; the freeze itself remains open pending the VPS leg | **Corrections to this row, recorded as corrections (B9):** **(1) "ONE writer" is wrong — there are THREE.** `POST /s2/activation` validated only `body.countries` and wrote the body verbatim, and the frozen 8-month full-calendar payload cannot have come from `computeS2Activation`'s 6-chunk loop (BTD range fidelity probed both directions). The published trajectory rested on a payload of unestablished provenance. That route now enforces shape, provenance and admission. **(2) "published projections are UNDERSTATED" is FALSIFIED.** Measured one-process, one KV snapshot, activation the only variable: fresh data moves gross −3.53 %, project IRR −66 bp, equity IRR −108 bp, NPV −1.19M, LCOS unchanged. Worse on every headline metric. The controlled comparison's SERIES direction held (the decline did stop); the inference from series to published numbers did not. **(3) The "~3× basis difference" DOES NOT EXIST.** Over identical bytes the two aggregations sit at ratios 1.13 / 1.22 / 0.98 / 0.50 / 1.00. What was compared was monthly medians against `initial_p50` (66.9 = the p50 of 2025-09, n=24 of ~2880) and `recent_avg_p50` (13.5 = a 3-month mean) — different months and different reductions, not different bases. **(4) The stated reach — `deriveCompression` → `effective_compression_rate` — carries ZERO of the effect.** Both the frozen and a fresh trajectory imply ~0.99 against a 0.15 ceiling, so both publish the clamp. Swapping the compression trajectory alone moves nothing, on any metric, and would have moved nothing across all 105 days. **What was actually costing money** was a defect independent of the freeze: `computeBaseYear` read the market-formation months (66.90 / 38.71 / 33.33 €/MW/h) at face value while `deriveCompression`, one function away, discounted them as "post-sync anomaly normalisation". One question answered twice (rule #4). Fixed by ONE `ACTIVATION_MONTH_RULE` consumed by both — coverage computed, regime boundary measured from BTD source bytes at 2026-03. Worth **10× the freeze on IRR** (−60 bp vs −6 bp). Shipped 2026-08-06, worker `49b01793`, disclosed in the methodology drawer. **Still open:** the VPS leg (§2) and the 54-config delta on the corrected basis (§3). Full write-up: `docs/investigations/2026-08-06-b076-frozen-vs-fresh.md`; superseded predecessor: `2026-08-04-s2-activation-frozen.md`. |
...
| B-044 | tech-debt | P3 | **PHANTOM ID — referenced but never filed** | 2026-08-03 Phase 46 | closed — does not exist | The Phase 46 prompt lists "B-044 TAM / APVA citability" as a backlog item to triage. It does not exist anywhere in this repository: `grep -c 'B-044' docs/handover.md` returned 0 before this row was written. Either it was filed in a channel outside the repo, or the ID is wrong and the intended item is one of the tabled ones. Tabled AS A PHANTOM rather than excluded from the B-066 check, because special-casing an id inside the gate is how the gate stops being one. Operator: confirm which item was meant, then repoint or delete this row. |
| B-045 | bug | P1 | Calculator full tier could not authenticate from a browser — CORS preflight omitted `authorization` | 2026-08-02 Phase 37.H1 | **CLOSED, DEPLOYED 2026-08-02** | **Tabled 2026-08-03 Phase 46: this row did not exist** (filed in prose at handover ~:668, ~:686). Premise was verified live before any edit: the pre-fix `OPTIONS /calculate` returned `Content-Type, X-Update-Secret` with no `authorization`. Fixed and deployed; the post-deploy read was the C8 edge-propagation case that produced the banked "poll to agreement" rule. |
| B-046 | bug | P2 | Weekly lifecycle-digest arming gate compared the constant against `wrangler.toml` only | 2026-08-02 Phase 37.B.1 | **CLOSED, DEPLOYED 2026-08-02** | **Tabled 2026-08-03 Phase 46: this row did not exist** (filed in prose at handover ~:650, ~:674). A cron declared in both places and HANDLED in neither would have passed the drift test and silently never sent. The widened gate drives the real `scheduled()` handler instead of comparing two declarations. |
| B-047 | tech-debt | ~~P2~~ **fixed 38.1** | Worker has no 404 — every unmatched path returned the S1 payload with 200, AND wrote the `s1` KV key | 2026-08-01 Session 100 | closed — fixed 2026-08-02 Phase 38.1 | Filed as a *control-strength* problem (a path-shaped probe could not distinguish route-absent from route-present, B11). **Phase 38 found the second half, which is worse:** the catch-all did not merely respond as `/s1`, it ran `computeS1()` and `put('s1')` — and `s1` is monitored by /health with a 24h threshold. So the monitor was measuring inbound 404 traffic, and it read green through the entire eight-tick S1 outage (2026-08-01T00:01Z → 2026-08-02T08:00Z). It also called `updateHistory`, appending a row to `s1_history` per stray request — see B-056. Escalated to P1 by the operator and fixed in the same batch. **Fix:** `/s1` is now an explicit read-only route (same status, shape and headers, so `diagnose.sh` and external probers are unaffected) and unmatched GETs return a 404 envelope. **The audit the row asked for, run before the change (A7 ALL-N):** 92 resolved route forms vs 32 distinct worker paths called from `app/`, `lib/` and `scripts/` — **0 relied on the catch-all**. The first pass of that enumeration reported 12 false misses by only matching `url.pathname === '...'`; the template-literal (`/${sig}`, `/${sig}/history`, `/${genSig}`) and regex (`/^\/(s[1-4])\/history$/`) route families had to be resolved before the count meant anything. |
| B-049 | enhancement | P2 | 37.B.2 — entity names on public fleet rows: the registry detectors' whole eligible population is private-tier | 2026-08-01 Session 101 | open | Measured, not inferred: `registry_terminated` / `registry_absent` are HEALTHY with **36 eligible rows and 0 of them public-tier**. Every public-fleet LV row's `source` applicant is a provenance NOTE, not a company (`AST · TSO operational`, `esn · Aretis Group`), so `isLegalEntity()` correctly rejects all 6 and the most reliable signal in the set **cannot retire anything published**. Retirement only affects supply via the public DB, so today the strongest detector is structurally inert there. Fix: carry a resolved legal-entity name on public fleet rows (the loaders have `applicant` / `company_name` at source — see the confirmed field lists), then re-run the runner. **Also the home for the 41 untracked live LV storage-named entities** the discovery sweep found (2026 wave: BESS 1/2/3/5/6/33, AVER Energy Storage 1-4, THE BATTERY STREET 2-8, Krustpils/Dobele/Pupuķi/Ķauķi BESS, BESS Parks Latvia, BESS Park Jelgava, Latgale/Baltic Energy Storage). They stay **CANDIDATES for review in /fleet, never projects** — name matches against the register are not evidence a project exists (rule #3). Full list in the gitignored `docs/_private/fleet-intel/lifecycle-proposals-latest.json`. |
| B-050 | enhancement | P2 | 37.B.3 — detector source coverage: 2 of 7 detectors cannot fire, and one population has no clock | 2026-08-01 Session 101 | open | Three measured gaps from the first real run. (1) **`press_negative` has NO SOURCE.** The lv_press tripwire is alive (150 items/day, 0 candidates) but scans for grid-scale storage COMMISSIONING; `grep -n "bankrot\|maksātnespēj\|insolven\|cancel\|atcel\|likvidāc" lv_press_scraper.py` → 0 hits in the keyword lists. Reporting its silence as "nothing was cancelled" would be B11 exactly, so the detector declines to speak. (2) **`vert_permit_expired` is BLIND at 0/243.** VERT serves 23 records of which **1** carries a `permit_expiry`; 224 LT rows match no permit holder by name. The field the signal depends on is effectively absent, so this is a source problem, not a matching problem. (3) **117 of 329 rows carry no evidence timestamp**, so `evidence_stale` cannot age them at all — a staleness clock that silently skips a third of the population. **Source access constraint, operator-confirmed:** VERT and lv_press exist only as files on the VPS; **SSH-to-prod must NOT be added as a secret surface**. Route those detectors through the worker or a published endpoint. |
| B-051 | bug | **P1 — STILL BLOCKED, mitigated 36.E0.3** | Scheduled evidence refresh cannot open its PR — `GitHub Actions is not permitted to create or approve pull requests` | 2026-08-02 Session 102 | open | Both scheduled runs (Sat 2026-08-01 `30686633957`, Sun 08-02 `30734890718`) completed the refresh and FAILED at the last step with `pull request create failed: GraphQL: GitHub Actions is not permitted to create or approve pull requests (createPullRequest)`. **The data is not lost** — branch `evidence-refresh/2026-08` is pushed at `174a166` (46 files, +2062/−841) and can be PR'd by hand. Fix is a repo/org setting (Settings → Actions → General → *Allow GitHub Actions to create and approve pull requests*), not code. Until then the PR-based review shape — the whole reason E0.1 refuses to push to main — silently does not exist, and the job is red every firing. **36.E0.3 UPDATE — verified by observation, not by the tick.** A `workflow_dispatch` on 2026-08-02 08:09 UTC (run `30739141676`) still got `GraphQL: GitHub Actions is not permitted to create or approve pull requests`. **The setting has NOT taken effect** — re-check it is the *repository* setting and that no organisation-level policy overrides it. **Mitigation shipped:** the PR step now falls back to opening a labelled `refresh-blocked` issue carrying the compare URL (demonstrated: issue #125), and the new gate-summary step fails the job — so a blocked PR is loud, not silent. Still P1: the fallback is a notification, not a review shape. |
| B-052 | bug | ~~P2~~ | Evidence-refresh cron fires ~11×/month, not once — `0 3 1-7 * 0` ORs day-of-month with day-of-week | 2026-08-02 Session 102 | closed — fixed 36.E0.3 | The workflow comment asserts "this fires on any of days 1-7 that is a Sunday — exactly one day per month". POSIX cron ORs the two fields when BOTH are restricted, so it fires **every day 1-7 AND every Sunday** ≈ 11 firings/month. Measured, not reasoned: it ran Sat 2026-08-01 (day 1) and Sun 2026-08-02 (day 2 + Sunday). Each firing re-fetches all eight sources and force-pushes the branch. Fix: `0 3 * * 0` plus a day-of-month guard in the job, or `0 3 1-7 * *` with a Sunday guard — one field restricted, never both. **Also observed:** GitHub delayed the 03:00 UTC schedule to 05:51/05:52 UTC on both days, so any "lands before X" margin must assume ~3 h of scheduler drift. **Fixed 36.E0.3:** `0 3 * * 0` + a `window` job guarding `date -u +%-d <= 7`. The comment is no longer trusted — `refreshWorkflow.test.ts` implements POSIX cron day-matching, enumerates 2026-2030, and asserts one firing per month AND that it is the first Sunday; it also re-derives the ~11×/month figure the comment cites. `matureMarkets.test.ts` had asserted the broken cron as the requirement and was green throughout the bug's life — inverted to assert its absence (B-036 precedent). |
| B-053 | bug | ~~P2~~ | Evidence-refresh `tests` gate structurally cannot fail — missing `pipefail` | 2026-08-02 Session 102 | closed — fixed 36.E0.3 | `if npx vitest run tools/consultancy/__tests__/matureMarkets.test.ts 2>&1 \| tail -40; then tests=pass`. Only the *Refresh* step sets `set -o pipefail`; this step does not, so the pipeline's exit status is `tail`'s, which is always 0. **Recorded `tests=pass` on 2026-08-02 while 3 tests actually failed** — the PR body and the report would have told a reviewer the loader gate was green. B2/B7 shape: a gate that measures the wrong thing. The 3 failures are themselves real but environmental — `loadDataset('activation')` exceeding vitest's 5000 ms default on a slower runner (41 s total on Actions vs 14 s and 71/71 green locally); they need an explicit `testTimeout`, separately from the pipefail fix. **Fixed 36.E0.3.** `set -o pipefail` added. Enumerated rather than spot-fixed (A7): 2 workflow files, exactly **1** `run:` block in either that branches on a shell pipe — this one (`fetch-btd.yml`'s pipe characters are all JS `||` and `join(' | ')` inside a heredoc, not shell pipes). `refreshWorkflow.test.ts` re-runs that enumeration every commit. **Demonstrated failable on real runs:** clean run `30739141676` → `refresh=pass tests=pass scope=pass`; injection run `30739276674` → `refresh=fail tests=fail scope=fail`, job red. **One layer out:** a run could finish green with `tests=fail` recorded, because gate results reached the PR body and nothing else — a `Gate summary` step (`if: always()`) now fails the job when any gate is not an explicit `pass`, treating "not reached" as failure. **The 3 failures diagnosed:** all `loadDataset` timeouts, not data defects. Local 14.4 s / de 2143 ms / au 1752 ms / activation 4138 ms vs 41.3 s on Actions = ~2.9× slower runner, putting de at ~6.2 s, au ~5.1 s, activation ~12.0 s against the 5000 ms default — predicting exactly the three that failed and `da` (748 ms → ~2.2 s) surviving. `testTimeout` 60 s; no assertion changed. The local suite was NOT the wrong one. |
| B-054 | enhancement | ~~P2~~ | No runner-specific staleness threshold — a dead weekly runner takes 30-45 days to surface | 2026-08-02 Session 102 | closed — fixed 36.E0.3 | 37.B.1a closed the "nothing schedules the runner" gap and `/health.fleet_lifecycle.detectors` now carries `last_run_at` + `age_hours`, aging independently of whether a digest ever fires. But `max_age_hours` is 720-1080 (30-45 d), set for the DETECTORS' source cadence, not for a **weekly** runner: if the cron dies the Sunday after next, nothing crosses a threshold for a month. Wanted: a runner heartbeat with its own ~10-day threshold, distinct from per-detector source staleness. Stated as an open gap in the 37.B.1a commit rather than claimed as solved. **Fixed 36.E0.3.** Premise correction first: `max_age_hours` is **not** a global constant — lifecycle-rules.json already carries five distinct per-signal values (336/336/720/720/720/1080/1440), each shaped to its source. The real gap is that a detector's `last_run_at` only advances when the RUNNER runs, so a dead runner freezes every stamp together and the first alarm is the smallest SOURCE ceiling (14 d; 60 d for the slowest). `runnerHeartbeat()` emits a record claiming only "the runner reached this line at NOW" — 240 h = 10 d, strictly below every source ceiling (asserted, not intended). It rides the existing detector map, so the worker's generic ageing gives the alarm with **no worker change**. **B8 table (days from death to detection):** queue_disappearance 14 · press_negative 14 · registry_terminated/absent/new_entity 30 · vert_permit_expired 45 · evidence_stale 60 · **runner_heartbeat 10**. Dead runner: 14 d before → 10 d after. |
| B-055 | tech-debt | P2 | E0 summary table's DE arbitrage series silently stops at 2025-09 — PT60M filter vs Germany's 15-min MTU | 2026-08-02 Phase 36.E1 | open | `build-summary-table.mjs:389` filters DE day-ahead to `resolution === 'PT60M'`. Germany moved to a 15-minute MTU on **2025-10-01** and ENTSO-E A44 has served PT15M only since, so the arbitrage denominator **ends 2025-09** while every DE capacity price series it is divided into runs to 2026-08. Eleven months missing, and missing SILENTLY: `floorSeg.months.map(m => arbByMonth.get(m.month)).filter(Boolean)` **drops** months with no arbitrage value rather than reporting them, so every published DE `floor_to_arbitrage_ratio` and `arbitrage_opportunity` is computed over fewer months than the floor beside it and nothing says so. **Measured consequence:** on the truncated series every post-crisis k trend loses significance purely from the lost window — DE aFRR down goes t = −1.43 truncated against **t = −2.36** on the full series, i.e. the truncation alone flips a supported decay rate into an unsupported one. **Controlled before 36.E1 switched its own calibration to both resolutions:** `arbitrageByMonth` collapses to hourly duration-weighted means first, so overlapping PT15M and PT60M rows for the same hour merge instead of double-counting; over the 84 months both cover the two filters agree to **2.57 % mean, 8.24 % max**. Fix: drop the resolution filter and rebuild the table. NOT done in 36.E1 — changing a published table mid-batch is out of scope, and the table's own `arbitrage_opportunity` figures move when it lands. **Related:** the table is published at RTE 0.85 while the engine's canonical `RTE_BOL.h4` is 0.83; 36.E1 pinned the published value in `arbitrage.mjs` as `ARB_RTE_E0_PUBLISHED` and single-sourced its own from the engine. Fold both into one rebuild. |
| B-056 | bug | ~~P1~~ **fixed 38.2** | `s1_history` is 90 rows over 9 distinct dates (8 at filing) — the published 90-day distribution is weighted by stray HTTP traffic | 2026-08-02 Phase 38.1 | closed — fixed 2026-08-03 Phase 38.2 (option 3, signed) | Discovered while fixing B-047. `updateHistory` **appends** unconditionally (no dedupe by date, unlike `appendSignalHistory` which does dedupe), and the pre-38.1 catch-all called it on every unmatched GET. Live measurement 2026-08-02T09:40Z: **90 rows, 8 distinct dates**, `2026-08-02` × 27 and `2026-07-29` × 25. `rollingStats` then reports `n: 90, days_of_data: 90` — a provenance claim that is false by a factor of 11 (playbook B10: the gate validates the values, not the evidence about the values). **This is not cosmetic:** `swing_stats_90d` drives PeakForecastCard's dot colour and its interpretation sentence, both public. **Two causes, one fixed:** 38.1 stops the stray-traffic contribution (the catch-all no longer calls `updateHistory`; `GET /s1` uses the new read-only `readHistory`). The 4-hourly cron still appends 6 rows/day, so even a clean array would cover ~15 days and call itself 90 — that predates B-047 and is untouched. **Measured delta if deduped to one row per date** (latest wins), computed on the live array: `spread_eur` p25 3.26 → **2.55**, p50 10.72 → **10.72**, p75 11.03 → **17.84**, p90 21.64 → **21.64**, n 90 → **8**; `lt_swing` p25 142.83 → **147.04**, p50 153.10 → **157.47**, p75 204.22 → **208.15**, p90 208.15 → **209.04**, n 90 → **8**. Deliberately NOT applied in 38.1: it moves a published distribution and the operator reserved number movements to 38.2 with a delta table. The dedupe, the honest `days_of_data`, and whether to retroactively collapse the stored array are one decision. **ENUMERATION (operator-requested, measured live 2026-08-02T15:58Z) — every published figure sourced from an S1 history array, distinct dates beside claimed days.** The two arrays are SEPARATE and only one is contaminated. `s1_capture_history` (written by `computeCapture`, which **does** dedupe: `history.filter(e => e.date !== today)` then push): **30 rows / 30 distinct dates / claims 30** — so the S1 card's `DAYS` tile (`S1Card.tsx:161`), its P25–P90 band, its 30-day sparkline and the `last N days` caption (`:411`) are all **HONEST**, and the €142 P50 sits on a genuinely 30-day distribution. `s1_history` (written by `updateHistory`, which does **not** dedupe): **90 rows / 8 distinct dates**, feeding `spread_stats_90d` and `swing_stats_90d`, both claiming `n=90, days_of_data=90` — an 11× overstatement — and through `swing_stats_90d` driving PeakForecastCard's dot colour and interpretation sentence. **ESCALATION, worse than this row was filed:** SpreadCaptureCard's *"14D swing history"* sparkline (`SpreadCaptureCard.tsx:51`, `h.slice(-14)`) resolves to **14 rows spanning 1 distinct date** — all `2026-08-02`. A published visualisation labelled a 14-day trend is one day repeated fourteen times. That is a rule-#2 shape on a live chart, not only a percentile drift. **RE-MEASURED 2026-08-03T05:35Z (Phase 38.2 CP, A3 — the figures above are superseded):** 90 rows over **9** distinct dates; the `slice(-14)` now spans **two** dates (2026-08-02, 2026-08-03), so the defect is one date less severe than filed and still a two-day chart labelled 14D. Deduped keep-last: `lt_swing` p25 142.83 → **156.13**, p50 153.10 → **179.99**, p75 204.22 → **204.22**, p90 208.15 → **209.04**; `spread_eur` p25 3.26 → **2.55**, p50 10.72 → **9.32**, p75 11.03 → **10.72**, p90 21.64 → **21.64**; n 90 → **9**. **Backfill answered:** `raw:s1:<date>` carries a 7-day TTL (`:8176`) and is written once per successful tick, so it can never hold more than `s1_history` already covers — forensic evidence, not an archive. `s1_capture_history` CAN rebuild `lt_swing` to 30 real dates; `spread_eur` has no surviving daily archive anywhere. **See B-060** — keep-last on the write stamp disagrees with the settled capture series on 7 of 9 days, so the CP recommends rebuilding swing from the capture history rather than deduping in place. |
| B-057 | bug | P2 | The 4-hourly cron's five parallel computes share one invocation's connection budget, and the hourly cron fires in the same second | 2026-08-02 Phase 38.1 | open | Measured in the 2026-08-02T12:00:33Z tail: **both crons stamped the same second**, the 4h invocation ran `cpu=83ms wall=50017ms` (I/O-bound, not compute-bound), and four of its five branches degraded at once — `[S1] Timed out after 30000ms`, `[S4] Timed out after 25000ms`, `[S3] AbortError`, `[FX] frankfurter.app AbortError` — while the hourly invocation took `[Gen/LT] energy-charts lt: HTTP 429`. computeS1's own completion logs are present in the same invocation as its timeout, so the work finishes and the wrapper discards it. **HYPOTHESIS, not measured:** the Workers per-invocation simultaneous-connection cap queues ~20 fetches through far fewer slots, with `computeHistorical`'s four ~425 KB XML windows as the long poles. 38.1 raised computeS1's budget to 60s, which is what the evidence directly supports; it did not touch the structure. **Candidate fixes:** stagger the `Promise.allSettled` block; cache the two `-120..-90` reference windows, which change daily rather than 4-hourly; move the hourly cron off the 4h-aligned minute (`0 * * * *` → `5 * * * *`). **S4 belongs to this row** — it timed out in the same invocation and was deliberately not widened into 38.1's scope. Externally the burst is not the problem: computeS1's exact 9-request ENTSO-E burst replicated from a laptop returned 48/48 HTTP 200 across four rounds at 9- and 15-request concurrency, so this is contention inside the invocation, not upstream rate-limiting. **Operator note 2026-08-02, cheapest candidate first:** stagger the schedules so the hourly and 4-hourly crons stop firing in the same second — the collision is what produced the energy-charts 429 and the connection contention. `0 * * * *` → `5 * * * *` is a one-line wrangler.toml change with no code impact, and it should be tried and MEASURED before anything is restructured. Not built in 38.1. |
| B-058 | bug | ~~P1~~ **fixed 38.2** | `baltic_total.installed_mw` has two writers — a stored assertion that wins over the sum of its parts | 2026-08-03 Phase 38.2 | closed — fixed 2026-08-03 same phase | `fetch-s1.js:10409` resolves `installed_mw: getVal('installed_storage_baltic_mw', ltMw + lvMw + eeMw)`. The `s4_buildability` KV holds `installed_storage_baltic_mw = 651, as_of 2026-03-28, confidence "derived"`, pushed daily by a VPS writer that is **not in this repo** (grep over `.` and `~/kkme-control-center` for the key returns zero hits outside the worker). Today 484 + 40 + 127 = 651, so the duplication is invisible — the classic B12 shape, a second writer that agrees. **The moment LV moves 40 → 80 the sum becomes 691 and `baltic_total` keeps publishing 651**, so the Baltic headline stops being the sum of the three country figures printed beside it and nothing detects it, because both writers are "working". Discipline rule #4 with a live trigger. **Recommended fix:** delete the assertion so the total is always derived; a quantity defined as the sum should be computed as the sum. Alternative is correcting both and keeping two writers of one number, which is the state that produced this row. |
| B-059 | bug | ~~P1~~ **fixed 38.2** | The A68 live-fetch script will delete all 16 `s4_buildability` assertions on its first success | 2026-08-03 Phase 38.2 | closed — fixed 2026-08-03 same phase — **on the worker, not the script** | `scripts/vps/fetch_entsoe_installed_capacity.py:207-228` builds `{assertions: {...}}` **from scratch**, containing only `installed_storage_<c>_mw_live` keys, and `POST /s4/buildability` (`fetch-s1.js:10224`) `put`s the body **wholesale**. The live KV read on 2026-08-03 shows **16 assertions and no `*_live` key among them**, which both confirms the script has never posted successfully (consistent with `installed_mw_live: null` for all three countries) and means nothing has yet exercised the path. Its first success deletes `installed_storage_{lt,lv,ee,baltic}_mw`, `installed_storage_lt_{gen_mw,mwh}`, `reserved_storage_lt_{mw,mwh}`, `intention_storage_lt_mw`, `apva_applied_storage_lt_mw`, `under_construction_storage_ee_mw`, `grid_capacity_caveat` and the four `litgrid_connection_*` keys — i.e. every installed figure on the S4 card falls back to the worker's hardcodes, and LT's `installed_mw_as_of` reverts to a hardcoded 2026-03-23. **Playbook B12 on a second artifact, and unlike B-048 it is armed rather than already fired.** Fix: merge into the existing assertions (`preserveAcquisitionMetadata` equivalent) rather than replace, and add an append-only gate on the worker's POST handler so a body that would REMOVE an assertion key is rejected. Same family as `lint:manifest-single-writer` — one canonical writer per artifact. |
| B-060 | bug | P2 | `updateHistory` stamps the WRITE date, and keep-last disagrees with the settled capture series on 7 of 9 days | 2026-08-03 Phase 38.2 | open — **entangled with B-056** | Found while measuring B-056's dedupe delta. `updateHistory` (`fetch-s1.js:4860`) writes `date: todayEntry.updated_at.split('T')[0]` — when the row was written, not the market day of the prices in it. Cross-checked against `s1_capture_history`, which dedupes by market date on write and is trusted: keep-last `lt_swing` vs capture `swing`, same date — 07-26 147.04/147.16 · 07-27 115.25/108.84 · 07-28 204.22/**102.07** · 07-29 208.15/198.41 · 07-30 209.04/209.04 ✓ · 07-31 156.13/126.02 · 08-01 157.47/153.10 · 08-02 191.23/**136.42** · 08-03 179.99/179.99 ✓. **Two of nine agree**, and no s1_history value matches any *other* capture date either, so this is not a simple write-date/market-date shift — cause not established, and deliberately not guessed. **Consequence for B-056:** deduping keep-last fixes the row count and publishes a swing series that disagrees with the S1 card's own settled series on 7 of 9 days. The 38.2 CP recommends rebuilding `lt_swing` from `s1_capture_history` (one canonical writer, rule #4) rather than deduping in place, which sidesteps this row for the swing series and leaves it standing for `spread_eur`, which has no archive anywhere. |
| B-061 | tech-debt | P2 | `daily_intel.py` carried a live production Postgres credential as an inline default, and is not version-controlled at its origin | 2026-08-03 Phase 38.2 | partly closed — vendored, credential withheld | The daily writer behind every assertion-backed figure on the S4 card (`installed_storage_{lt,lv,ee}_mw`, the LT reservation/intention tiers, the APVA estimate, `grid_capacity_caveat`, four `litgrid_connection_*`) existed **only on VPS disk**: `/opt/kkme/app` is a `kkme-control-center` checkout and `git status` there reads `?? sync/daily_intel.py`. Vendored to `scripts/vps/daily_intel.py` in 38.2 with a README. **One line withheld:** line 40 was `DATABASE_URL = os.environ.get("DATABASE_URL", "postgresql://kkme_loader:…@localhost:5432/kkme")` and this repo is **public** — committing it verbatim would have published a working DB password into permanent git history. Verified safe to drop before doing so: `/opt/kkme/config/.env` sets the byte-identical value and the wrapper is `#!/bin/bash` sourcing it under `set -a`, so the default was never reached; confirmed against the resolved environment on the host. Not previously published (grep across both repos = 0), so nothing is exposed and no rotation is forced — but the credential is still plaintext on the VPS, which is **B-003**. **Open half:** the vendored copy and the VPS copy have no sync mechanism, i.e. a second copy of a thing is a second writer of it — the failure this same phase paid for twice (B-058, B-059). Proper fix: move the script into `kkme-control-center`, commit it there, deploy from there. Until then the rule is edit-here-deploy-from-here, byte-compare before assuming they agree. |
| B-062 | enhancement | P3 | Three fields the worker computes reach `/s4` and are surfaced nowhere — including 36.D's `demand_basis` | 2026-08-03 Phase 38.2 | open — **Phase 38 surfacing backlog** | Fallout of 38.2's ALL-N on the `/s4` whitelist: the projection dropped eight fields, five had consumers that went dark, and **three had no consumer at all**. `baltic_weighted_net_mw` (the net S/D numerator — the gross is disclosed in the canonical caption, the net is not); `non_commercial_mw` (80 MW today — the MW excluded from the merchant supply curve, which is the reason the fleet total and the S/D numerator differ, and it is explained nowhere); and **`demand_basis`** — 36.D built it to name the demand scenario behind `eff_demand_mw` (`{source: demand-forecast-module, module_version: 1.0.0, scope: baltic-auction-demand, year: 2026}`) and it has never been rendered. Session 105 already recorded the sibling gap: `s2.demand_basis` carries no scenario name, so "Litgrid L TrSc" is nowhere on the site either. All three are now published on `/s4` by the whitelist inversion, so surfacing them is a frontend-only change. Deliberately NOT wired in 38.2 — routed here rather than bundled into a corrections sweep. |
| B-063 | bug | ~~P1~~ **RESTATED 38.4 — the premise was wrong** | The shipped and hourly engines were reported as disagreeing ~2.5× on arbitrage share of gross; like-for-like the gap is 1.32×, and the real disagreement is in energy, not revenue | 2026-08-03 Phase 38.3 | **restated 2026-08-03 Phase 38.4** — the mechanism is now filed separately as **B-065**; this row survives as the corrected measurement so the artefact is not re-derived | **As filed:** shipped `computeRevenueV7` books Y1 arbitrage at 27.1 % of gross, B1's hourly run at 10.7 %, "~2.5× apart, both passing every gate they own". **The 2.5× is largely an accounting artefact.** `rev_trd` is a SPREAD MARGIN — `gross_eur_mwh = avgDischarge − avgCharge` (`fetch-s1.js:4432`) — and the shipped engine has no charging-cost line anywhere. The hourly `arbitrage` line is net of the ENTIRE charging cost including energy later delivered as activation; `dispatch.mjs:461` says so in terms ("booking the entire charging cost against arbitrage understates the arbitrage line and overstates activation"). **Like-for-like table, measured live 2026-08-03, 2h/base/COD 2028 — recorded here so nobody re-derives it:** shipped `rev_trd` spread margin, no charging cost = **27.8 %** (`2,235,502 / 8,049,363`) · hourly `arbitrage` RAW, all charging cost on it = **10.7 %** (`545,579 / 5,085,873`) — *the figure that was quoted* · hourly `arbitrage_net`, charging cost allocated = **21.0 %** (`1,067,100 / 5,085,873`) — *the comparable one* · hourly `discharge_income`, no charging cost = 31.5 % (`1,604,227 / 5,085,873`). **Like-for-like: 1.32×, not 2.5×.** **Where the disagreement actually lives — energy, price-free and accounting-free (the B5 non-mirror check):** DA delivered 747.0 vs 210.9 MWh/MW/yr = **3.54×** · total 498 vs 219.1 EFC/yr = 2.27× · 1.36 vs 0.60 c/d = 2.27×. **A second premise in this row was also wrong.** It offered B1's 75.2-85.5 % simultaneity as the check that would "distinguish a physically-correct commitment rule from a merely conservative one". It cannot: 85.5 % is `constrained_gross / unconstrained_gross`, and €757,401 of the €860,968 LT-2025 delta is CAPACITY revenue — it measures what the SoC reservation costs the reserve stack, not how much MW is free to trade. Committing 72.5 % of MW and retaining 85.5 % of the unconstrained stack are consistent, not rival. Verified across all five price years; ratio and free-MW share do not covary (79.2/75.2/83.1/83.5/85.5 % against free-MW 0.286/0.266/0.271/0.279/0.275). **The check that does discriminate already existed and already passed:** the 36.B1 artifact's `cycle_attribution.da_coherence` — free-MW share 0.2751 vs DA-achieved-against-revenue-anchor 0.2890, a power share and an energy ratio computed independently, agreeing to 1.4 pp. **Full diagnosis:** `docs/investigations/2026-08-03-phase-38-4-dispatch-diagnosis.md`. |
| B-064 | tech-debt | P2 | The degradation model has no characterisation below 1.0 c/d, and until 38.3 said so nowhere | 2026-08-03 Phase 38.3 | partly closed — limit published, model unchanged | `sohYr` (`workers/fetch-s1.js`) does `cd = Math.max(cd_total ?? 1.0, 1.0)` and `getDegradation` floors its 2 c/d weight at zero. The three SOH curves are calibrated at 1.0 / 1.5 / 2.0 c/d test rates and nothing extrapolates below the slowest, so **the engine cannot represent a battery cycling at 0.6 c/d ageing any more slowly than one at 1.0 c/d.** Defensible — manufacturers do not characterise below 1 c/d either — but it is a LIMIT, and a silent limit reads as a result. It is also why 38.3's cutover moved project IRR only +0.07 to +0.15 pp: the entire benefit of 1.36 → 0.60 c/d is the partial blend toward the 2C curve, and everything below the floor is clamped away by construction. The stress scenario already sits at 0.92 c/d, so a third of the 54-config matrix does not move at all. **38.3 published the floor as drawer disclosure** (`app/lib/wearModelRange.ts` + `WearModelRangeNote`), computed from the payload and asserted against the engine's real behaviour by bisection rather than against the constant that describes it. **That is disclosure, not a fix.** A wear model valid only at ≥1.0 c/d cannot price a 0.6 c/d asset regardless of which dispatch wins, so characterising below 1.0 c/d — or formally adopting the floor as a published model boundary — is a PRECONDITION for the cutover. Owned by 38.4 Q2. **38.4 UPDATE — still open, and deliberately not closed.** 38.4 verified the floor at code level rather than from this filing (`sohYr:6368` `Math.max(cd_total ?? 1.0, 1.0)`; `getDegradation:1467` `w2 = min(1, max(0, cd − 1))` — zero weight on the 2C curve at any cd ≤ 1.0; both confirmed). It then established that the floor is an artefact of **parameterising by c/d**, which fuses two independent mechanisms — cycle wear depends on cumulative EFC, calendar wear on elapsed time — and that the separable form (superposition of a √t calendar term and a linear-in-EFC cycle term) is standard in the LFP literature. Fitted to the engine's OWN three curves (51 points, 1.0/1.5/2.0 c/d × Y1–Y17): `loss(t,cd) = 0.0602·√t + 1.463e-05·EFC`, worst residual **3.3 SOH pp** (per-curve max abs-error 3.25 / 2.32 / 1.77 pp). That is interpolation of fitted parameters, not extrapolation of curves, so it clears the bar 38.4 was set — **and the operator declined to adopt it, correctly.** Three reasons: the 3.3 pp residual is too coarse for a coefficient that drives revenue; the payoff is ~2 pp of SOH at Y10 (fit gives 0.778 at 0.6 c/d vs 0.756 at 1.0 c/d), consistent with the +0.07 to +0.15 pp IRR that 38.3 measured for the entire cutover; and adopting it would replace a *disclosed* limit with an *undisclosed* internal regression. **Resting state: the 38.3 validity-floor disclosure is the answer, and it already shipped.** Characterisation waits for citable coefficients from a primary source (cell datasheets, published low-C-rate cycle-life curves) per rule #3. This row is filed OPEN so the disclosure is not mistaken for the fix — and note that the fit's small payoff means characterisation will not change a cutover decision either way. |
| B-065 | bug | **P1** | `trading_fraction` derives a share of VALUE in euros and spends it as a share of ENERGY in megawatt-hours — and both sides of the asset are sold whole | 2026-08-03 Phase 38.4 | closed — primary path fixed by `aaac252` (38.6a, operator-signed, 2026-08-03); residue re-scoped as B-069 | Split out of B-063, which described a symptom. This is the mechanism, and it is worse than the row it came from. **The unit error.** `trading_fraction = min(0.70, (T/(T+R)) × 0.75)` (`fetch-s1.js:3407-3503`): `T` and `R` are both value densities in €/MW/h, so `T/(T+R)` is dimensionless **€/€ — a share of value**. At `fetch-s1.js:2283` it multiplies `da_mwh_per_mw_yr` [MWh/MW/yr], i.e. it is applied as **MWh/MWh — a share of energy**. Derived in euros, spent in megawatt-hours, on one line. **The engine is not missing a representation of the physical constraint — it has one and does not use it.** `base_year.time_model.effective_arb_pct` (`:3676-3680`) is a genuine power share (Σ MW-share-freed-in-slice × slice-probability, dimension MW·h/MW·h), computed on every request, published in every payload, and consumed by nothing (`:3690`, diagnostic). Measured live 2026-08-03 from a SINGLE `time_model` object, 2h/base/COD 2028: `effective_arb_pct = 0.141` beside `trading_fraction = 0.700` — **4.96× apart inside one payload**. The independently-derived hourly measurement is 0.275. The two dimensionally-comparable figures (0.141, 0.275) disagree by 1.95×, which is an ordinary modelling disagreement and is not what blocks anything. **And it is not a partition — both sides are sold whole.** `rev_bal` (`:2263`) carries NO `(1 − trading_fraction)` factor, while `RESERVE_PRODUCTS` shares sum to exactly 1.00 (`:1777-1781`: FCR 0.16 + aFRR 0.34 + mFRR 0.50). The reserve model already claims the entire asset; the trading line then bills 70 % of a 1.5 c/d DA anchor on the same MW. Free MW under those shares is **0 %** (16 % pre-2028, because the DRR derogation drops FCR). The hourly run measures 27.5 % — above 16 % — only because the SoC reservation CUTS reserve commitment below its own ceiling (`dispatch.mjs:288`: the reservation is never relaxed, the commitment is). The hour loop's own invariant (`dispatch.mjs:419`, `totalCommitted + mwFree > mw` is a recorded violation) forbids what the shipped engine does. **Consequence, stated plainly: fixing this lowers IRR, materially.** DA revenue falls by roughly the ratio applied; the degradation offset from lower cycling is ~2 pp of SOH at Y10 (see B-064), nowhere near enough to compensate. The arc's three-phase sequencing rationale assumed the cutover moved IRR *up* — that assumption rested on the 498 → 222 EFC benefit while ignoring which side of the trade the revenue line sits on. Direction is not an input to the fix. **Not a constant swap.** Substituting 0.141 or 0.275 at `:2283` while `rev_bal` stays unpartitioned would still double-count MW, just less. A correct fix partitions both sides against one MW budget and re-derives `bal_calibration`, which is anchored to the current *unpartitioned* base year. **Scope owned by the partition phase** (queued after the card-defects phase): 54-config baseline captured from a CLEAN WORKTREE of the reference commit before anything changes (C6 — never via `git stash`). **Full diagnosis:** `docs/investigations/2026-08-03-phase-38-4-dispatch-diagnosis.md`. |
...
**Phase 33 follow-ups (filed Session 78, 2026-06-15):**
- **33.A (map feed, P1)** — Baltic-only allowlist for `s4.projects`. Root cause is upstream `~/kkme-control-center` (`esn_scraper`/`esn_loader` in `cron_daily.sh` → `kkme_sync.py` POST /s2/fleet, stored verbatim). Worker has no country filter (insertion point `fetch-s1.js:7077`); prompt's `POST /curate?action=purge_non_baltic` does NOT exist (needs building). 26 non-Baltic polluters (all `tso="Litgrid"`), 196 blank `cod`. **Scope expanded:** `/s4 storage_by_country.LT.installed_mw=484` is `as-of 2026-03-23` — a SEPARATE installed-MW pipeline frozen ~3 months, upstream of and untouched by today's VPS cron fix.
...
**CLOSED 2026-08-06 — the disclosure is live and verified end to end.** Pushed; Cloudflare Pages built; drawer read from rendered production DOM at kkme.eu, not from the bundle. It renders `BASE-YEAR MONTH EXCLUSION`, the principle, the rule with its measured boundary evidence (121× range through 2026-02 vs 3.5× from 2026-03), all six excluded months (2025-09 … 2026-02), and the measured effect with its direction stated. Three agreeing reads across independent reloads (C8): API `s2_months 4`, rule `0.2 / 2026-03`, `compression_rate_observed` absent, clamp disclosed `applied 0.15 / measured 1 / clamped max`. The Pages build and the worker deploy are separate propagation paths and they agree.

**Banked: C3b** — a production pre-state is either captured or explicitly waived with its reason, stated in the same breath as the deploy, not after it. Phase 53's waiver was sound (live KV moves every tick, so a production before/after would be confounded; the fixture is the controlled artifact) but it was stated at wrap rather than at deploy, which made it a justification instead of a decision.

**Roadmap edit needed, NOT applied (discipline rule #5 — operator/Cowork owns `_post-12-8-roadmap.md`).** Line 43 still reads "Direction established (published projections **understated**)… magnitude deliberately NOT given, because the available comparison sits on an unreconciled ~3× basis difference." Both halves are falsified: the direction reverses (fresh data is worse on every headline metric) and the ~3× difference does not exist. The B-076 register row in this file carries the corrected version.
...
**Pause A, four questions.** *(a) HYPOTHESIS vs verified.* The prompt's §1 framing (economic allocation vs physical constraint) was verified and turned out to understate the problem; its §2 framing (reconcile 27.5 % free MW against B1's 75.2-85.5 % simultaneity) was **wrong — those are not rival measures of one thing**, see B-063 restated. B-064's code claim re-verified at `fetch-s1.js:6368` rather than trusted from the filing. All live figures re-measured this session (A3); the 36.B1 artifacts are 2026-07-29 and one block is stale (A9). *(b) Consumes.* `trading_fraction` → `rev_trd` (`:2283`), cycle accounting (`:2288`), `assumptions_panel.cycles_breakdown`, RevenueCard "Trading %", and the `capture_*` back-solve (`:2703-2704`). *(c) Silent failure.* `effective_arb_pct` is computed and published on every request and consumed by nothing — a diagnostic that has disagreed with the shipped number by ~5× in production for months, with no gate. *(d) Layer/time.* Engine layer on `/revenue` plus a price-free energy check against the hourly artifact; nothing deployed, so no post-deploy window applies.

#### The phase was asked the wrong question, and said so

**§1 was the first thing tested, and the answer decided everything after it.** Three quantities were candidates for one slot; two are the same kind of thing and the third — the one in the revenue path — is not. `trading_fraction` is **€/€**, a share of value (`T` and `R` are both €/MW/h densities). `effective_arb_pct` and the hourly free-MW share are both **MW·h/MW·h**, shares of power. Line `:2283` multiplies `da_mwh_per_mw_yr` [MWh/MW/yr] by the €/€ ratio. **Derived in euros, spent in megawatt-hours.**

**The engine is not missing a representation of the physical constraint — it has one, publishes it, and ignores it.** Live, 2h/base/COD 2028, from a *single* `base_year.time_model` object: `effective_arb_pct = 0.141` beside `trading_fraction = 0.700`, **4.96× apart in one payload**. The hourly measurement is 0.275. The two dimensionally-comparable figures disagree by 1.95× — ordinary, and not what blocks anything.

**And it is not a partition.** `rev_bal` (`:2263`) has no `(1 − trading_fraction)` factor while `RESERVE_PRODUCTS` shares sum to exactly 1.00. Free MW under those shares is 0 % (16 % pre-2028 via the DRR derogation). The hourly 27.5 % exceeds 16 % only because the SoC reservation *cuts* commitment below its ceiling. Both sides of the asset are sold whole; the composition is the defect.

#### The 2.5× was largely an accounting artefact

`rev_trd` is a spread margin (`gross_eur_mwh = avgDischarge − avgCharge`, `:4432`) and the shipped engine has no charging-cost line at all; the hourly `arbitrage` line carries the entire charging cost including activation's share, and `dispatch.mjs:461` says so. Like-for-like: shipped 27.8 % · hourly raw **10.7 % (the figure quoted)** · hourly attributed-net **21.0 % (the comparable one)** · hourly discharge-income 31.5 %. **1.32×, not 2.5×.** The real disagreement is in energy: DA delivered 747.0 vs 210.9 MWh/MW/yr = **3.54×**; 498 vs 219.1 EFC/yr; 1.36 vs 0.60 c/d. Full table in B-063 so it is never re-derived.

**The independent non-mirror check (B5)** was energy, price-free and accounting-free. The check that *does* discriminate already existed and already passed — 36.B1's `cycle_attribution.da_coherence`, free-MW 0.2751 vs DA-achieved/anchor 0.2890, agreeing to 1.4 pp.

#### Degradation — tested, and deliberately not adopted

The 1.0 c/d floor is an artefact of parameterising by c/d, which fuses cycle wear (∝ EFC) with calendar wear (∝ time). Fitted the standard separable form to the engine's own three curves: `loss(t,cd) = 0.0602·√t + 1.463e-05·EFC`, worst residual **3.3 SOH pp**. That clears the "interpolation not extrapolation" bar — **and the operator declined it, correctly**: 3.3 pp is too coarse for a revenue coefficient, the payoff is ~2 pp SOH at Y10, and it would swap a disclosed limit for an undisclosed regression. The 38.3 validity-floor disclosure is the resting state. **B-064 stays OPEN** so the disclosure is not mistaken for the fix.

#### Decisions taken at the checkpoint

1. **B-063 restated** (documentation only) and carrying the like-for-like table. 2. **The unit error filed separately as B-065, P1** — B-063 was a symptom, B-065 is the mechanism and it is much worse. 3. **36.B1's `engine_branch_gap` block marked stale (A9)** in the committed diagnosis artifact — those JSONs are untracked and carry an `output_hash`, so they were not amended; the 550-vs-385 row describes an engine 36.B5 already retired. 4. **`effective_arb_pct` stays published** — a diagnostic that has been disagreeing in production for months is evidence; removing it while the model is wrong would be the wrong kind of tidy. 5. **Three card defects → their own phase, BEFORE the partition**, each with its own commit and its own delta, `capture_eur_mwh` first (rule-#2 shape on a live field). Folding them into the partition would blend three attributable movements into one unattributable one — exactly what C3 exists to stop. 6. **Partition → its own phase after that.** Not scoped this session.

**Direction, recorded plainly:** fixing B-065 **lowers IRR, materially**. The degradation offset is ~2 pp SOH, nowhere near enough. The arc's three-phase sequencing rationale assumed the cutover moved IRR *up*; that assumption rested on the EFC benefit while ignoring which side of the trade the revenue line sits on. Direction was not an input to the recommendation.

#### Roadmap delta — REPORTED, NOT APPLIED (rule #5)

`docs/phases/_post-12-8-roadmap.md` is operator/Cowork-owned and was not touched. Needed changes: (i) the hourly-cutover thread should be **closed as malformed**, not left queued as a decision; (ii) insert a **card-defects phase** (three defects, three commits, three deltas — `capture_eur_mwh` first) ahead of everything else in this thread; (iii) insert a **partition phase** after it — unit error at `:2283`, `rev_bal` partitioned against one MW budget, `bal_calibration` re-derived (it is anchored to the current *unpartitioned* base year), 54-config baseline captured from a **clean worktree** of the reference commit before anything changes (C6, never `git stash`); (iv) degradation characterisation is **unscheduled**, waiting on citable coefficients.

#### The 38.2 open item — watched, not assumed

The last open item from 38.2 is the 08:00Z cron tick on `/read`. 38.2 shipped with a stated lag: `/read` returns the cron-written `s1` KV, so `spread_stats_90d` keeps publishing the pre-fix window until the 4-hourly cron next fires, while the route-level `/s1` already serves the corrected figures. **Baseline captured this session at 07:03Z** — `/read` `updated_at 2026-08-03T04:01:04.074Z`, `spread n=90 days=90`, `swing n=90 days=90`; `/s1` at the same moment served `spread n=9 days=9`, `swing n=90 days=90`. So the discriminating field is `spread_stats_90d`, which should move 90/90 → 9/9 at the tick; `swing_stats_90d` at 90/90 is already correct and must NOT move (it is the post-38.2 windowed figure, not the pre-fix one — the 400-day bug was caught and fixed before deploy).

A watcher is armed and polls to two-consecutive-read agreement before reporting (C8), because a single post-tick read cannot distinguish a propagated write from an edge that has not caught up. **Result pending at the time of writing — this entry is completed when the watcher fires, and is deliberately left open rather than pre-written (B1: no success claim without the verifying output).**

#### Gates

No code touched, so the code gates are not the evidence here — the absence of change is. `git diff --stat` against `main` is documentation-only; `/revenue` untouched and unredeployed; `docs/_private/` never staged. Every live figure in this entry was measured this session with the command output in the transcript, not carried from 38.3 (C4).
...
**Pause A, four questions.** *(a) HYPOTHESIS vs verified.* Every §5 premise re-checked live. Four of the prompt's seven mechanical items had already shipped in 38.1 (§5 1-3 and the S2 data-stamp half of 6) — the "7 mechanical" was §5's original count, not the remainder. Two prompt premises did not survive: the hero was **not** dark (it fetches `/s4/fleet`, which the whitelist never touched — the two dark sites are S4Card and the ticker), and §5 item 12's LV `coverage_note` **already renders** in the card's LV tab. *(b) What consumes what changes.* The `/s4` fleet projection feeds S4Card, SignalBar and the hero's second `/s4` state; `baltic_total`/`storage_by_country` have six consumers, all display, none in the revenue engine — so the LV movement cannot move IRR, by grep. *(c) What fails silently.* The whitelist itself: omission produces no error, no null, no log, which is why a signed-off fix stayed dark for two arcs. Inverted to publish-by-default so the next aggregate is not dropped the same way. *(d) Layer and time.* Worker at the real router; frontend at SSR markup and at the live DOM in a browser against production data; `/revenue` at the 54-config route probe.

#### Stage 1 — the whitelist, and the ALL-N underneath it

`GET /s4` was a hand-maintained 10-field projection that copied `baltic_weighted_mw`'s neighbours and not the field itself. **8 of 20 stored fields dropped**, 26 references across 5 `app/` files, and the caption every call site guards on that field never reached a browser after 36.D shipped. Fixed by inversion, not by widening: everything `processFleet` computes now reaches `/s4` unless explicitly excluded (`raw_entries`, republished as `d.projects`; `demand`, the operator override).

**What else the whitelist hid:** the ticker's FLEX FLEET tooltip lost 2 of its 5 sentences; the hero's quarantine tooltip rendered a hardcoded five-asset roll-call beside a computed 0; and three dropped fields have **no consumer at all** — `baltic_weighted_net_mw`, `non_commercial_mw`, and 36.D's `demand_basis`, built and never surfaced.

**Proved dark in a browser, on production data, before the fix:** the S4 card renders `FLEET TRACKER · 188 PROJECTS · 2.91× S/D` immediately followed by the Flex-fleet/pipeline/demand tiles, with nothing between them — `querySelectorAll('[data-testid="sd-formula-caption"]').length === 0` and no `S/D = ` anywhere in the DOM. The block mounts, the tiles populate, the disclosure is absent.

Also: six frontend literals duplicating a worker default removed (1545 ×3, 3232, 45000000, 3204 ×2) — all unreachable today, so nothing moved; three left standing (1395, 3700, 484) because they feed arithmetic rather than display, and logged rather than silently skipped.

#### Stage 2 — five copy corrections, all unflattering

"782 MW (BESS + pumped hydro)" over 18 operational entries that are **all batteries** — corrected in five rendered places plus the two `metricRegistry` declarations. `= TSO BESS 651 MW + Kruonis flex share 131 MW` replaced by `= 651 MW in national registries · 131 MW tracked above them`, decomposed per country from the payload (EE +9 · LT +63 · LV +59) by new `app/lib/fleetCoverage.ts`, which returns null rather than a partial answer when a tracked country has no registry counterpart. The hero's quarantine roll-call now reads the worker's own `quarantined[]`. RenewableMix and ResidualLoad cite `energy-charts.info`, and the masthead — which listed ENTSO-E and omitted energy-charts entirely — now lists both. S1's `today's gross capture` is the payload's own date. APVA relabelled per B2: the figure stays, the reason it cannot be confirmed is now on the surface.

#### The checkpoint — and what the KV read found

**3a · LV 40 → 80.** Primary source verified through the browser (`ast.lv` 403s both curl and WebFetch): *"The Rēzekne battery has a capacity of 60 MW / 120 MWh, while the Tume system adds 20 MW / 40 MWh, bringing the total capacity to 80 MW / 160 MWh"*, balancing reserves from 2025-10-30. The published 40 carries `as_of 2025-10-01` — four weeks **before** the assets entered service — and `confidence: "official"`. The 99 is not a rival value: registry and tracker answer different questions and both are right.

**Two findings the delta uncovered, each larger than the delta.** `installed_storage_baltic_mw = 651` is a **stored assertion that wins over `ltMw + lvMw + eeMw`** at `fetch-s1.js:10409`; today the sum happens to equal it, so move LV to 80 and `baltic_total` keeps publishing 651 while silently ceasing to be the sum of the three numbers beside it. And `s4_buildability` holds **no `*_live` keys**, confirming `fetch_entsoe_installed_capacity.py` has never posted — it builds the body from scratch and the worker `put`s it wholesale, so its **first success deletes all 16 assertions**, every installed figure on the S4 card included. B12, armed, in production now.

**3b · B-056.** `s1_history` is **90 rows over 9 distinct dates** today (8 at 38.1), and the 14D sparkline resolves to **two** dates, not one. Deduped: swing p50 153.10 → 179.99 (+17.6 %), spread p50 10.72 → 9.32 (−13.1 %), n 90 → 9. **The finding that blocks a straight dedupe:** `updateHistory` stamps the write date, and keep-last disagrees with the settled `s1_capture_history` swing on **7 of 9** shared dates — not an off-by-one, and not a cause I can name from here. Backfill answered: `raw:s1` has a 7-day TTL and can never hold more than `s1_history` already covers; `s1_capture_history` can rebuild `lt_swing` to 30 real dates; `spread_eur` has no archive anywhere.


#### Post-checkpoint — the six answers, in the order they were signed

**Q4 first, and it earned its place.** `POST /s4/buildability` `put` the request body wholesale, so every poster owned the whole key. Fixed **on the worker**, per the operator: merge by default, `mode:"replace"` the only way to destroy, the flag consumed rather than persisted, and a 409 behind the merge that is unreachable today and exists so a future edit cannot quietly reintroduce deletion. The guard covers every future poster, not the one script — the same argument as the whitelist inversion. Inject-then-revert with the wholesale `put` restored: **5 failed, 1 passed**, and the failure line is what production would have printed — *"assertion apva_applied_storage_lt_mw was deleted"*. The survivor was the 401 spec; auth was never the broken half. **Live proof after deploy:** `{ok: true, mode: 'merge', assertions_pushed: 15, assertions_stored: 15, assertions_preserved: 0, dropped: []}`.

**Q3.** `installed_storage_baltic_mw` deleted from the resolution path; `baltic_total` is `ltMw + lvMw + eeMw` with no override, deliberately. The test perturbs a country and checks the total followed, because *"equals 651 today"* passed all the way through the defect — inject-then-revert: **3 failed, 2 passed, and the two that stayed green are exactly the equals-today spec and the no-assertions spec.** A duplicate writer that agrees today is invisible to any test written against today.

**Q2 — and the instruction had to be deviated from.** `sync/daily_intel.py` was untracked at its origin (`?? ` in the `/opt/kkme/app` checkout). Vendored to `scripts/vps/` with a README — **but not byte-for-byte as-is**: line 40 carried a live production Postgres credential as an inline default and this repo is **public**. Committing it verbatim would have published a working DB password into permanent git history. One line withheld, flagged in place and in the commit; verified safe first (`/opt/kkme/config/.env` sets the byte-identical value, wrapper is bash with `set -a`, so the default was never reached). Deployed back with a backup, proven on the host: import OK, DB reachable, 16 current assertions. **B-061.** And the finding that reshaped Q1: **the values are not in the script** — JOB 7 pushes every `assertion WHERE is_current = TRUE` from Postgres, so LV 40 was a database row.

**Q1 — LV 40 → 80, live.** Superseded rather than overwritten (`assertion 17 supersedes 13`), on AST's own publication with the specific release URL rather than the bare `https://www.ast.lv/` the old row cited. The grade was corrected as its own act: row 13 `official` → `estimate`, because a 2025-10-01 figure for assets commissioned 2025-10-30 is a pre-commissioning estimate and *"official"* was a grade it never earned. `baltic_total` 651 → **691**, verified across three consecutive agreeing reads. `TwoPopulationsNote` renders the 80-vs-99 framing in the LV tab, computed from the payload, silent when they agree.

**Q5/Q6 — option 3.** `lt_swing` moved to `s1_capture_history` (one canonical writer, dedupes by market date); `spread_eur` stays on `s1_history` publishing n = 9 honestly. `days_of_data` counts distinct dates everywhere. `updateHistory` dedupes on write **and** `/s1/history` on read, so consumers are correct without waiting for a cron tick — live, that array went **90 rows / 9 dates → 9 rows / 9 dates**, and SpreadCaptureCard's sparkline moved onto `/s1/capture.history` with a computed `{days}D` label. PeakForecastCard's window comes from `days_of_data` instead of a hardcoded `90D`, and the P90 comparison — and the green dot — are withheld below 10 market days.

**Two of my own specs did not discriminate, and injection is what found it.** The 14-row-tail spec appended clean dates *after* the contaminated ones, so `slice(-14)` landed in clean data and passed **with the bug still in**; in production the repeats are last. And `days_of_data` had no behavioural assertion at all, because no route exposes it — so injecting the row count changed nothing. `rollingStats` and `dedupeByDateKeepLast` are now exported and asserted directly. Sharpened, the injections give `expected 2 to be 14` and `expected 90 to be 9`. **B13 corollary, again: when an injection fails to go red, suspect the harness before concluding the code is safe.**

#### The one the gates could not have caught

Post-deploy verification of the live payload returned `swing_stats_90d: {n: 400, days_of_data: 400}`. `s1_capture_history` is not the 30 rows `/s1/capture` displays — it holds **400 market days**, 2025-05-02 onward, and `readSwingSeries` took all of them. **The mirror image of B-056**: that label overstated its window, this one understated it, and both are a label that does not describe its own data. Windowed to the last 90 market days; a spec now asserts the window against a 400-day archive. Suite green, gates green, defect visible only in the deployed payload — B2 in one line.

#### Deploy and verification

`main` `55dee3c` = `origin/main`, working tree clean, deployed with `npx wrangler deploy` (versions `c01dfd10` then `15f7c7ac`). **C8 honoured and it mattered twice** — on both deploys a mid-sequence read returned the pre-deploy artifact and would have read as a failed deploy; polled to agreement instead. Final live state: all 8 previously-dropped `/s4` fleet fields present · LV 80 · `baltic_total` 691 = the sum · `/s1/history` 9 rows over 9 dates · `/s1` swing n=90/days=90, spread n=9/days=9 · buildability merge envelope confirmed.

**Known lag, stated rather than papered over:** `/read` still serves `n: 90, days_of_data: 90` because it returns the cron-written `s1` KV, and the 4-hourly cron next fires at **08:00Z**. PeakForecastCard reads `/read`, so its sentence carries the old window until that tick. The route-level `/s1` already serves the corrected figures. B3 — every verification states when it is valid.

#### Gates

`/revenue` **54/54 byte-identical** vs main at the route layer, re-run at every stage including all six post-CP items · vitest **2114/2114, 114 files** (+41 tests, +7 files, none deleted or weakened) · `next build` clean · eslint delta **zero** (per-file/per-rule diff; the raw 197-vs-195 is two untracked probe scripts the stash round-trip removes) · `lint:no-editorial-chips`, `lint:no-raw-spacing`, `lint:manifest-single-writer` PASS · `docs/_private/` never staged · **both new gates proven failable by inject-then-revert** — restoring the whitelist gives `expected '' to contain 'sd-formula-caption'` (4 red, 1 green, and the survivor is the `/s4/fleet` spec, reproducing the production asymmetry exactly); re-injecting the pumped-hydro label and the ENTSO-E citation gives 2 red.
...
Rule #1's ~25 % class, and it went 4 for 4. Worth recording against the base rate: visual inference on *one's own* site, where the observer knows what the number should be, is a better-calibrated instrument than a third-party visual audit. Two of the four were confirmed as defects but not the defect suspected — B3's two fleet numbers are correctly different populations reading canonical fields, and the defect is that one is labelled "(BESS + pumped hydro)" over a population with **zero** pumped-hydro entries, beside a derived line publishing "Kruonis flex share 131 MW" computed as `max(0, 782 − 651)`.

35 rendered surfaces traced (hero, ticker, every card's hero metric, chip, impact line and footer; drawer interiors sampled, not enumerated — stated rather than implied). Findings routed to 38.2/38.3.

#### 38.1 — what the tail actually said, and why it mattered

The operator's condition was: settle the cause before changing the branch. That condition earned its keep — **the hypothesis was wrong and the fix written against it would have left the cause live.**

Pre-fix, from outside: computeS1's exact 9-request ENTSO-E burst replicated with the real key, alone and at the 15-request concurrency of the 4h-aligned collision with the hourly cron, four rounds — **48/48 HTTP 200, zero throttling.**

Then `wrangler tail` across the 2026-08-02T12:00:33Z tick, on the unfixed worker (`[observability]` was off, which is why this had to be watched live rather than read):

```
cron '0 */4 * * *' at 12:00:33Z — outcome=ok cpu=83ms wall=50017ms
  [error] [S1] cron failed: Error: Timed out after 30000ms
  [error] [S4] cron failed: Error: Timed out after 25000ms
  [error] [S3] scrape failed: AbortError: The operation was aborted
  [error] [FX] frankfurter.app failed, using fallback rates: AbortError
  [log]   [S1/PL] pl_avg=107.02 lt_pl_spread=-57.41€/MWh (-53.6%)
  [log]   [S1] coupling_spread=9.32€/MWh intraday_swing=191.23€/MWh
cron '0 * * * *'   at 12:00:33Z — [error] [Gen/LT] energy-charts lt: HTTP 429
```

**computeS1 did not reject — it timed out, and its own completion logs sit in the same invocation as the timeout that discarded them.** The work finished, late, and the result was thrown away. `cpu 83ms / wall 50017ms` says the invocation waits on connections rather than computes. And the hourly cron fires in the *same second* as the 4-hourly one at every 4h-aligned hour, with energy-charts returning 429 to one of them.

Ninth consecutive failure confirmed at 12:03Z: `s1_capture` still `2026-08-01T00:01:40.584Z`, `raw:s1:2026-08-02` still absent.

**Diagnosis reconstructed before the tail, from `raw:s1:<date>` TTL arithmetic** (`expiration − 604800` = that date's last successful S1-branch write), because there were no logs:

| date | last S1-branch success |
|---|---|
...
Not a new break — intermittent for at least the seven days the TTL window covers.

#### What shipped in 38.1

| commit | what |
|---|---|
| `96bc9b1` | `computeCapture` hoisted out of the `s1Result.status === 'fulfilled'` branch (it takes only `env`; it was nested there by code position); `fetchBznGuarded` on the LT/SE4 legs; `[observability]` on; `s1_capture` → `STALE_THRESHOLDS_HOURS` at 12h; Telegram alert on the failing tick; `/read` always prefers the canonical capture key; **B-047 closed** |
| `72e902a` | S2 badges the data stamp (`marketDayEndStamp`), window rendered, fetch time demoted to the title |
| `edd1b13` | 10 gates, each proven failable by inject-then-revert first |
| `8f4464c` | tail-driven correction: computeS1 budget 30s → 60s, retry delay 750 → 400 ms, `fetchEnergyCharts` retries once on 429/5xx |

**B-047 turned out to be worse than filed.** It was filed as a control-strength problem (a probe cannot distinguish route-absent from route-present). The catch-all did not merely *respond* as `/s1` — it ran `computeS1()` and `put('s1')`, and `s1` is the key /health monitors with a 24h threshold. The monitor was measuring inbound 404 traffic and read green through the whole outage. The A7 ALL-N audit the row asked for: **92 resolved route forms vs 32 distinct worker paths called from `app/`, `lib/`, `scripts/` — 0 relied on the catch-all.** The first enumeration reported 12 false misses by matching only `url.pathname === '...'`; the template-literal and regex route families had to be resolved before the count meant anything.

#### Inject-then-revert (B13) — green proves nothing until it has been made red

```
inject 1  delete s1_capture from STALE_THRESHOLDS_HOURS  → 4 red / 3 pass → revert 7/7
inject 2  restore the write-on-any-GET catch-all         → 2 red / 5 pass → revert 7/7, sha256 identical
inject 3  marketDayEndStamp guesses instead of nulling   → 1 red / 4 pass → revert 5/5
```

**Injection 2 took three attempts, and both failures were the same mistake in two costumes.** Draft 1 passed *with the bug reinjected*: unmocked `fetch` made computeS1 reject, so the write path was never reached — it verified the mock, not the router. Draft 2 used `mockResolvedValue(new Response(...))`, one shared instance, and a Response body can only be read once, so every fetch after the first rejected and it passed for the same wrong reason again. Only a fresh Response per call exercises the path. Running the test and seeing green would not have caught either.

#### B8 — what tells us next time, and how fast

| surface | before | after |
|---|---|---|
| `s1_capture` staleness | **nothing, ever** — absent from the threshold table | `/health` flags at 12h = 3 missed ticks |
| `s1` staleness | present but unfalsifiable — any stray 404 refreshed it | cron is the sole writer; the entry means something |
| S1 branch rejection | `console.error` to a worker with no log sink | Telegram on the failing tick, naming the rejection + the knock-on |
| retrospective diagnosis | TTL arithmetic on `raw:s1:<date>` | Workers Logs, `head_sampling_rate = 1` |
| `da_tomorrow` | 36h threshold, already flagging, nobody watching | unchanged — it was the one honest alarm and it worked |
...
- **B-056 (P1)** — `s1_history` is 90 rows over 8 distinct dates; `rollingStats` publishes `days_of_data: 90`. Drives PeakForecastCard's dot colour. Deduped delta measured. Number movement → 38.2.
- **B-057 (P2)** — five heavy computes share one invocation's connection budget while the hourly cron fires in the same second. S4's 25s timeout is the same root cause.
...
**B-053 (P1) — the gate that could not fail.** `set -o pipefail` added; the pipeline was returning `tail`'s status. Enumerated rather than spot-fixed (A7): **2 workflow files, exactly 1 `run:` block in either that branches on a shell pipe.** `fetch-btd.yml`'s pipe characters are all JS (`||`, `join(' | ')`) inside a heredoc. `refreshWorkflow.test.ts` re-runs that enumeration on every commit.

**Every gate demonstrated failable on real Actions runs, not argued:**

| gate | clean run `30739141676` | injection run `30739276674` |
|---|---|---|
| refresh | pass | **fail** |
| tests | pass | **fail** |
| scope | pass | **fail** |
| PR opened | **no — blocked** | no — blocked |
| job status | red (PR block) | **red** |

Injection commit `2009346`, reverted in `832ccfc`; `git diff main -- tools/consultancy/mature-markets/refresh-mature-markets.mjs` empty afterwards. **My first version of the pipefail test was itself unfailable** — it matched the word `pipefail` in the explanatory *comment* above the code, and passed with the pipefail deleted. Caught by injecting; fixed by stripping comments before asserting. Recorded because it is the same defect one level up.

**One layer out:** a run could finish GREEN with `tests=fail` recorded, because gate results reached the PR body and nowhere else. A `Gate summary` step (`if: always()`) now fails the job when any gate is not an explicit `pass` — "not reached" included.

**The 3 Actions-only failures diagnosed (a separate finding, not a side effect).** All three are `loadDataset` timeouts, not data defects, and **the local suite is not the wrong one.** Local 14.4 s wall / de 2143 ms / au 1752 ms / activation 4138 ms against 41.3 s on Actions = ~2.9× slower runner → de ~6.2 s, au ~5.1 s, activation ~12.0 s against vitest's 5000 ms default. That predicts exactly the three that failed, the one that nearly did (au), and `da` (748 ms → ~2.2 s) surviving — which it did. `testTimeout` 60 s; no assertion changed.

**B-052 — cron.** `0 3 * * 0` + a `window` job guarding day-of-month ≤ 7. The comment is computed now, not asserted: the test implements POSIX cron day-matching, enumerates 2026-2030, asserts one firing per month AND that it is the first Sunday, and re-derives the ~11×/month figure the comment cites. `matureMarkets.test.ts` asserted the broken cron as the requirement and was green for the bug's whole life — **inverted to assert its absence** (B-036 precedent), not quietly corrected. GitHub's ~3 h scheduler drift noted, not fought.

**B-051 — verified by observation, and the answer is no.** The tick has **not** taken effect: a `workflow_dispatch` at 08:09 UTC still got `GitHub Actions is not permitted to create or approve pull requests`. Fallback shipped and demonstrated: labelled `refresh-blocked` issue with the compare URL (**issue #125**), plus a red job. Left open for the operator — re-check it is the repository setting and that no org policy overrides it.

**B-054 — premise corrected, then fixed.** `max_age_hours` is **not** a global constant (five distinct per-signal values, each source-shaped), and the prompt framed it against `check-freshness.mjs` while the handover entry is about `fleet_lifecycle`. Real gap: a detector's stamp only advances when the RUNNER runs, so a dead runner freezes them together and the first alarm is the smallest *source* ceiling. `runnerHeartbeat()` — 240 h, strictly below every source ceiling — rides the existing detector map so the worker's generic ageing gives the alarm with **no worker change** (this phase forbids `workers/` edits). Dead runner: **14 d → 10 d**.

**Part 5 — 2026-08 refresh: ACCEPT both, applied neither.** `docs/research/evidence-refresh-2026-08-review.md`. `se`'s 72 restated rows are one delivery day whose implied second-auction tranches land on **3.00 / 15.00 / 10.00 EUR/MW** — clean clearing prices, not corruption. `da` is two events, not one: 24 removed PT60M rows were the only hourly rows in a quarter-hourly dataset on two days that already had complete PT15M coverage (**coverage grew 20,366 → 20,630**; the `rows_removed` alarm fired on growth), and the 191 restatements are two delivery days, not eight. **The recurring pattern:** both sources tripped on their trailing edge because the refresh reads a market before it has finished publishing. It will trip every month; recommended fix is a 2-day exclusion or lag on the comparison window — filed, not applied.

**Gates:** 1977/1977 · eslint clean · `git diff main -- workers/ app/` empty · no `docs/_private/` staged. Evidence branch preserved at `archive/evidence-refresh-2026-08-174a166` before any proof run, because the workflow force-pushes that branch.

**Open follow-ups filed:** trailing-edge window exclusion; how 24 PT60M rows entered the `da` shard; B-051 repo setting.

### Session 102 — 2026-08-02 — 37.B.1a (runner scheduled) + 36.E0.2 (manifest single-writer) + Sunday-refresh report (Claude Code, autonomous) — PR #124, branch `phase-37-b1a-runner-schedule`, HEAD `c5d3c91`

**Pause A, four questions.** (a) HYPOTHESIS vs verified: the prompt's "Host is decided: the VPS" and "one bypassing writer at `fetch-activation-prices.mjs:590`" were BOTH false and both re-checked at execution time — the VPS had no Node runtime at all, and there are eight manifest writers, not one (590 is `fetch-de-regelleistung.mjs`'s line, activation's is 647). (b) What consumes what changed: `writeManifest` is consumed by 8 fetchers + the orchestrator, and the orchestrator now applies preserve TWICE per run, so it was made idempotent; the runner change is consumed by cron only. (c) What fails silently: a manifest write that drops `last_successful_refresh` exempts its source from the staleness surface permanently (B12) — now gated; and a cron whose log directory is missing dies before its command (36.C) — now asserted in the wrapper. (d) Layer/time of verification: the cron path under `env -i`, not an interactive shell; the manifest gate under an inject-then-remove control; `/revenue` at the offline 54-config harness, not the live route.

**Part 0 — Sunday's refresh (evidence only, not fixed).** It fired and **all eight sources served from Actions runner IPs** — the `fetch-btd.yml` blocking precedent did not repeat (6 ok / 2 anomalous / **0 failed**). Four findings filed, none fixed: (1) **no PR opened** — `GitHub Actions is not permitted to create or approve pull requests`, a repo setting; the branch `evidence-refresh/2026-08` IS pushed at `174a166` (46 files, +2062/−841). (2) **The cron fires ~11×/month, not once** — `0 3 1-7 * 0` ORs day-of-month with day-of-week, so it ran Sat 08-01 AND Sun 08-02; the workflow comment asserts "exactly one day per month". (3) **The tests gate cannot fail** — `if npx vitest run … | tail -40` with no `pipefail` takes `tail`'s exit status; the run recorded `tests=pass` with 3 tests actually failed (`loadDataset` exceeding the 5 s default on a slower runner; 71/71 green locally in 14 s). (4) `append_only` FAIL is the gate working: `se`/`da` restated boundary-hour settled prices and `da` withdrew 24 rows; both correctly withheld `last_successful_refresh`. **Also: GitHub delayed the 03:00 UTC schedule to 05:5x both days** — relevant to any "lands before X" margin.

**Part 1 — the runner is scheduled, Sunday 22:00 UTC.** Operator decided to install Node once the no-Node finding landed. Footprint far smaller than a repo clone: the runner's module graph is **7 files, zero external packages**, so no `npm ci`, no GitHub credentials on prod. Node 24.18.1 at `/usr/bin/node`; deployed under `/opt/kkme/fleet-intel`; wrapper at `/opt/kkme/bin/fleet_lifecycle_runner.sh` (in-repo at `scripts/vps/fleet_lifecycle_runner.sh`); crontab backed up to `/root/crontab.bak.20260802`. **The `env -i` proof caught a real bug**: run 1 died on `ENOENT data/lifecycle-rules.json`, a runtime `readFileSync` no import trace can see; run 2 exit 0, four sources acquired, `POST → 200`. Cold run **7 s** (register.csv mtime proves the 128 MB pull is inside it). Runs `--write-health` (new flag) — posts detector health, never a transition, because scheduling `--write` is the unattended wiring `run-lifecycle.mjs`'s own header warns against. Three defects fixed en route: hardcoded dated report filename (would rewrite 37.B.1's report every Monday, rule #2), mode banner reading `REPORT-ONLY` while writing (rule #2), and `readVpsJson` SSHing to the VPS from the VPS (now reads locally — no SSH key on prod, B-050 respected).

**Part 2 — 36.E0.2.** New `tools/consultancy/mature-markets/manifest-writer.mjs` is the only sanctioned manifest path; all 8 fetchers + the orchestrator route through it. **Omission is repaired** (carried forward from disk), **destruction fails the write** — got wrong first, because carry-forward ran before the assert and the gate could never fire; its own test caught it. Per queue §2.55, provenance-absence-as-ERROR was **deliberately not built**. `npm run lint:manifest-single-writer` proven failable by inject-then-remove.

**CC-error self-flags banked.** (1) The first version of the manifest gate reported PASS on a live violation: `[^\n]*` in a grep bracket expression means "not backslash, not n", so it never matched `fs.writeFile(path.join(OUT, …` because of the `n` in `join`. Found only by running the control, not by reading the pattern — **a grep gate is unvalidated until a known violation makes it fail**. (2) An import-graph trace is NOT a deployment manifest: it cannot see runtime `readFileSync`. (3) Two temporal-dead-zone bugs self-introduced by placing `const MODE`/`REPORT_OUT` above the `const`s they read.

**Open, NOT claimed as solved.** `/health.fleet_lifecycle.detectors` now carries `last_run_at` + `age_hours` and ages independently of any digest (populated this session, `age_hours 0`) — but `max_age_hours` is 720-1080, so a dead WEEKLY runner takes 30-45 days to cross it. A runner-specific threshold is the remaining gap (B-051 below).

**Gates:** suite **1959/1959** (+14, none deleted; two updated and called out) · eslint clean · `/revenue` **54/54 byte-identical** · 0 files under `workers/`/`app/` · `docs/_private/` never staged · nothing deployed to the worker (no worker change).

### Session 101 — 2026-08-01 — Phase 37.B.1: detector runner, first real run, digest armed (Claude Code, semi-autonomous, one checkpoint)

**Branch `phase-37-b1-detector-runner`, pushed, 3 commits. Suite 101 files / 1945 tests (1895 → 1945, +50). One worker deploy. Digest ARMED.**

**ORIGIN-SHA COMPARE — before the deploy:**
```
local  phase-37-b1-detector-runner : 281dcc4ea061ad37d376e95e2737c8552148c6c9
origin phase-37-b1-detector-runner : 281dcc4ea061ad37d376e95e2737c8552148c6c9   equal
git status --short -- workers/ wrangler.toml tools/fleet-intel/ : empty
→ worker version 67280b44-ab3e-438a-ba35-ea693d4a2e69
→ Cloudflare confirmed triggers incl. `schedule: 30 7 * * 1`
```

**What was missing.** 37.B shipped seven signals, a rename guard, a retirement policy, a transition log and an ingest endpoint — and no caller. `fleet_lifecycle:detectors` had one writer fed by nothing. New: `tools/fleet-intel/lib/sources.mjs`, `lib/detectors.mjs`, `run-lifecycle.mjs`; `lifecycle.mjs` gains `applyEligibility`.

**First run REPORT-ONLY, 0 proposals — and the zero is defended, not asserted.** B11 controls run every run and suppress the registry detectors on failure: known-good names resolve 3/3, nonsense 0/2, known-terminated entities read back terminated 3/3. All 36 registry-eligible rows resolve ACTIVE/current.

**The three kinds of zero, per detector (this is the phase's actual product):**

| Detector | verdict | why |
|---|---|---|
| registry_terminated / registry_absent | checked | 36 eligible, **all private-tier** — cannot produce a PUBLISHABLE transition (→ 37.B.2) |
| vert_permit_expired | **blind** | 0/243 LT rows eligible; VERT serves 23 records with **1** expiry date (→ 37.B.3) |
| press_negative | **no-source** | lv_press alive (150 items/day) but scans for COMMISSIONING; no insolvency vocabulary (→ 37.B.3) |
| queue_disappearance | checked | 182 eligible; baseline established by this run |
| evidence_stale | checked | 212 eligible, nothing past 6 months; 117 rows carry no timestamp (→ 37.B.3) |
| new_entity_unmatched | checked | **41 untracked live LV storage-named entities**, incl. a 2026 wave |

**Discovery find (report-only, CANDIDATES not projects — 37.B.2 owns review).** BESS 1/2/3/5/6/33, AVER Energy Storage 1–4, THE BATTERY STREET 2–8, Krustpils/Dobele/Pupuķi/Ķauķi BESS, BESS Parks Latvia, BESS Park Jelgava, Latgale/Baltic Energy Storage. First sweep returned 56, mostly noise (`debess` = sky, a surname, a family doctor); tightened to word boundaries for short tokens, morphological prefixes kept for Latvian → 41.

**Write step: secret never entered this shell.** CC held no `UPDATE_SECRET`; it lives in `/opt/kkme/config/.env`, which the VPS crons already source. `run-lifecycle.mjs --emit-payload` writes exactly the body `--write` would POST, and the POST was executed on the VPS. Same payload, one fewer place the secret has been. **Correction to the Pause-A report: CC had earlier reported the secret simply unavailable — that check was broken (a zsh glob miss aborted the command before it read `.env`).**

**`/health.fleet_lifecycle` before → after:** `{detectors:{}, status:"never_run", transition_log_size:0, digest:not_armed}` → 7 detectors, 5 capable / 7 total, `status:"degraded"` (2 not healthy, correctly), `transition_log_size:0`, `digest:{armed:true, cron:"30 7 * * 1", status:"armed_never_sent"}`. `appended:0, rejected:[]` — zero proposals meant zero transitions, exactly as signed off.

**B8 answers, built because they did not exist.** (1) `/health` no longer trusts a stored status forever — it ages `last_run_at` against each detector's own `max_age_hours` and overrides to `stale`, and treats "healthy with no run stamp" as `never_run`. Without it a stopped runner leaves every detector reading healthy while its stamp ages: B-048's self-exempting shape. (2) `applyEligibility` — a detector passing its invariants over ZERO eligible rows is BLIND, not healthy.

**Digest = egress path, leak-tested like a route.** Private-field transitions withheld and COUNTED; contact-shaped content anywhere in the rendered payload BLOCKS the send without stamping `last_digest_at`. Proven failable: injected leak → **6 red**, removed → green, worker `sha256 b289c737…dc18` identical either side.

**Arming gate widened.** B-046's drift test compared the constant against `wrangler.toml` only — a cron declared in both and handled in NEITHER would have passed and silently never sent. The new test drives the real `scheduled()` handler, asserts a message left and stamped, and carries a vacuity guard. Proven failable: `sha256 9cad961c…d473` identical either side.

**Two tests changed, both called out (B7), neither weakened.** `distinguishes a genuine quiet week from a broken one` — 37.B fixture seeded `{status:'healthy'}` with no `last_run_at` and asserted "genuine quiet week"; that record is `never-run`, so the FIXTURE was sharpened and the inverted case asserted alongside (B-036 precedent). `reports the digest as deliberately unarmed` — asserted the unarmed state through the live constant; now state-aware, with the status vocabulary asserted as arithmetic so neither arming nor disarming makes it vacuous. B-046's author flagged this exact trap in a comment.

**Gates.** Suite 1945/1945 · route-level `/revenue` vs main, frozen KV, 54 configs → **54/54 identical** (re-run at each commit) · live `/revenue` post-deploy **54/54 identical** across two same-UTC-hour captures · eslint 0 errors, worker warnings **29 = 29** vs main, new files 0 · `docs/_private/` never staged (gate PASS before and after) · C8 honoured: polled until two consecutive `/health` reads agreed before concluding.

**Honest gap in the deploy evidence:** no PRE-deploy LIVE `/revenue` capture exists for this deploy, so the live leg is a post-deploy determinism check (two captures, same UTC hour) rather than 37.H1's before/after pair. The byte-identity claim rests on the route-level probe vs main, which ran green at every commit.

**Open, and it matters:** the RUNNER is still manual — nothing schedules it. Detector stamps age from here; `queue_disappearance` and `press_negative` pass their 336h ceiling around **2026-08-15**, at which point the digest correctly starts reporting them `stale`. That is the staleness surface working, not a regression. Host decision stands: GitHub Actions, VERT/lv_press routed through the worker or a published endpoint per 37.B.3. **SSH-to-prod is NOT being added as a secret surface** (operator-confirmed).

**First digest firing: Monday 2026-08-03 07:30 UTC = 10:30 Europe/Vilnius (EEST, UTC+3).**
...
**Checkpoint artifact:** `docs/investigations/2026-07-31-phase-37-a-coverage-report.md` (generated, not hand-written). Pause A: `docs/investigations/2026-07-31-phase-37-a-pause-a.md`.

**Privacy held throughout.** `docs/_private/` unstaged, untracked and ignored at every commit — asserted by `scripts/assert-no-private-staged.sh`, which is now a gate. No contact, comment or APVA value appears in any commit, fixture, test output, report or payload. All fixtures synthetic (`example.invalid`). The generated private payload writes to a gitignored path.

**Five prompt premises corrected at Pause A, all file-verified:** 141 projects not 143 · LT `MWH` column empty on all 84 rows · **APVA is a binary flag (`Gavo` x55 / `Negavo` x29), not scheme identifiers** · unit chaos bounded to 24 cells of 141 rows, not pervasive · AST's "polite UA workaround" is dead (403 to polite AND browser UA; Cloudflare bot-management + CAPTCHA — not evaded, route is the AST relationship).

**APVA got its own investigation and no schema assumption.** Balanced 14-row sample against `esinvesticijos.lt` (the national EU-beneficiary register): `Gavo` 0/8, `Negavo` 0/6. The symmetry is the finding — a source that answers identically for both arms cannot verify the column, whatever the reason. APVA's own published schemes are household-scale. Stored **opaquely** as `apva_flag`, private tier, never published, never scored, per the arc's own private-until-corroborated rule. **Open question for you: which APVA scheme does `Gavo` refer to?** One sentence decides whether 55 LT rows gain a citation source or stay private testimony permanently.

**Coverage: 0/141 public-confirmed — and that number measures wiring, not verifiability.** `esinvesticijos` is a working endpoint (`kaupimo`→57, `Ignitis`→3 discriminate) and its 84/84 zeros are real. Everything else is deferred or excluded. Three registries that demonstrably serve data are build spikes, not dead ends: **BIS** (33.A.2.b was a resolver gap, not a data gap — confirmed), **registrucentras** JAR, and **EE `avaandmed.ariregister.rik.ee` bulk open data** (the strongest of the three — free bulk download, no scraping).

**Correction to my own Pause-A table, logged as a correction (B9).** I marked Lursoft "Viable" on a 200 whose body contained `SIA` and an LV reg number — both true of the page, neither meaning search worked. The run's 0/36 across LV legal entities was implausible enough to check: the endpoint returns a byte-similar ~107 kB page for every term, never echoes the query, and its one reg number is Lursoft's own. Control case: **`Latvenergo` returns the same page as a nonsense string.** Those zeros were a broken probe, not evidence — reporting them would have told you 36 real Latvian SPVs have no registry trace. Now excluded and unprobed. **Rule extracted: a source is viable only if a known-good control returns something different from a nonsense term.** Same technique that made APVA decisive; I applied it there and not here.

**Match rates:** LT 84/84 · LV 0/42 (41 new-to-us — consistent with only 6 LV entries in the public fleet) · EE 2/15 matched, 5 probable. LT's 100% is genuine (61 exact name matches; the tail was hand-checked twice).

**The finding with teeth for 37.D — the public fleet counts hybrid grid connections as batteries.** For pure-BESS rows the public `mw` tracks the battery rating (40/45); for hybrids it tracks the **site** grid connection (40/47). Across matched hybrids: private BESS components **1320 MW** vs **4572 MW** carried publicly — **3.46x, +3251 MW**. The arc assumed the feeds under-count hybrids; in MW terms they **over-count** them. Supply feeds `sd_ratio` feeds cannibalisation feeds IRR, so this is not cosmetic. Generated into the coverage report, not asserted in prose, so 37.D re-derives it from the artifact (C4). **HYPOTHESIS** — rests on the private BESS column (operator testimony, unverified) and covers only the matched subset.

**Two defects in my own match engine, found by auditing output rather than by tests (B4).** All 39 unit tests passed while `bareName` stripped only *leading* legal forms (the public fleet writes `Name, UAB`, the workbook writes `UAB "Name"` — exact matching never fired), and 2-char tokens were filtered so `Anykščiai PV` and `Anykščiai BS` both collapsed to the placename and matched *each other* while the correct counterpart scored lower. Both surfaced from asking whether 89% was too good, then whether 100% was too good as well.

**Leak tests proven live, not merely green.** Injecting a real leak into `/s2/fleet` (merging the overlay into the public response) turned both leak tests red; removing it turned them green; worker verified byte-identical after. Also fixed a vacuity hole in my own test — the public-route sweep originally ran against an empty KV, so the fleet routes answered `{"fleet":null}` and the sweep asserted that an empty response contains no secrets. It now seeds a synthetic public fleet and carries an explicit vacuity guard.

**Architecture note:** `tools/fleet-intel/lib/tiers.mjs` makes **public an allowlist, not a denylist** — a field nobody declared public is private, so a new parser field cannot leak by default. Verification tier is **derived** from the evidence array, never assigned; zero cited sources ⇒ `private-only` by construction. Two detectors: by field name at any depth, and by **content** (email / Baltic phone shapes) so a renamed field is still caught.

**A7:** `s4_fleet` = 6 writers + 9 readers in-repo + 1 tool, **plus `kkme_sync.py` out-of-repo — true writer count 7, not 6.** 37.A does not write `s4_fleet` at all. `fleet_private:*` and dossier keys had 0 pre-existing references.

**B10:** `POST /admin/fleet-private` refuses a batch smaller than what is stored unless `allow_shrink` is set.

**GIT STATE — needs your decision (C1, rule #5).** `main` arrived diverged: local `dabb549` unpushed, 5 origin commits unmerged. I branched off `origin/main` and did **not** touch `dabb549`, because it edits the operator-owned roadmap. **`origin/main` therefore still lacks playbook rows B9 and B10** until you land it. Recommend landing it separately from this phase's PR. The worktree also held 11 modified + 26 colliding untracked files; I compared every one byte-for-byte first — 32 were already identical to `origin/main`, 5 carried unique local content (the known B-043 fetcher churn) and were preserved and verified.

**STOPPED AT THE CHECKPOINT. 37.B not started** — the coverage report shapes its lifecycle rules, per the prompt.

### Session 96 — 2026-07-30 — B-036 (settled activation-price source) + 36.E0.1 (evidence-base refresh automation) (Claude Code, autonomous batch)

**Branch `phase-36-b036-activation-source` off `9aa2fbc`. No engine, worker or public-site change — `git diff main -- workers/ app/` empty and gated in CI and in the refresh workflow itself.**

**B-036 verdict: a settled German activation-price source exists. E0's "A84 empty for DE" was an EIC-choice artefact.**

`netztransparenz.de` — the scoping target — publishes no settled activation-price *series*. It publishes the derived reBAP and the AEP modules, and its own AEP-Module page states in prose that the aFRR/mFRR VWAPs feeding AEP Modul 1 are published on ENTSO-E under "Balancing/Prices of Activated Balancing Energy" — `documentType=A84`. The scoping target pointed at the answer rather than holding it. Germany publishes A84 per **TSO control area**, not against the DE-LU bidding zone; isolated one parameter at a time, `controlArea_Domain=10YDE-VE-------2` returns data on the same day that the BZN, the DE CTA and the German block all return "No matching data found".

**Acquired:** `tools/consultancy/data/mature-markets/activation/` — 286 135 rows, 5.3 MB gz, 16 year shards. DE aFRR 225 953 rows (2022-06-21 →), DE mFRR 685, AT aFRR 54 117 (2025-08-31 →), AT mFRR 5 380 (2023-06-13 →). New `price_basis: vwap_activated`. Total committed evidence base 72 MB → 77 MB.

**What E2/E3 now calibrate activation on, and the four things that constrain it:**
1. **No usable accession before/after.** The German series *starts* at its own PICASSO accession (first rows 2022-06-21, first full day 2022-06-22 — the primary-sourced calendar date). Austria was acquired believing it spanned the break; under the standard-product definition its aFRR starts 2025-08-31 (three years after its own accession) and its mFRR 2023-06-13. The only pre-accession sample anywhere in reach is **49 Austrian mFRR quarter-hours against 5 331 after**. Any activation-break magnitude must come from the capacity evidence, labelled as such.
2. **German mFRR activation is rare** — 685 activated quarter-hours in four years, frequency 0.002-0.004. No mFRR activation-revenue term should carry material weight on it.
3. **No activated volumes.** A84 carries price only; activation revenue is price × energy. Route found, unauthenticated and fetched at pin time — filed as **B-041**.
4. **Break applicability is not asserted.** The calendar records a break's market and products but not which *mechanism* it affects, so the DE aFRR activation series is split at 2025-09-01 by the ČEPS/ALPACA event — a **capacity** cooperation. The measured step is large (segment mean 199.66 → 95.34 EUR/MWh) and must not be read as an ALPACA effect without an argument that ALPACA touches activation at all. Flagged in the summary table, decision left to E2.

**Four data traps found by measurement, three of them my own errors corrected mid-phase:**
- **`standard_MarketProduct=A01` is load-bearing, not optional.** A DE mFRR document carries four TimeSeries: two declaring the standard product with every point non-zero, and two declaring none that pad every unactivated quarter-hour with **price 0**. Dropping the filter to widen Austrian coverage (which I did, before understanding why it existed) admits ~190 fabricated "activated at €0" quarter-hours per day per product.
- **curveType A03 is block compression.** I first emitted one row per point and asserted contiguity on the strength of a single day of aFRR. DE mFRR publishes positions 1, 3, 5 across a five-quarter-hour Period, so that reading covered 45 of 75 minutes — 4 465 losses in the first acquisition. Points are now expanded across their block with `extra.block_isps`, and the asserted invariant is that rows **tile** each Period exactly (measured: 0 anomalies).
- **DE and AT publish the same dataItem in two styles.** Germany: one short Period per episode, 0.1 % zeros. Austria: a dense step function where 0 is the resting value — 5.1 % of AT aFRR points, 61.6 % of AT mFRR. Zeros are dropped and counted **per series** (AT mFRR: 214 090 dropped against 5 380 kept), with the limit of the justification stated — A84 has no volume column, so unlike the Swedish case there is no second column proving a zero means absence.
- **Amprion publishes nothing.** Three of four German TSO control areas publish byte-identical values; Amprion returns no rows for every reserve type. A probe landing on Amprion reproduces E0's false negative by a different route. `--verify-publishers` re-measures it (verified: 0 differing ISPs).

**Independent check (B5):** the series is verified against a *different publisher of the same settled quantity* — netztransparenz's own AEP-Modul-1 CSV, committed as a fixture — not against another view of the same response. 78 of 92 priced quarter-hours agree to the cent (65 up, 13 down; Modul 1 follows the imbalance direction). The 14 that differ sit on the Modul-1 side, which is a derived module with its own combination rules; the residual is **not explained** and is not claimed to be. Also hand-checked from raw committed bytes bypassing the loader: DE aFRR 2026-01 up=2552 down=2196 reproduces the manifest exactly, and 2026-01-05T00:00Z = 139.25 matches the netztransparenz CSV.

**B-038 piggyback confirmed, nothing built.** `A15/B95/A01` still serves LT procured balancing capacity (A51 204, A47 724 instances) with NL/BE/AT/CZ/FI as live positive controls in the same run; DE/LV/EE empty. Routed to 36.C's arc as a one-line note per the prompt.

**36.E0.1 — periodic refresh as a scheduled GitHub Actions workflow.**

Pause-A viability verified rather than assumed: Actions enabled, `workflow` scope on the token, repo **public** so minutes are unmetered, `ENTSOE_API_KEY` added as a repository secret (reversible with `gh secret delete`). Both ENTSO-E fetchers now prefer `process.env.ENTSOE_API_KEY` so they run unchanged where there is no `.env.local`. **No `pull_request` trigger**, so a fork PR can never run the job with the secret in scope; a test asserts that, plus no `set -x` and no echo of the variable.

Shape: monthly (`0 3 1-7 * 0` = first Sunday 03:00 UTC), per-source incremental, **PR-based and never a push to main**. Six in-workflow gates: all sources served · append-only · checksums+schema · summary table rebuilds · summary table deterministic · human-owned files untouched. `comparability.md` is human-owned and the gate measures what *this run* changed (snapshot before, compare after) rather than what the working tree already carried.

**Running it against Sweden for real found four data-destroying bugs that every checksum passed.** SE went **146 733 → 15 261 rows**; the append-only gate reported 26 277 removed.
1. **The year shard is not a safe boundary.** SvK publishes local wall clock stored as UTC, so a `2026-01` window starts at `2025-12-31T23:00Z` — inside the 2025 shard. Fixed by `reconcileShards()`, which merges row-wise and derives the window from the new data's own earliest row, so it is immune to the local-versus-UTC reasoning that caused the bug.
2. **The manifest's `files` list forgets.** A windowed run listed 2 shards beside 7 files on disk; `loadDataset` reads the manifest, so the dataset shrinks silently. Fixed by `mergeManifestFiles()`, sha256-verified before carrying anything forward.
3. **The manifest's *metadata* forgets — the worst of the four.** The windowed run replaced `coverage_verification`, dropping SE's `all_zero_rows_dropped: 5137` and its 74-month table: E0's evidence about *how that source lies*. The prices would still have been right and the reason to trust them gone. Fixed by `preserveAcquisitionMetadata()`; only a `full`-window source may replace a manifest wholesale.
4. **The manifest was not written on anomaly**, leaving disk and manifest disagreeing. Now always written; only `last_successful_refresh` is withheld.

After the fixes the same Swedish refresh reports `no_change`, all six gates PASS, 146 733 rows and 7 files preserved, coverage evidence intact, and **not one shard byte changed**. Each of the four has a regression test, and each exists because the bug happened rather than because it was imagined. The SE dataset was restored from HEAD after every destructive trial.

**Staleness surface (B8):** `check-freshness.mjs`, wired into `scripts/diagnose.sh` and `npm run evidence:freshness`. Stale = older than **two** cadence cycles; `never_refreshed` is reported and does **not** fail, so adding the check does not turn the pre-automation state of the base into a red light. Deliberately not a vitest test — a gate that goes red purely because time passed would go red on unrelated PRs and get ignored.

**Also fixed in passing:** the fetcher ran `main()` at module scope, so any `import` fired network requests (it happened during development); now guarded, and `parseDoc` is exported and unit-tested offline against a committed single-day A84 document. Bounded concurrency added — the platform ran at ~1 s/request when healthy and ~40 s during a post-maintenance backlog, and six concurrent requests completed in 25.6 s wall against ~220 s sequential, which is what makes the acquisition finish in minutes rather than hours when degraded.

**External blocker hit and ridden out:** the ENTSO-E platform entered scheduled maintenance mid-acquisition (HTTP 503 with an HTML page, on every dataItem including a control) for ~80 minutes, then returned degraded. This is the concrete instance behind the refresh's per-source failure classification (`source_unavailable` / `rate_limited` / `schema_drift` / `network` / `unknown`).

**Gates:** 1697 tests pass (+25 vs 1672 at 36.E0) · `tsc --noEmit` 29 errors, identical to main's baseline · `lint:no-editorial-chips` PASS · `lint:no-raw-spacing` PASS · `git diff main -- workers/ app/` empty · summary table rebuilds offline and deterministically. **No test deleted; three rewritten because they encoded beliefs the measurements overturned, and the rewrites are strictly sharper** (the AT before/after test now asserts the absence it originally asserted the presence of). No deploys.

**Artifacts:** `docs/investigations/2026-07-30-phase-36-b036-pause-a.md` (Pause A, four questions, the full probe record) · `tools/consultancy/mature-markets/fetch-activation-prices.mjs` · `refresh-mature-markets.mjs` · `check-freshness.mjs` · `.github/workflows/refresh-mature-markets.yml` · `docs/research/mature-market-comparability.md` §6-7 rewritten.

**Filed:** B-041 (P2) activated volumes not acquired — recipe pinned and fetched · B-042 (P3) FI activation not acquired, NL/BE stopped publishing.
...
**Branch:** `phase-36-c-reserve-fallback` off `1466a53` · 5 commits.
**PR:** https://github.com/kastiskemezys-tech/kkme-website/compare/main...phase-36-c-reserve-fallback
**DEPLOYED** — worker version `98f721cd-e041-41da-8f24-67fe32fb1d3c`. VPS crons installed.
Audit: `docs/investigations/2026-07-29-phase-36-c-pause-a-source-audit.md`.

---

#### Premise correction (discipline rule #1) — the phase's own opening claim was false

The prompt opened on *"BTD has been host-down since 2026-07-17 — 12+ days."* **It was not.**
From the VPS, BTD returns HTTP 200 with a valid Let's Encrypt cert and complete PT15M
days across the entire supposedly-dark window. The cert's `notBefore` is **2026-07-18
17:40 UTC** — the previous one lapsed around 07-17, AST reissued the next day, and BTD
recovered. KKME did not recover with it.

Had this gone unchecked, the phase would have spent 2-3 days replacing a working feed.
Estimate dropped 2-3 days → ~1.

#### The two real faults

1. **Cloudflare gets a persistent 526** from BTD's origin (confirmed twice, ~40 min
   apart, via a throwaway `wrangler --remote` worker; litgrid 200 on the same runs as
   control). `computeS2` returned `null` **by design** and deferred to the Mac cron.
2. **The Mac cron was dead two ways.** BTD's origin returned it *zero TLS bytes* — no
   peer certificate — while serving the VPS normally from the same IP. And the crontab
   line could not execute at all: `/Users/Kastis/kkme/logs/` no longer existed, and the
   shell opens `>>` before it execs, so `node` never started.

Each leg was written to assume the other was covering. Neither was.

#### What shipped

- **VPS is now the S2 primary** — `scripts/vps/fetch_btd.py`, 4-hourly, with a pre-flight
  validation gate (all-null window / truncated fetch / moved column layout rejected
  before the worker sees them).
- **Admission control on every S2 write**, keyed on `data_window_end` — the last BTD
  delivery date a payload was computed from. Freshness wins outright; source rank only
  breaks ties. Three near-identical write blocks collapsed into one commit path.
- **`_meta.source` now names the serving leg** (`vps` / `worker-direct`).
- **Worker-direct leg retained** as opportunistic secondary — self-heals if the 526 clears.
- **Mac cron retired**, fetcher archived to `~/kkme-cron/retired-36c/`.
- **Litgrid ordered-capacity scraper deleted** — five ranges spanning 2025-03 → 2026-07
  return zero data cells; `ordered_price`/`ordered_mw` were always null.
- **Country column offsets resolved from BTD's own `header_groups`** instead of hardcoded 10.
- **Cert tripwire** — `scripts/vps/cert_watch.py` daily, `POST /admin/cert-watch`,
  Telegram below 7 days. Worker checks the tripwire is still reporting.
- **299 days of per-delivery-day clearing prices** backfilled into a new
  `s2_daily_clearing` KV (`GET /s2/daily-clearing`), 2025-10-01 → 2026-07-26, **zero
  holes, every day a full 96-ISP PT15M day**.
- **B0-G forecast plumbing fixed across all THREE `da_tomorrow` writers** (the audit had
  named two; `fetchNordPoolDA` is the one that actually populates the KV).

**Result: S2 went from 286.9h stale to 0h.** `_meta.source: "vps"`, `validation_passed: true`.

#### Scope correction on the backfill

The brief assumed only 07-17→07-29 was recoverable "while BTD's export window covers
them". **That premise does not hold** — BTD serves complete days back to 2025-10-01
(2025-09-15 and earlier return all-null rows). The 12 days were never at risk, and the
same effort yielded 299 days instead. The 12-day gap is included and complete.

#### mode=forecast serves for the first time — and doing so exposed two defects in it

Verified live at both durations (`?dur=2h` → €180/MW/day, `?dur=4h` → €237/MW/day,
`sources: ["KV:da_tomorrow","KV:s2_rolling_180d"]`).

Getting there required more than the plumbing fix:

- **NordPool's endpoint now returns HTML, not JSON** (`data.nordpoolgroup.com/api/v1/auction/prices/areas`),
  verified from two independent networks — an upstream change, not an egress problem.
  Both original `da_tomorrow` writers depended on it. Meanwhile `computeS1` was already
  computing tomorrow's day-ahead from ENTSO-E and storing it at `s1.da_tomorrow` — a
  **different key the forecast consumer never reads**. The working source and the
  starving consumer were one key apart. Now mirrored.
- **`synthesizeBTDFromRolling` indexed the DA curve as `daP[floor(i/4)]`** — correct only
  for a 24-slot hourly array. At PT15M that reaches index 23 of ~96: the **first six
  hours of the day**, stretched across all 96 ISPs, with the evening peak never seen.
  Latent only because `prices_24h` was never populated. Replaced with a proportional map.
  Evidence it mattered: 2h activation revenue went 0 → €17/MW/day.
- **Delivery date was read from `daTomorrow.date`, a field no writer produces**, so the
  forecast labelled itself with tomorrow's date regardless of which day's prices it held
  (discipline rule #2). Fixed and verified live.

#### Open — NOT fixed here, deliberately

- **`capacity_eur_day` is 0 in forecast mode.** `s2_rolling_180d.products.*.cap_avg` is
  `null` — the Phase 33.B finding that the engine reads a never-produced field and falls
  back to calibrated constants. 33.B chose not to remap it to avoid re-introducing an IRR
  swing; reversing that mid-phase is not CC's call. **Routes to 33.B.2 operator review.**
- **`peak_offpeak_ratio_decimal` reads negative** (−20.32 / −38.23). It divides *revenues*
  and off-peak revenue is negative by construction, so the arithmetic is right and the
  name is misleading. Changing it moves a public number → operator decision.
- **`/revenue` currently reads `irr_2h` 22.35%, `min_dscr` 2.39, bankability Pass.**
  The S2 refresh is a deliberate input change (fcr 56.33 → 41.25, afrr_up 48.31 → 32.69 —
  real market compression over the dark window, visible in the backfilled series). **I did
  not capture a pre-refresh IRR baseline, so this delta is unattributed** — 36.B's arc also
  deployed today and several of its commits explicitly moved public numbers. Worth an
  operator eyeball rather than an assertion of stability I did not measure.

#### Filed

- **Phase 36.E** — `docs/phases/phase-36-e-entsoe-new-api-prompt.md`, LOW priority. The
  ENTSO-E sweep found nothing for the Baltics **but the positive control failed too**
  (NL, DE, BE, FR, AT all empty, 2024 windows included, on a token returning A44 fine).
  Verdict is *"this API surface serves nobody"*, not *"the Baltics don't publish"*. B1's
  conclusion holds operationally; its stated reason does not.
- **AST contact draft** — `docs/drafts/2026-07-29-ast-btd-contact.md`. Operator to send.
- **DECISIONS.md** — cron redirects open before the command runs; grep crontabs before
  removing a directory.
- **Pinned for 36.D** (requested mid-audit): Litgrid flexibility needs assessment
  `.../sistema/lankstumo-poreikiu-vertinimo-ataskaita/36615`. Flag: the prompt's
  "973 MW / 3.12 GW" figures do not obviously match its public summary (4.36 → 7.13 GW
  total flexibility demand). Not assumed interchangeable.

**Tests:** 1570 pass (26 new, fixture-pinned on a real recorded BTD response).
...
**Why.** Phase 33.B's `[revenue/s2-capacity-watch]` log (`flagOutOfBandS2Capacity`, every `/revenue` compute) was `console.log`-only → ephemeral. 33.B.2 (capacity-basis review, 2026-06-29) needs ≥2 weeks of trend data to judge whether `fcr_avg=€63.73/MW/h` is a transient post-CE-sync spike or sustained. Persist it to KV.

**Shipped.** (1) `accumulateCapacityWatch(prev, s2, nowIso)` — pure helper, deduped per distinct `s2.timestamp`, tracks `min/max/last/above_50_pct` for all 5 fields (fcr + afrr_up/down + mfrr_up/down). (2) `persistCapacityWatch(env, s2)` via `ctx.waitUntil` (off response path), key `s2_capacity_watch:<YYYY-MM-DD>`, 30-day TTL. (3) `GET /admin/capacity-watch?days=N` (default 14), `X-Update-Secret` auth. (4) `:2871` `prices_source` `'BTD measured'` → `'BTD parsed; calibrated capacity (review pending)'` (rule #4).

**Pause A refinements (empirical, rule #1 — 5th phase running).** Two corrections to the prompt's design: (a) **KV key uses colon** `s2_capacity_watch:<date>` — codebase date-series convention is colon (`raw:s1:<date>`, `trading:<date>:raw`), not the prompt's underscore guess; (b) **throttle by `s2.timestamp` dedup** — s2 KV refreshes ~6×/day (4h Mac cron), so per-`/revenue`-call writes (prompt's "100-10000/day") would be quota-risk noise; deduping makes `samples` = distinct snapshots (~6/day, the meaningful unit) and bounds writes to ~6/day. Operator added down-direction tracking (33.B.2 may want up-vs-down or asymmetric treatment).

**Post-deploy verified.** `/revenue` prices_source = relabel, IRR 20.0% stable, caps 7.06/19.74/0.36 (no engine change). `GET /admin/capacity-watch?days=1` returns today's summary (all 5 fields, clip_events_count 1). 401 w/o secret. **Dedup proven:** ~15 `/revenue` calls → `samples:1` (one distinct s2 snapshot). Origin-SHA matched before deploy.

**Process change (operator, memorialized).** From next phase, CC's Pause C runs `npx wrangler deploy` **directly** after the origin-SHA check passes — no operator-runs round-trip. Saved to memory `feedback_cc_runs_wrangler_deploy`.

**Out of scope (untouched):** 33.B.2 capacity-basis review (2026-06-29, now has KV data to read); 33.A.2 installed_mw frozen pipeline (operator's next pick).

### Session 80 — 2026-06-15 — Phase 33.B: BTD capacity-price documentation + `prices_source` relabel (Path C) (Claude Code)

**Branch:** `phase-33-b-btd-cap-reparse` off main. **Commit:** `ec49286`. **Worker deploy:** `82df190f-aa4d-44bf-aac4-4745ab327988`. **Three pause points.** One-file change: `workers/fetch-s1.js` (+20/−2), **zero behavioural change**. Evidence: `docs/visual-audit/phase-33-b/EVIDENCE.md`.

**Pause A finding (the substance) — 4th consecutive rule-#1 premise correction.** Traced the BTD→S2 path end-to-end: BTD fetcher is a **Mac cron** (`~/kkme-cron/fetch-btd.js`, every 4h), which fetches `price_procured_reserves` (BTD's **capacity** dataset) and routes the raw payload to `/s2/update`, where `s2ShapePayload()` (`fetch-s1.js:4565`) extracts cols 10-14 into `s2.{fcr_avg, afrr_up_avg, afrr_down_avg, mfrr_up_avg, mfrr_down_avg}`. **These are directional capacity-reservation prices €/MW/h, NOT activation prices €/MWh.**

**⚠ Premise-correction footnote (Phase 33 diagnosis was off, fix still correct).** Phase 33's "activation/capacity conflation" narrative was empirically wrong: `afrr_up_avg` is a *capacity* price from `price_procured_reserves`, not an activation price. The actual bug was — *the engine reads `s2.*_cap_avg`, which NO parser path ever produces; the `?? *_up_avg` fallback was a misnamed band-aid for a permanently-missing field.* Phase 33's `capPrice` bound still landed correctly (calibrated constants keep IRR in-band) — right behaviour, clearer mental model now: calibrated capacity until operator review, not "energy-vs-capacity category error." (This is the 4th data point for `feedback_prompt_premises_empirically_verified.md` — Pause A doing exactly its job.)

**Why Path C (no remap), operator-confirmed.** BTD *does* carry capacity prices and the parser *already* extracts them — but live they run **2–177× the calibrated constants** (afrr_up 72.7 / mfrr_up 38.9 / fcr 63.7 vs 7.06 / 19.74 / 0.36; FCR especially anomalous, €551k/MW/yr-implied = implausible). A mechanical `*_up_avg → *_cap_avg` remap would re-introduce the silent IRR swing Phase 33's bound exists to prevent. So: document the truth at the `capPrice` fallback site, relabel `prices_source` honestly, keep constants canonical pending operator sign-off.

**Shipped.** (1) Multi-line comment at the `capPrice` fallback (`:1062`). (2) `prices_source: 'BTD partial'` → `'BTD parsed; calibrated capacity (review pending)'` (computeRevenueV7 + V6). No new test; `capPriceBound.test.ts` keeps guarding. **Post-deploy verified:** `/revenue` prices_source = new label, signal_inputs still 7.06/19.74/0.36, **project_irr 20.0% identical before/after** (behavioural-no-op); `/s2` `*_cap_avg` documented-null, `afrr_up_avg/fcr_avg` unchanged. Origin-SHA matched before deploy.

**Follow-ups filed:**
- **33.B.2 (expanded)** — one coherent operator review of all three products (FCR + aFRR + mFRR) post-CE-sync capacity-pricing basis, **2026-06-29** (~2wk clock starts now). Not three separate decisions.
- **33.B.3 (separate, ~30 min, own A-B-C)** — replace the ephemeral `[revenue/s2-capacity-watch]` `console.log` with a KV accumulator (`s2_capacity_watch_{YYYY-MM-DD}` = `{date, fcr_avg, afrr_up_avg, mfrr_up_avg, clip_events_count}`, 14-day retention) so 33.B.2 can reason about persistence vs transient spikes. Different risk class (KV write + admin endpoint + test) → its own ship, not bundled.
...
**Out of scope (untouched):** the remap itself (deferred to 33.B.2 decision); 33.A.2 installed_mw frozen pipeline (still blocked on operator's projects sample).

### Session 79 — 2026-06-15 — Phase 33.A: Baltic map-feed allowlist + reject-on-flag + purge endpoint (Claude Code)

**Branch:** `phase-33-a-baltic-allowlist` off main. **Commit:** `7619785`. **Worker deploy:** `2306c828-8728-4e97-b2b6-525e3b4b79a2`. **Three pause points.** Two-file scoped change: `workers/fetch-s1.js` (+85/−4) + new `app/lib/__tests__/fleetAllowlist.test.ts`. Evidence: `docs/visual-audit/phase-33-a/EVIDENCE.md`.

**What shipped.** Worker-side defence at the last hop before the public Baltic BESS map:
- `BALTIC_COUNTRIES = {LT,LV,EE}` single-source constant (rule #4). Blank-country drops too (verified safe — only the ESN scraper emits blanks, all foreign).
- `filterFleetEntries()` gate: rejects non-Baltic country **OR** HIGH-severity contradiction flag (C-01/C-07); C-11 (MW>500, MEDIUM) kept. **Gated at POST before `processFleet`** (not at the old `:239` quarantine point) so aggregates compute on survivors; Phase 12.10's quarantine machinery left intact-but-inert (0 HIGH entries today → no regression).
- `POST /s2/fleet` wired to the gate — per-entry drop (never 400s a batch), returns `{accepted, dropped_non_baltic, dropped_flagged}`. Refuses (400) only if **all** entries filter out (won't overwrite KV with empty fleet).
- `POST /admin/purge-non-baltic-fleet` (`X-Update-Secret`) — one-shot retroactive cleanup, recomputes aggregates from survivors, idempotent.

**Count delta.** `/s4` 196 → **170** projects (purged 26). After: LT:147 / EE:21 / LV:2, zero non-Baltic. Purge response `{purged:26, remaining:170}`; auth-guard 401; idempotent re-run `{purged:0}`. Live ingest gate verified: synthetic 170-Baltic+1-PH POST → `{accepted:170, dropped_non_baltic:1}`, PH dropped, fleet self-restored.

**⚠ Prompt-premise correction (rule #1).** Prompt assumed "allowlist is fleet-scoped, doesn't touch revenue / IRR stays 18.5%." **False.** The 26 polluters (4967 MW, all `announced`) were inflating `baltic_pipeline_mw` (19034 → 14067 after purge; delta == Σ purged MW exactly). Pipeline feeds `sd_ratio` (2.39 → 1.86) → trajectory CPI (0.30 → 0.33) → **`/revenue` project_irr 18.5% → 20.0%** (still v7.3). `baltic_operational_mw` unchanged (331) confirms operational/spread inputs untouched, so this is the purge *correcting a polluted revenue input*, not daily drift and not a regression. The foreign pollution was never cosmetic map dots — it was corrupting the supply/demand model. **Operator: the headline IRR moved 18.5%→20.0% as a data-quality consequence; confirm any published IRR figure is updated accordingly.**

**Blank-cod root cause = (c) upstream → filed Phase 33.A.3.** All 196 entries (LT included) carry empty `cod` (`""`×189 / `null`×7). The worker never writes/normalises `cod` — POST stores entries verbatim; `cod` is only *read* (sort comparator `fetch-s1.js:~147`, hardcoded EE entries). So blank cod = upstream `kkme_sync.py`/esn never populates it. No worker change this phase. **33.A.3 (upstream, Cowork-side):** populate `cod` in the feed — would also improve the `:147` "specific-date sorts first" comparator quality (currently every entry ties on blank cod).

**Notes for next Cowork pass (not phases):**
- CLAUDE.md "Worker: never cat whole file (7740 lines)" is **stale → 9394 lines**. Small inline fix.
- Single-entry endpoint `POST /s2/fleet/entry` (`:~7165`) is **not** gated by the allowlist — only the batch full-replace POST is (the real kkme_sync ingest path). Minor surface; gate it too if manual single-entry upserts ever resume.

**Out of scope (untouched):** 33.A.2 installed_mw frozen pipeline (needs operator's 5-10 project sample); 33.A.3 upstream country filter; Meralco TSO mislabel (moot — dropped by allowlist).

### Session 78 — 2026-06-15 — Phase 33: revenue capacity-price bound + VPS dash/cron fix + s1_capture dual-writer close (Claude Code)

**Branch:** `phase-33-engine-drift-and-feed-quality` off main. **Commits:** `a761893` (bound + regression test), `6f3d9cb` (dual-writer lock). **Worker deploys:** `0972c7c4-…` then `06b9d774-…`. **Three pause points; map feed (A.3) split to Phase 33.A.**

**Pause A diagnosis (code-grep + curl + VPS-log + git triangulation):**
- **Revenue (IRR 49.6% on prod):** the prompt's lead (`per_product_at_cod.fcr.sd_ratio=28.5` vs fleet 2.69) is a **confirmed red herring** — `cpiCurve()` (fetch-s1.js:2037-40) floors at 0.30 for any high S/D, so `clampSdRatio` would be a **no-op on IRR** (rule #1 win). Real lever: capacity-price source gap — `s2.afrr_cap_avg` absent (Litgrid "ordered capacity" page emptied, "data moved to BTD", per `logs/btd.log`), so every site fell back `*_cap_avg ?? *_up_avg ?? const`, substituting the aFRR **activation** price (`afrr_up_avg=37 €/MWh`) for a **capacity** price (~7 €/MW/h) → ~3× balancing capacity revenue → gross €13.65M. `v7.3` confirmed serving.
- **VPS cron (baltic_index 457h stale):** ROOT CAUSE = `/bin/sh → dash`, crontab has no `SHELL=`, the 4 inline crons use `source` (a bash builtin **dash lacks**) → `.env` never loads → `KKME_DB_URL`/`DATABASE_URL`/`ENTSOE_API_KEY` unset → scripts error. **Not** Session 73's mode; Session 73 misdiagnosed it as ".env orphan lines" and its `&&→;` change was necessary-but-insufficient (only made the failure visible). Wrapper crons (`cron_daily.sh`, `#!/bin/bash`) worked, masking it. (Memory: `vps-cron-dash-source-bashism`.)

**Pause B build + Pause C ship:**
- **Revenue bound (B.1):** single-source `capPrice(product, observed)` (fetch-s1.js:1025-32) — constants `{afrr:7.06, mfrr:19.74, fcr:0.36}`, ceiling clamp `[0,50]` + clip log; never an activation price. Wired into 6 served sites + V6. `fcr_avg=63` dropped (>50). Added `flagOutOfBandS2Capacity()` watch log + `capPriceBound.test.ts` (8 cases) + named exports. `clampSdRatio` NOT added.
- **VPS fix (B.2):** `SHELL=/bin/bash` + `source`→`.` on all 4 inline crons (validated before install), backup `crontab.bak.20260615`. Caught up all 4 crons (exit 0, no env errors). `/health`: baltic **457h→0h**, s2→fresh.
- **Deploy regression + recovery (Pause C):** deploy 1 fixed the IRR but flipped to **v6_fallback** (12.6%, bankability "Fail") because the `ingest_daily` catch-up **clobbered `s1_capture.monthly` (12→0 months)**. Recovered via worker `POST /admin/trigger-s1-capture` (rebuilds `.monthly` from the 400-day `s1_capture_history`, which the VPS push can't touch) → v7.3 restored.
- **Dual-writer close (deploy 2):** worker `computeCapture` and VPS `ingest_daily` both write `s1_capture`; masked 19 days by the broken cron. Removed `s1_capture` from the `/kv/set` allow-list (fetch-s1.js:9217) → worker sole writer. Verified: `POST /kv/set key=s1_capture` → **400 not-in-allowlist**; `/revenue` still v7.3.

**Final prod state (verified):** `project_irr=0.1852` (**18.5%**, inside ch_benchmark 6-31%, just above central 16.6%), `equity_irr=32.4%`, `moic=5.15`, `gross_revenue_y1=€7.32M`, `bankability=Pass`, caps `7.06/19.74/0.36`, `prices_source="BTD partial"`. Evidence: `docs/visual-audit/phase-33/EVIDENCE.md`.

**Caveats for next session:** (1) **Probe calibration** — the local Vitest IRR probe ran ~+9pp hot (showed the broken state ~59% vs live 49.6%); trust prod curl over the probe for absolute IRR. (2) **`prices_source` stays "BTD partial"** until 33.B re-parses `afrr_cap_avg` — the bound is load-bearing meanwhile. (3) **installed_mw frozen pipeline** — see 33.A.
...
**Root cause:** `/opt/kkme/config/.env` edited 2026-05-06 left 3 orphaned secret values on lines 12-14 (bare `60e41fd4…` hash, no `KEY=` prefix). When sourced, each runs as a shell command → "command not found" → `source` returns non-zero. The **4 inline crons** chain with `source .env && export && cd && python` — the `&&` short-circuits on the failed source → python never runs → no log file ever created (redirect is on the unreached python). The **2 wrapper-script crons** (`cron_daily.sh` with `set -uo` + own-line source; `daily_intel_wrapper.sh` with `source .env || true`) were immune and ran daily throughout — confirmed by unbroken `cron_*.log` + `daily_intel_*.log` mod-times through today.

**Blast radius (4 inline crons, frozen ~May 6 → May 27):**
- `baltic_storage_index.py` (06:30) — `/index/baltic` served April data, 501h stale (threshold 36h)
- `kkme_sync.py --all` (07:00) — fleet/project sync to worker (feeds S4); S4 input went stale → S4 timestamp drifted to 11-24h (the watchdog alerts)
- `hourly_grid.py` (:05 hourly) — genload KV push (hero map gen/load overlays)
- `ingest_daily.py` (07:45) — ENTSO-E historical ingest (s1_capture, revenue_trailing, s2_rolling_180d)

**NOT affected (ran fine all along):** Cloudflare worker-native 4h cron (s1-s9 fresh), `cron_daily.sh` main pipeline (scrapers→loaders→intelligence→sync_to_website→sheets), `daily_intel_wrapper.sh` intel feed.

**Fix applied (VPS-side):**
1. Commented out `.env` lines 12-14: `sed -i "12,14s/^/#/" /opt/kkme/config/.env`; `mkdir -p /opt/kkme/logs`.
2. Manually caught up all 4 frozen crons — all succeeded: baltic index pushed May 2026 (`LT/2h=364.2 LT/4h=382.8`); kkme_sync pushed 189 entries (S/D 2.53, MATURE); hourly_grid genload 200 (LT gen=2285/load=1192; EE transient ENTSO-E read-timeout, self-heals next :05); ingest_daily s1_capture/revenue_trailing/s2_rolling_180d all 200.
3. Post-fix worker `/health`: S4 → 0h (was drifting 11-24h), baltic_index → 0.4h (was 501h). Only `da_tomorrow` missing = pre-publication (ENTSO-E ~13:00 CET; recheck after 14:00 CET).
4. **Hardened crontab** (prevents recurrence): swapped `source .env &&` → `source .env ;` on all 4 inline crons via `crontab -l | sed "s#config/.env &&#config/.env ;#g" | crontab -`. Backup at `/opt/kkme/config/crontab.bak.20260527`. Now a future `.env` typo can't short-circuit the chain — export/cd/python run regardless of source exit code.

**Open follow-ups filed:** (a) **Watchdog coverage review** — baltic_index was 20d/501h stale but didn't page as loudly as S4; a multi-day silent outage should be impossible to miss on return (escalation tier for signals >Nx threshold). (b) **"Trailing 12m: 2 months"** — `ingest_daily` reported only 2 months in the 12-month trailing window; possible PostgreSQL history-depth gap, low priority, investigate later. (c) `.env` lines 12-14 are commented (preserved) — operator may want to identify what the orphaned `60e41fd4…` value was meant to be and either re-key it properly or delete.

**Discipline:** rule #1 (audit-triage) — the watchdog blamed S4, but empirical health-sweep + log-mod-time forensics found the true root cause (a single `.env` line, 20-day blast radius across 4 crons) rather than chasing the S4 symptom. No repo commits this session — pure VPS ops; this entry is the record.

---

### Session 72 — 2026-05-27 — Phase 18.2.2 — Pipeline-bar on-colored-bg label contrast (Claude Code)

**Headline:** Branch `phase-18-2-2-pipeline-bg-contrast` off latest main (`8a4a1f4`). Two-pause-point session, ~2h CC. Cleared the 4 axe-core pipeline-bg violations Phase 18.2.1 deferred (text-primary on `#298672` mint and `#947434` amber chips at light/dark 1440). Scoped 3-token approach in `app/globals.css` + 4-line edit in `app/components/S4Card.tsx`. axe-core delta: 8 → 4 (all in-scope cleared; remaining 4 = 4 irreducible documented exceptions = text-ghost decorative source-footer + white-on-mint chip elsewhere, × 2 modes). **a11y arc complete.**

**Pause A — locate + propose (~30 min):**

- **Probe:** Cloned `scripts/_phase-18-2-1-probe.mjs` → `_phase-18-2-2-probe.mjs` (OUT_VIS=`docs/visual-audit/phase-18-2-2/`). devDep `playwright@^1.x` added via `PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD=1 npm i -D playwright` (browser binaries pre-cached). Re-scan: dark-1440 4 + light-1440 4 (1024/414 returned 0 — render not reached within `networkidle + 800ms`; not a regression).
- **Chip location:** `app/components/S4Card.tsx:466-495` — BESS PIPELINE BAR, 3 flex chips. Mint chip lines 468-476 (`background: var(--teal-strong)`, text `color: var(--text-primary)`). Amber chip lines 477-485 (`background: var(--amber-strong)`, text `var(--text-primary)`). Intention chip lines 486-494 (`background: var(--amber-subtle)`, text `var(--text-primary)` — passes; subtle alpha reads as bg-page).
- **Token defs:** `app/globals.css:101` `:root --teal-strong: rgba(0,180,160,0.75)` → `#038a7b`; `:107` `--amber-strong: rgba(212,160,60,0.75)` → `#a27b31`; `:336` `[data-theme=light] --teal-strong: rgba(19,122,101,0.90)` → `#298672`; `:341` `--amber-strong: rgba(138,102,32,0.90)` → `#947434`. All tokenized (no inline hex).
- **Premise correction (rule #1):** Prompt pick #2 assumed "existing light/white label text" — but chip text was `var(--text-primary)` which **inverts** (light mode: dark text #1b2326; dark mode: light text #d6dbd1). Pure "darken chip" only works in dark mode; in light mode darker chip = worse contrast against already-dark text. Resolution: swap text to non-inverting token + per-mode darker chip-bg.
- **Cross-card audit (rule #4) clean:** Other `--teal-strong`/`--amber-strong` chip-bg uses (`S7Card:143-146`, `S9Card:158-161`, `globals.css:1171`) have no text inside the chip. The text-on-strong-chip pattern is **S4Card-only**. `--teal-strong`/`--amber-strong` also used as text COLOR in S5/S7/S8/S9/HeroMarketNow/IntelFeed → token edit would have cross-card regression risk → recommended scoped tokens.

**Mid-Pause-B token contradiction (Pause B sub-stop):**

- Operator's pick (1) was `color: var(--white)` (NOT raw `#fff`) per CLAUDE.md never-raw-rgba rule. Pre-flight measurement caught `--white` mode-inversion at `globals.css:30,293` (`#e8e2d9` dark / `#1a1a1f` light). With `var(--white)` and proposed darker mint `#1f7c66`: light mode text resolves to near-black on darker chip → contrast 3.36 (regression from 3.61, worse than starting state). Flagged to operator; authorized 3rd scoped token `--pipeline-chip-text = #e8e2d9` non-inverting in `:root` only.

**Pause B — apply + verify (~45 min):**

- `app/globals.css:101-119` (`:root` block, near teal/amber alpha variants): added 3 new tokens — `--pipeline-chip-text: #e8e2d9` (cream, non-inverting), `--pipeline-chip-mint: #016b5d` (dark mode), `--pipeline-chip-amber: #7d5c1c` (dark mode). With Phase-18.2.2 comment documenting scoping + intent.
- `app/globals.css:341-345` (`[data-theme="light"]`): added overrides `--pipeline-chip-mint: #1c705c`, `--pipeline-chip-amber: #7c601c`. Text token NOT redefined (inherits cream from :root — non-inverting by design).
- `app/components/S4Card.tsx:468-485`: 4 swaps — `background: var(--teal-strong)` → `var(--pipeline-chip-mint)` (line 470); `color: var(--text-primary)` → `var(--pipeline-chip-text)` on mint span (line 473); `background: var(--amber-strong)` → `var(--pipeline-chip-amber)` (line 479); `color: var(--text-primary)` → `var(--pipeline-chip-text)` on amber span (line 482). **Intention chip at line 491 explicitly NOT touched** — cream-on-amber-subtle in light mode = 1.20 contrast (would create new violation). Deviation from operator's "3 chip spans" instruction noted; engineering-safer to leave the passing chip alone.
- **Contrast targets re-measured against cream-white** (`#e8e2d9` L=0.766, not pure white L=1.0) — initial proposal hexes shifted ~10-15 RGB units darker to clear 4.5:1:

| Mode | Chip | Hex | Contrast vs #e8e2d9 |
|---|---|---|---|
| Light | Mint | `#1c705c` | 4.61 |
| Light | Amber | `#7c601c` | 4.61 |
| Dark | Mint | `#016b5d` | 5.01 |
| Dark | Amber | `#7d5c1c` | 4.77 |

**Distinguishability:** Mint deep-teal vs amber deep-olive — hue families clearly separable in both modes (screenshots confirm). Legend squares (lines 505-507) keep original `--teal-strong`/`--amber-strong` tokens — minor cosmetic hue drift between chip and legend dot but acceptable at xs size.

**Gates (all green):**

- `npx tsc --noEmit`: clean (0 output)
- `npm run test`: **919/919** deterministic
- `npm run lint:no-raw-spacing`: clean
- `npm run lint:no-editorial-chips`: clean
- `npm run build`: 7/7 routes compiled (`/`, `/_not-found`, `/dev/hero-preview`, `/dev/map-calibrate`, `/intel`, `/methodology`, `/regulatory`)
- `npm run lint`: 134 problems (39 errors, 95 warnings) — pre-existing baseline drift in unrelated files; zero new in `S4Card.tsx` or `globals.css`

**Verification (Pause B):**

- **Smoke test:** `npm run build && npx serve@latest out -p 3100`. 5 routes (`/`, `/intel`, `/methodology`, `/regulatory`, `/dev/hero-preview`) → 200; 8 sampled JS chunks under `/_next/static/chunks/` → 200.
- **axe-core re-scan** (`node scripts/_phase-18-2-2-probe.mjs`):
  - dark-1440: **2 violations** (was 4; both pipeline-bg cleared — mint 3.02 → pass, amber 2.79 → pass)
  - light-1440: **2 violations** (was 4; mint 3.61 → pass, amber 3.65 → pass)
  - 1024/414 configs: 0 (unchanged — render-timing artifact in probe, not regression)
  - Remaining 4 total = 4 documented exceptions × 2 modes (text-ghost decorative + white-on-mint chip elsewhere)
- **2 PNGs** at `docs/visual-audit/phase-18-2-2/` via `scripts/_phase-18-2-2-shot.mjs`: `s4-pipeline-bar-light-1440.png`, `s4-pipeline-bar-dark-1440.png`. Both show the 3-tier bar (installed mint / TSO-reserved amber / intention pale-amber) with cream labels readable on the 2 colored chips and dark label on the pale intention chip; mint vs amber hue separation preserved.

**Roadmap delta (Cowork applies per rule #5):**

- **Phase 18.2.2 → Shipped appendix.**
- **axe-core baseline now 4** unique selectors across light-1440 + dark-1440 (was 8). Remaining 4 = irreducible documented exceptions (text-ghost decoratives + white-on-mint chip elsewhere). **a11y arc complete.**
- **Update "Currently active" pointer** away from 18.2.2 to the operator-pick across the three eligible Tier 1 threads (7.7g-a-2, 12.12 #1+#2, or 29).
- **devDep delta noted:** `playwright@^1.x` added to `devDependencies` (closes the flag from commit `3a83767`).

**Deviations to note:**

- Operator's "3 chip spans" → applied to **2 chip spans** (mint + amber only; intention span unchanged to avoid new violation).
- Operator's `var(--white)` text token → escalated to operator at mid-Pause-B for `--white` inversion; authorized 3rd scoped token `--pipeline-chip-text`.
- Initial proposed chip hexes (`#1f7c66 / #84641c / #028274 / #966e24`) → darkened to (`#1c705c / #7c601c / #016b5d / #7d5c1c`) after re-measure against cream-white #e8e2d9.

**Branch push:** `phase-18-2-2-pipeline-bg-contrast` to remote pending operator OK. Per `feedback_pr_workflow_minimal.md`: no PR body, no branch delete. Operator opens PR + merges.

### Session 71 — 2026-05-18 — Phase 19.2.3 — Parent-opacity Financing-arrow residual fix (Claude Code)

**Headline:** Branch `phase-19-2-3-financing-arrow-opacity` off latest main (`1faf0f8`). Two-pause-point session, ~45 min CC. Cleared the 2 axe-core residuals Phase 18.2.1 deferred — light-1440 + dark-1440 "↓ Financing" driver-chip violations (light 1.72, dark 1.86). Single component edit in `app/components/S3Card.tsx`. Pick (a) per phase prompt: drop wrapper opacity + swap chip text color to `var(--text-muted)` when stale. axe-core delta: 5→4 per side (light-1440 + dark-1440), 10→8 total flagged sites.

**Pause A — locate + confirm (~15 min):**

- Grep `Financing` + `↓` led to `S3Card.tsx:369` (12M trend text) and `:394` (driver chip text). axe target selector `div:nth-child(13) > div:nth-child(4) > span:nth-child(1)` + axe HTML `<span style="...color: var(--teal-accent-text);">↓ Financing</span>` triangulated to the driver chip at line 394 (chips ordered Battery hardware/Electrical-PCS/HV grid/Financing — Financing is `nth-child(4)`).
- **Parent-opacity mechanism confirmed at line 393:** wrapper `<div style={{ ..., opacity: stale ? 0.35 : 1 }}>` compounds with full-opacity `var(--teal-accent-text)` token. Composited foreground math at α=0.35: light fg = `rgba(14,97,85,0.35)` over `#edeae5` → `#9fbab3` (axe-reported); dark fg = `rgba(0,180,160,0.35)` over `#0f0f12` → `#0a4944` (axe-reported). Both match α≈0.35 precisely. Phase 19.2.1's audit missed this because Phase 19.2.1 looked for `opacity:` on the text element itself — this opacity is on the WRAPPER.
- **Sibling check (rule #4):** one parent-opacity-wrapper sibling in same component at `S3Card.tsx:565` — Drawer "Raw inputs" table rows with `opacity: stale ? 0.35 : 1`. Wraps `text-tertiary`/`text-primary`/`text-secondary` spans (NOT accent tokens) inside a collapsed Drawer (axe didn't reach it). Recommended NOT folding into this phase — different visual context, different color tokens, no axe flag.
- **Fix shape:** pick (a) — drop wrapper opacity entirely, swap chip text color from `chipColor(drv.direction)` to conditional `stale ? var(--text-muted) : chipColor(drv.direction)`. `--text-muted` is the token designed for muted-but-readable text; passes WCAG AA on `--bg-elevated` in both themes. Color shift (accent → muted-grey) replaces alpha wash as the stale signal. All 4 chips share this wrapper pattern → all inherit the fix uniformly.

**Pause B — apply + verify (~20 min):**

`app/components/S3Card.tsx:393–394` (2-line surgical edit):

```jsx
// before
<div ... style={{ ..., transition: 'border-color 0.2s, opacity 0.2s', opacity: stale ? 0.35 : 1 }}>
  <span style={{ ..., color: chipColor(drv.direction) }}>{drv.symbol} {drv.driver}</span>

// after
<div ... style={{ ..., transition: 'border-color 0.2s' }}>
  <span style={{ ..., color: stale ? 'var(--text-muted)' : chipColor(drv.direction) }}>{drv.symbol} {drv.driver}</span>
```

**Gates (all green):**

- `npx tsc --noEmit`: clean
- `npm run test`: **919/919 deterministic** (Phase 19.2.2 baseline preserved)
- `npm run lint:no-raw-spacing`: clean
- `npm run lint:no-editorial-chips`: clean
- `npm run build`: 7/7 routes
- `npm run lint`: 132 problems (39 errors, 93 warnings) — pre-existing baseline drift in untouched files; zero new in touched file

**Verification (Pause B):**

- **Smoke test:** `npm run build && npx serve out -l 3100`. 5 routes (`/`, `/intel`, `/regulatory`, `/methodology`, `/dev/hero-preview`) → 200.
- **axe-core re-scan** (`SHOTS=1 node scripts/_phase-19-2-3-probe.mjs`):
  - light-1440: **4 violations** (was 5; "↓ Financing" cleared)
  - dark-1440: **4 violations** (was 5; "↓ Financing" cleared)
  - Remaining 4 per side = 1 ghost-text `aria-hidden` source-footer + 3 pipeline-bg/mint-badge sites (all Phase 18.2.2 turf, OOS).
- **DOM probe** (`scripts/_phase-19-2-3-dom-probe.mjs`): for "↓ Financing" span → `parent_opacity=1`, `span_opacity=1`, color resolves to `rgba(26,26,31,0.65)` light / `rgba(232,226,217,0.55)` dark. Composited contrast: light ≈5.06, dark ≈5.3 (both ≥4.5).
- **2 PNGs** at `docs/visual-audit/phase-19-2-3/`: `light-1440.png`, `dark-1440.png`.

**Roadmap delta (Cowork applies per rule #5):**

- **Phase 19.2.3 → Shipped appendix.**
- **Update "Currently active" pointer** away from 19.2.3.
- **axe-core baseline now 8** unique selectors across light-1440 + dark-1440 (was 10). Drops to 4 after Phase 18.2.2 ships (pipeline-bg + 4 remaining selectors).

**Branch push:** `phase-19-2-3-financing-arrow-opacity` to remote. Per `feedback_pr_workflow_minimal.md`: no PR body, no branch delete. Operator opens PR + merges.

### Session 70 — 2026-05-18 — Phase 18.2.1 — Baltic palette retune (medium scope, accent text-on-cream WCAG AA) (Claude Code)

**Headline:** Branch `phase-18-2-1-baltic-palette-retune` off latest main (`8943399`). Three-pause-point session, ~3h CC. Resolved the residual 19 accent text-on-cream WCAG 2.1 AA violations + 2 pipeline-on-colored-bg labels Phase 19.2.1 left for 18.2.1. Pick #4 sentiment-lock honored — `--teal`/`--amber` primitives stay, three dedicated `--<color>-accent-text` light-mode tokens added (new-token discipline rule's overlap-mitigation exception). `--rose` (no sentiment coupling) shifted directly. axe-core delta across 6-config (dark/light × 1440/1024/414): **43 → 10 raw violations (−33; light-1440 27→5, light-1024/414 5→0, dark-1024/414 already 0).** Remaining 10 = 4 Phase 19.2 doc'd exceptions (white-on-mint + text-ghost) + 4 pipeline-on-colored-bg (Pick #1 → Phase 18.2.2 candidate) + 2 faded-teal "↓ Financing" arrows (DEFERRED → Phase 19.2.3 per Pause B pick #3; residual parent-opacity site Phase 19.2.1 missed).

**Pause A — audit + propose retune (~45 min):**

- 6-config axe probe: 27 violations at light-1440, 5 each at light-1024/414, 6 at dark-1440, 0 at dark-1024/414. De-duplicated to ~24 unique sites.
- **Sentiment-overlap audit (pick #4 critical finding):** `--positive` ≡ `--teal` and `--warning` ≡ `--amber` (overlap → text tokens needed). `--negative` ≡ `--accent-rose` ≠ `--rose` (no overlap → `--rose` shiftable directly).
- **Chart palette verification (chartTheme.ts):** all chart data colors resolved via `getComputedStyle` from accent tokens; no hardcoded hex outside `DARK_FALLBACK` SSR fallback. Since `--teal`/`--amber`/`--teal-medium` primitives stay locked, charts inherit current shipped hex; `--rose` shift propagates to chart paint via existing resolution path.
- **Cascade-surface counts:** `--teal` 140 sites, `--amber` 80, `--rose` 28, `--teal-medium` 9. Migration path only touched TEXT-color sites; borders/backgrounds untouched.

**Pause A → operator decision picks (all approved at start of Pause B):**

1. Add `--teal-accent-text` / `--amber-accent-text` / `--teal-medium-accent-text` tokens; migrate text sites.
2. `--rose` direct shift to `#b82828` (28 sites inherit auto; charts auto-pick).
3. Defer faded-teal "↓ Financing" arrow → Phase 19.2.3 (parent-opacity residual, not accent-token work).
4. Migrate **all 9** `--teal-medium` tier3-impact sites (rule #4 consistency), not just the 1 axe-flagged.
5. Acknowledge spec-vs-prompt hex mismatch — proceed minimum-delta-from-shipped.
6. Pipeline-bg (Phase 18.2.2 candidate), ghost + white-on-mint exceptions preserved.

**Pause B — token shifts + cascade migration (~1.5h):**

`app/globals.css`:

| Token | Mode | Before | After | Ratio |
|---|---|---|---|---|
| `--teal-accent-text` | dark (`:root`) | n/a | `var(--teal)` | dark passes (8+ vs `--bg-page`) |
| `--teal-accent-text` | light | n/a | `#0e6155` | 4.44 → ~6.23 vs `--bg-card` cream |
| `--amber-accent-text` | dark | n/a | `var(--amber)` | dark passes |
| `--amber-accent-text` | light | n/a | `#7a5a1c` | 4.44 → ~5.37 |
| `--teal-medium-accent-text` | dark | n/a | `rgba(0,180,160,0.85)` | 3.68 → 5.82 (alpha bump 0.65→0.85) |
| `--teal-medium-accent-text` | light | n/a | `rgba(14,97,85,0.95)` | 3.19 → ~5.61 |
| `--rose` | light | `#dc3535` | **`#b82828`** | 4.08 → ~5.27 |
| `.inline-cta a:hover` | light | `var(--amber)` | `var(--amber-accent-text)` | text path |
| `.card-footnotes a:hover` | light | `var(--teal)` | `var(--teal-accent-text)` | text path |

Cascade migrations (TEXT-color contexts only; borders/backgrounds left untouched per pick #4 sentiment lock):

| File | Migrations |
|---|---|
| `S3Card.tsx` | 13 — `chipColor` function + 12 inline sites (trend arrows, EU-turnkey, ↓/↑ driver chips, 3 anchors, observed-references header, range-review warning, do-not-average warning, status reference colorizer) |
| `RevenueCard.tsx` | 8 — `WARRANTY_STYLE` lookup, `irrColor`/`dscrColor` helpers, 4 inline ternaries |
| `S4Card.tsx` | 7 — `StatusBadge` lookup, legend ●dots, EE/LV buttons + MW serif callouts, hover handlers, tier3-impact `--teal-medium → -medium-accent-text` |
| `HeroBalticMap.tsx` | 3 — Y1-base delta arrow, amber awaiting-TSO callout, `--amber-strong` IRR title (the one I missed in first probe pass, surfaced by post-Pause-B re-scan → migrated to `--amber-accent-text`) |
| `IntelFeed.tsx` | 10 — `CATEGORY_COLORS` + `SOURCE_TYPE_COLORS` lookups, `OfficialBadge`, `MagnitudeChip` helper, 5 anchor/link/tab sites, tier3-impact tab, 3 hover handlers |
| `{S7,S8,S9,SolarCard,WindCard,LoadCard}.tsx` | 6 × tier3-impact `--teal-medium` (rule #4 cross-card cascade) |
| `{S1,S2}Card.tsx` | 3 — duration/price-period/country toggle button text ternaries |
| `TradingEngineCard.tsx` | 5 — `qualityColor` helper, Toggle button text ternary, headline anchors |
| `{AssetDetailPanel, RegulatoryItem, RegulatoryPreview, RegulatoryFilters, StickyNav, ContactForm, PeakForecastCard, S5Card, CardFooter, CopyButton, SignalIntel, SourceFooter, RenewableMixCard, ResidualLoadCard, SpreadCaptureCard, ConfidenceBadge, ConfidencePanel, DataClassBadge}.tsx` | 22 — anchor links, helper-return colors, lookup tables, hover handlers, ternaries |
| `page.tsx` + `methodology/page.tsx` | 4 — anchor links |
| `app/lib/types.ts` | 1 — `sentimentColor()` positive/caution branches |
| `app/lib/freshness.ts` | 1 — `colorToken` for LIVE/STALE bands (no consumer churn — components re-resolve via `var()`) |
| `app/components/__tests__/CyclesBreakdownChart.test.tsx` | 2 — assertion updates (`var(--teal)` → `var(--teal-accent-text)`) for warranty-chip color test |
...
- `npm run lint:no-editorial-chips`: clean
- `npm run build`: 7/7 routes
- `npm run lint`: 129 problems (39 errors, 90 warnings) — pre-existing baseline-drift family (Session 68 noted 127; +2 carry, **zero new in touched files**)

**Pause C — local-build verification + visual evidence (~30 min):**

- **Smoke test:** `npm run build && npx serve out -l 3100`. 5 routes (`/`, `/intel`, `/regulatory`, `/methodology`, `/dev/hero-preview`) → 200. 8 chunks (`/_next/static/chunks/*.js`) → 200.
- **6-config axe re-scan** (`SHOTS=1 node scripts/_phase-18-2-1-probe.mjs`):
  - dark-1440: 5 (all pre-approved categories), dark-1024: 0, dark-414: 0
  - light-1440: 5 (all pre-approved), light-1024: 0, light-414: 0
- **DOM probe** (`scripts/_phase-18-2-1-dom-probe.mjs`) at light-1440:
  - Token values resolve correctly: `--teal-accent-text=#0e6155`, `--amber-accent-text=#7a5a1c`, `--teal-medium-accent-text=rgba(14,97,85,0.95)`, `--rose=#b82828`.
  - 5 sampled axe-previously-flagged selectors all route through new tokens (`Revenue Engine` anchors `rgb(14,97,85)`, S3Card `EU turnkey` span `rgb(14,97,85)`, Hero `awaiting-TSO` `rgb(122,90,28)`, S3Card `Battery hardware` driver chip `rgb(14,97,85)`).
  - 12/12 chart canvases rendered (width × height > 0). `--rose` shift propagates via `getComputedStyle` resolution. **0 console errors on render.**
- **6 PNGs** at `docs/visual-audit/phase-18-2-1/`: `{dark,light}-{1440,1024,414}.png`. Compare against `docs/visual-audit/phase-19-2-1/{dark,light}-1440.png` for visual continuity (Pause C operator-side acceptance).

**Spec-vs-prompt hex mismatch (handover flag for Cowork to update spec-reference memory):**

Phase 18.2.1 prompt cited spec P2-2 hexes **`#1a3833` (teal) / `#d4a574` (amber) / `#a8324a` (rose)** as starting point. Empirical verification: **none of these three hex strings appear in `docs/specs/2026-05-06-designer-developer-spec.md`.** The actual spec doc (lines 52-54) prescribes `#00b4a0` (positive=teal), `#d4a03c` (warning=amber), `#d65858` (negative=rose) — all LIGHTER than shipped, would fail WCAG worse on cream. Of the prompt-cited values: `#a8324a` happens to match the shipped `--accent-rose` light token (which is sentiment-coupled to `--negative`, not `--rose`); `#d4a574` matches shipped (dark) `--accent-amber`; `#1a3833` doesn't appear anywhere in the codebase or specs. **Recommendation:** either (a) the prompt's spec hexes came from an unmerged spec revision and `docs/specs/2026-05-06-designer-developer-spec.md` needs updating, or (b) the prompt sourced from a different (memory-stored?) reference and the wording "spec P2-2" was imprecise — Cowork to reconcile. Phase 18.2.1 proceeded with minimum-delta-from-shipped per pick #5 since neither prompt nor spec hexes are WCAG-viable starting points on the existing cream backgrounds.

**Chart-palette spec divergence (deferred):**

Per pick #4, `--teal`/`--amber`/`--teal-medium` primitive hex stays at shipped values; only the new `*-accent-text` tokens (used at text sites) carry darker hexes. Charts (chartTheme.ts → `getComputedStyle` on the primitives) therefore continue to render with the existing shipped teal/amber hex. The "chart palette consistency" concern Phase 19.2 pick #8 deferred remains unresolved — charts still use `#137a65 / #8a6620 / rgba(19,122,101,0.80)` (light). Resolved-rose now matches across text + chart (both `#b82828` via `--rose` shift). Filed as Phase 18.2.x candidate (suggest bundling with 18.2.2 pipeline-bar work, or scoping as separate chart-palette pass).

**Cross-card-consistency rule #4 application:** All `--teal-medium` tier3-impact text sites (9 across S4/S7/S8/S9/SolarCard/WindCard/LoadCard/IntelFeed) migrated together, not just the 1 axe-flagged at the time of scan. Same principle for `--teal` and `--amber` text-color uses — sweeping migration ensures no card renders with two different darknesses of "the same accent." Borders and backgrounds left at the unshifted primitives (decorative usage, not WCAG-text-critical).

**Roadmap delta (Cowork applies per rule #5 + sequencing memory step 10):**

- **Phase 18.2.1 → Shipped appendix.** axe-core baseline now **≤2 docs'd exceptions + 4 OOS (Phase 18.2.2) + 2 deferred (Phase 19.2.3)** = 10 unique selectors across 6 configs.
- **Update "Currently active" pointer** away from 18.2.1.
- **File Phase 18.2.2** (pipeline-bar background colors + 2 pipeline-on-colored-bg label sites; ~half-day; would close the remaining 4 unique selectors and finish the WCAG accent pass on cream). Also opportunistic candidate to address chart-palette spec divergence noted above.
- **File Phase 19.2.3** (faded-teal "↓ Financing" arrow — residual parent-opacity site Phase 19.2.1 closing scan missed; light ratio 1.72, dark 1.86; needs grep+screenshot triangulation per rule #1 before scoping). Likely single-file edit.
- **Spec doc reconciliation** per the mismatch flag above — Cowork to update spec-reference memory and/or `docs/specs/2026-05-06-designer-developer-spec.md` per whichever source is authoritative.

**Branch push:** `phase-18-2-1-baltic-palette-retune` pushed to remote with one commit. Per `feedback_pr_workflow_minimal.md`: no PR body, no branch delete. Operator opens PR + merges.

### Session 68 — 2026-05-18 — Phase 19.2.1 — Inline-opacity audit (component-level cleanup) (Claude Code)

**Headline:** Branch `phase-19-2-1-inline-opacity-audit` off latest main (`62d590d`). Two-pause-point session, ~1.5h CC. Component-level cleanup of inline `opacity:0.35–0.9` overlays on text tokens flagged by Phase 19.2's closing audit. 14 edits across 6 files; no token shifts; pick (b) "drop inline opacity, swap to a darker existing token" applied uniformly. axe-core delta: **54 → 21 raw violations (−33; light/dark/mobile combined).** All 10 inline-opacity sites flagged by Phase 19.2's closing scan now cleared. Remaining 21 are accents (Phase 18.2.1 turf) + 2 docs-exception ghost decoratives + 2 pipeline-bar labels still failing due to colored-background contrast (also Phase 18.2.1 — bg color, not opacity).

**Audit-triage finding (rule #1 in action):** prompt scoped "13 inline-opacity sites"; Phase 19.2's closing handover said "~9". Re-running `scripts/_phase-19-2-probe.mjs` showed empirical reality: **8 source-lines, 10 rendered instances, all in `app/components/S4Card.tsx`** (Phase 19.2's "LitGrid stage breakdown" hot-site confirmed). Grep also surfaced 6 cross-card siblings with the same `--text-muted opacity:0.65–0.7` pattern in drawer-only contexts (not currently axe-flagged because drawers were closed during scan, but pattern-identical per rule #4). Pause A held; operator approved Scope A+B = 14 total edits.

**Pause B — edits applied (pick (b) uniform):**

| # | File:Line | Element | Before | After |
|---|---|---|---|---|
| 1 | `S4Card.tsx:451` (×3 buttons rendered) | Stage-filter button count chip | `marginLeft: 5px, opacity: 0.5` (inherited tertiary/secondary) | `marginLeft: 5px, color: var(--text-muted)` |
| 2 | `S4Card.tsx:473` | Pipeline-bar "Installed" mono label | `color: text-primary, opacity: 0.9` | `color: text-primary` |
| 3 | `S4Card.tsx:482` | Pipeline-bar "TSO reserved" mono label | `color: text-primary, opacity: 0.9` | `color: text-primary` |
| 4 | `S4Card.tsx:491` | Pipeline-bar "Intention" mono label | `color: text-secondary, opacity: 0.8` | `color: text-primary` |
| 5 | `S4Card.tsx:543` | LitGrid credibility row 1 "Grid agreement + tech project" | `opacity: 0.7` (inherits muted) | `color: var(--text-secondary)` |
| 6 | `S4Card.tsx:544` | LitGrid credibility row 2 "Development permit only" | `opacity: 0.5` | `color: var(--text-tertiary)` |
| 7 | `S4Card.tsx:545` | LitGrid credibility row 3 "TSO reservation / protocol" | `opacity: 0.4` | (drop; inherits muted from parent) |
| 8 | `S4Card.tsx:546` | LitGrid credibility row 4 "APVA applied" italic + hollow `○` | `opacity: 0.35, fontStyle: italic` | `fontStyle: italic` (italic + hollow dot already marks weakest) |
| B1 | `WindCard.tsx:133` | Drawer coverage paragraph | `color: text-muted, opacity: 0.7` | `color: text-muted` |
| B2 | `SolarCard.tsx:136` | same pattern | same | same |
| B3 | `LoadCard.tsx:135` | same pattern | same | same |
| B4 | `RevenueCard.tsx:1831` | Source footer line | `color: text-muted, opacity: 0.7, cursor: pointer` | `color: text-muted, cursor: pointer` |
| B5 | `RevenueCard.tsx:1850` | Assumptions inline disclosure | `color: text-muted, opacity: 0.65` | `color: text-muted` |
| B6 | `TradingEngineCard.tsx:551` | Source footer line | `color: text-muted, opacity: 0.7` | `color: text-muted` |

Visual hierarchy preserved in dark (secondary 0.70 > tertiary 0.62 > muted 0.55 > muted-italic). In light (Phase 19.2 flattened muted=tertiary=secondary=0.65), row 5 reads slightly heavier (0.70), rows 6-8 land uniform with row 8 differentiated by italic + hollow `○`. Accepted per pick #4 visual-continuity test.

**axe-core delta (playwright probe, `scripts/_phase-19-2-1-probe.mjs`):**

| Run | Pre (Phase 19.2 baseline re-scan) | Post | Δ |
|---|---|---|---|
| dark 1440 | 13 | 6 | −7 |
| light 1440 | 36 | 10 | −26 |
| dark 414 | 0 | 0 | 0 |
...
(Pre-scan total 54 is higher than Phase 19.2's closing "28" because closing was deduplicated by selector + grouped exceptions; current raw axe-cli count is the apples-to-apples baseline at Phase 19.2's tip. Δ is what matters: 33 fewer raw flagged nodes.)

**DOM verification (3 previously-failing inline-opacity sites, post-fix):**

```
S4Card.tsx:544 LitGrid row 2 "Development permit": color rgba(232,226,217,0.62) on rgb(12,12,15) → ratio ≈ 4.86 ✓ (was 2.11)
S4Card.tsx:451 button count chip:                   color rgba(232,226,217,0.55) on rgb(7,7,10)  → ratio ≈ 5.74 ✓ (was 2.36)
S4Card.tsx:482 pipeline-bar TSO reserved (dark):    color rgba(232,226,217,0.92) on rgb(162,123,49) → ratio ≈ 2.67 (was 2.67; still failing — bg color issue, deferred to 18.2.1)
```

Pipeline-bar inner labels (#473/482/491) remain flagged because the bg color (`--teal-strong`, `--amber-strong`, `--amber-subtle`) crashes contrast even at full alpha. Phase 18.2.1 turf (accent palette).

**Editorial-regex regression check** (per Phase 19.2 protocol via `lint:no-editorial-chips`): clean. Rule #6 unbroken.

**21 remaining (post-Phase-19.2.1) — exception categorization:**
- **Phase 18.2.1 candidates (accents)** (~17): light-mode `--rose` / `--amber` / `--accent-amber` / `--teal` on cream (rebalanced to fail by slim margins post-19.2); 2 dark `--teal` / `--teal-medium` outliers; 2 dark pipeline-bar inner labels on colored bg (text-primary on teal-strong / amber-strong).
- **Pick #7 documented exceptions** (~2): `€···` hero skeleton + `MODEL INPUT → CAPEX reference` decorator (aria-hidden landed in Phase 19.2; axe-cli still surfaces visually).

**Local-build smoke-test:** `npm run build && npx serve out -l 3100`. 5 routes all HTTP 200. 8 first-listed chunks all HTTP 200.

**Multi-viewport visual evidence:** 2 PNGs at `docs/visual-audit/phase-19-2-1/`:
- `light-1440.png` (6.1 MB) · `dark-1440.png` (5.6 MB)

(Lighter than Phase 19.2's 4-PNG grid per pick #5 — changes are surgical.) Phase 19.2's PNGs were briefly overwritten by an unscoped `OUT_VIS` env var on the original probe; restored from git and Phase 19.2.1 captured to its own dir via cloned `scripts/_phase-19-2-1-probe.mjs`.

**Gate deltas vs baseline:** `npx tsc --noEmit` = 0 errors. `npm run lint:no-raw-spacing` = clean. `npm run lint:no-editorial-chips` = clean. `npm run build` = 7/7 routes compiled. **`npm run lint` = 127 problems (39 errors, 88 warnings); zero new errors in any touched file** — same baseline-drift family as Phase 19.2 (125→129); current 127 is a slight reduction vs 129. **`npm run test` = 918/919 passed; 1 pre-existing time-of-day flake** in `freshnessBadge.test.tsx:39 "renders TODAY label between 6h and 24h"`. Test runs `Date.now() - 12 * HOUR` and asserts TODAY; at 08:09 UTC (= 11:09 EET Vilnius), 12h prior crosses Vilnius midnight, so `isSameVilniusDay(ts, now)` is false at `freshness.ts:117` and the branch falls through to STALE. Unrelated to Phase 19.2.1 edits — neither `FreshnessBadge.tsx` nor `freshness.ts` is touched in this phase. Pre-existing flake worth a follow-up phase (file as Phase 19.2.2 candidate or fold into freshness primitive review).

**Roadmap delta needed (Cowork applies per rule #5):**
- Phase 19.2.1 → Shipped appendix.
- Update "Currently active" pointer.
- Note: 21 remaining axe violations now all bucketed as Phase 18.2.1 (accents + bg-color contrast) + 2 documented exceptions.
- Optional file: Phase 19.2.2 candidate — `freshnessBadge.test.tsx` TODAY-band Vilnius-midnight flake.

**Branch state:** `phase-19-2-1-inline-opacity-audit` ready to push. 6 files modified (`app/components/S4Card.tsx`, `WindCard.tsx`, `SolarCard.tsx`, `LoadCard.tsx`, `RevenueCard.tsx`, `TradingEngineCard.tsx`) + 1 script + 2 PNGs. Operator opens PR + merges per `feedback_pr_workflow_minimal.md`.

### Session 67 — 2026-05-17 — Phase 19.2 — WCAG 2.1 AA color-contrast pass (Claude Code)

**Headline:** Branch `phase-19-2-color-contrast-aa` off clean main (`a361231`). Three-pause-point session, ~2h CC. Token-level WCAG 2.1 AA contrast fix for `--text-muted` cascade across dark + light modes. 4 token-value edits in `app/globals.css` + 2 `aria-hidden="true"` annotations on decorative `--text-ghost` elements. axe-core delta: **259 → 28 violations (−89%)** across 4 mode×viewport scans; **0 `--text-muted` violations remain.** 28 remaining are all in documented-exception categories (accents → Phase 18.2.1, inline-opacity × tertiary → Phase 19.2.1 candidate, two `--text-ghost` decoratives → annotated per pick #7).

**Audit-triage finding (rule #1 in action):** prompt scoped "~7 violations" off Phase 19's snapshot. Re-running axe-core at full DOM at 2×2 mode×viewport surfaced 259 nodes — 99 dark 1440, 121 light 1440, 17 dark 414, 22 light 414. Aggregation by `(fg, bg, ratio, font-size)` collapsed to one dominant cause (`--text-muted` alpha too low across both modes) + a long tail of pre-existing accent / opacity-multiplied / decorative-ghost issues. Pause A held; operator picked **A3 dark** (rebalanced 3-step), **B3 light** (muted-joins-collapse), defer accents to 18.2.1, file opacity-multiplied as 19.2.1, treat both ghost decoratives as in-scope aria-hidden fixes.

**Pause B — token shifts (per-violation closures):**

| Token | Mode | Old → New | Worst-case ratio before → after | Cascade-surface |
|---|---|---|---|---|
| `--text-muted` | dark | `rgba(232,226,217, 0.45)` → `0.55` | 3.73 → ~5.74 | 388 call sites |
| `--text-tertiary` | dark | `rgba(232,226,217, 0.55)` → `0.62` | (was passing 5.11) → ~6.34 | 169 call sites |
| `--text-secondary` | dark | `rgba(232,226,217, 0.65)` → `0.70` | (was passing 7.1) → ~7.8 | 168 call sites |
| `--text-muted` | light | `rgba(26,26,31, 0.60)` → `0.65` | 4.22 → ~4.71 | 388 call sites |
| `--text-tertiary` | light | unchanged (0.65) | passing | — |
| `--text-secondary` | light | unchanged (0.65) | passing | — |

Light B3 accepts the pre-existing Phase 18 tertiary=secondary=0.65 collapse; muted joins them. Tight WCAG margin (~4.7) operator-acknowledged. Dark A3 preserves the muted<tertiary<secondary rank with 7–8 point gaps instead of the prior 10-point gaps.

**Pause C — aria-hidden annotations (pick #7 exceptions, in-scope trivial):**
1. `HeroBalticMap.tsx:710` — wrapped the `€···` placeholder span (`fmt(null)` fallback when `lr?.today_total_daily == null`) with `<span aria-hidden="true">`. This is a hero-skeleton state; when live data lands, `<AnimatedNumber>` replaces it.
2. `S3Card.tsx:619` — added `aria-hidden="true"` to the `MODEL INPUT → CAPEX reference · Financing cost` decorator (always-rendered footnote in serif-mono ghost; intentional brand decoration).

**Honest caveat on aria-hidden + axe color-contrast:** axe-core 4.x's `color-contrast` rule still flags `aria-hidden` elements (it asserts visual contrast regardless of a11y-tree exclusion). axe-cli's final scan still surfaces `€···` as ratio 1.4:1; playwright probe at 1440×1800 doesn't render `€···` (data hydrates within `waitForTimeout(800)` after `networkidle`) so it's absent from the post-fix totals, but MODEL INPUT (always rendered) remains in the playwright count as a single ratio-1.4 entry. The aria-hidden is still the right *semantic* fix (screen readers correctly skip decorative content); axe just doesn't honor it for contrast checks. Treating both as the pick #7 documented-exception count.

**axe-core delta (playwright probe, `scripts/_phase-19-2-probe.mjs`):**

| Run | Pre | Post | Δ |
|---|---|---|---|
| dark 1440 | 99 | 13 | −86 |
| light 1440 | 121 | 10 | −111 |
| dark 414 | 17 | 0 | −17 |
| light 414 | 22 | 5 | −17 |
| **Total** | **259** | **28** | **−231 (−89%)** |

axe-cli final scan against `http://localhost:3100/` (headless default viewport, no theme override, default rendering state): 1 color-contrast violation (the `€···` aria-hidden span, ratio 1.4 — see caveat above). Down from prod axe-cli baseline of 17.

**28 remaining — exception categorization:**
- **Phase 18.2.1 (accent palette retune) candidates** (~17 light, 2 dark): `--rose #dc3535` on cream (3.86–4.08), `--amber #8a6620` on cream (3.94–4.44), `--accent-amber #947334` on cream (3.73), `--teal #137a65` on cream (4.18–4.44), `--teal #0a4944` / `#04796d` on dark (1.86 / 3.68), `--white` on `--mint` chip badge (2.02).
- **Phase 19.2.1 candidate (inline-opacity × token)** (~9 dark): LitGrid stage breakdown uses `opacity: 0.35–0.7` inline on top of `--text-tertiary`, compounding alpha and crashing computed ratio to 1.59–3.03. Token-level fix won't help; needs per-site rewrite.
- **Pick #7 documented exceptions (in-scope aria-hidden landed; axe still flags visually)**: `€···` hero skeleton + `MODEL INPUT → CAPEX reference` decorator.

**DOM verification probe** (`scripts/_phase-19-2-domprobe.mjs`, single-run sample of 3 previously-failing selectors):

```
hero MW span:    color rgba(232,226,217,0.55)  10px on rgb(7,7,10)   → ratio ≈ 5.74 ✓
footer text-muted: color rgba(232,226,217,0.55)  14px on rgb(7,7,10)   → ratio ≈ 5.74 ✓
tier1 KPI:       color rgba(232,226,217,0.55)  14px on card-elevated → ratio ≈ 5.6  ✓
```

All three previously-failing 3.73–3.79 sample sites now compute to 5.6–5.74 against their respective backgrounds (well over AA 4.5).

**Local-build smoke-test:** `npm run build && npx serve out -l 3100`. 5 routes (`/`, `/intel`, `/methodology`, `/regulatory`, `/dev/hero-preview`) all HTTP 200. 8 first-listed chunks under `_next/static/chunks/` all HTTP 200.

**Editorial-regex regression check** (per Phase 19 protocol): `grep -rEn "elevated|above-normal|high renewable|tight system|wide envelope|favorable|supporting" app/components/` — 27 raw hits, all `var(--bg-elevated)` token references (false positive on `elevated` substring) + 1 `supporting metrics` JSX comment in `HeroMarketNow.tsx`. **0 user-facing editorial prose matches.** Rule #6 unbroken.

**Multi-viewport visual evidence:** 4 PNGs at `docs/visual-audit/phase-19-2/`:
- `light-1440.png` (6.1 MB) · `light-414.png` (4.1 MB) · `dark-1440.png` (5.6 MB) · `dark-414.png` (4.1 MB)

Operator-side acceptance check: editorial character preserved? Tokens shifted by 7–10 alpha points; muted-text reads marginally crisper but the cream-against-charcoal / charcoal-against-cream Bloomberg-terminal palette is intact. No structural visual shift expected.

**Gate deltas vs baseline:** `npx tsc --noEmit` = 0 errors. `npm run test` = 919/919 passed (60 files), no delta. `npm run lint:no-raw-spacing` = clean. `npm run lint:no-editorial-chips` = clean. `npm run build` = 7/7 routes compiled. **`npm run lint` exit 1 with 129 problems (39 errors, 88 warnings); identical pre/post Phase 19.2 stash — pre-existing baseline.** Lint baseline drifted **125 → 129 across recent ships in `RevenueCard.tsx`** (observation noted per operator; not a Phase 19.2 regression — Phase 19.2's two-file edit produces zero new lint output).

**Roadmap delta needed (Cowork applies per rule #5):**
- Phase 19.2 → Shipped appendix.
- File Phase 19.2.1 candidate: inline `opacity:0.35–0.7` × `--text-tertiary` LitGrid stage breakdown rewrite (~9 axe nodes, needs per-site refactor; token-level shifts can't reach this).
- Reaffirm Phase 18.2.1 (Baltic accent palette retune) as a separate scoped phase — covers the ~17 light-mode accent violations + the dark teal/mint chip outliers.
- Update "Currently active" pointer.

**Branch state:** `phase-19-2-color-contrast-aa` ready to push. 4 files modified (`app/globals.css`, `app/components/HeroBalticMap.tsx`, `app/components/S3Card.tsx`, `scripts/_phase-19-2-probe.mjs` new). Operator opens PR + merges per `feedback_pr_workflow_minimal.md`.
...
**Pause A audit (rule-#1 triangulation):**

Pre-decided picks vs current code grep + phase-31-a-1 screenshot revealed two operator-misread items per discipline rule #1:
1. **"Hollow circle on SpreadCapture" — DROPPED.** Code (`SpreadCaptureCard.tsx:96`) used identical filled `<span>` dot as cards 1-3; screenshot confirmed. Misread treated as null. Operator confirmed drop.
2. **"Wire to each card's DetailsDrawer open action" — WIDENED.** Cards 1-4 had **no** `DetailsDrawer` infrastructure pre-phase (only S7/S9 did). Premise corrected at Pause A: pick 7 required *adding new drawer scaffolds + methodology prose*, not just link wiring. Operator confirmed widen with three-paragraph (Source / Computation / Limitations) drawer content.

Curl-verified `swing_stats_90d` shape at Pause A: `{p25:122.23, p50:150.93, p75:160.56, p90:202.38, n:90, days_of_data:90}` against worker `/read`; today's swing 149.47 (just below P50). Confirmed the marker-bar viz is data-feasible.

**Pause B revised pick (operator-driven):** Pick 3 originally said `--type-display-md` flat (1.5rem / 24px) for all 6. Operator revised: align S7/S9 down to `clamp(1.5rem, 3vw, 1.75rem)` matching cards 1-4 — *smallest-change-wins*. Cards 1-4 untouched. Result: S7 €49.8 and S9 75.6 now render at 28px @1440 and 24px @414, matching the 28/24 envelope of cards 1-4.

**Diff:** 6 components + 1 new script, ~+150 / −42 lines.
- `app/components/RenewableMixCard.tsx`: +21 — import `DetailsDrawer`, append drawer.
- `app/components/ResidualLoadCard.tsx`: +21 — same shape.
- `app/components/PeakForecastCard.tsx`: +43 — import `DetailsDrawer`, replace "90D median € · P90 €" line with P25/P50/P90 marker bar (+ cursor at today's swing), append drawer.
- `app/components/SpreadCaptureCard.tsx`: +21 — import `DetailsDrawer`, append drawer.
- `app/components/S7Card.tsx`: −21 / +21 — drop `MetricTile`/`StatusChip`/`Sentiment`/`ratioSentiment` helper, rebuild hero inline with filled-dot header, add subtype-bar comment.
- `app/components/S9Card.tsx`: −21 / +21 — same surgery.
- `scripts/_phase-31-probe.mjs`: +120 — playwright probe (3 viewports) + screenshot capture per Phase-31-A probe template + memory `feedback_chrome_mcp_orphans.md`.

**Verification gates (baseline reference: tsc 0, vitest 919/919, lint 125):**

| Gate | Baseline | Post-edit | Δ |
|---|---|---|---|
| `npx tsc --noEmit` | 0 errors | 0 errors | 0 |
| `npm run test` | 919/919 | 919/919 | 0 |
| `npm run lint` | 125 problems | 125 problems | 0 (none in changed files) |
| `npm run lint:no-raw-spacing` | pass | pass | 0 |
| `npm run lint:no-editorial-chips` | pass | pass | 0 |
| `npm run build` | success | success (3.9s compile, 9/9 static) | 0 |

**Pause C verification:**

*Local-build smoke-test:* `npm run build && npx serve out -l 3100`. 5 routes (`/`, `/intel`, `/methodology`, `/regulatory`, `/dev/hero-preview`) all HTTP 200. 8 chunks (7 `.js` + 1 `.css`) all HTTP 200. Clean.

*Multi-viewport playwright probe (`node scripts/_phase-31-probe.mjs`):* 1440 / 1024 / 414. All 6 cards across all 3 viewports rendered: filled colored header dot (✓), no `observed` pill above hero (✓), "▸ View [thing] detail" footer drawer trigger (✓), hero font-size 28px @1440-1024 and 24px @414 (✓). PeakForecast P25/P50/P90 labels present at all 3 viewports (✓). One probe-regex caveat: `/elevated/i` hit at all viewports, but localized to S7 drawer methodology copy `HIGH >50 €/MWh · ELEVATED >30 · NORMAL 15–30 · LOW <15` (pre-existing prose, collapsed drawer, not chip, not regression from this phase). The discipline-rule-#6 target is chip-style state labels, not methodology references; `npm run lint:no-editorial-chips` passes.

*Visual evidence:* 3 PNGs at `docs/visual-audit/phase-31/structural-1440.png` / `1024.png` / `414.png`. Compare against `docs/visual-audit/phase-31-a-1/structural-1440.png` (pre-phase) — S7 €59.8 and S9 75.6 hero shrink from ~52px to 28px is the largest visible delta; green "observed" pill removed from S7+S9; "▸ View … detail" affordance now visible across all 6 cards; PeakForecast marker bar replaces plain-text "90D median € · P90 €" line.

**Out of scope (untouched per prompt):**
- Worker / engine math / schema (frontend-only).
- Interpretation copy (locked to 31.A.1 ship).
- Color/palette work (pick 8, deferred).
- SpreadCapture 2-chart layout (pick 6 confirmed no-op).
- `<MetricCard>` primitive extraction — filed as **Phase 31.1 follow-up candidate** at Pause A. 6 cards share rhythm but diverge in sub-rows, viz, and drawer content; wide-scope abstraction was the right thing to defer.

**Roadmap delta needed (Cowork applies per rule #5 + `feedback_cowork_cc_sequencing.md` step 10):**
- Mark Phase 31 SHIPPED in `docs/phases/_post-12-8-roadmap.md`.
- Update "Currently active" pointer.
- File Phase 31.1 candidate: `<MetricCard>` primitive extraction for tier-3 cards (header+dot · hero · sub-row · viz · interp · footer drawer rhythm).

**Branch push:** `phase-31-tier3-visual-normalization` pushed to origin. Operator opens PR + merges per `feedback_pr_workflow_minimal.md` (no body, no branch delete).

---

### Session 57 — 2026-05-08 — Phase 18.3.1 hot-fix + Phase 31/32 candidate filings (Cowork; orphan-discovered 2026-05-13)

**Headline:** Phase 18.3.1 universal card hover-lift shipped Cowork-direct on `phase-18-3-animation-activation` post-Session-56 (commit `a0f36ea`). Operator-rebooted scope from 3-card opt-in (`card--interactive` modifier) to all-cards-except-hero after rule-#1 chrome-MCP probe surfaced that the hover-lift was only firing on RevenueCard's investable pill (the one site WITHOUT `.card`). Plus Phase 31 + Phase 32 candidates filed to `docs/phases/_post-12-8-roadmap.md` (separate commit `bb96655` on 2026-05-13 — see "Orphan-discovered" note below).

**Bug diagnosis (rule #1 verified, not visual-inference):**

Operator post-Session-56-merge feedback: "Hover lift only works on the 50MW reference asset" (RevenueCard's investable pill). Chrome MCP DOM probe found 3 `.card--interactive` wrappers (RevenueCard pill at `RevenueCard.tsx:1641` / S4Card at `page.tsx:78` / TradingEngineCard at `page.tsx:152`), but only the pill was actually lifting on hover. S4Card + TradingEngineCard wrappers had stale `transform: matrix(1, 0, 0, 1, 0, 0)` from a different mechanism overriding `:hover`.

**Root cause:** `.card { animation: cardFadeIn 0.4s ease-out both; }` at globals.css:730. The `both` fill-mode keeps the keyframe's `to { transform: translateY(0) }` applied as an active animation value indefinitely. Per CSS spec, animation values rank ABOVE regular cascade — silently outranking `.card--interactive:hover { transform: translateY(-2px) }`. Same conflict on `.card-tier1-feature` (line 904) and `.card-tier3` (line 922). RevenueCard's pill wrapper at `RevenueCard.tsx:1641` has class `"card--revenue card--interactive"` (no `.card`) → no cardFadeIn animation → no fill-state conflict → hover transform fires cleanly.

**Scope-decision walkthrough (operator-driven, three iterations):**

1. **Cowork proposed: targeted 3-card hot-fix** — `.card.card--interactive { animation: none }` disables cardFadeIn on the specific 3 wrappers. Surgical; preserves all other behavior.
2. **Operator pushback:** "I think most except of hero should have it or none should have it, no? what's the logic you're thinking?" — exposed that the original 3-card pick was design-arbitrary. Cowork re-examined: lift = "drilldown destination" rule was thin (~all cards have drilldowns); only the hero is a "look at me" non-clickable centerpiece. Two coherent positions: all-except-hero OR no-lift-anywhere.
3. **Operator pick: all-except-hero.** Shipped: `.card:hover, .card-tier1-feature:hover, .card-tier3:hover { transform: translateY(-2px) !important; box-shadow: 0 8px 24px rgba(0,0,0,.18); }`. `!important` was needed because animation fill-state outranks regular cascade — `!important` promotes `:hover` above the animation per CSS spec. The `.card--interactive` modifier kept alive as opt-in for non-`.card` surfaces (the pill at `RevenueCard.tsx:1641`).

**Tone-down iteration:** operator: "I like shadow, maybe even less lift" → shipped `translateY(-0.5px) + box-shadow 0 3px 10px rgba(0,0,0,.08)` (half the lift, less than half the shadow opacity, tighter blur).

**Rejected expansion (operator-prompted self-correction):**

Cowork then proposed expanding lift to inline CTAs / drawer toggles / hero map markers / form submit. After grepping ("158 click targets across 30 files; only `.inline-cta a` has a stable class hook"), shipped `.inline-cta a:hover { transform: translateY(-0.5px); box-shadow: ... }`.

Operator: "rethink if it's the best way design wise?" → Cowork re-examined and reverted. **Principle articulated:** lift is the *card* hover signal (physical-panel metaphor). Each surface type has its own affordance pattern — cards lift, links shift color, buttons press-scale, icons rotate, markers reveal tooltips. Applying lift to non-cards stacks two motion vectors (color shift + Y-translate) for the same affordance, dilutes the card-specific signal, and creates motion fatigue across 158 click targets. Revert restored inline-cta to original color/border/bg-shift treatment.

**Files touched (2 modified, commit `a0f36ea`):**

```
M  app/globals.css                              +18/-2 (transition expansion + universal hover block + modifier kept as opt-in)
M  app/page.tsx                                  className simplification (2 sites — dropped now-redundant card--interactive modifier)
```

`RevenueCard.tsx:1641` intentionally untouched — pill keeps `card--interactive` modifier because it has no `.card` class.

**Phase 31 + Phase 32 candidate filings (separate commit `bb96655` on 2026-05-13):**

Operator-surfaced via "Structural market drivers" screenshot during same Cowork session. Screenshot exposed (a) two distinct card templates in the tier-3 grid (canonical-anatomy cards 1-4 vs threshold-pill-first cards 5-6 TTF/Carbon); (b) three status-indicator patterns for the same freshness concept; (c) hero type drift; (d) viz density inconsistency (PeakForecast has none, PeakTrough has two charts). Operator escalation: "I think the data is nonsense in some, they need a full revision/rebuild."

**Phase 31 — Tier-3 card data integrity + structural normalization** filed as two-workstream. **31.A — Data integrity (ship first, ~1 day CC)** captures 4 visible-from-screenshot reconciliation bugs at Pause-A triage gate per discipline rule #1 (still visual-inference; code-grep required before scoping fix): (a) **PeakForecast hero €142/MWh "Today's DA swing" but peak h20 €154.3 − trough h12 €99.9 = €54.4** — hero is ~3× actual swing; either field-mapping wrong or hero is price-level mislabeled; (b) **TTF Gas hero 44.4 €/MWh + threshold pill "0.89× / 50 €/MWh" (below-threshold) BUT interp "Above-normal gas — supporting wider peak spreads"** — threshold-direction logic appears inverted in copy generator; (c) **RenewableMix 143% headline + "Wind 52% · Solar 91% · Thermal 0%" sub-bar** — sub-bar values' semantic meaning unclear, don't reconcile to 143% aggregate; (d) **PeakTrough "Buy €15 · Sell €153" (€138 spread) vs hero €135.5** — rounding from underlying but displayed numbers don't add up exactly; cross-border `€-6.9/MWh LT-SE4` semantically unclear. Math-reconciles cards (ResidualLoad, EU ETS Carbon) noted. **31.B — Structural normalization (~3-4h tight / ~6-8h wide with `<MetricCard>` primitive extraction)** ships after 31.A.

**Phase 32 — Reference asset recalibration from Bitėnai cascade** filed. Bitėnai 4-layer math architecture (`~/Documents/KKME/01_Deals/Prosperus_48MW_96MWh/`) produces realistic POI RTE 79.38-83.12% per `11_RTE_SOH_cascade_check.py` — substantially below industry-typical published claims (85-90%). Operator wants this to recalibrate KKME's 50 MW Baltic reference asset, anonymized + padded "+10-15%" (interpreted as conservative-published-numbers: cost padded UP, performance padded DOWN). **Confidentiality gates documented up-front per discipline rule #3 + Bitėnai project rule "Never share supplier A's pricing with supplier B":** no supplier-specific config patterns published (no PE-PCSM-with-Hithium-18×6.25 etc.); aggregate ranges only ("POI RTE 76-82%" not "80.53%"); no trade names / model numbers / audit-letter refs (FSIC20042026AI etc.); internal trace memo stored OUTSIDE the public repo at `~/Documents/KKME/.../private/`. Depends on Phase 31.A shipping first (need to know which fields require recalibration).

**Discipline rules applied:**

- **#1 audit-triage** — bug verified empirically via chrome MCP DOM probe + globals.css code-grep BEFORE shipping fix. Original pre-summary diagnosis (ScrollReveal inline transform from `useScrollReveal` hook overriding hover) was wrong; actual diagnosis (cardFadeIn animation fill-state outranking hover) only surfaced after grepping globals.css for transform-bearing rules. Pre-fix wrong-diagnosis course-correction is itself a rule-#1 win.
- **#3 named-entity verification** — Phase 32 candidate carries the discipline explicitly in roadmap text. Confidentiality gates are load-bearing (would prevent Phase 32 from shipping safely if dropped).
- **#5 roadmap edit-conflict** — Phase 31 + 32 filed by Cowork after operator explicit "add it to the list" / "add new phase, yes" instructions. Roadmap commit `bb96655` separate from hover-lift commit `a0f36ea` (single-logical-change per commit).
- **#6 no-editorial-state-label** — no chips, no state strings; pure motion design.

**Foundation gates:** NOT run in Cowork session per `feedback_cowork_sandbox_limits.md` (vitest + next-build unreliable in Cowork sandbox; reliable in operator's Mac shell). Hover-lift verification was operator-local `npm run dev` smoke + chrome MCP DOM probe of `.card:hover` transform application. Local-build smoke-test (per `feedback_local_build_verification.md`) NOT executed pre-commit; risk class LOW (CSS-only edit; no new code paths, no chunk emission changes).

**Orphan discovery (2026-05-13):**

Session 56 PR #78 (`8cf30eb`) was merged to main BEFORE commit `a0f36ea` (Phase 18.3.1 hover-lift) was added to the feature branch. Commit `a0f36ea` therefore landed on `phase-18-3-animation-activation` but is NOT in main. Production frontend has Phase 18.3 (3-card `card--interactive` modifier) but NOT Phase 18.3.1 (universal `.card / .card-tier1-feature / .card-tier3` hover-lift). Empirical confirmation via `curl https://kkme.eu/ + grep card.*hover`: production CSS contains `.card--interactive:hover { transform: translateY(-2px) }` (Phase 18.3 original); does NOT contain `.card:hover` / `.card-tier1-feature:hover` / `.card-tier3:hover` / `translateY(-0.5px)`. **Recovery:** roadmap commit `bb96655` added to same branch on 2026-05-13; branch pushed; operator opens new PR (no body, no branch delete per `feedback_pr_workflow_minimal.md`) and merges to land both 18.3.1 + Phase 31/32 candidates.

**Process lesson worth memorializing:** Cowork hot-fix commits on a feature branch AFTER the operator merges the parent PR become orphans. Pattern: PR #78 merged → operator says "perfect, let's keep this" → Cowork ships hot-fix to same branch → no new PR opens because state isn't refreshed → 5-day gap → next session discovers orphan. **Mitigation pattern:** after any post-merge Cowork hot-fix on a feature branch, explicitly verify the merge state and either (a) open a new PR for the hot-fix immediately, or (b) cherry-pick onto a fresh branch. Memory-entry candidate.

**Next operator action:** open PR for `phase-18-3-animation-activation` against main → merge → hard-refresh kkme.eu → verify universal hover-lift fires on all cards except hero map. Then Phase 31.A is the natural next CC pick (data integrity before visual normalization, before Bitėnai recalibration).
...
**Pattern set refined at Pause A based on empirical corpus check** (210 curations + 8 feed items):

- DROPPED `/Ä[A-Z]/` — JS regex `[A-Z]` only matches U+0041–U+005A; the empirical mojibake bigrams (`Ä„`, `Ä—`, `ÄŠ`, `Ä™`, `Ä¯`, `Ä…`) all have non-A-Z chars after `Ä` (extended-Latin like `Š`, punctuation like `„` `—` `™`). The regex was empirically dead — zero hits across the 210-item corpus.
- DROPPED bare `/Ä/` — false-positive risk on legitimate Estonian content (`Tähtsamad`, `Käiku` — `Ä` is an Estonian letter). Future Eesti regulator items would have been falsely rejected.
- DROPPED bare `/Å/` — false-positive risk on Swedish/Finnish/Norwegian/Danish content (Stora Enso etc.) which is in scope as Baltic-adjacent.
- ADDED 7 explicit Lithuanian Win-1252-misdecode bigrams (`Ä„` `Ä…` `Ä—` `Ä™` `Ä¯` `Å³` `Å«`) covering the full LT special-char table (`ą č ę ė į š ų ū ž`).

**Final 10-bigram set:** `/Ä„/` `/Ä…/` `/Ä—/` `/ÄŠ/` `/Ä™/` `/Ä¯/` `/Å¡/` `/Å¾/` `/Å³/` `/Å«/` — zero false-positives against current 210-curation + 8-feed corpus.

**4-test verification protocol (worker version `e150b2d8`):**

| # | Input | Expected | Actual | Verdict |
|---|---|---|---|---|
| 1 | title `Test ÄŠgumas` (mojibake) | 400 / field=title / pattern=ÄŠ | as expected | ✅ |
| 2 | clean Lithuanian `Test plėtoti pajėgumas` | 201 / id returned | `mownfpv9-6kweqy` | ✅ |
| 3 | raw_text `Test ÄŠgumas dirty` | 400 / field=raw_text / pattern=ÄŠ | as expected | ✅ |
| 4 | legitimate Estonian `Eesti küsimused energiavarustuse` + `Tähtsamad küsimused` | 201 / id returned | `mownfqhs-nwli7n` | ✅ |

Test 4 is the load-bearing defensive verification — proves the gate doesn't false-positive on legitimate non-Lithuanian Baltic content (Estonian `ü`, `ä`). If the prompt's bare `/Ä/`/`/Å/` patterns had stayed, Test 4 would have rejected.

**Test entries cleaned via `/feed/delete-by-id`** by operator post-merge: `cur_mownfpv9-6kweqy` + `cur_mownfqhs-nwli7n` (UPDATE_SECRET in operator's shell, not CC's). Both entries had `status: 'rejected'` in `feed_index` because `source: 'test'` fails the source-tier gate, so they were never publicly visible at `/feed`; 30-day TTL on `/curations` would have reaped them anyway.

**Existing 7 mojibake duplicates of `cur_mowid0zn-siyurd` "[PDF] Leidimai plÄŠtoti..."** still in `/curations` — invisible at `/feed` (de-duped to 1 item; that 1 was already deleted earlier per Phase 4G's `cur_mo87wkt8-65w5pc` purge). 30-day TTL will reap. Filed as future cleanup task; not 4G.1 scope (gate stops new writes; doesn't retroactively scrub).

**Discipline rules applied:**

- **#1 audit-triage** — Pause A's empirical false-positive sweep against the 210-curation corpus produced the pattern-set refinement. Following the prompt's literal pattern set verbatim would have broken Estonian content. Direct operating-discipline win.
- **#3 named-entity verification** — Phase 4G.2 candidate filed for the entity-verification layer at `/curate` ingest (cross-check `source_url` against allowed commercial registries: `registrucentras.lt`, Lursoft, `inforegister.ee`). Out of 4G.1 scope to keep ship size tight.
- **#5 roadmap edit-conflict** — no roadmap edits; needed deltas reported below for operator/Cowork.
- **#6 no-editorial-state-label** — error message is operator-honest: `"Title contains mojibake (cp1257-misdecoded-UTF-8). Re-paste from a UTF-8 source."` Explains what the validation fail is + the re-paste fix; not vague "validation failed".

**Foundation gates (all baseline-exact, no regression):** tsc 0 / vitest **924/925** (pre-existing `freshnessBadge` "renders TODAY label between 6h and 24h" failure carries forward as known baseline; introduced by Phase 21's EET-day-aware freshness logic; time-of-day-dependent assertion fails in certain windows; foldable into 12.10b housekeeping or own micro-phase later) / lint 127 (40e/87w) / no-raw-spacing exit 0 / no-editorial-chips exit 0 / build 7 routes (clean).

**Local-build smoke-test (per `feedback_local_build_verification.md` REQUIRED gate):** `npm run build && npx serve out -l 3100` → all 4 routes HTTP 200 (`/` 105 KB, `/intel` 34 KB, `/methodology` 329 KB, `/regulatory` 26 KB). Worker-only change; smoke-test redundant by construction but cheap.

**Files touched (1):**

- `workers/fetch-s1.js` — `MOJIBAKE_PATTERNS` constant + `detectMojibake(s)` + `validateCurationContent(body)` helpers added in the curation-helpers section (line ~5054); validation wired into `POST /curate` handler immediately after the required-fields check (line ~7343); new error envelope `{error: 'Encoding validation failed', field, pattern, message}` at HTTP 400. ~57 lines added, 0 lines deleted.

**Risk class LOW confirmed** — pure-string validation; no engine math, no KV schema change, no chunk emission, no frontend touch. The only structural change is a 400-status-return path added before `storeCurationEntry`/`appendCurationToFeedIndex`; both downstream functions remain unmodified.

**Operator workflow:** open PR → click merge; no PR body draft, no branch delete (per `feedback_pr_workflow_minimal.md`). Future operator pastes via `POST /curate` are now validated.

**Roadmap delta needed (operator-side after merge, Cowork applies per discipline rule #5):**

- Phase 4G.1 → Shipped appendix
- Currently-active update: 4G.1 SHIPPED; next CC pick across remaining Tier 1 queue (4G.2 entity verification candidate, 18.3 animation activation, 12.12 #1+#2 schema validation gates, 7.7g-a-2 spacing tokens rollout, 29.1 per-country DA capture, 22 institutional S2)
- **Phase 4G.2 candidate filed** — entity verification at `/curate` ingest per discipline rule #3. Cross-check `source_url` against allowed commercial registries (`registrucentras.lt`, Lursoft, `inforegister.ee`) + regulator decision URLs + named-press-release paths. Estimated 1-2h. Closes the third operator-paste-quality layer (after encoding 4G.1 + the existing source-tier gate).

**Tier 1 sequence:** 4G.1 ✅ → next CC pick across remaining queue (4G.2 entity verification, 18.3, 12.12 #1+#2, 7.7g-a-2 rollout, 29.1, 22).

**Next operator action:** open PR; merge; after merge fire the 2 cleanup curls (`cur_mownfpv9-6kweqy` + `cur_mownfqhs-nwli7n` via `/feed/delete-by-id` with `$UPDATE_SECRET`); future operator pastes via `POST /curate` are now validated end-to-end.
...
- (4b) Sensitivity tornado SVG — `<svg width={width}>` (literal 360 attribute, non-responsive) → `viewBox + preserveAspectRatio="xMidYMid meet" + width:100% + maxWidth:width`. Renders at 246px (parent-track width) at 360 viewport instead of escaping to native 360px.
- (4c) MonthlyHeatmap — `gridTemplateColumns: '60px repeat(13, 1fr)'` → `'60px repeat(13, minmax(0, 1fr))'`. 13 month cells resolve to 13.5px each at 360 viewport instead of max-content 22px.
- (3) S3 BreakdownBar — `.breakdown-bar` family classNames added (`__label / __range / __scope / __bar / __pcs-note`) + **6 split @media blocks** in globals.css (Tailwind v4 silent-drop pattern: each rule in its own `@media (max-width: 720px)` block; multi-selector blocks ALSO drop, not just stacked-inside-existing). Bars stack to 3 visual rows on <720px (label / range+scope / bar). `flex-wrap: wrap !important` confirmed via getComputedStyle at 360.
- (1) DSCR triple panel — `min-width: 0` + `overflow-wrap: anywhere` on inner cells + sublabel; new `.rv-dur-dscr` class wrapping DurationOptimizer + DSCRPanel with `minmax(0, 1fr) minmax(0, 1fr)` desktop / single-column at <720px (replacing inline `'1fr 1fr'` that resolved to max-content under DurationOptimizer's pill-row pressure).
- (2) AssumptionsPanel grid — **THE OPERATOR-SCREENSHOT BUG** — replaced inline `gridTemplateColumns: 'auto 1fr'` with `.assumptions-grid` class (`auto minmax(0, 1fr)` desktop, single-column at <720px). Dropped `whiteSpace: 'nowrap'` on **5 label cells** (RTE row + v7.3 cycles row + v7.2 fallback row + 3 standard rows). Added `overflowWrap: 'anywhere'` to italic notes. New `.assumptions-grid__label` + `.assumptions-grid__note` classNames. `getComputedStyle` at 360 confirms `gridTemplateColumns: '212px'` (single col); both label and note 212px wide. **No vertical-character-per-line rendering** — operator-screenshot bug closed with empirical visual evidence at `docs/visual-audit/phase-18-1-3/360-assumptions-panel.png`.
- (4a) chart-canvas track — `.rv-main > *, .rv-analytics > * { min-width: 0 }` injected into RevenueCard's inline `<style>` block. Chart.js Line + Sensitivity tornado SVG canvases shrink to grid track width instead of becoming 360px auto-minimum.

**Both band-aids dropped successfully** — `html { overflow-x: hidden }` (Phase 18.1 page-level clip, restored after 18.1.1 revert) AND `body { overflow-x: hidden }` (older defensive trim) both REMOVED at `app/globals.css:486 / :502`. `body { overscroll-behavior-y: contain }` retained (not a clip rule). Matches Phase 18.1.1 success pattern (rule #4 cross-card consistency). 5-viewport `scrollWidth=clientWidth` verified at 360 / 414 / 768 / 1024 / 1440 against the freshly-rebuilt `out/` artifact via cache-bypassed reload — `getComputedStyle().overflowX === "visible"` confirms band-aid actually gone in served bundle (not just in source). All 5 viewports PASS.

**Chrome MCP gate restored end-to-end** — first chrome-MCP-clean ship since Session 47 (5+ consecutive sessions had recurring browser-lock condition forcing curl-only verification with prose state docs). The `cc` alias fix per `feedback_chrome_mcp_orphans.md` is **empirically confirmed** this session: `mcp__chrome-devtools__navigate_page`, `resize_page`, `emulate`, `evaluate_script`, and `take_screenshot` all worked end-to-end. **Process-recovery milestone alongside the Phase 18.1.3 ship.** Visual evidence captured this session (7 screenshots) where prior 5 sessions delivered prose-only:

- `360-assumptions-panel.png` — operator-bug closed, single-col stack with paragraph-block note wrapping
- `360-dscr-triple.png` — DurationOptimizer + DSCRPanel stacked, no overflow
- `360-s3-breakdown.png` — bars wrap to 3-row layout cleanly
- `360-tornado.png` — SVG scales to 246px parent track (was escaping to 360px native)
- `360-heatmap.png` — 13 month cells fit inside parent
- `414-page.png`, `1440-page.png` — desktop overview, no regressions

**Pause B verification (band-aids removed, fresh build):**
| Viewport | scrollWidth | clientWidth | html overflowX | Status |
|---|---|---|---|---|
| 360 | 360 | 360 | visible | ✅ PASS |
| 414 | 414 | 414 | visible | ✅ PASS |
| 768 | 757 | 757 | visible | ✅ PASS (11px scrollbar) |
| 1024 | 1013 | 1013 | visible | ✅ PASS (11px scrollbar) |
| 1440 | 1429 | 1429 | visible | ✅ PASS (11px scrollbar) |

**Risk class LOW confirmed** — phase touched zero of: `HeroBalticMap.tsx`, `MobileBalticMap`, GSAP, dynamic imports, lazy boundaries. Phase 18.1.1's ChunkLoadError came specifically from MobileBalticMap (~200 lines) + GSAP guard tweak — neither touched here. Local-build smoke-test (per `feedback_local_build_verification.md` REQUIRED gate): `npm run build && npx serve out -l 3100` → home page HTTP 200; **20/20 JS chunks** referenced from `index.html` return HTTP 200 → 18.1.1-class ChunkLoadError class **structurally absent**.

**Bundle delta:** CSS chunk `40,479 B` (smaller than 18.1.1's 43,470 B reference — 7.7g-a-3 typography pruning earlier in the day was load-bearing for the smaller envelope; net 18.1.3 CSS additions of new `.assumptions-grid / .rv-dur-dscr / .breakdown-bar*` blocks ~1.5KB are absorbed by the prior pruning headroom). Frontend-only, no `model_version` bump, no worker deploy.

**Foundation gates (all baseline-exact):** tsc 0 / vitest 925/925 / lint 127 (40e/87w) / no-raw-spacing exit 0 / no-editorial-chips exit 0 / build 7 routes (clean).

**Phase 18.1.4 candidate:** **NOT FILED** — both band-aids removed cleanly across all 5 viewports; no regressing site surfaced; honest empty-state. If post-merge operator hard-refresh exposes any new edge case, Phase 18.1.4 can be filed at that time with the concrete file:line target (per discipline rule #1: visual-inference audit claims need triangulation, not preemptive scoping).

**Discipline rules applied:** #1 (audit-triage) — re-grepped line numbers at Pause A vs trusting prompt's 18.1.1-era line citations; surfaced +28 drift on DurationOptimizer wrapper; #4 (cross-card consistency) — uniform 720px breakpoint across all reflows, uniform `min-width: 0` + `overflow-wrap: anywhere` combo, uniform class naming (`.{component}__{slot}`); #5 (roadmap edit-conflict) — no roadmap edits; needed delta reported below for operator/Cowork.

**Operator workflow:** open PR → click merge; no PR body draft, no branch delete (per `feedback_pr_workflow_minimal.md`).

**Roadmap delta needed (operator-side after merge, Cowork applies per discipline rule #5):**
- Phase 18.1.3 → Shipped appendix
- Currently-active update: 18.1.3 SHIPPED; next CC across remaining Tier 1 queue (18.3 / 19 / 12.12 #1+#2 / 7.7g-b reduced / 18.2.4 / 18.2.5 / 18.2.1 / 20 / 18.1.1.1 / 21.1 / 18.1.2)
- AssumptionsPanel right-column-vertical-text bug closed (was the Phase 18.1.3 trigger)
- No Phase 18.1.4 follow-up needed

**Next operator action:** open PR; merge; hard-refresh kkme.eu mobile + desktop; verify AssumptionsPanel right column wraps cleanly (operator-bug closed with empirical visual evidence in this session — should be a quiet confirmation, not a debugging session).

**Tier 1 sequence:** 18.1.3 ✅ → next CC pick across remaining queue.

### Session 51 — 2026-05-07 — Phase 21.2 — S2 monthly trajectory + capacity_monthly KV fix (Claude Code)

**Headline:** Operator opened S2 drawer "Monthly trajectory — Baltic aggregate" 2026-05-07 and saw a single bar at "Mar 26"; production curl confirmed `capacity_monthly[0] = { month: "2026-03", days: 1 }`. Discipline-rule-#2 anti-pattern (no-hardcoded-temporal-label without computing it; "monthly" must mean monthly). Phase 21.2 ships four sub-items in one PR — (a) `fmtMonth` format `Mar 26` → `2026-03` (operator-picked ISO) cross-card across S1 + S2; (b) KV diagnostic confirming dual-write architecture mismatch (worker-direct cron writes `s2_btd_history`, Mac-cron POST `/s2/update` did NOT — when BTD's edge-side SSL flickered to 526 and Mac cron stepped in, the daily history accumulator silently no-op'd); (c) shared `appendBtdHistory(env, payload)` helper paired into **three** write paths (4h cron at `fetch-s1.js:6329`, 09:30 watchdog at `:6205`, POST `/s2/update` at `:7269`) + new UPDATE_SECRET-gated `POST /s2/btd-history/backfill` endpoint as one-shot recovery tool that reads `s2_history` (consistently-written across all paths) and re-emits to `s2_btd_history`; (d) dynamic empty-state fallback at threshold `< 3` months with data-derived accrual date (rule #6 compliant). **Worker deploy `b2152be9-ed8b-4e71-9f09-9c11d96d273a` live**; backfill endpoint returns 401 unauthenticated as expected.
...
Expected: `{ ok: true, source_count: ~90, target_after: ~90, added: ~89 }`.

Verify post-backfill:
```bash
curl -s https://kkme-fetch-s1.kastis-kemezys.workers.dev/s2 | jq '.capacity_monthly | length'
# Expect: ≥3-6 (re-activates the chart)
```

**Without this curl, the empty-state fallback continues rendering until the daily cron accumulates ≥3 distinct months naturally (~3 months of clock time).** Don't bury this instruction.

All 6 gates baseline-exact (tsc 0, vitest 925/925, lint 127 (40e/87w), no-raw-spacing exit 0, no-editorial-chips exit 0, build 7 routes). 3 files modified, ~25 insertions / ~7 deletions net (excluding unrelated `logs/btd.log`). Bundle delta ≈ zero (fmtMonth bodies shrunk to identity; capacityEmptyStateText helper ~12 lines; worker change is server-side, not in client bundle). **Visual-audit gap (recurring 4th consecutive session — 47 + 48 + 49 + 50 + 51)**: chrome-devtools-mcp browser-lock condition recurred; operator pre-approved curl-only verification path with prose state description at `docs/visual-audit/phase-21-2/STATE.md`. Cross-session friction observation already filed as roadmap operator-action-item (chrome MCP `--isolated` userDataDir investigation).

**Process honesty note (rule #3 spirit applied to write-path discovery, not to entity-naming):** Pause A diagnosis identified TWO write paths into `s2_btd_history` (worker-direct cron + Mac-cron POST). During implementation I discovered a **THIRD** write path I missed at scope-time: the **09:30 UTC daily S2 watchdog** at `fetch-s1.js:6189`. It also wrote `s2` KV but skipped the `s2_btd_history` append. Now also paired. Surface revision noted in Pause B report; the no-silent-drops principle is working as designed — flagging the gap rather than burying it. The fix shape didn't change (same helper, same call site pattern), only the call-site count (3 paths now paired, not 2).

**No specs added** — `fmtMonth` body becomes `return m` (tautological identity); `capacityEmptyStateText` is time-dependent (`new Date()` inside; date arithmetic against today). Operator approved skip; if regression hits either, file Phase 21.2.1 with a minimal `vi.useFakeTimers()` mocked-Date pattern.

**Cross-card check (rule #4):** `s1_capture.monthly` curl-verified GREEN — 15 months populated (2025-03 through 2026-05). Different KV pipeline (`s1_history` via `updateHistory()` at `fetch-s1.js:3421-3442`; shape includes `gross_2h`, `gross_4h`). **NO Phase 21.3 follow-up needed.**

**Branch:** `phase-21-2-monthly-trajectory-kv-fix` off `main` at `b50ad91` (post-Phase-21-merge + phase-21-2 setup commit).

**Pause A — discovery + KV diagnosis:**

| Discovery | Finding |
|---|---|
| `fmtMonth` cross-card consumer audit | Duplicated across `S1Card.tsx:69-73` (consumed at line 507 — MonthlyChart x-axis) and `S2Card.tsx:164-168` (consumed at lines 781, 793, 866, 884, 897 — MonthlyTrajectoryChart + tooltip + PinReadout fmtKey + CapacityChart + CapacityChart tooltip). Both definitions identical. Mild pre-existing rule #4 violation (helper duplicated, not centralized) — keeping surgical fix; hoisting to `app/lib/` deferred. |
| KV diagnostic — production state | `capacity_monthly[0] = { month: "2026-03", afrr_avg: 7.7, mfrr_avg: 21.54, fcr_avg: 0.35, days: 1 }`, length=1. Empirical confirmation of prompt's smoking gun. |
| `computeCapacityMonthly` audit | `fetch-s1.js:4404-4417`. Correct. Groups by `date.slice(0,7)`, counts via `v.afrr.length`, no filter that drops months. NOT the source of under-population. |
| Root cause | **Dual-write architecture mismatch** (revised at impl-time to **triple-path mismatch**). Worker cron paths at `:6329` (4h) and `:6205` (09:30) AND Mac-cron POST `/s2/update` at `:7269` all write `s2` KV, but only the 4h cron appended to `s2_btd_history`. When BTD has SSL 526 issues (worker comments at `:4113`, `:4334`, `:6290` — frequent), Mac cron fills in but the daily-accumulator silently no-op'd. |
| Fix proposal | Lift inline append into `appendBtdHistory(env, payload)` helper near `appendSignalHistory`; pair into all three write sites; add `POST /s2/btd-history/backfill` UPDATE_SECRET-gated endpoint that copies missing dates from `s2_history` (consistently written across all paths) into `s2_btd_history`. **No external BTD historical API call needed** — recovery source already in production KV. |
| Empty-state fallback | Threshold `< 3` months. Muted line text data-derived: `"Capacity history accruing — N months collected (YYYY-MM list); full trajectory available YYYY-MM-01."` Computes accrual date as `today + (3 - capMonthly.length) months, day=1`. Rule #6 compliant. |
| Cross-card s1_capture check | `s1_capture.monthly.length = 15` (2025-03 through 2026-05). Different KV pipeline. **GREEN** — no Phase 21.3. |
| Refined estimate | ~2.5h vs prompt's 2-4h. |

**Pause B — foundation gates + worker deploy + smoke-test:**

| Gate | Baseline | Post-edit | Status |
|---|---|---|---|
| `npx tsc --noEmit` | 0 | 0 | ✅ baseline-exact |
| `npx vitest run` | 925/925 | 925/925 | ✅ baseline-exact (no new specs added per skip-decision) |
| `npm run lint` | 127 (40e/87w) | 127 (40e/87w) | ✅ baseline-exact |
| `npm run lint:no-raw-spacing` | exit 0 | exit 0 | ✅ |
| `npm run lint:no-editorial-chips` | exit 0 | exit 0 | ✅ |
| `npm run build` | 7 routes | 7 routes | ✅ baseline-exact |
| Worker deploy | `2b3c6bc5-...` (Phase 21) | `b2152be9-ed8b-4e71-9f09-9c11d96d273a` | ✅ live |
| Backfill endpoint smoke | n/a | `POST /s2/btd-history/backfill` → 401 unauth (gate working) | ✅ |
| Local prod-build smoke-test | 20 chunks 200 / 4.5 MB | 20 chunks 200 / 4.5 MB | ✅ baseline-exact (18.1.1 ChunkLoadError class structurally absent) |

**Files modified (excluding unrelated `logs/btd.log`):**

| File | +/− | Notes |
|---|---|---|
| `app/components/S1Card.tsx` | +1 / −4 | `fmtMonth` body → `return m` (identity) |
| `app/components/S2Card.tsx` | +18 / −5 | `fmtMonth` body → `return m`; new `capacityEmptyStateText(months)` helper; drawer threshold `> 0` → `≥ 3` with dynamic empty-state line |
| `workers/fetch-s1.js` | +52 / −19 | New `appendBtdHistory(env, payload)` helper paired with `appendSignalHistory`; replaced inline 20-line block at 4h-cron site with helper call; added helper call at 09:30-watchdog site + POST `/s2/update` site; new `POST /s2/btd-history/backfill` UPDATE_SECRET-gated endpoint (~25 lines) |

**Pause C — visual + commits:**

Visual audit prose state description authored at `docs/visual-audit/phase-21-2/STATE.md` (per recurring chrome MCP lock pattern, 4th consecutive session); covers (1) the load-bearing operator backfill action highlighted at top, (2) UI surface descriptions pre-fix vs post-fix vs post-backfill, (3) curl-based verification evidence, (4) cross-card s1_capture verification, (5) discipline-rule application notes.

**Out of scope reminder (per prompt §):** engine math; other monthly aggregations beyond capacity_monthly; historical KV architecture redesign; backfill earlier than `s2_history` 90-day cap; forward curves; scenario analysis; roadmap edits (operator/Cowork-owned per discipline rule #5).

**Roadmap delta needed (operator-side after merge — discipline rule #5 forbids CC commit):**

- Phase 21.2 → Shipped appendix
- Currently-active update: 21.2 SHIPPED; next CC across (18.2.4 / 18.2.5 / 18.2.1 / 18.1.2 / 18.3 / 12.12 #1+#2 / 7.7g-a-3 / 7.7g-b / 19 / 20 / 18.1.1.1 / 21.1)
- Phase 21.3 candidate **NOT FILED** (s1_capture verified healthy)
- Phase 21.2.1 conditional candidate filed (minimal mocked-Date specs for `fmtMonth` + `capacityEmptyStateText` if regression hits)

**Tier 1 sequence:** 21.2 ✅ → next CC pick across (18.2.4 / 18.2.5 / 18.2.1 / 18.1.2 / 18.3 / 12.12 #1+#2 / 7.7g-a-3 / 7.7g-b / 19 / 20 / 18.1.1.1 / 21.1).

**Next operator action:**
1. Open PR; click merge.
2. **Run the load-bearing backfill curl above** as soon as Cloudflare Pages stabilizes.
3. Verify `capacity_monthly | length ≥ 3` post-backfill.
4. Hard-refresh kkme.eu mobile + desktop dark + light; confirm S2 drawer "Monthly trajectory — Baltic aggregate" shows multiple `YYYY-MM`-formatted bars (post-backfill) OR honest accrual line (if backfill not yet fired).
