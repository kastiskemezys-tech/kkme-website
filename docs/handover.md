...
| B-063 | bug | P1 | The shipped engine and the hourly engine disagree ~2.5× on arbitrage share of gross, and both pass their own gates | 2026-08-03 Phase 38.3 | open — **blocks the cutover, owned by 38.4** | Surfaced while measuring 38.3's cutover. Shipped `computeRevenueV7` books Y1 arbitrage at **27.1 %** of gross (`rev_trd` 2,188,823 of 7,994,239, frozen KV, 2h/mid/2028/base); B1's hourly run over LT-2025 books **10.7 %** (arbitrage 545,579 of gross 5,085,873). Same asset, same market state resolution (the runner resolves reserve inputs through the worker's own `capPrice`, rule #4), largest merchant line, ~2.5× apart — and **neither is failing anything**: the hourly run passes energy balance to 1.6e-9 MWh, zero constraint violations over 8760 hours, warranty never breached; the shipped engine passes 54/54 byte-identity and every reconcile band. **This is B5 in its purest form** — the two are compared only against each other and against gates each already satisfies, so the disagreement has never had to resolve. Mechanically it traces to the DA throughput each is allowed: shipped applies `trading_fraction` 0.70 (an ECONOMIC switching model, `min(0.70, (T/(T+R))×0.75)`), hourly finds ~27.5 % of nameplate MW physically free after reserve commitment. **These are not composable and not obviously the same quantity** — one is an allocation of value, the other a constraint on MW — which is exactly why 38.3 refused to multiply or substitute them. Until it is reconciled, no cutover can be adopted in either direction, because the choice would be made by which number is prettier. Independent check available and specified in 38.4: B1's own simultaneity measurement (75.2-85.5 % of the unconstrained stack) distinguishes a physically-correct commitment rule from a merely conservative one. |
| B-064 | tech-debt | P2 | The degradation model has no characterisation below 1.0 c/d, and until 38.3 said so nowhere | 2026-08-03 Phase 38.3 | partly closed — limit published, model unchanged | `sohYr` (`workers/fetch-s1.js`) does `cd = Math.max(cd_total ?? 1.0, 1.0)` and `getDegradation` floors its 2 c/d weight at zero. The three SOH curves are calibrated at 1.0 / 1.5 / 2.0 c/d test rates and nothing extrapolates below the slowest, so **the engine cannot represent a battery cycling at 0.6 c/d ageing any more slowly than one at 1.0 c/d.** Defensible — manufacturers do not characterise below 1 c/d either — but it is a LIMIT, and a silent limit reads as a result. It is also why 38.3's cutover moved project IRR only +0.07 to +0.15 pp: the entire benefit of 1.36 → 0.60 c/d is the partial blend toward the 2C curve, and everything below the floor is clamped away by construction. The stress scenario already sits at 0.92 c/d, so a third of the 54-config matrix does not move at all. **38.3 published the floor as drawer disclosure** (`app/lib/wearModelRange.ts` + `WearModelRangeNote`), computed from the payload and asserted against the engine's real behaviour by bisection rather than against the constant that describes it. **That is disclosure, not a fix.** A wear model valid only at ≥1.0 c/d cannot price a 0.6 c/d asset regardless of which dispatch wins, so characterising below 1.0 c/d — or formally adopting the floor as a published model boundary — is a PRECONDITION for the cutover. Owned by 38.4 Q2. |
