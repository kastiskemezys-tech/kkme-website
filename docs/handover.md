...
Last updated: 2026-06-15 (Session 81 — Phase 33.B.3: KV-persisted capacity-watch accumulator + `:2871` label fix (deploy `7928299a`): `flagOutOfBandS2Capacity` was console-only; now persists daily `s2_capacity_watch:<date>` (deduped per s2 snapshot → ~6 writes/day, min/max/last/above_50_pct for fcr+afrr_up/down+mfrr_up/down, 30d TTL) via `ctx.waitUntil`; read via `GET /admin/capacity-watch?days=N` (X-Update-Secret). Feeds 33.B.2 (2026-06-29). IRR stable 20.0%, no engine change. Process change memorialized: CC runs `npx wrangler deploy` directly post-SHA-check. Full entry in Session log below. ‖ Prior — Session 80 — Phase 33.B: BTD capacity-price documentation + `prices_source` relabel (Path C, deploy `82df190f`): zero-behavioural-change doc ship. Pause A found BTD DOES carry capacity prices (parser extracts `price_procured_reserves` cols 10-14 as `s2.afrr_up_avg/fcr_avg` = directional capacity €/MW/h, NOT activation — 4th rule-#1 premise correction); engine reads never-produced `*_cap_avg` → calibrated constants always active. No remap: live values 2-177× constants (FCR anomalous), would re-introduce the IRR swing Phase 33's bound prevents. Constants canonical until 33.B.2 operator review (2026-06-29, all 3 products). Filed 33.B.3 (KV-persisted watch). IRR stable 20.0%. Full entry in Session log below. ‖ Prior — Session 79 — Phase 33.A: Baltic map-feed allowlist + reject-on-flag + `/admin/purge-non-baltic-fleet` (deploy `2306c828`): /s4 196→170 (26 ESN polluters purged), zero non-Baltic. Premise correction: purge removed 4967 MW fake foreign pipeline → sd_ratio 2.39→1.86 → /revenue IRR 18.5%→20.0% (data-quality fix, not regression; operational_mw unchanged). Blank-cod = upstream → filed 33.A.3. Full entry in Session log below. ‖ Prior — Session 78 — Phase 33: revenue capacity-price bound (IRR 49.6%→18.5%, v7.3 restored) + VPS dash/cron fix (baltic 457h→0h) + s1_capture dual-writer close. Full entry in Session log below. ‖ Prior — Session 73 — PRODUCTION INCIDENT (Cowork, VPS-side, no git commits): operator returned after ~1 week, Telegram flagged S4 staleness. Audit found 4 VPS inline crons silently frozen ~20 days by malformed `.env` lines 12-14 (orphaned `60e41fd4…` values, no `KEY=` → broke `source .env && …` chain). Fixed `.env` (commented 12-14) + caught up all 4 (baltic_index, kkme_sync, hourly_grid, ingest_daily) + hardened crontab (`source .env &&` → `source .env ;`, backup at `/opt/kkme/config/crontab.bak.20260527`). S4 → 0h, baltic_index → 0.4h, all worker signals green (da_tomorrow pre-publication). Follow-ups filed: watchdog-coverage review, trailing-12m data-depth, .env orphan-value identification. Full entry in session log below. Previously Session 72 — Phase 18.2.2 (pipeline-bar on-colored-bg label contrast) shipped on `phase-18-2-2-pipeline-bg-contrast` off latest main (`8a4a1f4`): two-file scoped edit in `app/globals.css` (3 new pipeline-chip tokens — `--pipeline-chip-text` non-inverting cream `#e8e2d9` in `:root` only; `--pipeline-chip-mint` `#016b5d` dark / `#1c705c` light; `--pipeline-chip-amber` `#7d5c1c` dark / `#7c601c` light) + `app/components/S4Card.tsx:468-485` (2 chip-bg + 2 chip-text token swaps on mint + amber pipeline segments; intention chip span at line 491 explicitly NOT touched — cream-on-amber-subtle would create new violation). axe-core delta: dark-1440 4→2, light-1440 4→2 (8→4 total, all 4 in-scope pipeline-bg violations cleared in both modes; 4 remaining = 2 doc'd exceptions × 2 modes = text-ghost decorative source-footer + white-on-mint chip elsewhere). Premise correction (rule #1 + Pause-A pre-empt): prompt pick #2 assumed "existing light/white label text" but chip text was `var(--text-primary)` which inverts (dark text in light mode, light text in dark mode) — pure "darken chip" would worsen light-mode contrast (text already near-black on now-darker chip). Resolution: text swap to non-inverting `--pipeline-chip-text` (cream-white in both modes) + per-mode darker chip-bg. Mid-Pause-B contradiction: operator's "var(--white) (NOT raw #fff)" instruction collided with `--white` token's mode-inversion (`#e8e2d9` dark / `#1a1a1f` light per `globals.css:30,293`); flagged + operator authorized 3rd scoped token `--pipeline-chip-text` to preserve scoping discipline. Final hex values darkened ~10-15 RGB units from initial proposal because cream-white `#e8e2d9` (L=0.766) is darker than pure white (L=1) and required lower L_bg to clear 4.5:1. Cross-card audit clean: `--teal-strong`/`--amber-strong` retained for S4 legend squares + S5/S7/S8/S9/HeroMarketNow/IntelFeed accent-text uses (no cross-card regression). Distinguishability preserved: mint (deep teal #016b5d/#1c705c) clearly distinct from amber (deep olive #7d5c1c/#7c601c) in both modes. devDep added: `playwright@^1.x` (`PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD=1`; browser binaries pre-cached at `~/Library/Caches/ms-playwright/chromium-*`) — closes the "flag gsap bump + playwright devDep" note from commit `3a83767`. Gates: tsc 0, test 919/919, lint:no-raw-spacing clean, lint:no-editorial-chips clean, build 7/7 routes, smoke-test 5 routes + 8 chunks all 200 (`npx serve out -p 3100`), lint pre-existing baseline (zero new in touched files). 2 PNGs at `docs/visual-audit/phase-18-2-2/`: `s4-pipeline-bar-light-1440.png`, `s4-pipeline-bar-dark-1440.png`. Roadmap delta (Cowork applies per rule #5): mark 18.2.2 SHIPPED, axe-core baseline now 4 (= 4 irreducible documented exceptions — a11y arc complete). Prior: 2026-05-18 (Session 71 — Phase 19.2.3 (parent-opacity Financing residual fix) shipped on `phase-19-2-3-financing-arrow-opacity` off latest main (`1faf0f8`): single-file 2-line component edit in `app/components/S3Card.tsx` driver-chip wrapper — dropped `opacity: stale ? 0.35 : 1` from wrapper at line 393, swapped chip text color at line 394 to conditional `stale ? var(--text-muted) : chipColor(drv.direction)`. axe-core delta: light-1440 + dark-1440 each 5 → 4 (−2 total). Both "↓ Financing" violations cleared (light 1.72→pass, dark 1.86→pass). 4 sibling driver chips inherit same fix (Battery hardware, Electrical/PCS, HV grid equipment, Financing). Sibling at line 565 (Drawer raw-inputs rows) deferred per rule #4 — different context (text-primary/text-secondary inside collapsed drawer, axe didn't flag). DOM probe confirmed parent_opacity=1, span color=`rgba(26,26,31,0.65)` light / `rgba(232,226,217,0.55)` dark, both ≥4.5 contrast. Gates: tsc 0, build 7/7, test 919/919 deterministic, lint:no-raw-spacing clean, lint:no-editorial-chips clean, lint pre-existing baseline (zero new in touched file). 2 PNGs at `docs/visual-audit/phase-19-2-3/`. Roadmap delta (Cowork): mark 19.2.3 SHIPPED, axe-core baseline now 8 (drops to 4 after Phase 18.2.2 ships).) Prior: 2026-05-18 (Session 70 — Phase 18.2.1 (Baltic palette retune, accent text-on-cream WCAG AA) shipped on `phase-18-2-1-baltic-palette-retune` off latest main (`8943399`): 3 new dedicated `--<color>-accent-text` light-mode tokens + `--rose` light direct shift `#dc3535 → #b82828` in `app/globals.css`; ~80 text-color migrations across ~30 component files (S3Card 13, IntelFeed 10, RevenueCard 8, S4Card 7, HeroBalticMap 3, S7/S8/S9/Solar/Wind/Load tier3-impact 6, others 30+). Pick #4 sentiment-lock honored (`--positive` ≡ `--teal` stays; new tokens used only at text sites). axe-core delta 43 → 10 across 6 configs (light-1440 27→5, light-1024/414 5→0); remaining 10 = 4 doc'd exceptions + 4 OOS (Phase 18.2.2 pipeline-bg) + 2 deferred (Phase 19.2.3 faded-teal Financing arrow). Gates: tsc 0, build 7/7, test **919/919** deterministic, lint:no-raw-spacing clean, lint:no-editorial-chips clean. Lint 129 (baseline-drift +2 carry from 127; zero new in touched files). 6 PNGs at `docs/visual-audit/phase-18-2-1/`. DOM probe confirmed all sampled accent sites route through new tokens; 12/12 chart canvases render; `--rose` shift propagates to chart paint. **Spec-vs-prompt hex mismatch flagged** for Cowork: prompt cited `#1a3833/#d4a574/#a8324a` as spec P2-2 — none appear in `docs/specs/2026-05-06-designer-developer-spec.md`; proceeded minimum-delta-from-shipped per pick #5. Roadmap delta (Cowork): mark 18.2.1 SHIPPED, file Phase 18.2.2 (pipeline-bg + 4 remaining selectors + chart-palette divergence) + Phase 19.2.3 (faded-Financing residual). See session log for full details.) Prior: 2026-05-18 (Session 68 — Phase 19.2.1 (inline-opacity audit) shipped on `phase-19-2-1-inline-opacity-audit` off latest main (`62d590d`): 14 component-level edits across 6 files (8 in `S4Card.tsx` LitGrid stage breakdown + pipeline bar; 6 cross-card-consistency siblings per rule #4 in WindCard/SolarCard/LoadCard/RevenueCard/TradingEngineCard). Pick (b) uniform — drop inline `opacity:0.35–0.9`, swap to darker existing token. No token shifts. axe-core delta 54 → 21 raw violations (−33); all 10 inline-opacity sites flagged by Phase 19.2 closing scan cleared. Remaining 21 = accents (Phase 18.2.1 turf, ~17) + 2 pipeline-bar labels on colored bg (Phase 18.2.1) + 2 docs-exception ghost decoratives. Gates: tsc 0, build 7/7, lint:no-raw-spacing clean, lint:no-editorial-chips clean. `npm run lint` 127 (baseline-drift family; zero new in touched files). `npm run test` 918/919; 1 pre-existing time-of-day flake in `freshnessBadge.test.tsx` (TODAY-band crosses Vilnius midnight at run-time 08:09 UTC; unrelated to opacity edits). 5-route + 8-chunk smoke-test green. 2 PNGs at `docs/visual-audit/phase-19-2-1/`. Roadmap delta (Cowork applies): mark Phase 19.2.1 SHIPPED + reaffirm Phase 18.2.1 (accents + bg-color contrast) + optional Phase 19.2.2 candidate (freshness Vilnius-midnight test flake). See session log for full details.) Prior: 2026-05-17 (Session 67 — Phase 19.2 (WCAG 2.1 AA color-contrast pass) shipped on `phase-19-2-color-contrast-aa` off clean main (`a361231`): 4 token-value edits in `app/globals.css` + 2 `aria-hidden="true"` annotations on decorative `--text-ghost` elements. axe-core delta 259 → 28 across 4 mode×viewport scans (−89%); zero `--text-muted` violations remain. 28 remaining all in documented-exception categories: Phase 18.2.1 candidates (accents, ~19), Phase 19.2.1 candidate (inline `opacity:0.35–0.7` × `--text-tertiary`, ~9), pick #7 ghost-decoratives (2, aria-hidden landed; axe-cli still flags visually). Gates flat vs baseline: tsc 0, vitest 919/919, build success, lint pre-existing 129 (baseline drift 125→129 noted across RevenueCard.tsx, not Phase 19.2 regression). 5-route + 8-chunk smoke-test green. 4 PNGs at `docs/visual-audit/phase-19-2/`. Roadmap delta (Cowork applies): mark Phase 19.2 SHIPPED + file Phase 19.2.1 (opacity-multiplied tertiary rewrite) candidate; reaffirm Phase 18.2.1 (Baltic accent palette retune). See session log for full details.) Prior: 2026-05-17 (Session 62 — Phase 31 (tier-3 card structural normalization) shipped on `phase-31-tier3-visual-normalization` off `08e26e7`: 6 cards under "Structural market drivers" now share filled-dot status + `clamp(1.5rem, 3vw, 1.75rem)` serif hero + "▸ View … detail" drawer footer; S7/S9 lost their `MetricTile size="hero"` 40-64px hero, green `observed` pill, and `StatusChip`; PeakForecast gained P25/P50/P90 marker bar; cards 1-4 gained Source/Computation/Limitations drawers. Gates flat vs baseline: tsc 0, vitest 919/919, lint 125, build success. 5-route + 8-chunk smoke-test green. 3 PNGs at `docs/visual-audit/phase-31/`. Roadmap delta (Cowork applies): mark Phase 31 SHIPPED + Phase 31.1 `<MetricCard>` primitive candidate. See session log for full details.) Prior: 2026-05-17 (Session 61 — Phase 31.A.1 (tier-3 editorial-copy sweep per discipline rule #6) shipped on `phase-31-a-1-tier3-rule6-sweep` off latest main (`cd33d0e`, post-31.A.3 merge). **Three-pause-point session, frontend-only, 4 files modified, 4 `interpretation()` functions rewritten, ~1-2h CC.** **Pause A code-grep + live curl** confirmed editorial-word leakage confined to the four cards' `interpretation()` returns with zero inline JSX leakage: `RenewableMixCard.tsx:23-27` ("High renewable penetration"/"midday surplus likely"/"charging window open"/"Thermal-dominated"/"narrow BESS spreads expected"), `ResidualLoadCard.tsx:26-31` ("well below norm"/"excess renewable supply"/"Tight system"/"elevated evening prices likely"/"Balanced"), `PeakForecastCard.tsx:42-48` ("Exceptional spread"/"Above-average"/"favorable for 2-cycle dispatch"/"Compressed"/"Normal spread conditions"), `SpreadCaptureCard.tsx:35-40` ("Wide/Moderate/Thin/Normal envelope"/"supports 2+ profitable cycles"/"single-cycle viable"/"hold or flex"). None of the four carry an `impact()` function — only S7/S9 do (Phase 31.A reference). Live curl of `/s_wind` (`baltic_mw=130`, `avg_7d_mw=642`), `/s_solar` (`1242`/`904`), `/s_load` (`2561`/`2805`), `/read` (`lt_daily_swing_eur_mwh=149.47`, `swing_stats_90d={p25:122.23, p50:150.93, p75:160.56, p90:202.38, n:90}`, `intraday_capture=146.5`, `bess_net_capture=145.1`, `p_high_avg=155.7`, `p_low_avg=9.2`), `/s1/capture` (canonical `gross_4h=99.09`) confirmed available fields support the data-derived rewrites. **Pause B replacements** (operator-approved S7/S9 rhythm "observed · ratio/comparison · derived implication"): RenewableMix → `Renewables ${pct}% of ${load} MW load (${renewableMw} MW); thermal residual ${thermalMw} MW (${thermalPct}%).` with `renewableMw>totalLoad` split-case `"… — exceed demand by ${Δ} MW (net export window)."` (operator-requested export-day guard); ResidualLoad → `Thermal + imports cover ${residualMw} MW (${pct}% of demand); renewables displace ${load−residual} MW.` with `residualMw<0` split-case `"Renewables exceed load by ${abs} MW (net export window)."` (operator-picked symmetry with RenewableMix's export-window split) + `totalLoad<=0` graceful guard `"Demand data unavailable"`; PeakForecast → `Swing €${swing}/MWh · ${(swing/p50).toFixed(2)}× the 90D median (€${p50}), ${(swing/p90).toFixed(2)}× the P90 (€${p90}).` with `!stats?.p50 \|\| !stats?.p90` graceful fallback `"Swing €${X}/MWh"`; SpreadCapture → `Gross €${g}/MWh; net €${n}/MWh after RTE losses — €${(g−n)}/MWh drag (${pct}%).` (operator-picked alt over the Sell/Buy parenthetical — avoids rule #4 duplication with the existing Sell/Buy sub-row above), plus `netCapture==null` fallback `"Gross €${g}/MWh."`. Call-site signature updates for three of four (RenewableMix `interpretation(renewablePct,renewableMw,totalLoad,thermalMw,thermalPct)`; ResidualLoad `interpretation(residualMw, totalLoad)` — was `(residualMw, residual7d)`, `residual7d` still used inline by dot-color + delta-arrow row; SpreadCapture `interpretation(grossCapture, netCapture)` and dropped local `interpValue`). PeakForecast keeps `(swing, stats)` signature. **Live-DOM-rendered samples** (Pause C probe at 2026-05-17 09:00Z worker snapshot): RenewableMix `"Renewables 54% of 2,561 MW load (1,372 MW); thermal residual 1,189 MW (46%)."` · ResidualLoad `"Thermal + imports cover 1,189 MW (46% of demand); renewables displace 1,372 MW."` · PeakForecast `"Swing €149/MWh · 0.99× the 90D median (€151), 0.74× the P90 (€202)."` · SpreadCapture `"Gross €147/MWh; net €145/MWh after RTE losses — €1/MWh drag (1%)."` (today's RTE drag is unusually thin because `intraday_capture` is the raw peak−trough range not the cycle-block; canonical 4h capture footnote tells the cycle story). **Gates** (baseline `tsc 0 / vitest 919 / lint 126`): `tsc --noEmit` 0 errors; `vitest run` 919/919 passed; `npm run lint` **125 problems (39 errors + 86 warnings) — Δ −1 vs baseline** (dropping `interpValue` local in SpreadCaptureCard removed one `no-unused-vars` warning), zero issues on the four touched files; `lint:no-raw-spacing` exit 0; `lint:no-editorial-chips` exit 0; `npm run build` 7 routes clean (`out/` regenerated). **Pause C local-build smoke-test** (`npx serve out -l 3100` per `feedback_local_build_verification.md`): 5/5 routes HTTP 200 (`/`, `/intel`, `/methodology`, `/regulatory`, `/dev/hero-preview`), 8/8 sampled JS chunks HTTP 200. **DOM probe via chrome-devtools-mcp** at 1440×900: 6 `.tier3-interp` nodes (the 4 in-scope cards + S7 + S9 reference) all populated with live data; 2 `.tier3-impact` nodes (S7 + S9 only — re-confirms no `impact()` on the four in-scope cards); phase-prompt regex `/elevated\|above-normal\|high renewable\|tight system\|wide envelope\|favorable\|supporting/i` matched 0 nodes; extended regex also covering "moderate/thin/normal envelope"/"midday surplus"/"charging window"/"thermal-dominated"/"excess renewable supply"/"charging opportunity"/"exceptional"/"above-average"/"compressed"/"insufficient data" matched 0 nodes. Console: 2 pre-existing entries unrelated to the change (a 404 plus a THREE.js `Clock` deprecation warning from the 3D hero). **Visual evidence**: `docs/visual-audit/phase-31-a-1/structural-1440.png` (1440×900, 927 KB) — Structural market drivers section with all 6 tier-3 cards rendering the new data-derived prose. **Risk class LOW confirmed**: rollback = revert 4 files; no engine math touched, no schema, no worker, no chart wiring, no JSX structural change. **Roadmap delta needed** (Cowork applies per rule #5 + `feedback_cowork_cc_sequencing.md` step 10): Phase 31.A.1 → SHIPPED appendix + update "Currently active" pointer in `docs/phases/_post-12-8-roadmap.md`. Discipline rules applied: **#1 audit-triage** (Pause A grep + live-curl triangulated the audit's editorial-copy claim against actual code — all four cards carry rule-#6-violating `interpretation()` branches as flagged; zero inline JSX leakage); **#4 cross-card consistency** (all four rewrites match the S7/S9 rhythm "observed · ratio · derived implication" so all six tier-3 cards now share structural copy rhythm; SpreadCapture's alt version specifically picked to drop the Sell/Buy parenthetical that duplicated the existing sub-row); **#5 roadmap edit-conflict** (zero roadmap edits; delta reported here for Cowork); **#6 no-editorial-state-label** (central rule — all regime words removed across the four `interpretation()` functions; CI gate `lint:no-editorial-chips` still passes; DOM regex confirms no residual prose leakage). Branch pushed for PR. Previously Session 60 — Phase 31.A.3 (`lt_hourly_24` downsampler robustness fix) shipped on `phase-31-a-3-hourly24-downsampler` off Session 59 HEAD (branch-base (i) per operator pick — branched off the unmerged 31.A.2 tip so the live worker baseline matches the deployed 31.A.2 arithmetic; `main` does not yet carry 31.A.2). **Two-pause-point session, worker-only, 1 file modified, 1 bug closed, ~30 min CC.** **Pause A code-grep + live curl** confirmed downsampler bug at `workers/fetch-s1.js:3367`: `Number.isInteger(perHour)` gate where `perHour = N/24` returns `null` on any N not divisible by 24. Pre-fix live curl: `hourly_lt_len=95`, `lt_hourly_24=null`, `lt_peak_price=158.41`, `lt_trough_price=8.94`. **Consumer audit corrects prompt premise**: only one app/ consumer (`SpreadCaptureCard.tsx:85` reads `data.lt_hourly_24 ?? []` then gates `length >= 2` for sparkline render). Net behavior on null is **sparkline silently hides** — NOT "regresses to slice-of-quarter-hours visualization" as prompt suspected; no slice-fallback path exists. Either way the fix removes the null path; consumer requires no change; registry entry at `metricRegistry.ts:108` keeps shape (24-entry float array). **Fix shape (b) operator-approved**: `Math.round(h*N/24)` slice-and-average bucketing replaces the integer-perHour gate. Mirrors the exact arithmetic shipped 18 lines down at L3382-3383 by the 31.A.2 evening-premium fix; paired with the inverse `Math.floor(idx*24/N)` peak/trough formula at L3359-3360. For N=95: produces 24 buckets of 3-or-4 entries each. Output: always 24-entry float array on N≥24 (null branch retained only when N<24, a degenerate non-day case). **Post-deploy curl reality-check** (worker version `c86116d8-af0d-421e-ab9f-8f48a36fa30a`): `hourly_lt_len=95`, `lt_hourly_24_len=24`, `lt_hourly_24_min=9.39`, `lt_hourly_24_max=153`, `lt_peak_price=158.41`, `lt_trough_price=8.94`. Sanity-bracket holds: bucket-avg min (9.39) ≥ global trough (8.94) and bucket-avg max (153) ≤ global peak (158.41) — exactly as expected (averaging within a bucket can't push a per-bucket value beyond the global extreme; small peak dilution because the peak hour bucket spans ~4 sub-bars around the spike). **Gates** (baseline-exact against 31.A.2): `tsc --noEmit` 0 errors; `vitest run` 919/919 passed (Session 59 noted 918/919 with a pre-existing `freshnessBadge` boundary failure; today the boundary lands inside the passing window — no delta from our change either way); `npm run lint` 126 problems (39 errors + 87 warnings, baseline-exact); `lint:no-raw-spacing` exit 0; `lint:no-editorial-chips` exit 0; `npm run build` 7 routes clean. 1 file touched (`workers/fetch-s1.js`), 13 LOC inserted, 8 LOC removed. No engine math, no schema change, no registry entry, no frontend touch. **Risk class LOW confirmed**: rollback path = revert single file; downstream consumer (SpreadCaptureCard sparkline) benefits unconditionally (now renders on N=95 days instead of hiding). **Roadmap delta needed** (Cowork applies per rule #5): mark 31.A.3 SHIPPED appendix + update "Currently active" pointer. Discipline rules applied: **#1 audit-triage** (Pause A live-curl re-confirmed Session 59's downstream-bug hypothesis empirically; one corrected sub-detail — consumer fallback is `?? []` → render-hidden, not slice-fallback as prompt framed; flagged transparently); **#4 cross-card consistency** (zero shape change to `lt_hourly_24`; SpreadCaptureCard + metricRegistry contracts preserved; arithmetic now mirrors the 31.A.2-shipped `Math.round(h*N/24)` ↔ `Math.floor(idx*24/N)` inverse pair, tightening within-engine consistency); **#5 roadmap edit-conflict** (zero roadmap edits; deltas reported); **#6 no-editorial-state-label** (no chips touched). Branch pushed for PR. Previously Session 59 — Phase 31.A.2 (evening-premium resolution-aware slicing fix) shipped on `phase-31-a-2-evening-premium-slicing` off latest main (`cc6d692`). **Two-pause-point session, worker-only, 1 file modified, 1 bug closed, ~30 min CC.** **Pause A code-grep + live curl** confirmed bug at `workers/fetch-s1.js:3377-3382`: `ltPrices.slice(17,22)` / `slice(10,15)` assume hourly indexing while `ltPrices` is 15-min resolution on ISP days. Live curl `hourly_lt_len = 95` (not 96 per prompt premise — likely ENTSO-E publishing one-off; flag for future investigation if recurring, not blocking); pre-fix `lt_evening_premium = -9.6` (slop between two early-morning quarter-hour windows, slice(17,22) on N=95 ≈ local 04:15-05:30). **Pause A frontend grep surfaced rule #4-relevant nuance**: `lt_evening_premium` (the buggy field) has **zero frontend consumers** — only type definition at `lib/signals/s1.ts:24`; the UI's "Eve premium" chip at `S1Card.tsx:640` reads `shape.evening_premium` which is a **separate, already-resolution-aware** field computed by `priceShapeMetrics(prices, timestamps, resolutionMin)` at `:3097-3151` (both callers pass `resolution` correctly; hour-bucket logic groups quarter-hours into UTC hours). So no labeling-bug follow-up needed and no rule #4 cross-card violation; pure backend-data-correctness fix. **Fix shape (a-improved) operator-approved**: `Math.round(h*N/24)` index math for both `ltEvening` and `ltShoulder` slices — works for any N (24/95/96/…), mirrors the inverse of Phase 31.A's `Math.floor(idx*24/N)` peak/trough idx→UTC-hour math at `:3359-3360`. Prompt's (a) ternary `length===96?4:1` and (b) `lt_hourly_24` paths both fail on N=95: (a) re-introduces the bug, (b) gets null because `lt_hourly_24` downsampler at `:3367` gates `Number.isInteger(perHour)` and `95/24 = 3.958…` is not integer. **Post-deploy curl reality-check** (2026-05-17 08:47 UTC, fresh `updated_at`, cache-bust): `lt_evening_premium = 120.65` (vs pre-fix `-9.6`; Δ = €130.25/MWh, sane against today's `lt_peak_price=158.41 @ h20` + `lt_trough_price=8.94 @ h14` profile — evening ~€130-150 minus midday ~€10-30 ⇒ ~€120 premium). **lt_hourly_24 null-on-N=95 surfaced as Phase 31.A.3 follow-up candidate for Cowork to file in roadmap** (rule #5 — CC does not edit roadmap): same resolution-bug family as 31.A; `Number.isInteger(perHour)` gate at `workers/fetch-s1.js:3367` skips downsampling on any N not divisible by 24, leaving `lt_hourly_24=null` on days like 2026-05-17 (N=95); fix would generalize to hour-bucket aggregation matching `priceShapeMetrics`'s pattern. Out of approved 31.A.2 scope (single-field correction). All 6 gates baseline-exact (deltas pre-existing on main, confirmed by stashing fix + re-running on unchanged worker file): tsc 0 ✓, vitest **918/919** (1 pre-existing failure in `freshnessBadge.test.tsx:42` — date-boundary "TODAY" expectation; unchanged on main; baseline drift since prompt was written), lint **126 (39e/87w)** (baseline drift +1 vs prompt's stated 125; pre-existing on main), no-raw-spacing exit 0 ✓, no-editorial-chips exit 0 ✓, build 7 routes 3.5s ✓. **Worker deploy `b6bd77e3-832e-4cd2-87b6-41db92b79112` live** via `npx wrangler deploy` (no `npm run deploy:worker` script wrapper exists; ran wrangler CLI directly). Worker-only change — 1 file (`workers/fetch-s1.js`), 6 LOC inserted (3-line index pre-computation + slice arg update), 3 LOC removed. No engine math, no schema change, no registry entry, no frontend touch, no chunk emission. **Risk class LOW confirmed**: rollback path = revert single file; downstream consumers unaffected (top-level `lt_evening_premium` is emitted but unconsumed; `shape.evening_premium` untouched). Discipline rules applied: **#1 audit-triage** (Phase 31.A's live curl pre-verified premise; 30-min Pause A added empirical N=95 surprise + frontend-consumer scan validating no rule-#4 surface — preempts the "premise correct in shape but wrong in detail" failure mode); **#4 cross-card consistency** (grep confirmed two parallel `evening_premium` computation sites; the buggy one has no display consumer, the correct one is already resolution-aware — no cross-card divergence to register); **#5 roadmap edit-conflict** (zero roadmap edits; 31.A.3 candidate surfaced for Cowork to apply); **#6 no-editorial-state-label** (no chips touched). Branch pushed for PR. Previously Session 58 — Phase 31.A (tier-3 card data integrity rebuild) shipped on `phase-31-a-tier3-data-integrity` off latest main (`34ee5b2`). **Three-pause-point session, 4 confirmed data-correctness bugs closed + 1 SKIP per rule #1, 8 files modified, worker+frontend, ~1 day estimate.** **Pause A discovery** confirmed 3 of 4 visible-from-screenshot bugs (75% hit-rate, vs ~25% historical visual-inference baseline) via per-card code-grep + live-curl: (1) **PeakForecast** — `app/lib/peakForecast.ts:75-88` heuristic mis-classifies 96-entry 15-min ISP arrays as 24-entry hourly when adjacent-bar variance > €2 (normal Baltic volatility); `slice(-24)` returns last-6h on 96-entry arrays while worker `lt_daily_swing_eur_mwh` reads full 24h → hero €108 vs peak/trough rows reconcile to €66 (37% mismatch); confirmed live (mean adj diff 5.52 > 2 threshold). (2) **S7 TTF Gas** — dual-threshold incoherence at `workers/fetch-s1.js:5610-5612` (regime ELEVATED at >30) vs `app/components/S7Card.tsx:31` pill `/50` (regime says "above-normal", pill says 0.92× of 50 = below); editorial `gasInterpretation`/`gasImpact` regime-derived (rule #6 violation). (3) **RenewableMix 143% claim — SKIP per rule #1** (math reconciles: 52% + 91% + 0% = 143% by algebraic identity when renewable > load; operator misreading, not bug). (4) **SpreadCapture net/gross labeling** — hero `bess_net_capture` (NET, RTE-adjusted) while subline `'raw daily envelope, top-4h avg − bottom-4h avg'` (claims GROSS) + Buy/Sell sub-row implies gross spread → hero/label/sub-row inconsistent triplet; additionally `todayCurve = hourly.slice(-24)` on 96-entry array showed last 6h not 24h (same root cause as #1). 3 additional bugs surfaced beyond screenshot: (A) S7 worker `data.interpretation` field never consumed (dead worker field); (B) S7+S9 editorial prose helpers rule #6 surfaces; (C) S9 CSS typo `transform: 'translateX(-50)'` missing `%` unit at `S9Card.tsx:166`. **Pause B fixes** (operator-approved scope, all 6 gates clean): worker `computeS1` at `workers/fetch-s1.js:3341` emits new `lt_peak_hour_utc`, `lt_peak_price`, `lt_trough_hour_utc`, `lt_trough_price`, `lt_hourly_24` fields all derived from same `ltPrices` array as `lt_daily_swing_eur_mwh` (resolution-aware downsample: N=24 pass-through, N=96 averages 4 sub-bars/UTC hour); UTC-hour anchoring via `floor(idx*24/N)`. `fetchTTFGas()` + `fetchEUCarbon()` returns: deleted dead `interpretation` fields (will purge from KV on next 4h cron; frontend already overrides). `app/lib/peakForecast.ts`: deleted `computePeakTrough` heuristic + `PeakTroughResult` interface; kept `formatTomorrowLine`/`intradayRange`/`crossZoneSeparationPct`. `app/lib/__tests__/peakForecast.test.ts`: removed 5 specs for deleted heuristic (−6 specs total expected, vitest 925→919). `app/components/PeakForecastCard.tsx`: reads worker fields directly, `hasPt` guard composed from 4 not-null checks. `app/components/SpreadCaptureCard.tsx`: hero swapped to `intraday_capture` (GROSS, operator pick) + new "Net after RTE charge leg: €N/MWh" disclosure line below; sparkline reads `lt_hourly_24` (full 24h). `app/components/S7Card.tsx`: data-derived `gasInterpretation(price)` ("Gas at {ratio}× the 50 €/MWh threshold; gas peaker marginal cost €{ttf*1.667}/MWh.") + `gasImpact(price)` ("P_high floor (gas only): €{peaker}/MWh — combine with EUA carbon for full peaker displacement floor."); single-anchor pill stays at `/50` per operator decision; sentiment from `ratioSentiment(price)` (data-derived, ≥1.0× → caution; <0.3 → positive); tooltip secondary label "Regime" → "Threshold". `app/components/S9Card.tsx`: matching pattern — `carbonInterpretation`/`carbonImpact` data-derived using `CARBON_THRESHOLD=70` + `CARBON_BREAKEVEN=55` + `CARBON_INTENSITY=0.45`; impact line shows ±delta vs 55 breakeven; CSS typo `translateX(-50)` → `translateX(-50%)` fixed. `app/lib/metricRegistry.ts`: registered 3 new metrics (`lt_peak_price_eur_mwh`, `lt_trough_price_eur_mwh`, `lt_hourly_24`) under `s1.*` workerPath prefix per rule #4 + existing `^s\d\.` registry test guard. **Worker deploy `71faa3df-e4b3-465d-b72d-4183ee9ffce4` live**. **Gates** (deltas vs main baseline 127 (40e/87w) / 925/925): tsc 0 ✓, vitest **919/919** (−6 specs deleted dead-code tests, expected), lint **125 (39e/86w)** (−2 problems vs 127, net cleanup win), no-raw-spacing 0 ✓, no-editorial-chips 0 ✓, build 7 routes 3.9s ✓. **Pause C verification**: local-build smoke-test green — `npm run build && npx serve out -l 3100` → all 5 routes HTTP 200 (`/` 105 KB, `/intel` 34 KB, `/regulatory` 26 KB, `/methodology` 329 KB, `/dev/hero-preview` 39 KB); 8/8 sampled JS chunks HTTP 200 → **18.1.1-class ChunkLoadError structurally absent**. **Chrome MCP gap recurring (6th consecutive session — 47/48/49/50/51/52/55/this; Session 53 was the lone restore)**: chrome-devtools-mcp server disconnected mid-session; deferred-tool fetch returned no match. Fallback to **playwright direct via `scripts/_phase-31-a-probe.mjs`** (operator pre-approved curl-only path is the discipline-rule pattern; playwright DOM probe is a stricter empirical extension of it). **16/16 DOM probe checks passed** + 2 unrelated console errors (pre-existing `/api/dispatch?dur={2h,4h}&mode=forecast` 404s from worker, not Phase 31.A-introduced). PeakForecast peak/trough swing reconcile: hero €108 ≈ (peak €181.0 − trough €72.8) = €108.2, **Δ=0.2 well within 2€ tolerance** — data-correctness bug closed at DOM render time. SpreadCapture: hero €100 + "Net after RTE charge leg: €89/MWh" disclosure rendered + Buy €76 / Sell €176 (176−76 = 100 ≈ hero ✓; 176 − 76/0.87 = 88.6 ≈ €89 net ✓). S7+S9 prose verified free of forbidden editorial regex (`Above-normal|Expensive|Cheap|Mid-range|strengthening BESS|supporting wider peak`); S9 ▲ marker `translateX(-50%)` unit confirmed in computed style. **Live curl post-deploy reality-check** (2026-05-13 09:16 UTC): `/read.lt_peak_price=180.96 @ lt_peak_hour_utc=21`; `/read.lt_trough_price=72.83 @ lt_trough_hour_utc=15`; **peak−trough = 108.13 = `lt_daily_swing_eur_mwh` exact (Δ=0.00)** ← data-layer bug closure proof; `lt_hourly_24` len 24 range [78.67, 169.65] (4-bar-averaged downsample, narrower than 96-entry array's [72.83, 180.96] extremes as expected). `/s7` + `/s9` still emit `interpretation` field in KV (4h cron hasn't refreshed yet — auto-purges by next scheduled run; frontend overrides regardless, no user-visible impact). **Visual evidence** at `docs/visual-audit/phase-31-a/structural-drivers-1440.png` — element-bounding-box capture (1176×727 logical → 2352×1454 PNG @ deviceScaleFactor=2), all 6 tier-3 cards visible with fixes rendered. **Follow-up candidates for Cowork to file in roadmap** (per rule #5, CC reports, Cowork applies): (i) **Phase 31.A.2 — evening-premium resolution-aware slicing**: `workers/fetch-s1.js:3346-3349` `ltPrices.slice(17, 22)` for "h17-21 evening" assumes hourly indexing; on 96-entry 15-min arrays this slices quarter-hours 17-21 = local hours ~04:15-05:30, NOT 17-21 evening; `lt_evening_premium` is silently wrong on every 15-min-resolution day (pre-existing, surfaced during Phase 31.A discovery, out of approved scope); fix shape: `const evening = perHour===4 ? ltPrices.slice(17*4, 22*4) : ltPrices.slice(17, 22)` paired with the existing perHour derivation. (ii) **Phase 31.A.1 — tier-3 editorial-copy sweep**: `interpretation()` band-derived prose functions in `RenewableMixCard.tsx:23-27` + `ResidualLoadCard.tsx:26-31` + `SpreadCaptureCard.tsx:35-40` + `PeakForecastCard.tsx:41-47` still surface rule #6 editorial copy ("High renewable penetration", "Tight system — thermal at high utilization", "Wide envelope — supports 2+ profitable cycles", "Above-average spread — favorable for 2-cycle dispatch"); apply the S7/S9 data-derived rewrite pattern to these four functions; estimate ~1-2h. **Discipline rules applied**: **#1 audit-triage** (RenewableMix SKIP after math reconciliation — direct operating-discipline win; 3-of-4 hit rate on this batch is operator-pre-diagnosed quality, well above baseline); **#4 cross-card consistency** (3 new metrics registered in `metricRegistry.ts` under `s1.*` namespace; S7 + S9 share same regimeLabel(price/threshold).toFixed(2)× shape post-fix); **#5 roadmap edit-conflict** (zero roadmap edits this session; 2 follow-up candidates surfaced for Cowork). **#6 no-editorial-state-label**: S7+S9 prose now strictly data-derived; lint:no-editorial-chips exit 0 (its scope is `phase: 'STATE'` props, not prose; the prose helpers were a separate rule-#6 surface closed by the rewrite; `RenewableMix`/`ResidualLoad`/`SpreadCapture`/`PeakForecast` interp() functions remain — see Phase 31.A.1 candidate above). Risk class MEDIUM confirmed in practice (no engine-math changes, additive worker fields + frontend reads; rollback path = revert the 7 frontend files; worker rollback via Cloudflare prior version pin). Branch pushed for PR. Previously Session 57 — Phase 18.3.1 hot-fix + Phase 31/32 candidate filings; Cowork-direct on `phase-18-3-animation-activation`. Universal card hover-lift shipped 2026-05-08 (commit `a0f36ea`) post-Session-56-merge but discovered 2026-05-13 to be ORPHANED — PR #78 (`8cf30eb`) merged to main BEFORE the hot-fix landed; commit not in main. Roadmap commit `bb96655` files Phase 31 (tier-3 data integrity + visual normalization, expanded from visual-only after operator "the data is nonsense in some, they need a full revision/rebuild" pushback; 4 visible-from-screenshot data bugs flagged at Pause-A) + Phase 32 (Bitėnai cascade reference asset recalibration; confidentiality gates per rule #3 documented up-front). Hover-lift design walkthrough: 3-card opt-in → operator most-or-none challenge → all-except-hero pick → !important to beat cardFadeIn animation fill-state outranking :hover transform → tone-down to translateY(-0.5px) + softer shadow → rejected inline-cta expansion after design-coherence rethinking (lift = card affordance only). Cowork-side: no foundation gates run (Cowork sandbox limits per `feedback_cowork_sandbox_limits.md`). Risk class LOW (CSS-only). Branch pushed for re-PR; operator opens new PR + merges. Previously Session 55 — Phase 4G.1 (POST /curate encoding-validation gate) shipped on `phase-4g-1-curate-encoding-gate`. **Single sub-item, single PR, worker-only.** `validateCurationContent(body)` + `detectMojibake(s)` + 10-bigram `MOJIBAKE_PATTERNS` const wired into `POST /curate` handler immediately after required-fields check; rejects mojibake (cp1257-misdecoded-UTF-8) in `title` or `raw_text` at ingest before persisting to KV with operator-honest error envelope `{error, field, pattern, message}` at HTTP 400. Operator-paste path that produced both Phase 12.10.0 Saulėtas Pasaulis hallucinated entity AND Phase 18.1.3-era `cur_motnhb6l-o8mv3k` mojibake deletion (2026-05-07) now gated. **Pattern set refined Pause A** based on empirical 210-curation + 8-feed corpus check: dropped `/Ä[A-Z]/` (regex matches no real bigram — `[A-Z]` only U+0041-005A, mojibake bigram followers are extended-Latin/punctuation), dropped bare `/Ä/` (Estonian false-positive risk: `Tähtsamad`, `Käiku`), dropped bare `/Å/` (Swedish/Finnish/Norwegian false-positive risk); added 7 explicit Lithuanian Win-1252-misdecode bigrams (`Ä„`, `Ä…`, `Ä—`, `Ä™`, `Ä¯`, `Å³`, `Å«`) covering full LT special-char table. Final 10-bigram set: `/Ä„/` `/Ä…/` `/Ä—/` `/ÄŠ/` `/Ä™/` `/Ä¯/` `/Å¡/` `/Å¾/` `/Å³/` `/Å«/` — zero false-positives across 210-curation corpus (only 7 hits all from the same `cur_mowid0zn-siyurd` "[PDF] Leidimai plÄŠtoti..." VERT batch, invisible at `/feed`, 30-day TTL will reap). **4-test verification protocol** (worker version `e150b2d8-8878-49b9-ab0e-fc0ef8588ea5`): Test 1 mojibake-title→400 `{field:title, pattern:ÄŠ}` ✅; Test 2 clean Lithuanian `Test plėtoti pajėgumas`→201 `id=mownfpv9-6kweqy` ✅; Test 3 mojibake-raw_text→400 `{field:raw_text, pattern:ÄŠ}` ✅; **Test 4 load-bearing defensive verification: legitimate Estonian `Eesti küsimused energiavarustuse` + `Tähtsamad küsimused`→201** `id=mownfqhs-nwli7n` ✅ — proves no false-positive on legitimate non-LT Baltic content; if prompt's bare `/Ä/`/`/Å/` had stayed, Test 4 would have rejected. Test entries cleaned via `/feed/delete-by-id` by operator post-merge (`cur_mownfpv9-6kweqy` + `cur_mownfqhs-nwli7n`; UPDATE_SECRET in operator's shell, not CC's). All 6 foundation gates baseline-exact (tsc 0, vitest 924/925 with pre-existing `freshnessBadge` time-of-day-dependent assertion carrying forward as known baseline — introduced by Phase 21's EET-day-aware freshness logic; foldable into 12.10b housekeeping or own micro-phase later, lint 127 (40e/87w), no-raw-spacing exit 0, no-editorial-chips exit 0, build 7 routes). Local-build smoke-test (per `feedback_local_build_verification.md` REQUIRED gate): all 4 routes HTTP 200 (`/` 105 KB, `/intel` 34 KB, `/methodology` 329 KB, `/regulatory` 26 KB). Worker-only change; pure-string validation; no engine math, no KV schema change, no chunk emission, no frontend touch. **Risk class LOW confirmed** — only structural change is 400-status-return path added before `storeCurationEntry`/`appendCurationToFeedIndex`, both downstream functions unmodified. Discipline rules applied: **#1 audit-triage** (Pause A empirical false-positive sweep refined pattern set, direct operating-discipline win); **#3 named-entity verification** (Phase 4G.2 candidate filed for entity-verification layer at `/curate` ingest — cross-check `source_url` against `registrucentras.lt`/Lursoft/`inforegister.ee`/regulator URLs); **#5 roadmap edit-conflict** (no roadmap edits; deltas reported); **#6 no-editorial-state-label** (error message operator-honest: explains mojibake + re-paste fix, not vague "validation failed"). 1 file touched (`workers/fetch-s1.js`), ~57 insertions, 0 deletions, no `model_version` bump. Branch pushed for PR. Previously Session 54 — Phase 25 — P0 right-rail clipping + audit-3 credibility-disclosure pass shipped on `phase-25-credibility-disclosure-pass` (7 sub-items + 1 SKIP, 8 files; right-rail clipping <900px → <1228px breakpoint bump closes 1066-1227px viewport range; S/D ratio + Kruonis + investable IRR pill + €20400 thousands-separator sweep + vs Y1 base + dispatch dup removed + ThemeToggle SKIP per code-grep; chrome-MCP gate validated end-to-end across 9 viewports; see structured Session log entry below). Previously Session 53 — Phase 18.1.3 (mobile per-component reflow, extracted from reverted 18.1.1) shipped on `phase-18-1-3-mobile-component-reflow`. **Six sub-items in one PR; 4 files modified.** All reflows applied verbatim from reverted commit `1839328`'s pattern with line-number drift accommodated post-7.7g-a-3 typography phase: (4b) Sensitivity tornado SVG → `viewBox + maxWidth + width:100%` (was non-responsive `<svg width={width}>` literal 360); (4c) MonthlyHeatmap grid `1fr` → `minmax(0, 1fr)` per month cell; (3) S3 BreakdownBar reflow with `.breakdown-bar*` className family + **6 split @media blocks** in globals.css per Tailwind v4 silent-drop pattern, bars stack to 3 rows on <720px; (1) DSCR triple panel cells `min-width: 0` + `overflow-wrap: anywhere` + new `.rv-dur-dscr` class wrapping DurationOptimizer + DSCRPanel (`minmax(0, 1fr) minmax(0, 1fr)` desktop / single-column at <720px); (2) **AssumptionsPanel grid — operator-screenshot bug** — replaced inline `gridTemplateColumns: 'auto 1fr'` with `.assumptions-grid` class, dropped `whiteSpace: 'nowrap'` on 5 label cells, added `overflowWrap: 'anywhere'` to italic notes. New `.assumptions-grid__label` + `.assumptions-grid__note` classNames; (4a) chart-canvas track `.rv-main > *, .rv-analytics > * { min-width: 0 }` injected into RevenueCard inline `<style>` block. **Both band-aids removed** — `html { overflow-x: hidden }` (Phase 18.1 page-level clip, restored after 18.1.1 revert) AND `body { overflow-x: hidden }` (older defensive trim) at `app/globals.css:486 / :502`. Matches Phase 18.1.1 success pattern. **Chrome MCP gate restored end-to-end — first chrome-MCP-clean ship since Session 47** (5+ consecutive sessions had recurring browser-lock condition forcing curl-only verification). The `cc` alias fix per `feedback_chrome_mcp_orphans.md` empirically confirmed. Process-recovery milestone alongside the Phase 18.1.3 ship. **5-viewport `scrollWidth=clientWidth` PASS** at 360 / 414 / 768 / 1024 / 1440 with band-aids actually removed in served bundle (verified via cache-bypassed reload + `getComputedStyle().overflowX === "visible"`). 7 visual-audit screenshots captured at `docs/visual-audit/phase-18-1-3/` (assumptions-panel + dscr-triple + s3-breakdown + tornado + heatmap at 360 + 414 page + 1440 page). **Operator-screenshot bug (vertical-character-per-line in AssumptionsPanel right column) closed with empirical visual evidence**, not just code-review reasoning. **Risk class LOW confirmed** — phase touched zero of: `HeroBalticMap.tsx`, `MobileBalticMap`, GSAP, dynamic imports, lazy boundaries (Phase 18.1.1's ChunkLoadError sources untouched). Local-build smoke-test (per `feedback_local_build_verification.md` REQUIRED gate): 20/20 JS chunks return HTTP 200 → 18.1.1-class ChunkLoadError class structurally absent. **Bundle delta: CSS chunk 40,479 B** (smaller than 18.1.1's 43,470 B reference — 7.7g-a-3 typography pruning earlier in the day was load-bearing for the smaller envelope). All 6 gates baseline-exact (tsc 0, vitest 925/925, lint 127 (40e/87w), no-raw-spacing exit 0, no-editorial-chips exit 0, build 7 routes). Frontend-only, no `model_version` bump, no worker deploy. Phase 18.1.4 follow-up **NOT FILED** — both band-aids removed cleanly across all 5 viewports; honest empty-state. Branch pushed for PR. Previously Session 52 — Phase 7.7g-a-3 (typography rationalization + spacing residuals + per-side CI extension) shipped on `phase-7-7g-a-3-typography-rationalization`. **7 sub-items in one PR (Path A); ~57 files modified.** (g) `lint:no-raw-spacing` regex extended to per-side variants (`padding(Left|Right|Top|Bottom|Inline|Block|InlineStart|InlineEnd|BlockStart|BlockEnd)?` + `margin*` + `gap`/`rowGap`/`columnGap`); BSD-grep portability fix replaced `\x27` with `['"'"'"]` literal-quote char-class; 5 on-scale per-side migration targets surfaced (SignalBar / intel + regulatory headers / regulatory footer) and migrated. (a) 9-step modular type scale per spec CC-1 added to `globals.css :root`: `--type-display-2xl`/`-xl`/`-lg`/`-md` + `--type-body-lg`/`-md`/`-sm` + `--type-label` + `--type-mono-xs`. Existing 6 `--font-*` tokens preserved as back-compat aliases of new tokens (414 existing consumers unaffected); only `--font-lg` rebound 22→24px to align with `--type-display-md` (single consumer at `RegulatoryItem.tsx:84`). (b) **185 inline `fontSize` sites migrated** across 49 files via Python regex pass `scripts/_phase-7-7g-a-3-fontsize-migrate.py` (scratch, excluded from commits per Phase 21+21.2 convention). 30+ distinct rendered values consolidated to **8 distinct token-rendered sizes** (`mono-xs`/`body-md`/`body-lg`/`label`/`display-md`/`body-sm`/`display-lg`/`display-2xl`) — spec target ≤9 hit cleanly. Off-scale residuals migrated to nearest token (0.5625rem→`mono-xs`; 0.9375rem→`body-md`; 1.125rem→`display-md`; etc.); `clamp()` responsive hero patterns (~20 sites) preserved as deliberate exception. **(c) + (d) Unbounded fully removed.** SPEC DEVIATION from P3-1's "wordmark-only" — Pause A discovery found the broadsheet masthead wordmark at `HeroBalticMap.tsx:332-333` is a PNG `<img>` (kkme-{white,black}.png), not text — Unbounded was zero-load-bearing. 31 `var(--font-display)` line hits across 16 production files (excl. dead-code `HeroMarketNow.tsx`) bulk-migrated `var(--font-display)` → `var(--font-serif)` (editorial); hand-corrected 5 sites to `var(--font-mono)` (data discipline): `HeroBalticMap.tsx` country-name SVG label + S/D + CPI + PHASE tiles + `ChartTooltip.tsx` value row. **`--font-display` token DELETED from `globals.css`**; **`Unbounded` next/font import + className DELETED from `app/layout.tsx`**. **⚠️ Operator post-merge action: hard-refresh kkme.eu and verify the masthead wordmark renders correctly** — if pixel-art PNG looks wrong, Phase 7.7g-a-3.1 hot-fix re-adds Unbounded for the wordmark site only (defined `--font-wordmark` token). Newsreader 200 (hairline) verified active at 5 hero-tile sites (S1Card / RevenueCard / S2Card / BalticStorageIndexCard / MetricTile primitive) → KEPT LOADED; total post-fix font weights = 5 (Plex 400/500 + Newsreader 200/400/400-italic), Latin-effective load ~167 KB — over spec target ≤4 weights / ≤120 KB by 1 weight / ~47 KB; deliberate exception per Phase 18 broadsheet hairline + italic identity commitments. (e) **202 shorthand `padding`/`margin` 2-3-4-value sites expanded to per-side** across 51 files via Python regex pass `scripts/_phase-7-7g-a-3-shorthand-migrate.py`; on-scale values resolved to `var(--space-*)` tokens, off-scale literals preserved. **Bug-and-fix mid-implementation**: initial migration script emitted bare `var(--space-xs)` (invalid JS — `var` is keyword); 227 unquoted refs across 51 files repaired by `_phase-7-7g-a-3-shorthand-fix.py` wrapper script; one duplicate-property tsc error in `IntelFeed.tsx:641-647` resolved by combining the two sites. (f) no-op (deliberate) — 113 off-scale 6/10/14/18 px residuals preserved as operator's design rhythm; gate is value-aware so off-scale isn't a CI violation. **All 6 gates baseline-exact** (tsc 0, vitest 925/925, lint 127 (40e/87w), no-raw-spacing exit 0 (extended to per-side), no-editorial-chips exit 0, build 7 routes 4.3s). **Bundle delta: −215 KB Unbounded files eliminated** (5 next/font hash files removed from emission); 19 @fontsource files unchanged. Production CSS confirms 9 `--type-*` tokens + 8 `--space-*` tokens emitted; `--font-display` / `--font-unbounded` NOT present. Local prod build smoke-test green: 21 chunks all return HTTP 200 at localhost:3100, all 5 routes (/, /intel, /regulatory, /methodology, /dev/hero-preview) HTTP 200, Plex + Newsreader Latin .woff2 served — 18.1.1-class ChunkLoadError absent. **Cross-card consistency (rule #4)**: post-fix grep returns 0 production code references to `var(--font-display)` or `Unbounded` (only documentation comments remain). **Visual-audit gap (recurring 5th consecutive session — 47/48/49/50/51/52)**: chrome-devtools-mcp browser-lock condition recurred; operator pre-approved curl-only verification path with prose state description at `docs/visual-audit/phase-7-7g-a-3/STATE.md`. Cross-session friction observation now operator-action-item bumped to "actually do this" priority — investigation no longer optional. **Phase 7.7g-a-3.1 conditional candidate filed**: if operator post-merge masthead wordmark visual regresses, restore Unbounded for wordmark-only via new `--font-wordmark` token. Frontend-only, no `model_version` bump, no worker deploy. Branch pushed for PR. Previously Session 51 — Phase 21.2 (S2 monthly trajectory + capacity_monthly KV fix) shipped on `phase-21-2-monthly-trajectory-kv-fix`. **Four sub-items in one PR; 3 files modified.** (a) `fmtMonth` `Mar 26` → `2026-03` (operator-picked ISO) cross-card across S1Card.tsx + S2Card.tsx; (b) KV diagnostic confirmed dual-write (revised at impl-time to **triple-path**) architecture mismatch — worker cron paths at `:6329` (4h) and `:6205` (09:30) AND Mac-cron POST `/s2/update` at `:7269` all wrote `s2` KV but only the 4h cron appended to `s2_btd_history`; (c) new shared `appendBtdHistory(env, payload)` helper paired into all three write sites + new UPDATE_SECRET-gated `POST /s2/btd-history/backfill` endpoint (~25 lines) reading `s2_history` (consistently-written across all paths) and re-emitting missing dates to `s2_btd_history` — **operator runs once post-merge for full recovery**; (d) dynamic empty-state fallback at threshold `< 3` months with data-derived accrual date `"Capacity history accruing — N months collected (YYYY-MM list); full trajectory available YYYY-MM-01."` (rule #6 compliant). All 6 gates baseline-exact (tsc 0, vitest 925/925, lint 127 (40e/87w), no-raw-spacing exit 0, no-editorial-chips exit 0, build 7 routes). Worker deploy `b2152be9-ed8b-4e71-9f09-9c11d96d273a` live; backfill endpoint smoke-confirms `POST /s2/btd-history/backfill` → 401 unauth (gate working). **⚠️ LOAD-BEARING operator post-merge action**: `curl -s -X POST -H "X-Update-Secret: $UPDATE_SECRET" .../s2/btd-history/backfill` to recover up to 90 days of lost history; without it, empty-state line continues rendering until daily cron accumulates ≥3 months naturally (~3 months clock time). Cross-card check (rule #4): `s1_capture.monthly.length = 15` GREEN — different KV pipeline, unaffected, no Phase 21.3 needed. **Process-honesty note**: Pause A scoped 2 write paths; impl-time grep surfaced a 3rd; revision flagged transparently in Pause B (no-silent-drops working as designed). **No specs added** — `fmtMonth` body becomes identity; `capacityEmptyStateText` is time-dependent; operator approved skip; Phase 21.2.1 candidate if regression hits. **Visual-audit gap (recurring 4th consecutive session)**: chrome-devtools-mcp browser-lock recurred; operator pre-approved curl-only verification path with prose state description at `docs/visual-audit/phase-21-2/STATE.md`. Branch pushed for PR. Previously Session 50 — Phase 21 (S2 professional polish: hero inversion to LT up-only operational reality + new derived `s2.afrr_up_avg_90d_delta` worker field + IMB interpretive line + `MarketThicknessChip` rule-#6 alignment via per-product quantitative depth anchor) shipped on `phase-21-s2-professional-polish`. **Three operator-confirmed sub-items + 1 quiet integrity fix in one PR; 7 files modified.** S2 hero swap: `act.afrr_p50` 3m P50 combined LT (€13.5/MW/h) → `data.afrr_up_avg` 7d mean LT up-only (€7.91/MW/h); €5,913k/yr footer auto-corrects to €3,464k/yr (operationally honest annualized for one-direction BESS at 50 MW). Hero label literal updates `up+down combined` → `LT · UP ONLY · 7D`; subline transformed from mislabeled "(up only, Baltic avg)" → methodology-disclosure "Combined up+down (LT, 3m P50): €13.5/MW/h" — discipline rule #1 (audit-triage) caught the "Baltic avg" mislabel during scope validation; `data.afrr_up_avg` is **Lithuania-only** per worker comment at `fetch-s1.js:4395` + methodology note at `:7113` (BTD `price_procured_reserves` column 11). New `effectiveCountry: Country = prod === 'aFRR' ? 'LT' : country` render-time derivation pattern preserves user's prior LV/EE pick across aFRR product-flip round-trips (eliminates `setCountry` in `useEffect` that would have tripped `react-hooks/set-state-in-effect` lint). CountryToggle disabled on aFRR mirroring FCR pattern; LT highlighted as canonical. New worker `computeAfrrUp30dVs60dDeltaPct(history)` (~25 lines) reads `s2_history` KV (90-day cap, daily snapshots from `appendSignalHistory`); current 30d mean vs prior 60d mean fits exactly within MAX_HISTORY=90; null-safety triggers when window underfilled (current 48 days → null). New `AfrrDeltaChip` frontend component renders muted `Δ — / 90d` boxed chip on null with explanatory `title=` tooltip; will auto-activate to `Δ ±N% / 90d` quantitative micro-descriptor (no sentiment palette per rule #6) ~2026-05-19; full window ~2026-06-18. `MarketThicknessSpec.quantitative_anchor: string` added; `MarketThicknessChip` renders `{label} · {level} · {anchor}` — Option A pick at Pause A keeps institutional vocabulary (THICK = price-taker zone) AND adds quantitative anchor satisfying rule #6 spirit: aFRR · THICK · ≥100 MW / mFRR · MEDIUM · 50–100 MW / FCR · THIN · ≤50 MW. Anchor propagates automatically to `TradingEngineCard.tsx:497-499` via the same `<MarketThicknessChip>` consumer pattern (no per-card retrofit). New IMB interpretive line beneath imbalance tiles: muted-mono prose "Imbalance volume drives activation: more imbalance → more aFRR called → activation revenue layers on top of the capacity reservation above. Today's `{N}%` >100 MWh ≈ 1-in-`{Math.max(2, Math.round(100/N))}` settlement periods stressed." Data-derived from `data.pct_above_100`. New `afrr_up_avg_90d_delta` registry entry in `metricRegistry.ts` per discipline rule #4 (Phase 12.10 seed `/^s4\./` regex broadened to `/^s\d\./` to admit s2-namespace metrics). All 6 gates baseline-exact (tsc 0, vitest 925/925 with 3 chip-shape specs added + 1 registry-regex broadened, lint 127 (40e/87w), no-raw-spacing exit 0, no-editorial-chips exit 0, build 7 routes (4.8s)). Worker deploy `2b3c6bc5-e9a3-4819-9919-cd1ec3d85952` live; cache-busted curl `/s2 | jq '.afrr_up_avg_90d_delta'` returns `null` per null-safety branch. **Visual-audit gap (recurring 3rd consecutive session — 47 + 48 + 49)**: chrome-devtools-mcp browser-lock condition recurred; operator pre-approved curl-only verification path with prose state description at `docs/visual-audit/phase-21/STATE.md` substituting the 8-screenshot mobile/desktop × light/dark grid. Cross-session friction observation already filed as roadmap operator-action-item (chrome MCP `--isolated` userDataDir investigation). 18.1.1 ChunkLoadError class structurally absent (28 chunks, 4.5 MB raw; 20 referenced from `index.html` all return HTTP 200 at localhost:3101; Phase-21 literals embedded in static chunks). Bundle delta below noise floor (~+50 lines source). **Phase 21.1 conditional candidate filed** (per-country aFRR up-only worker engine extension; trigger: operator post-merge feedback that LT-anchoring feels too restrictive); **Phase 21.2 conditional candidate filed** (deriveChip audit on mFRR + FCR — pre-existing always-zero StatusChip bug; out of Phase 21 scope); **Phase 22 conditional candidate filed** (institutional-grade S2: forward curves, P95 tail, cross-region benchmarks). Branch pushed for PR. Previously Session 48 — Phase 18.2.2 (chart crosshair UX hotfix + tooltip-tighten + small-chart height bump) shipped on `phase-18-2-2-crosshair-ux`. **Three sub-items in one PR; 9 files modified.** Operator hard-refresh feedback on Phase 18.2 ship: "Hover works only on lines, it should be more like I see the + across the whole chart when I hover. Not only on the lines and the peaks since the chart is small and it's hard to find it." → crosshair fix shipped; mid-session, operator hard-refreshed against the open PR branch and reframed the issue as **visual hierarchy between tooltip and chart** (tooltip box was ~50-67% of canvas height on small charts; target 20-25%). α tooltip-tighten + β small-chart height bump added to the same PR; γ (M2 mobile tooltip pinning) deferred to Phase 18.2.3 for risk isolation per Phase 18.1.1 lesson. Sub-item summary: (1) **crosshair "+"** — `CHART_INTERACTION = { mode: 'index', intersect: false, axis: 'xy' } as const` exported from `chartTheme.ts`, applied at all 10 plugin-bearing chart sites; `makeCrosshairPlugin` extended to draw horizontal line at `tooltip.caretY` alongside existing vertical at `caretX`. 5/10 sites lacked the option entirely (S1Card 2 + S2Card 3 — operator's "small charts hard to find" complaint); 5/10 had inline literal (RevenueCard 3 + RevenueBacktest 1 + TradingEngineCard 1) migrated to const for cross-card consistency rule #4. Both gaps closed. (2) **α tooltip typography + padding tightened** in sitewide `ChartTooltip.tsx` primitive (single file → propagates to all 19+ consumers): value 17→13px, padding 10/12→6/8, minWidth 128→96, headline/secondary 10.5→10, line-heights 1.05→1.15, vertical rhythm trimmed throughout; net value text −24%, padding −33-40%, minWidth −25%. (3) **β small-chart heights bumped** in S1Card + S2Card: Sparkline 120→170, Monthly 140→190, History 120→170, Trajectory 120→170, Capacity 140→190. Single height value per site (no media-query split — chart.js `responsive: true; maintainAspectRatio: false` reshapes width-only). Card-layout impact: S1Card +100px / S2Card +150px total; hero band intentionally scroll-rich per Phase 18.1's principle. Tooltip/chart ratio moved from ~58-67% → ~31-35% (close enough to operator's 20-25% target band). 18.1.1 risk-class pre-check (β): wrapper pattern unchanged, ResizeObserver target shape invariant, no new lazy boundaries. Visibility lift kept at `colors.textMuted` / 0.5px (Pause A pick a; 18.2.3 conditional candidate filed for M2 mobile tooltip pinning OR fallback brighten/thicken variant if either need surfaces post-merge). (A) new `CHART_INTERACTION = { mode: 'index', intersect: false, axis: 'xy' } as const` exported from `chartTheme.ts`, applied at all 10 plugin-bearing chart sites — makes crosshair fire whenever cursor is anywhere over canvas; (B) `makeCrosshairPlugin(colors)` extended to draw horizontal line at `tooltip.caretY` alongside existing vertical at `caretX`, forming "+" pattern using `chartArea.left/right`. Visibility lift kept at `colors.textMuted` / 0.5px (Pause A pick a, minimal-change-first; 18.2.3 conditional candidate filed if operator post-merge says still hard to read). **5/10 sites lacked `interaction` option entirely** (S1Card 2 + S2Card 3 — these were the operator's "small charts hard to find" complaint); **5/10 had inline `intersect: false` literal** (RevenueCard 3 + RevenueBacktest 1 + TradingEngineCard 1 — already fired anywhere but missed the "+" half) migrated to `CHART_INTERACTION` const for cross-card consistency rule #4 alignment. Both gaps closed; all 10 sites now have always-fire interaction AND "+" pattern. All 6 gates baseline-exact (tsc 0, vitest 925/925, lint 127 (40e/87w), no-raw-spacing exit 0, no-editorial-chips exit 0, build 7 routes — vitest 924/925 from prompt was the 1 pre-existing freshness boundary failure, now incidentally passing because today's calendar date lands inside the boundary it tests). Bundle delta below 0.2 KB gzipped (~+15 lines source). Frontend-only, no `model_version` bump, no worker deploy. **Visual-audit gap (recurring)**: chrome-devtools-mcp browser locked by parallel CC sessions — same condition as Session 47; operator approved curl-only verification path again (build clean + all 20 JS chunks return 200 + new `kkme-crosshair` plugin id + `axis:"xy"` literal embedded in static chunks → 18.1.1 ChunkLoadError class structurally absent); in-browser hover / theme-toggle visual confirmation owned by operator post-merge via Cloudflare Pages preview deploy. Cross-session friction observation filed: chrome MCP lock recurred on consecutive sessions — future Cowork investigation should consider `--isolated` userDataDir mode or browser-ownership coordination protocol. Branch pushed for PR. Previously Session 47 — Phase 18.2 (chart editorial polish) shipped on `phase-18-2-chart-editorial-polish`. Four sub-items, single PR, 6 files: (A) crosshair plugin factory `makeCrosshairPlugin(colors)` in `chartTheme.ts` consumed across 9 chart sites; (B) `CHART_FONT_DISPLAY` constant + 4 ref-line callouts switched to italic Newsreader serif (Y1 model anchor, 0.80·augment, 0.70·EoL, 1.0·today); (C) cross-card consistency rule #4 enforcement via `SENTINEL_DASH = [4,4]` + `SENTINEL_LINE_WIDTH = 0.8` constants applied at 7 sentinel sites; (D) data-derived quantitative aria-labels on 13 chart wrappers (rule #6 — no editorial language). Two sub-items deferred: (E) Baltic palette retune (spec P2-2 hex `#1a3833` / `#d4a574` / `#a8324a` ≠ Phase-18-shipped tokens; cross-cutting, filed as Phase 18.2.1 candidate); (F) tornado pointer-events fix NOT REPRODUCIBLE per code-grep verification (spec P2-2 inaccurate against current code). Phase 7.7e UI track (Sessions 24+25) had absorbed most P2-2 work pre-emptively (chartTheme.ts centralization, useChartColors() getComputedStyle resolution, IBM Plex Mono CHART_FONT, ChartTooltip primitive). All 6 gates baseline-exact (tsc 0, vitest 924/925 with 1 pre-existing freshness boundary failure unchanged, lint 127 (40e/87w), no-raw-spacing exit 0, no-editorial-chips exit 0, build 7 routes). Bundle delta near-zero (~+1 KB gzipped). Frontend-only, no `model_version` bump, no worker deploy. **Visual-audit gap noted**: chrome-devtools-mcp browser locked by parallel CC sessions; operator approved curl-only verification (build clean + all 20 JS chunks return 200 + new exports `SENTINEL_DASH` / `kkme-crosshair` / `Newsreader` confirmed in static chunks — 18.1.1 ChunkLoadError class structurally absent); in-browser hover / theme-toggle visual confirmation deferred to operator post-merge via Cloudflare Pages preview deploy. Branch pushed for PR. Previously Session 45 — Phase 18.1 (mobile foundation pass) shipped on `phase-18-1-mobile-foundation`. Five sub-items, single PR, 11 files: Next 15 `viewport` export adds `width=device-width` meta; hero 3-col grid (≥1060px min) collapses to single-column flex stack at <900px; KPI ticker reflows desktop 6-col → tablet 3-col → mobile 6-col scroll-snap; eight touch-target sites bumped to WCAG 2.5.5 44×44 minimum on mobile via additive `.tap-target-mobile` class hooks (no inline-style refactor); smooth scroll + `overscroll-behavior-y: contain` + `:focus-visible` ring globalized. Pause B runtime: `scrollWidth === clientWidth` ✅ at 360 / 414 / 768 / 1024 / 1440px; tap targets ≥44×44 mobile, compact 32-39 × 25-30 desktop preserved. Pause C addendum: StickyNav uses `position: fixed`, zero `position: sticky` in `app/`, pinning verified at 360 + 1440. Three notable surprises: (1) Tailwind v4 / Turbopack silently dropped a stacked `.hero-map-wrapper` rule inside an existing `@media` block — hoisted to its own block to retain (memory-entry candidate); (2) SVG `<g>` past viewBox boundaries inflated `scrollWidth` independent of CSS clipping — fixed with `overflow: 'hidden'` on SVG inline (off-map city labels at viewBox-x > 1024 from geographic east of calibrated map area; **Phase 18.1.1 question for operator: were those labels intended-visible by design?**); (3) page-level `html { overflow-x: hidden }` added as safety net for internal sub-card overflow (DSCR triple panel + RTE assumption rows + cycles_breakdown wider than parent at 360 viewport) — proper per-component `min-width: 0` + label-truncation pass queued as **Phase 18.1.1**. All gates baseline-exact (tsc 0, vitest 925/925, lint 126 (40e/86w), no-raw-spacing exit 0, no-editorial-chips exit 0, build 7 routes). Bundle delta near-zero (CSS-only; production CSS chunk 42,421 B). Frontend-only, no `model_version` bump, no worker deploy. 15 visual-audit PNGs in `docs/visual-audit/phase-18-1/` (5 viewports × 3 sections). Branch pushed for PR. Previously Session 44 — Phase 12.11 (P0 functional bug reconciliation) shipped on `phase-12-11-p0-bug-reconciliation`. Audit-v2 P0 list reconciled: **5 fixes shipped** (fleet 822 vs 651 inline disclosure via "(BESS + pumped hydro)" subscript on hero + SignalBar; AFRR precision 2dp standardized across **all 5 consumer sites** of `s2.afrr_up_avg` per Pause B operator-approved cross-card fold-in; freshness "TODAY" boundary tuned to calendar-today-in-`Europe/Vilnius` per discipline rule #2; DISPATCH header `/MW` → `/MW/DAY` to match hero unit; LV/EE empty-cell sup² inline footnote anchor mirroring LT 2h sup¹) + **3 NOT REPRODUCIBLE deferrals** (COD 2027 stragglers — live `/revenue` matrix legitimately emits `cod=2027` rows, sensitivity test correctly mirrors; CAPEX tornado consistency — `buildTornadoBars` math verified consistent; intel >30d staleness — filter exists at `IntelFeed.tsx:1066-1077`, deferred upstream feed-source-refresh question to Phase 12.12 #3). Vitest 919 → **925** (+6 calendar-EET freshness specs). All other gates baseline-exact (tsc 0, lint 126 (40e/86w), no-raw-spacing exit 0, no-editorial-chips exit 0, build 7 routes). 8 files modified, ~160 insertions. Frontend-only, no `model_version` bump, no worker deploy. Visual audit captured in `docs/visual-audit/phase-12-11/` (6 PNGs); DOM evaluation across all surfaces confirmed inline disclosures + precision standardization render correctly on dev server. **Phase 12.11.1 follow-up filed**: tornado COD-axis framing as 2026 ages (cod=2027 still meaningful when no greenfield asset can hit it?). Phase 12.12 #5 candidate metrics surfaced for cross-card consistency CI gate. Branch pushed for PR. Previously Session 43 — Phase 7.7g-a-2 (spacing tokens + rollout) shipped on `phase-7-7g-a-2-spacing-tokens`. Canonical 8-value t-shirt scale (`--space-2xs:4 / xs:8 / sm:16 / md:24 / lg:32 / xl:48 / 2xl:64 / 3xl:96`) added to `app/globals.css`; the prior 5-token block was cleanly renamed because Pause A discovery found **zero consumers** codebase-wide. **361 inline-px sites migrated** across 56 files (288 quoted + 73 bare-number). New CI grep gate `npm run lint:no-raw-spacing` (value-aware: only flags raw px in single-prop `padding/margin/gap` when value matches the canonical 4/8/16/24/32/48/64/96 set; off-scale + shorthand left raw, gated separately for Phase 7.7g-a-3). All four gates baseline-exact (tsc 0, vitest 919/919, lint 126, build 7 routes). Frontend-only, no `model_version` bump, no worker deploy. Visual confirmation deferred — production-build CSS artifacts verified correct, 1:1 mechanical rename verified by diff inspection; chrome-devtools-mcp browser was locked by parallel session, dev server at :3000 (PID 54297) was serving stale CSS pre-HMR. Phase 7.7g-a-3 backlog created: ~233 off-scale + 171 shorthand + 16 missed top-level pages (intel / regulatory / layout) + 2 conditional ternaries (BalticMap / AssetDetailPanel) + per-side CI regex extension. Branch pushed for PR. Previously Session 42 — Phase 18 Baltic editorial visual identity ship (Plex Mono + Newsreader + sharp 0px corners + per-card accents + broadsheet masthead + footnote discipline + bracket SourceFooter + pull-quotes; 18 visual-audit PNGs; ~143 KB Latin bundle = -13% vs baseline). Previously Session 41 — Phase 29 (KKME Baltic Storage Index) shipped on `phase-29-baltic-storage-index`. **First public-facing Tier 1 product surface**: monthly per-country per-duration €/MW/month revenue benchmark. Worker `GET /index/baltic` + `POST /index/update` (UPDATE_SECRET-gated) live on version `3c77897f-62ec-4dff-90c7-4aa94f052f47`; LT/{2h,4h} canonical (option-ε at Pause A — €284 / €307 for Apr 2026 in production-engine smoke); LV, EE, and 1h slots ship `coverage_status` strings (`pending_phase_29_1` and `pending_engine_1h_physics` respectively) per discipline rule #1 + #6 transparency framing. **Phase 30 destination decision resolved**: standalone `/methodology` route shipped (was Operator-action-item #4) — Next.js server component renders `docs/methodology.md` at build time via `react-markdown` + `remark-gfm`, anchor-aware via slugified heading IDs. Methodology paper +1 new "KKME Baltic Storage Index" section documenting calculation (engine `/revenue.backtest` reshape × 30 / 50 MW), launch coverage profile, comparability framing vs Clean Horizon, annualization, source-of-truth + cadence; Scope time-horizon line + comparison table updated to remove "planned" tags. New VPS Python script `scripts/vps/baltic_storage_index.py` (~310 lines) + PostgreSQL append-only history table `scripts/vps/sql/002_baltic_storage_index.sql`. Five commits pushed. Worker deploy `3c77897f-62ec-4dff-90c7-4aa94f052f47` live; Pause C curl confirmed `GET /index/baltic` returns expected 404 with hint pre-VPS-push (no KV data yet) + `/health.signals.baltic_storage_index_latest` recognized as `missing` per the 36h threshold added to `STALE_THRESHOLDS_HOURS`. All four gates green: tsc 0, vitest 919/919, lint 126 (down from 170 baseline due to `.wrangler/tmp/` cleanup; new files contribute 0 lint), build 9 routes (was 8 — `/methodology` added). No `model_version` bump (engine unchanged; index is a derived aggregation surface). **VPS Python deploy block in Session 41 below — operator fires post-merge**: scp the Python file, `psql -f` the migration, `--dry-run` smoke, first live run, daily crontab entry. Phase 29.1 (per-country DA capture extension + 5-product cap-reservation extraction; ~3-4h estimate) is the dedicated follow-on that closes LV/EE; engine 1h SOC physics is a separate extension on demand. Branch pushed for PR. Previously Session 40 — Phase 12.10 follow-up shipped on `phase-12-10-followup-gap5-and-a68-boundary`. Phase 30 P1-critical Gap #5 reconciled as methodology disclosure (NOT engine bug): direct primary-source verification confirmed Clean Horizon's €77/€340 aFRR-up/down figures come from "Baltic S1 2025 Price Forecasts" (June 2025, aggregate-Baltic, Apr–mid-Jun 2025 launch window) — two known step-changes since (summer-2025 market deepening visible in CH's own October 2025 update, and Baltic-Continental synchronisation Nov 2025 visible in KKME's S2 monthly trend chart) explain the order-of-magnitude gap; KKME engine is computing the same metric Clean Horizon publishes. EE A68/fleet boundary (Phase 12.12 #15) closed at policy (a) strict-commissioned, codified in `docs/methodology.md`. Three commits: methodology paper +2 paragraphs + new "Installed-capacity definitions" subsection + dated Updates entry; worker `/s2.macro_context.afrr_methodology_note` one-line additive field; handover. Worker deploy `ea88ccc9-0b45-46e6-87cc-b0afbac7407d` live; Pause B curl confirmed `afrr_methodology_note` renders (626 chars). All four gates baseline-exact (tsc 0, vitest 919/919, lint 170, build clean). No `model_version` bump. Branch rooted at operator's freshly-pushed `face05f` roadmap delta (Phase 12.10b housekeeping bundle + Phase 12.12 #17 + Phase 12.13 #6 sequencing note). **Phase 29 (KKME Baltic Storage Index) unblocked** — Gap #5 was its only listed blocker per Session 31. Discipline rule #1 (audit-triage) hit secondarily: Phase 30's "P1 critical" label was authored from WebSearch summaries without primary-source verification; backlog item authored to consider tightening research-process guardrails. Mid-session git-state recovery noted (HEAD unexpectedly switched off feature branch during deploy/commit flow; methodology commit landed on local main on top of `face05f` then was preserved on the feature branch via ref-move + local-main reset to origin/main; no work lost; transparent record + future-session guard recommendation in Session 40 entry). Branch pushed for PR. Previously Session 39 — Phase 7.7g-a-1 dead font triplet drop shipped on `phase-7-7g-a-1-token-audit`. Previously Session 38 — Phase 12.10a CLAUDE.md discipline patch shipped on `phase-12-10a-claude-md-discipline`. Six discipline rules (audit-triage, no-hardcoded-temporal-label, named-entity verification, cross-card consistency, roadmap edit-conflict, no-editorial-state-label) baked into `CLAUDE.md` with originating incident-trace per rule; "Current phase" stale block (Hero v3 / Phase 2B-1) refreshed to point at Tier 0 closing → Tier 1 (12.12 + 7.7g, parallel). Documentation-only — `CLAUDE.md` +19 / −2 (1 file). All four gates baseline-exact (tsc 0, vitest 927/927, lint 170, build 8 routes). Last Tier 0 item; after merge + roadmap delta apply, Tier 1 begins. Branch pushed for PR. Previously Session 37 — Phase 4G intel encoding shipped on `phase-4g-intel-encoding`. Audit-#6 premise was wrong; CC's §10 discipline caught it at Pause A. Original 1–1.5h scope shrunk to ~20 min: (a) IntelFeed badge + View-all link converge on `allItems.length` denominator (1-line edit at `IntelFeed.tsx:1311`); (b) defensive `resp.encoding = 'utf-8'` on the 3 `requests.get` HTML scrapers in VPS `daily_intel.py` (audit hygiene, NOT a fix for the current mojibake — daily_intel.py has zero `unquote/cp1257/latin-1` references; live litgrid scrape returns `charset=utf-8` correctly); (c) single-item KV purge of `cur_mo87wkt8-65w5pc` via `POST /feed/delete-by-id`. The 1 production mojibake item came through `POST /curate` (`origin: 'curation'`), not the ingestion pipeline — UTF-8 `ė` (`\xc4\x97`) interpreted as cp1257 → `ÄŠ` happened in the operator's clipboard chain upstream of KKME. Phase 12.12 #16 backlog item authored: curation ingestion encoding-passthrough audit. Same audit-vs-reality pattern as Phase 12.8.0; the artifact itself is the discipline lesson. /feed: 8 → 7 items, mojibake regex matches: 1 → 0. 927 tests, baseline preserved. No worker engine changes. Branch pushed for PR. Previously Session 36 — Phase 12.9.3 default duration 4h → 2h. Session 29 — Phase 12.10.0 emergency hallucinated-entity purge on `phase-12-10-0-entity-purge` off post-merge `main`. New `POST /feed/delete-by-id` worker endpoint shipped (UPDATE_SECRET-gated, ~50 lines). `isGenericSourceUrl` + `hasHallucinationHedgeLanguage` helpers exported from `app/lib/feedSourceQuality.ts` for Phase 12.12 to wire into `evaluateFeedItemGates()` at worker line ~6602. Investigation traced the Saulėtas Pasaulis ingestion path: entry came in via `POST /feed/events` (id `mna2ne4x-xfri25` matches `makeId()` + typed-event field shape, no `cur_` prefix, no repo source) — operator hand-pushed via curl with LLM-drafted content from an external chat. Phase 12.12 #8's structural gate target is the typed-event endpoint, not curation. Worker deploy `043fd2cb-1146-4d96-95c2-0ecb2864f5d7` live; delete-call deferred to operator (UPDATE_SECRET not in this session's shell — endpoint deploys regardless, fire later). 882 → 893 tests. Branch pushed for PR; PR draft at `docs/phases/phase-12-10-0-pr.md`. Previously Session 28 — Phase 12.8.0 Tier 0 hot-fix bundle on `phase-12-8-0-tier0-hotfix`. Audit-investigated: the prompt's "highest-priority light-mode bug" was empirically false (152 root tokens, 114 light overrides, HeroBalticMap fully tokenized; visual screenshots confirm parity). Light-mode commit reduced to a single `ContactForm` chevron fix + investigation writeup. Three other sub-items shipped per Pause A decisions: percentile tiles → static stat-summary strip; keyboard shortcuts → SOT + outline flash + `?` overlay (fixes 2 dead bindings); ticker → pause-on-hover + edge fade + robustified reduced-motion selector. 866 → 882 tests. **Process finding: 3 of 4 audit-#2 visual claims confirmed hallucinated this session — see audit-credibility taxonomy in Session 28 entry + `docs/investigations/phase-12-8-0-light-mode-audit-vs-reality.md`.** Branch pushed for PR; PR draft at `docs/phases/phase-12-8-0-pr.md`. Cloudflare Pages preview verified. Previously Session 27 — Phase 12.8 Dispatch render-error fix + boundary upgrade; see `docs/investigations/phase-12-8-dispatch-render-error.md`.)
