...
- **Next CC pick — Phase 18.1.3 (operator-confirmed 2026-05-07):** mobile per-component reflow (extracted from reverted Phase 18.1.1). Prompt at `docs/phases/phase-18-1-3-prompt.md`. ~1-2h CC. Frontend-only, low-risk. Addresses operator AssumptionsPanel screenshot bug. 5 sub-items: (1) DSCR triple panel reflow, (2) AssumptionsPanel grid reflow (the operator-screenshot bug), (3) S3 BreakdownBar reflow, (4) chart-canvas track + tornado SVG + MonthlyHeatmap fixes from reverted 18.1.1 Pause A scope additions, (5) opportunistic band-aid removal verification gate. NO mobile map redesign (Phase 18.1.1.1 territory; deferred separately).
