...
- **Phase 33.A — Map feed Baltic allowlist + frozen installed_mw pipeline** (~3-4h, mix of worker + external `~/kkme-control-center` repo + worker backstop) — Two sub-problems with shared "map data is wrong" symptom: (1) **esn pollution / international entries:** Phase 33 Pause A traced 196 `/s4.projects` entries with 26 polluters (Meralco Philippines TSO mislabeled "Litgrid", entries from CL/SA/AU/MD/AT/BG/HU/RO/SE). Root cause is upstream `esn_scraper`/`esn_loader` in `cron_daily.sh` + `kkme_sync.py` pushes verbatim with no country filter. Fix: country allowlist (LT|LV|EE) at upstream PG → worker boundary + worker backstop at POST `/s2/fleet`. Reject-on-flag for `severity:HIGH` contradictions (Meralco's C-11 "MW unusually large for Baltic BESS" should have rejected, not just flagged). (2) **Frozen installed_mw pipeline:** Phase 33 Pause C `/s4` curl revealed `storage_by_country.LT.installed_mw = 484 (as-of 2026-03-23)` — unchanged by today's VPS catch-up. Means the installed-MW field has a SEPARATE pipeline frozen ~3 months, not the inline-cron set today's fix unblocked. Operator confirmed via mental model that real fleet has grown materially since March. Likely paths: (a) TSO operational ledger lag (Litgrid/Elering/AST own delays between commissioning and registering); (b) source coverage gap (no positive Baltic feed for fresh commissionings); (c) ingestion isn't reading the field that updates. Investigation + fix. **Operator action item embedded:** jot down 5-10 known-operational projects not on the map (name, MW, country, COD) — that sample diagnoses whether the gap is TSO-lag, source-coverage, or naming/ID mismatch.

  Also includes: blank-`cod`-field investigation (all 196 projects have empty `cod` — extraction never happens), `/curate?action=purge_non_baltic` endpoint build (doesn't exist; `/curate` is intel-feed only — needs a fleet-purge path or a filtered full POST replace).

- **Phase 33.B — BTD `afrr_cap_avg` re-parse + FCR disclosure decision** (~1-2h, P2) — `prices_source: "BTD partial"` will persist until the BTD parser is updated to extract `afrr_cap_avg` from whatever new schema/page the source moved to. Currently the Phase 33 capPrice bound papers over this with calibrated constants — defense-in-depth, but the bound being "always active" is a smell. Fix the parser to populate `*_cap_avg` from current BTD source, then the bound becomes the safety net it's meant to be.

  Companion decision: the `[revenue/s2-capacity-watch]` log will accumulate evidence on whether `fcr_avg=63 €/MW/h` is a transient anomaly or a real Baltic FCR market shift post-CE-synchronisation. If it persists for weeks at €50+, that's a material model assumption to reconsider (with operator sign-off on the IRR move, NOT silent re-routing).
