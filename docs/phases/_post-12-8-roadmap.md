...
- ~~**Phases 48-52 — the hardening arc**~~ **[SHIPPED 2026-08-03/04]** — overnight unsupervised run (10 items, PRs #141-#150) plus supervised follow-through (#152-#162). The through-line: **defects that produce a plausible number rather than an error, and gates that pass because they are broken.**

  **Phase 48 — endpoint auth.** `POST /feed/clean` was unauthenticated, remote and destructive: a caller-supplied `before` wrote straight to `feed_index`, and a malformed body fell through `catch { /* empty body ok */ }` to a 60-day default. Found when a probe expecting 401 got `200 {"cleaned":0,"remaining":4}` — it executed, and cleaned zero only because of the default. The A7 audit found **sixteen** unauthenticated writers, not four; thirteen remain, all reported, including nine GET routes that recompute-and-write on a public read. `/curate` deliberately left open until its VPS caller was observed sending a secret (22/22 verified) — gating it first would have killed ~30 intel items/day.

  **Phase 49 — deep debug, five defects with a class guard each.** `extractPrices` ignored `<position>` on ENTSO-E A44 `curveType A03`, so 92 of 94 values landed at the wrong time and a UTC-bounded request concatenated two market days; Elering as an independent control read 2/96 before and 96/96 after. **The € error is intermittent; the TIME LABELS were wrong every day** — peak 22→20 UTC, trough 16→14 — which nobody predicted and is the larger finding. `calcIRR` returned exactly **2** for any IRR above 200 % (upper bracket escaping as a value); now null at both edges. The B-065 residue reached `signal_inputs.s1_capture` at 818.59 against an observed €101.91 (8.03×) through a fallback nobody exercised. Five scheduled writers had no staleness threshold at all.

  **B-075 — the site overstated Baltic price separation by ~3.2×.** LT was paired against SE4 **by array index** across 30-day ranges, so each zone's A03 omissions shifted its series against the other and the code silently truncated to the shorter array (2916 vs 2932). `pct_hours_above_20` 21.8 % → **6.9 %**; `rsi_30d` −0.68 → −1.08; `trend_vs_90d` 0.92 → 2.05 (a difference, not a ratio — a probe that assumed ratio got the sign backwards). The older the window the worse the misalignment, so the 90-day figure was the most distorted — the opposite of what averaging implies.

  **Phases 50-52 — durability and the gates.** `s2_daily_clearing`'s importer was never scheduled at all (a 36.C one-off), and the "expiring archive" premise was false — BTD's boundary is a fixed data start, proven by identical coverage at identical dates on consecutive days. The KV backup **nearly published personal data**: the NDA gate blocked the commit on a false positive (numeric needles colliding with clearing prices), and chasing it down surfaced `contact_submissions` holding real names, emails and messages headed for a public repo. Restore tested against a scratch namespace: 1131/1131, after two fixes (a retry-less restore silently dropped a key; KV eventual consistency made 5 correctly-written keys read as missing).

  **The gates themselves were the richest seam.** `lint:no-editorial-chips` — CI-enforced since 12.9.1 — **passed because it broke**: `! grep … | grep -v …` inverts the last pipeline command, so a first-grep error became success, and it had no positive control. A single file with invalid UTF-8 could have silenced the NDA gate entirely (BSD grep aborts, and its error code is indistinguishable from "no match"). A refactor replacing every `KKME_SIGNALS.put()` with `admitSignalWrite()` took the writer scanner from 17 writers to 1, reported "0 with NEITHER", and passed — caught only by `gates:selftest`. **New playbook rows: B14** (gates passing locally for reasons that do not exist in CI — five causes in one workflow), **B15** (exit-code conflation: a tool's ERROR read as its negative RESULT), **B16** (diagnosis-destroying error handling: `computeS4` rejected every tick for 28 h and the cause survived in none of four places, while the compute itself completes in 380 ms).

  **B-076 — PARTLY CLOSED 2026-08-06, and four of its own claims were falsified by measuring them** (see Phase 53). Corrected: (1) `s2_activation` has **three** write paths, not one — `POST /s2/activation` validates only that `body.countries` exists and writes verbatim, so the frozen payload's provenance was unverifiable; (2) published projections were **OVERSTATED, not understated** — fresh data is worse on every headline metric (gross −3.53 %, project IRR −66 bp, equity IRR −108 bp, NPV −1.19 M); (3) the "~3× basis difference" **does not exist** — it was monthly medians compared against `initial_p50` and `recent_avg_p50`, different months and different reductions over identical bytes; (4) the stated mechanism via `deriveCompression` carries **zero** of the effect and would have across all 105 days, because both series sit on the `min(0.15, …)` clamp. **The real defect was `computeBaseYear` reading at face value the exact post-sync months its sibling function was written to distrust — 10× the freeze on IRR (60 bp vs 6 bp).** Shipped: a canonical month-eligibility rule (≥20 % settlement-period coverage AND month ≥ 2026-03, boundary measured — 121× price dispersion on partial coverage before, 3.5× with continuous coverage after), consumed by both functions, with the exclusion and its measured effect disclosed in the drawer. `compression_rate_observed` retired — it was the clamp wearing the word "observed" (rule #2). **Still open:** the VPS leg for `s2_activation` (fresh data remains behind its flag), the 54-config delta on the corrected basis, and matching Germany's population definition to collapse E2's transferred direction-split band. Also open: B-069, B-072 follow-through, the seven signed read-path removals, `/digest` write-on-GET.

- **Phase 38.3 CLOSED AS MALFORMED + Phase 38.4 — dispatch reconciliation [DIAGNOSIS, 2026-08-03]** — PRs #131 (38.3 disclosure-only) and #132 (38.4 documentation-only). No code, no deploy, no published number moved.

  **38.3's premise failed measurement and CC stopped before implementing.** The arc claimed the dispatch-card → hourly-engine cutover moves public IRR materially upward; measured across all 54 configs it is **+0.07 to +0.15 pp (median +0.14), 18 of 54 moving at all**, cycles/LCOS/gross unmoved — because **the degradation model is only characterised down to 1.0 c/d** (`sohYr:6368` `max(cd_total, 1.0)`; `getDegradation:1467` zero-weights the 2C curve at cd ≤ 1.0). Publishing 222 EFC/yr would mean publishing a cycle count the wear model cannot use. Shipped as disclosure only: the validity floor stated as a limit, and **the cycling answer as an answer** — the 550-720 band is merchant-battery research while this asset is reserve-led at 72.6 % of Y1 gross, so the comparison is a category error, said with our own numbers.

  **38.4 found the real defect, and it is not a disagreement between engines — it is a unit error plus a double-sold asset.** `trading_fraction = min(0.70, (T/(T+R)) × 0.75)` is derived in **€/€** and spent at `fetch-s1.js:2283` multiplying `da_mwh_per_mw_yr` in **MWh/MW/yr**. The engine already computes the physical quantity that line needs — `effective_arb_pct` = 0.141 — **publishes it every request, and consumes it nowhere**: a diagnostic that has contradicted the shipped number by ~5× in production for months with no gate. Worse: **`rev_bal` carries no `(1 − trading_fraction)` factor while `RESERVE_PRODUCTS` shares sum to exactly 1.00** — the reserve model already claims the whole asset and the trading line then claims 70 % of it again. The same megawatts are sold twice. **B-065 filed P1.**

  **B-063 restated — the "2.5×" was an accounting artefact.** `rev_trd` is a spread margin with no charging-cost line, while the hourly arbitrage carries the entire charging cost including energy later delivered as activation. Like-for-like: 27.8 % spread margin · 10.7 % raw · **21.0 % attributed-net** · 31.5 % discharge income → **1.32×, not 2.5×**. The real disagreement lives in ENERGY (DA delivered 747.0 vs 210.9 MWh/MW/yr = 3.54×), not in the revenue ratio. **Two Cowork premises corrected:** B1's 75.2-85.5 % simultaneity is `constrained_gross / unconstrained_gross` — it measures what SoC reservation costs the reserve stack and cannot discriminate free-MW share (committing 72.5 % of MW and retaining 85.5 % of the stack are consistent); and the three-phase sequencing rationale rested on the cutover raising IRR, which counted the EFC benefit while ignoring which side of the trade the revenue line sits on. **Correcting this lowers IRR materially** — and it is not a constant swap: substituting a smaller number into an unpartitioned model would double-count less, not stop double-counting.

  **Degradation below 1.0 c/d: internally fittable, deliberately not adopted.** A separable form (`loss = 0.0602·√t + 1.463e-05·EFC`, fitted to the engine's own three curves, 51 points) interpolates rather than extrapolates — but the 3.3 pp worst residual is too coarse for a revenue driver and the payoff is ~2 pp SOH at Y10. **B-064 left OPEN** so the shipped disclosure is never misread as the fix; adoption requires citable coefficients from a primary source. 36.B1's `engine_branch_gap` (550 vs 385 EFC) marked stale in the committed artifact (A9) — not in the untracked JSONs, which carry an `output_hash` that amending would damage.

- ~~**Phase 38.2 — corrections sweep**~~ **[SHIPPED 2026-08-03, semi-autonomous (1 CP, operator-signed)]** — PR #130, main `33d2c25`, worker `15f7c7ac`. CP artifact: `docs/investigations/2026-08-03-phase-38-2-cp-delta.md`. Suite 2077+ · `/revenue` 54/54 byte-identical at every stage.

  **The `/s4` whitelist fixed by INVERSION, not by widening** — everything `processFleet` computes now reaches `/s4` unless explicitly excluded; adding the four missing names would have fixed today's four and dropped the next aggregate in the same silence. **8 of 20 fields were being dropped.** It had kept 36.D's canonical S/D caption dark since it shipped (proved on rendered output in production: header, tiles, and nothing between them — `sd-formula-caption` count 0; after: SSR markup carrying `(2385 − 200) MW / 752 MW = 2.91×`), degraded the ticker's FLEX FLEET tooltip (2 of 5 sentences), and dropped three fields with NO consumer at all — including 36.D's `demand_basis`, built and never surfaced (B-062, routed to surfacing). **Audit correction:** the hero was never dark (it reads `/s4/fleet` unprojected) and `Strict-verified count: —` does not reproduce — rule #1 applied to Cowork's own audit document.

  **Two P1s found by a KV read the audit had reported as blocked** (`--binding` works where `--namespace-id` 401s): **B-058** — `baltic_total.installed_mw` read a stored assertion that WINS over `lt+lv+ee`; it equalled the sum, so the duplication was invisible, and moving LV to 80 would have left the headline publishing 651 while silently ceasing to be the sum of the three numbers beside it. Deleted; a test now perturbs a country and checks the total follows (**under injection 3 red / 2 green — and the two that stayed green are exactly the "equals 651 today" spec and the no-assertions spec: a duplicate writer that agrees today is invisible to any test written against today**). **B-059** — `fetch_entsoe_installed_capacity.py` builds its body from scratch with only `*_live` keys and the worker `put` it wholesale, so the script's FIRST success would have deleted all 16 assertions. Disarmed on the worker side (merge by default, `mode:"replace"` the only path to destruction, flag consumed not persisted); injection with the wholesale put restored prints what production would have: *assertion apva_applied_storage_lt_mw was deleted*. Live after deploy: `{mode:'merge', assertions_pushed:15, assertions_stored:15, dropped:[]}`.

  **LV 40 → 80 MW**, as-of 2025-10-30, cited to AST's own release (ast.lv 403s curl and WebFetch — verified by loading the pages in a browser). The 40 was a stale pre-commissioning estimate stamped `confidence: "official"`; **the grade was corrected as its own act** (row 13 official → estimate, superseded not overwritten, 17 supersedes 13). `baltic_total` 651 → 691. **80 and 99 are both right and both render:** 80 answers "what has the TSO published as installed", 99 answers "what do we track as operational". **The values were never in the script** — JOB 7 pushes every assertion `WHERE is_current = TRUE` from Postgres, so LV 40 was a database row. **B-061:** vendoring the pusher into the repo surfaced a live production Postgres credential as an inline default on line 40 — dropped before committing (this repo is public), verified safe first (`/opt/kkme/config/.env` holds the byte-identical value, wrapper is bash with `set -a`, the default was never reached), redeployed with backup and proven on the host. Never published; no rotation forced.

  **B-056 resolved as option 3 — two series, two sources, honest n.** `lt_swing` rebuilt from `s1_capture_history` (one canonical writer, rule #4); `spread_stats_90d` publishes n = 9 with `days_of_data` counting DISTINCT DATES everywhere; `/s1/history` 90 rows / 9 dates → 9 rows / 9 dates live; PeakForecastCard withholds the P90 comparison and the green dot below 10 market days. **B-060 filed:** `updateHistory` stamps the WRITE date, not the market date, and keep-last disagrees with the settled capture series on 7 of 9 days — not an off-by-one, cause not named because it cannot yet be evidenced.

  **Two of CC's own new specs passed with the bug reinjected** (the 14-row-tail spec appended clean dates after the contaminated ones so `slice(-14)` never touched the repeats; `days_of_data` had no behavioural assertion because no route exposes it) — B13's corollary earned a second time. **And a green suite could not have caught the last defect:** post-deploy the live payload returned `swing_stats_90d: n=400` — `s1_capture_history` holds **400 market days**, not the 30 `/s1/capture` displays. The mirror image of B-056: that label overstated its window, this one understated it. Windowed to 90, spec added against the 400-day archive, redeployed. C8 mattered on both deploys — a mid-sequence read returned the pre-deploy artifact each time.

- ~~**Phase 38 audit + 38.1 — the S1 outage and its blind monitoring**~~ **[SHIPPED 2026-08-02]** — PR #129 merged, main `bf66249`, worker `ebd5eba4` deployed from main with byte-identity confirmed on all three deployable files. Audit artifact: `docs/investigations/2026-08-02-phase-38-sync-audit.md` (35 surfaces traced).

  **All four screenshot-derived items CONFIRMED — none was operator misreading.** Recorded against rule #1's ~25 % base rate: *visual inference on your own site, where you know what the number should be, is a different instrument from a third-party visual audit.*

  **"Stale 33 h" was not the problem it looked like.** The S1 branch had failed **nine consecutive ticks** and intermittently for over a week — dated by TTL arithmetic on `raw:s1:<date>` (expiration − 604800), a clock nobody built. Then the tail refuted the throttling hypothesis twice: 48/48 HTTP 200 replicating computeS1's exact 9-request burst externally including the 4h-aligned collision, and the production log showing a TIMEOUT, not a rejection — `cpu=83ms wall=50017ms`, with computeS1's own completion logs sitting in the same invocation as the timeout that discarded them. **The work finished, late, and was thrown away.** The fix written against the rejection hypothesis (per-leg guards) would have shipped green while S1 stayed dead. Actual fix: budget 30 s → 60 s, retry delay shortened (dead wait comes out of the budget the timeout measures), `fetchEnergyCharts` guarded against the 429 that was actually observed; guards kept as correct-but-not-sufficient.

  **Recovery proven twice, with discriminating evidence rather than green.** Ticks 16:00Z and 20:00Z, each polled to agreement: `s1_capture` advancing proves `computeCapture` ran (it now runs even when computeS1 fails); a fresh `raw:s1` proves the branch itself completed. **The ordering flip is the fix's fingerprint** — capture now writes 1.70-1.88 s BEFORE the branch, where the last pre-fix success wrote it 18 s AFTER; a single green tick was always consistent with variance, an inversion reproduced on both ticks is not. Tick 1 ran `wall=78683ms`, 2.6× the old wrapper.

  **The monitor's first act was to report the outage it had been blind to** (`s1_capture {age_hours: 40, stale: true}` → 0 after tick 1). Before: `s1_capture` had NO threshold at all, and the key that WAS monitored (`s1`) was rewritten by any unmatched GET — `/health`'s entry measured probe traffic (**B-047 closed**, ALL-N audit: 92 resolved route forms vs 32 called worker paths, 0 relied on the catch-all; the first enumeration reported 12 false misses by matching only `url.pathname === '…'`). Three staleness surfaces demonstrated failable; **injection 2 took three attempts, passing twice WITH the bug reinjected for two different wrong reasons** — it was verifying the mock, not the router (playbook B13 corollary).

  **Public state moved in the unflattering direction, honestly:** S1 hero €142 (2026-08-01, 88 intervals, frozen 40 h) → €85.99 (2026-08-02, 96 slots); forecast dispatch 2026-08-01 @ €101/MW/day → 2026-08-03; S2 badge `RECENT · 2h ago` → `STALE · 66h ago · data through 2026-07-30` (it had been badging the fetch stamp while S1 badged the data stamp). **The number didn't fall — the clock started again.**

  **Filed:** **B-057** (both crons stamped the same second; `[Genload/hourly]` timeout, `[FX]` and `[S3]` AbortErrors persist — stagger candidate `0 * * * *` → `5 * * * *`, measure before restructuring). **B-056** grew and sharpened: the S1 card's "DAYS 30" is HONEST (deduped `s1_capture_history`, 30 rows / 30 dates), but `s1_history` is 90 rows over 8 dates behind `days_of_data: 90`, and **SpreadCaptureCard's "14D swing history" sparkline resolves to fourteen rows of a single date** — rule #2 on a live chart. Routed to 38.2 with a signed delta.

- ~~**Phase 36.E1 + E2 — FCR and aFRR price formation**~~ **[SHIPPED 2026-08-02, semi-autonomous (1 checkpoint, operator-signed with 5 conditions)]** — PR #128, origin `9048658`. Suite 2053/2053 · `/revenue` 54/54 byte-identical · summary table unchanged · **nothing wired: the modules sit behind their own seam until E6's continuity gate.**

  **The checkpoint's headline was a refusal.** The prompt, the arc and Cowork all expected a PICASSO break magnitude transferred from capacity evidence. CC counted the primary files and declined to produce one: neither market that could show the break has data on both sides of its own accession (AT aFRR **0** pre-accession quarter-hours vs 54,117 post; DE aFRR **9** vs 226,402; AT mFRR 49 vs 5,344; DE mFRR 101 vs 588), and our own Baltic clearing series starts 2025-10-01 — after MARI 2024-10-10, PICASSO Litgrid 2025-03-05 and Elering+AST 2025-04-11. **The break is already inside the level we calibrate on; applying a forward compression would double-count it.** Now methodology §08B.5, "an explicit non-application", test-bound to the calibration artifact so the counts cannot drift out of the prose. E3 verifies the same structure for MARI rather than assuming it.

  **Five premise corrections, four of them Cowork's.** (1) E6 replaces `reservePrice()`, NOT `cpiCurve()` — `cpiCurve` is disclosure-only, three call sites, all `cpi_*_at_cod`, no cash path, verified at code level. (2) German FCR's price ROSE while its ratio to arbitrage FELL — both true of different quantities; DE FCR procurement is flat (605 → 584 MW over 7 yrs), so the nominal rise is the arbitrage denominator rising 3.46 → 18.35 €/MW/h. (3) **The arc's endogenous floor is FALSIFIED** — "gross arbitrage net of cycling cost" sits above every floor in the evidence base, found by a reproduction test failing (Baltic aFRR down 17.85 modelled vs 10.04 measured), not by reasoning; `displacement` is now measured per market per product with NO default. (4) No PICASSO break. (5) **The E0 summary table is silently truncated** — DE arbitrage stops 2025-09 because it filters DA to PT60M and Germany went 15-min MTU on 2025-10-01 (B-055; the truncation alone flips DE aFRR down from t = −2.36 to t = −1.43). **Cowork's ≤1 % FCR expectation was an A8 anchor and was wrong:** FCR is 4.80 % of reserve-capacity revenue at Baltic volumes because Baltic FCR carries the stack's highest multiple (k = 3.03); the asserted bound is 10 %.

  **Five signing conditions, all applied.** RTE wired to the engine's canonical `RTE_BOL.h4` = **0.83, not 0.85** — and the consequence measured rather than argued: every parameter is a ratio to the arbitrage series, so scaling the series scales k by its inverse and the priced result is invariant (max absolute difference **0.0 €/MW/h across 87 months**); published table stays pinned via `ARB_RTE_E0_PUBLISHED`, folded into B-055's single rebuild. **Direction-split band:** down-activation revenue runs €3,025-5,714/MW/yr between a 50/50 split and the German shape, asserted materially wide (>1.5×) so it cannot shrink into a footnote. **DE FCR λ (t = −1.70) marked descriptive-only**, with a `|t| ≥ 2` adoption rule enforced in the calibration rather than left to discipline. **Baltic FCR no longer holds flat** — the real gap: `convergeK()` existed but nothing forced its use; `projectClearing()` now requires `lambda_per_yr` and `convergeK()` throws without it, so holding flat is available only as a stated choice (`lambda: 0`), never by omission. Adopted FCR rate is Germany's ex-crisis trend λ = 0.131/yr (t = −10.2, half-life 5.3 yr): k 3.11 → 1.65 by +10 yr → 1.26 by +20; its significance draws on a between-regime level shift, so it is an upper bound, adopted because over-decaying is the conservative error on a revenue line — both bounds in the register, tension stated in §08B.7. aFRR needs no such argument (own within-regime rates supported, t = −3.08 / −2.36).

  **Flagged because it reads as a sign error otherwise: aFRR DOWN converges UPWARD** — Baltic down sits at k 0.61 against Germany's 0.99, so convergence raises it. Asserted by its own test. Content pins updated and called out (register 47 → 65, methodology categories 7 → 8, `deliverable-notes.register_count` 47 → 65, one fixture date moved to keep the changelog chronological); no test deleted, no assertion weakened — the single narrowing (Baltic FCR displacement 1.0003) was tightened into its own scarcity assertion.

- ~~**37.B.1a + 36.E0.2 — runner scheduled on the VPS + manifest single-writer**~~ **[SHIPPED 2026-08-02, autonomous]** — PR #124. Suite 1959/1959 (+14, none deleted; 2 updated and called out) · `/revenue` 54/54 byte-identical · 0 files under `workers/` or `app/` · nothing deployed.

  **Sunday's scheduled refresh fired and answered E0.1's last open question: all eight sources served from GitHub Actions runner IPs** (6 ok / 2 anomalous / 0 failed, with real row counts) — the `fetch-btd.yml` blocking precedent did NOT repeat. The job still went red at the last step, producing four findings, filed not fixed: **B-051 (P1)** Actions is not permitted to create pull requests (repo setting, not code) — data is safe on `evidence-refresh/2026-08` at `174a166` (46 files, +2062/−841), but until the setting is flipped the PR-review shape E0.1 exists for does not exist; **B-052** `0 3 1-7 * 0` ORs day-of-month with day-of-week, so the cron fires ~11×/month not once (it ran both Sat 08-01 and Sun 08-02, and the workflow comment asserting "exactly one day per month" is wrong; GitHub also delayed 03:00 → 05:5x both days); **B-053** the tests gate CANNOT FAIL — `if npx vitest … | tail -40` without `pipefail` takes `tail`'s status, and it recorded `tests=pass` with **3 tests failing**; and the append-only gate WORKING as designed — se/da restated boundary-hour prices, da withdrew 24 rows, both correctly withheld `last_successful_refresh`.

  **The `env -i` proof earned its keep twice.** The prompt's "host is decided: the VPS" premise didn't hold — the box had no Node at all (37.B.1 had only run curl there); footprint after install is 7 files, zero npm packages. Then under `env -i`, run 1 died instantly on `ENOENT data/lifecycle-rules.json`, a runtime `readFileSync` no import-graph trace could see and an interactive shell would have sailed past. Run 2 clean: exit 0, four sources, POST → 200. Scheduled **Sunday 22:00 UTC**, cold run 7 s (register.csv mtime proves the 128 MB pull is inside it), and it runs a new `--write-health` flag rather than `--write` — scheduling `--write` would have been precisely the unattended wiring the runner's own header warns against.

  **E0.2 found eight writers, not one.** `fetch-calendar.mjs` was dropping `last_successful_refresh` that same day. All eight now route through one writer where omission is repaired and destruction fails; provenance-absence-as-ERROR correctly NOT built (§2.55). Two self-caught defects worth the record: carry-forward ran BEFORE the assert so the gate could never fire (its own test caught it), and the grep gate reported PASS on a live violation because `[^\n]*` in a bracket expression excludes the letter **n** — and `path.join` contains one; only the inject-then-remove control found it.

  **B-054 filed, not solved and not claimed:** `/health` now ages `last_run_at` independently of the digest, but `max_age_hours` is 720-1080 — a dead weekly runner takes 30-45 days to cross it.

- ~~**Phase 37.B.1 — the detector runner + digest arming**~~ **[SHIPPED 2026-08-02, semi-autonomous (1 checkpoint)]** — PR #123. Suite 1945/1945 · `/revenue` route-level 54/54 vs main at every commit · eslint clean · `docs/_private/` never staged · C8 polled to agreement.

  **37.B stopped being dormant code.** `/health.fleet_lifecycle` went `{"detectors":{}, "status":"never_run", "digest":{"armed":false}}` → 7 detectors with stamps, `capable_count: 5 / 7`, status `degraded` (correctly — 2 cannot fire), digest `armed`, cron `30 7 * * 1`, first firing **Monday 2026-08-03 07:30 UTC = 10:30 Europe/Vilnius**, confirmed present in Cloudflare's registered triggers.

  **Report-only first contact produced ZERO proposals — and the controls make the zero mean something:** known-good registers resolve 3/3 (Latvenergo, Sadales tīkls, Augstsprieguma tīkls with real regcodes), nonsense 0/2, known-terminated entities read back terminated 3/3 (the decay branch is live code, not dead), all 36 registry-eligible rows active. Controls now run every future run and a control failure SUPPRESSES the registry detectors rather than being believed. `appended:0, rejected:[]` — `transition_log_size: 0` is a populated state, not an empty one.

  **The three kinds of zero, kept distinct and now rendered.** The digest states how many detectors were CAPABLE of firing, per-detector verdict + eligible count + population unit, not a bare "all quiet": a week where nothing happened and a week where nothing could happen read differently at a glance. **The verdict is DERIVED, never posted** — the runner sends only measured facts (`source_usable`, `baseline_present`, `rows_eligible`, `population_unit`) and the worker computes the label, because a runner posting the string "checked" would leave a claim about the world sitting in KV after it stopped being true (rule #2, applied to machine-to-machine state). `classifyDetector` shared by `/health` and the digest with a drift test. Looking at the real payload rather than a fixture caught "412609 rows eligible" reading as 412,609 projects when it counts register name-keys — hence `population_unit`.

  **Checkpoint self-correction:** capable is 5 of 7, not 3 — the baseline run gave `queue_disappearance` its diff, and the registry pair CAN fire, they just cannot produce a *publishable* transition (the narrower claim, now in their reasons). **Secret handling improved rather than followed:** instead of copying a production secret onto the laptop, CC added `--emit-payload` and ran the POST from the VPS where `UPDATE_SECRET` already lives; it also corrected its own Pause-A claim that the secret was unavailable (a zsh glob miss had aborted the check before it read `.env`). **Arming was three edits, not two:** the scheduled handler had no branch for a digest cron, and B-046's drift test only compared the constant against `wrangler.toml` — a cron declared in both places and handled in neither would have passed the gate and silently never sent. New test drives the real `scheduled()` handler and asserts a message left and stamped; proven failable. Two tests changed, both SHARPENED (fixtures encoded the exact conflation this phase removes), inverted cases asserted alongside, called out in the commits.

  **Two honest residuals, self-flagged:** (1) **nothing schedules the runner** — stamps age from here and around 2026-08-15 the digest will correctly start reporting `queue_disappearance` and `press_negative` as stale; the heartbeat currently measures the digest, not the runner (→ 37.B.1a). (2) No pre-deploy live `/revenue` capture was taken for this deploy, so the live leg is a post-deploy determinism check (two captures, same UTC hour, 54/54) rather than 37.H1's before/after pair; byte-identity rests on the route-level probe vs main. Weaker evidence than last session, stated rather than implied.

- ~~**Phase 37.H1 — browser-auth fix (B-045) + digest staleness surface + 37.D counterfactual**~~ **[SHIPPED 2026-08-01, autonomous]** — PR #121, 9 commits, two deploys (`2eb99c6c`, `b0707bd6`) each after origin-SHA equality. Suite 1895/1895. `/revenue` 54/54 route-layer vs main AND 54/54 live pre/post deploy inside one cron hour.

  **B-045 closed with the proof that had been missing for the whole life of the defect.** Preflight before: `Content-Type, X-Update-Secret`; after: `+ Authorization`; controls `/revenue` and `/s2` unchanged. Then the round trip a test cannot do — operator signed in, CC drove COMPUTE from `https://kkme.eu`: `tier: "full"`, 17 keys, no SAMPLE chip, NPV €15.75M · MOIC 3.45× · payback 4 yr · gross capex €16.40M · project IRR 22.3 % post-tax geared. **Open question, not a defect:** first COMPUTE after sign-in rendered sample; full tier only after reload — one line to settle at next sign-in (if that tab, 35.2's "token → re-render full tier" isn't holding). **Playbook C8 banked:** the first post-deploy read returned the OLD headers and would have been reported as a failed deploy — correct ~20 s later. B3 at the deploy edge; verification now polls until two reads agree.

  **Two prompt premises false, neither routed around.** (1) "Non-additive" — CC scoped the CORS change on 37.C's precedent, making it additive; it ran the byte-identity gate anyway. (2) "Trigger the detectors once for real" assumed a runner that **does not exist** — `fleet_lifecycle:detectors` has one writer fed by a caller that isn't there (16 grep hits outside the worker, all tests). So 37.B's seven signals, rename guard and retirement policy are shipped code that has never executed against real data. Queued as **37.B.1**, a phase not a hygiene item. In its place CC built what the prompt made the precondition: `/health.fleet_lifecycle.digest` now surfaces armed / cron / `last_sent_at` / `age_hours` / status, unarmed is never "ok", and a drift test fails if the declared cron and `wrangler.toml` disagree. Digest correctly still unarmed.

  **37.D counterfactual — the number that justifies the fixture:** with the enrichment path rewired inert, **all 26 of batch-2's original assertions still pass and only the 4 new ones go red.** A citable row moves the payload 0 → 75 → 105 MW and removing it returns to baseline; `public-confirmed` + `bess_mw: 3583.5` behind a registry-only citation still contributes 0 MW (the conjunction rule, now test-enforced).

  **B-048 — investigated, escalated, then CORRECTED DOWN by CC itself (P1 → P3).** The initial diagnosis (a second writer deleting refresh-provenance keys, producing self-exempting corruption) was withdrawn: both changed shards match the worktree manifest's sha256, manifest and shards share mtime 2026-07-30 10:40:02 UTC, and the fetcher stamps `retrieved_at` at end-of-run — a run finishing 10:40:02 cannot stamp 10:25:46.517Z. **No second writer ran; manifest and data are consistent; `never_refreshed` is the correct reading and the "acquired by hand" explanation is true of this artifact.** CC restored `acquisition_retrieved_at` + `refresh_cadence_months` and **withheld** `last_successful_refresh` / `last_refresh`, because writing them would assert a refresh the dataset doesn't contain. **Cowork error banked as playbook A11:** my close-out instruction named the expected output ("reads `fresh / 2d` … paste that line as the proof") — an anchor pointing at the exact lie the finding was about. CC refused it and re-derived. Verification instructions name the check, never the expected value. **Closed same session as NOT-A-DEFECT, nothing ever lost:** the reflog named the cause to the second — `10:40:02 UTC` was a plain branch switch from `evidence-refresh/2026-07` back to `phase-36-b036-activation-source`, whose tip predated the refresh, so git rewrote the three tracked files to that branch's committed state and stamped them with the operation's wall clock. `1d8ae0a` is an ancestor of HEAD and HEAD's committed blobs hash to the shas the HEAD manifest records; the worktree was merely stale. The earlier restore was therefore INVERTED (it reconciled the manifest down to stale data) and redone with `git checkout main --`: rows tie 286,137, all five provenance keys, freshness `activation · fresh · 1m · 2026-07-30 · 2d · 60d` — true this time because the data it describes is on disk, and reached without re-running the refresh. **Three successive readings, each corrected by CC's own next check** (second-writer clobber → unexplained rollback → stale worktree). Habit banked: the deciding evidence sat in the session's FIRST reflog, unread past the top eight entries — reflog reads are date-bounded, never top-N. **B-047 filed** (worker has no 404; unmatched paths return the S1 payload with 200, so path probes can't distinguish absent from present).

- ~~**Phase 37 batch-2 — operator-only fleet CRM + forecast wiring (37.C + 37.D)**~~ **[SHIPPED 2026-08-01, semi-autonomous (1 CP, operator-signed)]** — branch `phase-37-batch-2`. Batch-1's worker deployed at step 0 (`2e558602`, from main = origin/main = `46d1bdf`, `workers/` byte-identical to origin, suite green first, B11 control run on the deploy itself: new endpoints 401 vs `/admin/zzqqxx-nonsense` 405 — they discriminate).

  **The headline is a negative, and it is the phase's most valuable output: the citable capacity contribution is 0 MW.** All 36 public-confirmed LV rows carry exactly one data.gov.lv citation — "entity resolves in the Latvian Uzņēmumu reģistrs, reg. NNNN, status active". Σ `bess_mw` = 0.0 MW. The only power figure (`site_total_mw` 3,583.5 MW) and the technology figure (`plant_type`) come from the private workbook, not the citation. **A registry entry proves a company exists; it proves nothing about a battery.** The arc's tier-mapping alone would have licensed publishing 3,583.5 MW of private testimony behind a registry citation — so publishability became a CONJUNCTION: verification tier decides the haircut, capacity-citability decides whether there is anything to haircut. That distinction is now the rule.

  **CP signed, all deltas zero — and the zero is explained, not assumed.** Baseline from a clean worktree of `46d1bdf`, both sides against one frozen KV snapshot copied byte-identical into each tree. Public: 54/54 configs byte-identical, largest relative movement across 54 × 4 metrics = 0.0000000000 % (reference config gross Y1 €7,994,239 · IRR 0.2228 · min DSCR 2.36 · NPV €16,139,595). Client: `portfolio.json` identical modulo timestamps (gross Y1 €12,770,114 · NPV@8 % €37,177,495 · three IRRs Δ +0.000000). Hybrid ships as a band (11,975.7 – 16,020.4 MW) whose UPPER bound is the status quo, so adoption moves no point; no midpoint is exposed and a test asserts the midpoint value appears nowhere in the object.

  **A defect shipped then self-caught by the built-artifact sweep:** `hybrid-band.json` imported into the client component — public-derived, so every leak test stayed green, but the build put 34 fleet entry names into a JS chunk fetchable at `/fleet` with no token. Band now travels server-side; regression test added. **Leak tests proven failable at BOTH layers** with byte-identity either side (API `38abae14…3476`, UI `40f9ba26…f033a`). Suite 100 files / 1874 green.

  **Digest still NOT armed, correctly:** `/health.fleet_lifecycle` = `{"detectors":{},"status":"never_run","transition_log_size":0}`, which lands in the renderer branch emitting "no detector has ever reported — this digest cannot distinguish a quiet week from a dead pipeline". The prerequisite is a first real detector run, not a second dry run. **Filed B-046.**

  **Live-verified defect found outside scope, left unfixed by gate discipline (B-045):** the calculator's full tier cannot authenticate from a browser — preflight omits `Authorization`, so the bearer token never leaves the browser and users see "Could not reach the engine". Endpoint tests pass because they call the worker directly (B2 exactly: the gate measured the wrong layer). One word in the shared CORS constant; non-additive, so out of this batch's gates. **Highest-priority hygiene item — it is the gated product's front door.**

  Caveat recorded: `run-portfolio.mjs` overwrote the gitignored `tools/consultancy/output/` set, so local Prosperus deliverables now reflect 1-Aug data rather than 29-Jul; internal consistency repaired by regenerating the whole runner chain at one vintage, but the vintage moved.

- ~~**Phase 37 batch-1 — fleet verification engine + corrective pass + lifecycle (37.A + 37.A.1 + 37.B)**~~ **[SHIPPED 2026-07-31, semi-autonomous (1 checkpoint)]** — branch `phase-37-batch-1` (head `e69e356`) · main `e4218b2`. Worker additive-only (132 insertions, 0 deletions), no deploy yet. Suite 1816 green.

  **Privacy held and PROVEN, not asserted:** `docs/_private/` unstaged/untracked/ignored, enforced by `scripts/assert-no-private-staged.sh` as a commit gate; zero private paths, zero contact/comment/APVA values across all commits, fixtures synthetic (`example.invalid`). Leak tests demonstrated failable — a real leak injected into `/s2/fleet` turned both red, removal turned them green, worker byte-identical after. **CC caught its own sweep being VACUOUS** (empty KV → routes answered `{"fleet":null}`, so "no secrets in this response" was true and meaningless); it now seeds a fleet and carries a vacuity guard. That self-audit is the B10 class caught unprompted.

  **Three probes corrected, one rule promoted.** Lursoft was marked Viable on a 200 containing "SIA" and a reg number — the control (Latvenergo) returns the same page as a nonsense string, so the run's 0/36 measured a broken probe, not 36 Latvian SPVs without registry trace. **Playbook B11 added: a source is viable only when a known-good control and a nonsense control return DIFFERENT responses.** Applied to esinvesticijos, the 84/84 zeros survived (probe validated: kaupimo 57 · Ignitis 3 · saulės elektrinė 221 vs nonsense 0) — the zeros are real but the register is likely the wrong one (Modernisation Fund ≠ structural funds; MF coverage thin at 1-6 hits). **APVA verdict: NOT CITABLE today**, `apva_flag` stays private/opaque/unscored, no scheme guessed; TAM (EU aid >€100k publication duty) recorded NOT EVALUATED — client-rendered behind a language gate — and named as the unblocker (**B-044**).

  **Latvia went 0 → 36/42 (85.7 %)** on the free daily Uzņēmumu reģistrs bulk open data (`register.csv` + `register_name_history.csv`), controls first; the 6 unconfirmed are descriptor rows naming no entity. It paid for itself immediately: the UR export writes `closed` as a single space, and an untrimmed check marked **all 486,509 entities terminated, Latvenergo included** — a decay path that would have retired the entire Latvian fleet while satisfying every rule (evidence present, citations resolvable, nothing deleted, all gates green). 37.B's meta-monitoring is built around that case; the name-history file is what stops a renamed live project being retired.

  **Hybrid finding — band, not correction.** Public fleet counts hybrid grid connections as batteries: pure-BESS rows track the battery (40/45), hybrids track the site connection (40/47); matched hybrids showed private BESS 1320 MW vs 4572 MW carried publicly (3.46×, +3251 MW). Because the correction moves the flattering way (supply down → sd_ratio down → cannibalisation down → **IRR up**) and rests on private testimony, it ships as `hybrid-band.json` (lower 11,975.7 / upper 16,020.4 MW, width 4,044.7), private magnitudes asserted absent by test, and the artifact declares its own incompleteness (24 of 45 hybrids carry a public technology signal → the band understates uncertainty). Named unblocker: public hybrid decomposition (VERT permits, Litgrid/Elering queues).

  **37.B lifecycle:** rules-as-data, 7 signals each with action/confidence/rationale/written B8 answer; exactly one may retire (test-enforced on `confidence: high`); rename guard cancels decay signals; unhealthy detectors suppressed-not-obeyed with suppression written to the log. Weekly digest deliberately NOT cron-armed — B10's corollary (proof run before first scheduled firing); manual trigger wired, arming left to the operator. Match rates LT 84/84 · LV 36/42 · EE 2/15; two matcher defects found by auditing output (39 tests green while `bareName` stripped only leading legal forms and 2-char tokens were filtered → Anykščiai PV matched Anykščiai BS), and the first golden case did NOT isolate the token defect — mutation testing found that, and both defects are now independently mutation-proven. **Silent-push root cause found:** local main was 5 commits behind origin while carrying `dabb549` on an older base (non-fast-forward); resolved by merging origin in an isolated worktree, not rebasing, so `dabb549`'s SHA survives unrewritten.

- ~~**B-036 + Phase 36.E0.1 — settled DE activation-price source + evidence-base refresh automation**~~ **[SHIPPED 2026-07-31, autonomous batch]** — branch `phase-36-b036-activation-source` (head `ac9f5e8`) merged as PR #118, first refresh delta merged as PR #117. No engine, no worker, no public site.

  **B-036 verdict — E0's "A84 empty for DE" premise was wrong (the 27th prompt-premise correction, this one against a premise E0 itself established).** netztransparenz publishes no settled activation-price series of its own, but its AEP-Module page names the channel that does: ENTSO-E `documentType=A84`, published **per TSO control area, not against the DE-LU bidding zone E0 probed**. 286,135 rows / 5.3 MB acquired under a new `price_basis: vwap_activated`, cross-checked against the German TSOs' own AEP-Modul-1 CSV (78 of 92 quarter-hours matching to the cent) and hand-checked from raw bytes bypassing the loader.

  **What E2/E3 calibrate activation on:** DE settled aFRR from 2022-06-21 with per-direction activated-ISP counts, plus AT as a second-market comparator. **What they cannot do: measure the accession effect.** Germany's series starts at its accession; Austria — acquired believing it supplied the before/after — starts three years after its own under the standard-product definition. The only pre-accession sample in reach is 49 Austrian mFRR quarter-hours against 5,331. **Any activation-break magnitude must come from the capacity evidence and be labelled as such** — carried into E2/E3 as a hard constraint.

  **E0.1 — monthly evidence refresh live** (`.github/workflows/refresh-mature-markets.yml`, first Sunday 03:00 UTC, `ENTSOE_API_KEY` repo secret never echoed, no `pull_request` trigger so forks can never hold it). Opens a PR with the per-source delta; operator merges. Six gates in-workflow; per-source `last_successful_refresh`; append-only asserts (a source restating history is an anomaly routed to review, not silently accepted).

  **The automation paid for itself before its first scheduled firing.** The proof run against Sweden destroyed 131,472 rows *while every checksum passed* — a windowed refresh overwrote `coverage_verification`, deleting E0's evidence of how Sweden lies and leaving prices correct with the reason to trust them gone. Four distinct bugs; the boundary defect proved general (3 of 4 network sources spill the calendar year; reconciliation carried 478,521 rows forward). **Playbook rows B9 + B10 added** (interim reports are hypotheses; integrity gates must assert on provenance metadata, not only values). Two of the phase's corrections were to CC's own mid-phase reports to the operator — recorded as corrections in docs and commit messages, with the test that asserted Austria's before/after inverted to assert its absence; the dangerous one was `standard_MarketProduct=A01` (dropping it would have admitted ~190 fabricated "activated at €0" quarter-hours per day per product). **B-043 filed:** AU/DA fetchers rewrite their own fixtures each run — churn in every refresh PR.

  **Open loose end:** source reachability from GitHub Actions runner IPs is unverified until the first scheduled firing or a post-merge `workflow_dispatch` — `fetch-btd.yml`'s precedent (BTD blocks Actions) makes this worth firing deliberately rather than assuming.

- ~~**Phase 36.E0 — mature-market evidence base**~~ **[SHIPPED 2026-07-30, semi-autonomous (1 checkpoint)]** — branch `phase-36-e0-evidence-base` (4 commits, head `9adfb00`) merged to main. Four markets committed + normalised + loader-tested (`tools/consultancy/data/mature-markets/`: de, se, gb, au + da, fx, calendar, summary-table.json). No engine changes, no deploy.

  **The evidence contradicted two arc premises (the phase existed for exactly this):** (1) German FCR did NOT collapse-to-floor — it ROSE 7.60 → 16.09 €/MW/h over the served window; **E1's skeleton needs a supply-tightness term, not two-regime decay.** (2) "mFRR saturates last because demand is deep" dropped; mechanism retained (deepest marginal provider sets the floor — usually NOT a battery for mFRR); Baltic ordering argued from Baltic supply composition. **Baltic structural calendar re-dated from TBD-verify to pinned:** MARI 2024-10-10 (all three TSOs) · PICASSO Litgrid 2025-03-05 · PICASSO Elering+AST 2025-04-11 — **past, partly inside the observation window** (no future-compression stacking on post-accession prices). Dataset corrections: regelleistung serves 2018-07-12+ (not 2011); Fingrid key-gated (B-037, declined for now → Sweden stands alone for hydro-floor, n=1 stated); AU = E4 spread evidence only (72 MB native 5-min, unreduced).

  **Filed:** B-036 (P2, GATES E2/E3 — no settled German activation prices held; netztransparenz.de scoping is the next CC job, ENTSO-E A84+A16 fallback verified serving AT/FI, empty DE) · B-037 (Fingrid key) · **B-038 (ENTSO-E A15/B95/A01 DOES serve LT procured balancing capacity, controls passing — corrects 36.C's "serves nobody"; genuine source redundancy exists, parameter shape recorded)** · B-039 (no installed-storage-MW series → E4 compression-coefficient gap) · B-040 (E5 has no evidence base). Honesty flag carried: saturation months for low-spread series (med/p10 < ~2×) are band-constant-set, quoted with bands not as facts — table exposes med/p10 + three-way tolerance sensitivity.

- ~~**Phase 36.D — Litgrid/tri-TSO demand-forecast integration**~~ **[SHIPPED 2026-07-30, semi-autonomous (2 checkpoints)]** — branch `phase-36-d-litgrid-forecast` merged to main. Verified stable across 4 cron ticks post-deploy.

  **The demand side went from an undocumented scalar (935, bare literal, no archaeology) to the Baltic TSOs' own published procurement forecast** — year-indexed, component-mapped, reconciliation-tested, tripwire-watched (3 armed: baltic-fcr, baltic-frr, fna). Basis: tri-TSO LFC-block FRR+FCR dimensioning series (752 → 922 MW to 2035, 2.29 % CAGR — the mystery 752 was fully sourced all along; nobody wrote it down). FNA (Litgrid flexibility assessment, 153 pp, archived with checksums) supplies component structure: FCR addressable · GAGAP+LT-PL+IZDR as SUPPLY ABSORPTION (not demand — the correctness-critical treatment) · short-term+DSO excluded (would double-count the dimensioning series). Step-not-linear interpolation (legal reservations are binary); component-trend extrapolation with FCR flat-after-2035 exception (trend implied Baltic reference-incident share tripling — physically implausible, CP-2 amendment). Energy Cells' 200 MW removed from competing supply year-indexed (EEĮ 48(1)(3) legal bar; returns 2033 with GAGAP +200 — net-zero identity asserted as permanent test).

  **CP-1 caught the p.10-vs-p.146 trap:** the LinkedIn "4.36→7.13 GW" was the TOTAL requirement; 973 MW is the UNCOVERED residual (table 43). Adopting the headline would have put LT S/D at 0.26-0.42 → CPI 1.86 vs 0.31 — a ~5× inflation in the flattering direction. **CP-2 caught the estimate being wrong by sign:** CP-1's ≈ −7 % single-axis estimate vs actual −0.98 % median (two approved decisions oppose: absorption recovers depth, per-year demand lifts mFRR acceptance); client portfolio moved UP (+13 % NPV, decomposed at Pause C per amendment). **Prompt risk premise also false:** CPI floor does NOT bound the blast radius — marketDepthFactor is unfloored (the −7 % channel).

  **Six defects, five pre-existing, three live-public:** S/D tooltip prescribing arithmetic yielding 8.99× beside a displayed 2.55× (three surfaces, now one canonical self-asserting caption) · hero ticker hardcoding "FCR 28+aFRR 120+mFRR 604" (rule #2) · reconcile.mjs scoring null-vs-null as PASS (whole check bank; now not_evaluable) · placeholder gate structurally unable to fire (prefix-vs-units) · syncLitgridFleet cosmetic-KV feedback (published S/D oscillated 3.17↔2.55 on cron order) · NUL byte making a source file unreviewable. Named "Litgrid L TrSc basis" scenario ships (+22.5 % gross/+43 % IRR — TSO's own projection, attribution unmissable by design). Cost of honesty this round: gross Y1 −0.98 % median, IRR −0.37pp, client NPV −1.78 %, two configs flip NPV-negative. 1622 tests (+52, sharpened not relaxed). Follow-ups routed: B-034 (tests grading untracked artifacts), B-035 (/s4/migrate-fleet no recompute), LV/EE flexibility assessments, Litgrid Q4-2026 flexibility-market plan watch, LT fleet tiering. CC-error self-flags banked: git-stash baselines (sign-wrong +12.9 %), tripwire URLs pinned unfetched, laptop-suspend clock (iteration counts aren't a clock — trust worker stamps).

- ~~**Phase 36.C — reserve-price path redundancy + S2 revival + forecast mode**~~ **[SHIPPED 2026-07-29, semi-autonomous (1 checkpoint)]** — branch `phase-36-c-reserve-fallback` merged to main. Worker deploy `98f721cd`.

  **Pause-A premise correction (the arc's biggest):** BTD was never dead — it recovered 2026-07-18 with a reissued cert (AST-hosted, Riga). BOTH KKME paths broke on OUR side: worker-direct gets a persistent CF 526 (chain-issue hypothesis, AST emailed); Mac cron got zero TLS bytes from origin AND its log redirect died when batch-4's hygiene commit removed `logs/` (cron opens the redirect before node starts — DECISIONS rule banked: grep crontabs for path deps before removing directories). Source audit killed the replacement thesis: Litgrid direct = 17 months of empty cells, Elering API = zero reserve procurement paths, AST = WAF-blocked, ENTSO-E balancing = serves nobody (positive controls NL/DE/BE/FR/AT all empty — filed honestly as 36.E-ENTSOE candidate, unresolved not guessed).

  **Shipped — path redundancy, not source redundancy:** VPS primary (Python port, validation gate inherited), worker-direct as self-healing secondary, ONE admission path keyed on `data_window_end` (freshness wins, source rank breaks ties), Mac cron retired, dead Litgrid scraper deleted. **S2: 286.9h stale → 0.9h.** Backfill premise also corrected: BTD serves complete days to 2025-10-01 — **299 days of full 96-ISP clearing history stored** (`s2_daily_clearing`, deliberately separate from the rolling-mean `s2_btd_history` — no in-place reinterpretation). Cert-lapse tripwire live on 4 upstreams (<7-day notAfter alert — the 12-silent-day class now gives a week's warning).

  **Forecast mode serves for the first time ever** — required 3 fixes beyond plumbing: a THIRD da_tomorrow writer the audit missed (the one that actually populates KV); NordPool endpoint now returns HTML (dead) — mirrored from computeS1's ENTSO-E data one key away; `synthesizeBTDFromRolling` PT15M indexing bug (read first 6 hours across 96 ISPs — latent until this phase activated the path; 2h activation 0 → €17/MW/day).

  **Honest residuals:** capacity_eur_day 0 (routes to 33.B.2) · peak_offpeak naming (public-number move, parked) · IRR 22.46 → 22.35 unattributed (two same-day causes: 36.B deploys + REAL market compression fcr 56.33 → 41.25 over the dark window; pre-refresh baseline not captured — run-registry prevents recurrence) · **Litgrid source-pin discrepancy: LinkedIn "973 MW / 3.12 GW" vs assessment public summary "4.36 → 7.13 GW" — exactly the 36.D CP-1 scenario, now known-input.** 1570 tests (+26, fixture-pinned on real BTD response).

- ~~**Phase 36.B batch-3 + batch-4 — measured-value cutover · B4 overlay · B0-H parser · B5 loop/dur_h · B6 governance — ARC CLOSED**~~ **[SHIPPED 2026-07-28/29]** — batch-3 (`5266d3f`→merged, worker deploy `061d2fce` after a two-stale-deploy recovery: silent push failure → diverged main → deploy-reads-working-tree shipped old code twice; recovered via rebase + redeploy; verification lesson banked: `/revenue` KV values flip at the HOURLY cron tick, not on deploy) + batch-4 (`4d285bf`→merged, no deploy needed).

  **Public site moved (all attributable to named decisions):** IRR 24.16 % → 22.46 % · LCOS €69.7 → €87.4/MWh · trading_realisation 0.85 → **0.7234 measured** · cycles 678 → 498 EFC/yr (dual-corroborated; declared Modo-band breach). Client portfolio Central NPV −13.8 %; Downside NPV €7.6M → €2.2M (−71 % — the contracted-overlay conversation is the counter-lever, B4 built it: floor lifts the tail 4.6× more than the median).

  **Arc-closing six numbers:** simultaneity 75.2–85.5 % of unconstrained stack, year-dependent (B1; the earlier "84.0 %" was single-year) · P50 −3.4 % vs Central, P90 unresolvable at N=5 — presented from 11-yr sensitivity with regime caveat (B2) · realisation 0.7234 measured (B3) · floor tail-lift 4.6× median-lift (B4) · 222.35 EFC/yr closed-loop vs B1's independent 219.1 (B5) · every delivered number traceable to a reproducible run_id (B6).

  **B6 shipped:** run registry (11 runners, content-fingerprint run_ids, artefact stamping — its own first finding was a self-defect: suite wrote into the committed governance log, caught by count-reconciliation not tests) · governed register changelog (founding entries: both cutovers + cycling revision with evidence trails) · `docs/methodology-lender.md` 25 pp / 11 sections / 20 binding tests · delivery bundle regenerated on measured basis · `logs/btd.log` finally degitted.

  **The arc's honest residual, in its own words:** adopting the measured trading realisation raised the STILL-ASSUMED reserve stack to 71 % of gross — while BTD (its only measurement source) has been down since 07-17. **→ Phase 36.C (reserve-price fallback: Litgrid + Elering direct publications, BTD demoted to one-of-N; includes B0-G forecast plumbing) jumps the queue ahead of the portal.** Phase 37 (dispatch-card cutover to hourly engine — moves public IRR materially UP) parked deliberately: sequencing optics after an honest-downward arc. 9 open items routed in Session 93 handover. Suite 1536/1536.

- ~~**Phase 36.B batch-2 — dispatch-card fix (DEPLOYED) + 36.B2 percentiles + 36.B3 measured realisation**~~ **[SHIPPED 2026-07-28, autonomous + 2 deploy touchpoints]** — branch `phase-36-b-batch-2` (head `5266d3f`) merged to main. Worker deploy `eeb4d5da` (dispatch-card corrections live).

  **Part 0 — dispatch card corrected, two compounding defects:** (1) RTE error had a compensating twin (RTE misapplied as discharge-power cap) — measured deltas −8.5 %/−1.3 % (2024 2h/4h), NOT the hypothesised −40 % (correction #22-class: quantified beats assumed, in both directions); (2) **the dominant defect was resolution**: `slice(0,24)` read 192-point PT15M payloads as hourly — the card had been dispatching against the first 6 hours of the day stretched to 24 since 2025-10-01. Fixed resolution-aware (exact-divisor detection + `Math.round(h*N/24)` bucketing per 31.A.2 precedent; DST ragged lengths test-pinned; no dates in logic per rule #2). Live-day delta: daily +14.2 %/+7.2 %, arbitrage un-clamped (was hiding −169/−105 €/MW/day losses), split now sums to ~100 %. Permanent mirror gate: card sits 7-14 % BELOW the hourly engine on every shape, never above.

  **36.B2 — bootstrap percentiles:** primary 2021-25 (N=5; 2026 excluded on 57.5 % coverage evidence), sensitivity 2015-25 (N=11). Reference lifetime gross P50 €136.6M (−3.9 % vs Central ✓). **P90 NOT RESOLVED from N=5 — reported as `resolved: false` with reason, not fabricated.** Lender presentation policy (operator-set): P50+P75 from primary; P90 from 11-year sensitivity with regime caveat stated.

  **36.B3 — measurements replace assumptions:** `trading_realisation` **measured 0.7234** vs assumed 0.85 (349 traded days, monthly 0.654-0.815 — also below the register's own declared range, which was understated); `RYSTAD_15MIN_UPLIFT` **measured 0.0885** vs asserted 0.14 (273 native PT15M days). Leakage checks pass (max day 0.997, r=−0.093). **Operator decision 2026-07-28: ADOPT BOTH** — cutover scheduled as batch-3 Part 0 with client-impact rerun. CC correctly declined to force the cutover mid-measurement-phase (register invariants turned 4 tests red — governance working as designed).

  **Defects logged for batch-3:** 36.B0-G `mode=forecast` structurally dead (reads fields no writer ever writes — has never served a forecast; folds into BTD-fallback work) · 36.B0-H `extractPrices` regex drops negative prices entirely, shifting indices — live on solar days, shared across routes, prerequisite for B5's negative-price rule. **BTD still down (11+ days, host-level) — fallback-source phase triggers 2026-08-01 if not recovered.** One declared batch-rule deviation (computeDayCapture export for denominator comparability) paid for with route probe 54/54. Suite 1395/1395. — branch `phase-36-b1-hourly-dispatch` (3 commits, head `9d10de9`) merged to main. Arc: `phase-36-b-arc.md` (bankable backend). Not deployed — nothing public changed; `/revenue` byte-identical at engine AND route layer (Session-88 lesson applied).

  **Corrections #14-21 at Pause A** — the three that mattered: (1) ENTSO-E token already existed as worker secret — B2 was never blocked; (2) hourly history is 11+ years (2015→now, LT/LV/EE), not 2-3 — the arc's percentile-extrapolation honesty constraint largely dissolves; (3) **computeDispatchV2 (live dispatch card) has an RTE arithmetic error** — discharge cap applied but round-trip loss never charged on the cycle, then negative days clamped to zero.

  **Shipped:** `tools/consultancy/lib/dispatch.mjs` Node-side (decided on measurement: 3.5 ms for 8760×20yr — runtime constrains nothing; Node placement makes byte-identity true by construction). Conservative-greedy policy, SoC continuity, reserve-energy reservation via canonical `RESERVE_PRODUCTS[p].dur_req_h`, cycle-budget throttle, negative-price rule, POI limits. 5-year real-price validation runs (2021-2025). **Headline measured finding: 84.0 % of the revenue stack is simultaneously achievable** — isolated by paired runs (SoC reservation on/off), a constraint measurement not a price artifact. Energy balance closes at 4.3×10⁻¹⁶ relative; zero constraint violations across 8,784 hours every year; 1,339 tests.

  **D-decisions (operator-approved at checkpoint):** D1 hourly-averaged for gates, 15-min delta measured at B3 vs the hardcoded 0.14 uplift · D2 quantify-don't-fix · D3 B3 split (measured DA realisation; reserve realisation stays assumed with stated boundary) · D4 bootstrap 2021-2026 primary, 2015-2026 sensitivity.

  **D2 quantified: the dispatch card overstates arbitrage 40.3 %** (2024 full-year isolation; 30 of 366 days are actually losses that the clamp hides). One-function one-commit fix — **scheduled as micro-fix before batch B-2.** Second inconsistency found: engine charges cost on full-nameplate DA throughput (`:1287`) while billing revenue × trading_fraction (`:3178`) — partially self-cancelling, routed to B5.

  **Gate #3 (cycle count ±10 %) failed honestly** — documented deviation: the hourly sim reallocates cycles (aFRR slightly up, DA sharply down vs the throughput model); substitute proportionality gate holds within ~2pp on all five years. Three CC-made defects caught by the arc's own gates before ship (activation-energy 4.6×, backtest-window 4×, charging-cost misattribution) — the gate architecture works.

  **B2/B3 state:** 11-year hourly DA committed (~648 KB, 100 % coverage), 15-min 2025 file carries the resolution split. BTD outage (host down since ~07-17, 17 failed cron runs — external, self-heals; fallback-source phase if not back by 08-01).

- ~~**Phase 35.1 + 35.2 — BESS Revenue Calculator**~~ **[SHIPPED 2026-07-28, autonomous batch, LIVE]** — branch `phase-35-batch-1` fast-forwarded to main (`c0526df`). Worker deploy `743ac89d` (CALC_SECRET set). **Live + Cowork-verified:** kkme.eu/calculator 200 · sample tier computes with leak-check CLEAN (no full-tier keys reachable unauthenticated) · wrong-password 401 · `/revenue` regression untouched. Tiered `POST /calculate` (sample: 5 KPIs + 8-line bridge, 10/day/IP rate-limit; full: complete Phase-34 analysis incl. 3 client scenarios ported into the worker with exact overlay parity), `POST /calculator/login` HMAC token vs dedicated CALC_SECRET. **Soft-launched: zero links anywhere** — operator decides when/whether to link. 1305 tests (+93).

  **Batch findings:** (1) **dur_h calibration gap** — engine calibrated at exactly 2h/4h; the 2h-3h middle band mixes 4h throughput with 2h RTE (branch conditions `<=2` vs `>=3` never meet) — latent, unreachable from `/revenue`, calculator clamps + discloses bias direction. **Follow-up candidate: Phase 35.3 duration-calibration continuity.** (2) **Gate-scope limitation documented** — the 54/54 byte-identity gate calls the engine directly; a route-layer refactor broke `/revenue` while the gate stayed green (caught by CC's route-level probe, fixed). Future gates assert at the outermost user-touched layer. (3) 13th premise correction was CC's own, caught by CC's own batch-2 driver-echo test — the discipline is self-correcting. (4) Build gate caught a horizontal-scroll layout break all tests missed (Phase 18.1.1 class).

  **Next candidates:** nav/footer link when operator wants the page discoverable · Phase 35.3 dur_h continuity · public-calculator SEO/announce push · v1.0 extended items if Prosperus converts.

- ~~**Phase 34.6 + 34.7 (batch-3) — ARC CLOSED**~~ **[SHIPPED 2026-07-28, autonomous batch]** — branch `phase-34-batch-3` (3 commits, head `c51d639`) fast-forwarded to main. Worker untouched (diff empty). Delivery bundle (gitignored, reproducible via `node tools/consultancy/build-all.mjs`): Excel 8-tab model with 44 editable overrides + scenario dropdown (exceljs, spike-verified round-trip) · 16pp branded PDF summary · 11pp methodology annex · README. Template anchoring premise overturned (12th rule-#1 class correction): find-replace unviable against several hundred prose-embedded placeholder numbers — split on two asserted-unique anchors, sections 01-10 fully regenerated from runner JSON, upsell verbatim; part 2 contains zero numeric literals which makes the consistency gate real. **4 defects the gates caught pre-delivery:** project-Y1 vs calendar-Y1 conflation in Bridge tab (€4.43M column error) · €k/MW-yr divided by nameplate not operational MW-years · placeholder-blocklist false-positive on live-value collision · 3 operator notes carrying stale figures (now derived). Delivery-build numbers (live-KV recompute): NPV €44.1M / MOIC 3.764 — figures computed at generation time by design. 1212 tests · reconciliation 73/73 + 59/60 (known WARN). **Phase 34 arc complete: engine parameterised, scenarios, sensitivity, register, reconciliation harness, Excel/PDF generators — all permanent platform capability. Next client engagement of this shape: ~1 day.** Phase 35 (BESS Revenue Calculator — productised tiered web tool) prompts staged.

- ~~**Phase 34.4 + 34.5 (batch-2)**~~ **[SHIPPED 2026-07-28, autonomous batch]** — branch `phase-34-batch-2` (3 commits, head `df6dfa0`) merged to main. Worker READ-ONLY held (`git diff main -- workers/` empty). **11th rule-#1 correction:** none of the 6 client scenario drivers is param-reachable (all module constants; `pipeline_realisation` is the engine's echo, not an input). CC rejected the prompt's post-hoc-adjustment fallback as structurally unsound and built `scenario-overlay.mjs` — exact-anchored constant substitution in worker source imported as separate module instance; scenario runs are the engine's own arithmetic. Central invariant EXACT vs batch-1 (zero differing fields; companion test asserts Downside differs, no vacuous pass). Scenarios: Downside NPV €7.6M / Central €43.3M / Upside €74.9M, monotone. Sensitivity top-3 by 20-yr swing: capacity prices €82.6M · fleet realisation €36.6M · optimiser fee €25.5M. **2 of 6 drivers move zero cash** (spread_growth clamps at trading_fraction 0.70; cpi_floor disclosure-only) — reported as zero, operator decision: keep + disclose as analytic finding. Register 44 rows (not 39 — category math), all binding-tested to live code. Reconciliation 73/73 internal, 59/60 external (1 WARN: Bitėnai Upside IRR 33.2% vs CH band — by-design, disclosed). 1150 tests. 3 batch-3 candidates logged (B-031…033, worker-side).
...
- ~~**Phase 33.A.2 (W1a) — Operator-curated operational-confirmation allowlist + MW reconciliation**~~ **[SHIPPED 2026-06-15]** — PR `phase-33-a-2-status-refresh-and-coverage` (commits `6cd05af` impl, `7617ff4` evidence+handover) merged to main (`c449599`). Worker deploy `7a285bf0-2dac-4152-a4e8-18d746da66b9`. 2 files / +90 worker + new 13-test `knownOperational.test.ts`.

  **Operator's 2026-06-15 sample of 5 known-operational projects empirically diagnosed all three failure modes** (status-refresh broken / LV source coverage thin / MW mismatches across sources). Cross-check against `/s4.projects` found 4 of 5 in the data flagged "announced" (Hertz 1, Auvere, Vilnius BESS, Tausolos) + 1 entirely missing (Utilitas Wind LV). 7 "operational" entries were all Litgrid Kaupikliai/Energy Cells from 2022 — no commercial commissioning since flagged operational.

  **The 6th consecutive rule-#1 correction (extends `feedback_prompt_premises_empirically_verified.md`):** prompt's A.4 modelled `sd_ratio = baltic_pipeline_mw / baltic_operational_mw = 14067/331 = 42.5`, projecting major IRR-down on status flips. CC's Pause A traced the actual engine: `sd_ratio = baltic_weighted / eff_demand` (`fetch-s1.js:231`), where each project carries STATUS_WEIGHT (operational=1.0, announced=0.1). Flips ADD weighted supply (0.1 → 1.0 per MW) — direction is UP not down (1.86 → 2.08 observed). And IRR impact ≈ zero because spot + projected per-product sd_ratios all pin at cpiCurve's 0.30 floor. **No operator IRR gate needed.** Display fleet-cpi 0.33 → 0.31 cosmetic, not cashflow.

  **W1a 4-seed allowlist shipped:** Hertz 1 (100MW EE), Vilnius BESS (E energija, 65MW LT Trakai), Vėjo galia (41MW LT), Tausolos (30MW LT Telšiai). Auvere held per Pause A (75 vs 26.5 MW gap needs source resolution). All 4 primary-source cited per rule #3 (TV3 + LRT cross-confirmed). Mechanism: `KNOWN_OPERATIONAL` constant + `applyKnownOperational()` + `normName()` (smart-quotes/diacritics/suffix tolerance) wired into POST `/s2/fleet` **before** `filterFleetEntries` (so the operational flip survives the C-01 hard-reject gate via an appended operational-evidence source token). Response now reports `flipped_operational` count.

  **W3 folded in via allowlist** — worker receives only one collapsed `capacity_mw` per project (kkme_sync.py pre-collapses multi-source MW upstream), so multi-source disagreement can't be surfaced worker-side independently. Instead: when the allowlist's curated MW differs from the feed's, attach `_mw_disagreement: {feed, operator}` flag to the entry. 3 disagreements flagged live: Hertz1 (114.9→100), Vilnius (72→65), Vėjo galia (50→41). Tausolos matched (30=30).

  **Two rule-#1/#3 corrections to operator-supplied data mid-phase:**
  - Vilnius BESS: operator's casual recall "60 MW / 130 MWh" → primary-source cited TV3 + LRT confirmed **65 MW / 130 MWh** (used 65 per rule #3 / ~95% authoritative override of casual recall).
  - Tausolos: operator's "30 MW / 130 MWh" → primary-source LRT confirmed **30 MW / 67.7 MWh** (MWh corrected; MW matched).
  - Vėjo galia: confirmed real entity at LRT, 41 MW / 107.3 MWh / COD Dec 2025 (Lithuania's first commercial BESS park).

  **Live production impact:** `/s4` 159 announced / 11 operational (was 163/7); `baltic_operational_mw` 331 → 567 (+236); `pipeline_mw` 14067 → 13800 (−267); `sd_ratio` 1.86 → 2.08; `/revenue` IRR 20% → 21% (+1pp, within ±5pp tolerance, floored-CPI prediction held). C-01-survival invariant held (0 dropped). 401 enforced.

  **Operator-source-credibility taxonomy reaffirmed:** primary-source-with-citation (LRT / TV3 / Elering official) > casual operator recall > inference. Phase 33.A.2 mechanically applied this when operator's recall conflicted with cited sources.

  **W1b dropped** — Litgrid ArcGIS auto-confirm would only re-confirm the 4 Energy Cells already in the curated seed (zero new data today). Filed as 33.A.2.c for when transmission-connected commissioning warrants it.
...
- ~~**Phase 33.A.2.e — Estonia operational status-refresh allowlist**~~ **[SHIPPED 2026-06-15]** — PR `phase-33-a-2-e-ee-coverage` (commits `dc6a782` impl, `c4d72d9` evidence+handover) merged to main (`c0e64aa`). Worker deploy `a535130a-a6e6-433e-a26a-96ce2f9e44dd`. 2 files / +110 lines worker + tests (89 tests new in `knownOperational.test.ts`, 6 new EE cases + 2 obsoleted 33.A.2 cases updated).

  **The 8th consecutive rule-#1 correction — qualitative first:** prompt asserted "Hertz 2 operational 100MW/200MWh" based on operator's 2026-06-15 primary-source-cited paste (yuso.com, ebrd.com, evecon.ee). CC's Pause A independent re-fetch found: Hertz 2 is **under construction, COD end-2026**. Two independent sources confirmed (corsicasole.com developer page + ess-news Feb 2026 article). The cited "200MW/400MWh" was the **combined** Baltic Storage Platform total, not Hertz 2's individual nameplate. First time the prompt's premise was wrong DESPITE operator citing primary sources — meaning cited-primary-sources themselves carry transcription/attribution risk and need independent Pause A re-fetch. Nuances the audit-credibility taxonomy ("primary-source WITH citation ≈ 95% reliable" still holds at source level; the 5% gap is mis-attribution in operator's pre-paste). Memory `feedback_prompt_premises_empirically_verified.md` updated with this data point + the citation-attribution risk.

  **3 EE allowlist additions shipped:**
  - **Enefit Auvere** (Ida-Virumaa) → operational; MW 75 → **26.5**, MWh → 53.1; COD 2025-02-01; energy-storage.news + ERR + Eesti Energia cited. The Auvere 75 in Elering register was the industrial-complex permit envelope, not the BESS; resolves the W1a-held mismatch.
  - **Rummu** (Harju County) → operational; MW 14 → **9**, MWh → 18; COD 2025-04; enery.energy cited. Path-b resolved: Elering's 14 MW "hübriidelektrijaam" is the hybrid grid-connection rating; BESS nameplate is 9 MW paired with separate 20 MW PV. Flip with MW correction.
  - **Hertz 2** (Aruküla) → `target_status: 'under_construction'` (NEW mechanism); MW 113.5 → **100**, MWh → 200; corsicasole.com + ess-news cited. Will re-verify + flip to operational when it energizes (~end-2026).

  **Mechanism extension:** `KNOWN_OPERATIONAL` entries gained an optional `target_status` field (defaults `'operational'`). `applyKnownOperational` now handles both operational flips AND pre-COD `under_construction` markers via single allowlist. C-01 evidence-token append only fires for operational targets (correct — under_construction shouldn't carry "operational-evidence" source token). Backward-compatible with all prior LT/LV entries unchanged.

  **Latent bug fix folded in (operator-approved):** `applyKnownOperational` was appending the C-01 evidence token on every POST round-trip without idempotency, causing Hertz 1's source field to accumulate `· operator-confirmed operational ×5`. Harmless but ugly. Fixed with idempotency guard + regression test.

  **`estonia_loader.py` audit (light):** Elering connection-queue scraper reads status `application/offer/contract/connected` — vocabulary tops out at "connected → connection_agreement," never "operational." Same structural finding as 33.A.2 LT W1a + 33.A.2.b LV: no upstream loader writes operational state. Curated allowlist is the load-bearing fix; no 33.A.2.e.1 loader-wireup needed.

  **Broader EE operational fleet research (A.1.5):** no other operational grid-scale EE BESS found beyond the 4 (Sunly Pikkori is 1.7 MW sub-grid-scale; Sunly Raba/Risti, Kirikmäe, Zirgu/Tsirguliina, the 800 MWh park = all pipeline/under-construction; Enefit Green / KSK / Roplant / Baltic Energy Partners = no energized BESS found). EE operational fleet is genuinely 3 grid-scale + Hertz 2 pre-COD.

  **Live impact:** EE operational 1 → 3 (Hertz 1 + Auvere + Rummu); EE under_construction +1 (Hertz 2); `baltic_operational_mw` 666 → 702 (+36 Auvere + Rummu); `sd_ratio` 2.10 → 2.22; `cpi` 0.312 → 0.303 (at floor — IRR-safe); IRR ≤ +1pp. 0 dropped. 978 worker tests.

  **Day-end position:** 7 ships today (33 → 33.A → 33.B → 33.B.3 → 33.A.2 W1a → 33.A.2.b → 33.A.2.e). 8 consecutive rule-#1 corrections banked. Revenue engine bounded + VPS hardened + map polluters purged + capacity-watch persisted + 9 operational BESS flipped across LT/LV/EE (4+4+3-1-from-pre-COD-Hertz-2-skip) with full primary-source citations.
...
- **Phase 33.A.2.d — Display-dedup pass** (~1-2h, P2) — Phase 33.A.2 surfaced that E energija Vilnius BESS now renders TWICE in S4Card: once as an operational pin in curated `storage_by_country.LT.assets` (65 MW), once as a flipped projects-feed entry (post-W1a). Engine uses only the projects feed so no double-count in sd_ratio (cashflow safe), but display dups are confusing. Reconcile: either drop the assets-layer duplicate when a matching projects-feed entry exists, or canonicalize on one source. Triggers more visibly as W1a allowlist expands to more curated overlapping entities.
