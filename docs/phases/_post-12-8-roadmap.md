...
- **Phase 31 — Tier-3 card data integrity + structural normalization** (NEW 2026-05-08; SCOPE EXPANDED 2026-05-08) — Operator-surfaced via "Structural market drivers" screenshot. Operator framing post-initial-filing: "the data is nonsense in some, they need a full revision/rebuild." Two distinct workstreams now folded into this phase:

  **A. Data integrity (~1 day CC; ship FIRST)** — Code-grep each of the six tier-3 cards (RenewableMix, ResidualLoad, PeakForecast, SpreadCapture, S7 TTF Gas, S9 EU ETS Carbon) to verify: (i) hero metric reconciles with sub-row labels and source field; (ii) interpretation copy isn't inverted vs threshold logic; (iii) cross-card consistency per discipline rule #4 (same metric in N locations derives from one canonical worker field). **Visible-from-screenshot bugs to triage at Pause A** (visual inference per rule #1 — verify code-side before scoping fix): (a) **PeakForecast** — hero claims €142/MWh "Today's DA swing" but peak h20 €154.3 − trough h12 €99.9 = €54.4. Hero is ~3× the actual swing; either field-mapping wrong or hero is showing price-level mislabeled as swing; (b) **TTF Gas** — hero 44.4 €/MWh + threshold pill "0.89× / 50 €/MWh" (i.e., below-threshold) + interpretation "Above-normal gas — supporting wider peak spreads" (claims above-normal). Threshold-direction logic appears inverted in copy generator; (c) **RenewableMix** — 143% headline + "Wind 52% · Solar 91% · Thermal 0%" sub-bar — sub-bar values' semantic meaning unclear (capacity factor? share?), and don't reconcile to 143% aggregate. Possibly fine if "% of load" is the headline definition but needs explicit unit disambiguation; (d) **PeakTrough** — "Buy €15 · Sell €153" (€138 spread) vs hero €135.5; rounding from precise underlying value but displayed numbers don't add up exactly which erodes trust; cross-border `€-6.9/MWh LT-SE4` semantically unclear (spread? net? average?). Math-reconciles cards: ResidualLoad ✓, EU ETS Carbon ✓.

  **B. Structural normalization (~3-4h CC tight / ~6-8h wide; ship AFTER data integrity)** — Visible inconsistencies: (a) two distinct templates in the grid — cards 1-4 follow canonical anatomy; S7 TTF + S9 EU ETS use a different template (threshold-pill at top + green "observed" badge + hero number BELOW the pill + horizontal scale bar at bottom); (b) three status indicator patterns (filled dot, hollow circle, green "observed" pill) for the same freshness concept; (c) hero metric type-size drift — TTF/Carbon numbers visually larger than rest, possibly a token-application slip; (d) PeakForecast has no viz where every other card has chart/bar/scale; (e) SpreadCapture/PeakTrough has TWO charts stacked while every other card has one or zero; (f) footer affordance gap — TTF/Carbon offer "▸ View [thing] detail" drilldown links, others don't; (g) card heights misalign within rows; (h) color usage incoherent across cards (green progress / teal sparklines / multi-color scale gradient / triangles). **Tight scope:** pick canonical template per CLAUDE.md card anatomy ("header → hero metric → status → interpretation → viz → impact line → source footer → drawer"), apply to all 6, drop the second template, single status pattern, hero type token-aligned, single chart-or-no-chart rule, one footer affordance. **Wide scope:** same plus extract a `<MetricCard>` primitive so all 6 share the same JSX skeleton, configurable via props (closes part of the Phase 7.7g-b primitives gap). **Discipline rule #1 caveat for B:** visual-inference audit. TTF/Carbon may legitimately deserve a distinct subtype (commodity overlays vs market-state cards have different content); design call to make at Pause A.

  **Sequencing:** A first, B after. Visual polish on top of nonsense numbers is wasted work; if A's rebuild changes which fields each card consumes, B has to redo. Total estimate: ~1.5-2 days CC for combined ship; or split across two CC sessions (A as Phase 31.A, B as Phase 31.B). Prompt(s) to be authored when picked.
