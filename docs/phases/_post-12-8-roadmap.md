...
6. **Chrome MCP lock investigation** [NEW 2026-05-07; ~30-60min Cowork] — chrome-devtools-mcp browser-already-running lock has now recurred 3 consecutive CC sessions (47 + 48 + 49) under parallel-CC condition; CC has been falling back to curl-only local-build verification (per `feedback_local_build_verification.md`). Investigate: test `--isolated userDataDir` flag for chrome-devtools-mcp (avoids profile lock), OR codify browser-ownership coordination protocol (e.g., named workspace per session). Without fix, chrome MCP visual verification gate is permanently degraded to curl-only. Cowork-side investigation; small phase.
...
**Why:** Operator hard-refresh feedback 2026-05-07 — "I don't understand this one at all? what is mwh/h? why I can only see once a month? what does this even mean?" Energy-expert audit followed (memory: `project_s2_audit_phase_21.md`): S2 Balancing card is methodologically honest but operationally thin. Three specific gaps to close for "professional but not institutional" target:

1. Headline €13.5/MW/h up+down combined overstates opportunity ~70% to non-expert readers (real BESS bid one-direction); operational reality (€7.91/MW/h up only) is buried in subline.
2. Snapshot-only S2 has no directional signal — investor reads €X today, doesn't know if market is recovering, stable, or deteriorating post-Feb-2025 Continental synchronization regime change.
3. IMB stats sit decoratively without interpretation; "AFRR · THICK" chip likely violates discipline rule #6 (editorial state-name).

**Scope (~3-4h, single PR, three pause points):**

**(a) Hero inversion** (~30-45 min, frontend-only) — lead with operational reality (up-only €7.91/MW/h) as primary hero number; up+down combined €13.5/MW/h becomes methodology subline. Auto-corrects €5.9M/yr footer translation → €3.46M/yr (the more honest annualized number for one-direction BESS at 50 MW). Single-file display change in `S2Card.tsx`. Math is already correct — this is reframing, not recalculation.

**(b) 90d delta signal** (~1-2h, includes worker deploy) — quantitative micro-descriptor chip near hero showing direction: `Δ −12% / 90d` style. Worker-side derived field `s2.afrr_up_avg_90d_delta`. Per rule #6 (no editorial chips). Tells regime story without chart redesign.

**(c) IMB interpretation + THICK chip fix** (~45-60 min, frontend-only) — one-line interpretive connector below IMB stats tying to revenue logic; verify and fix `AFRR · THICK` chip per rule #6 (replace with quantitative micro-descriptor like `Δ €1.5 spread` or remove if not load-bearing).

**Pause points:** Pause A (audit current S2Card.tsx state + verify THICK chip semantics + confirm 90d delta computation approach), Pause B (foundation gates + local-build smoke-test per `feedback_local_build_verification.md`), Pause C (visual + commits).

**Sequencing:** any time after Phase 18.2.3 ships. Operator-pickable from Tier 1 queue.

**Estimate:** ~3-4h CC. Single PR. Worker deploy needed for sub-item (b) only.

**Prompt:** authored at scope-time (`docs/phases/phase-21-prompt.md`).

**Discipline rules engaged:** #1 audit-triage (verify THICK chip semantics + check for similar editorial chips elsewhere), #4 cross-card consistency (hero metric naming convention should propagate if other cards have similar misleading-headline issues), #6 no-editorial-state-label (THICK chip fix).

**Acceptance:** S2 hero leads with up-only operational reality (€/MW/h up); 90d delta chip shows direction; IMB stats have one-line interpretive connector tying to revenue logic; THICK chip is quantitative or removed.

**Out of scope:** forward curves, P95/P99 tail risk lines, cross-region benchmarks (Baltic vs Continental aFRR), cross-product side-by-side, cross-country side-by-side, multi-period decomposition, scenario analysis, card layout restructuring. Deferred to Phase 22+ if operator later wants institutional-grade S2.

**Audit method reusable:** the energy-expert audit protocol applied to S2 (code-grep state → plain-language explain → multi-audience expert critique → ranked small improvements) is reusable for S4 / S7 / S9 / RevenueCard / etc. when operator surfaces "I don't understand this card" feedback. See `project_s2_audit_phase_21.md`.
