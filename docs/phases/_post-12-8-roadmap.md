...
- **Phase 19.2.2 — `freshnessBadge.test.tsx` Vilnius-midnight flake fix** (NEW 2026-05-18 from 19.2.1 Pause C gate run; ~30-60 min CC, test-only) — `app/lib/__tests__/freshnessBadge.test.tsx:39` ("TODAY" band) intermittently fails when test execution time crosses Vilnius midnight relative to the test's assumed boundary. Mechanism per CC's investigation: `freshness.ts:117` falls through to `STALE` because `isSameVilniusDay` returns false at execution. Carrying as known flake since Phase 21 (EET-day-aware freshness logic introduction). Phase 19.2.1 hit it again — Pause B vitest = 918/919 with this single test failing. Fix shape: either (a) freeze test clock via `vi.useFakeTimers()` + `vi.setSystemTime()` to a deterministic mid-Vilnius-day timestamp, or (b) restructure assertion to be Vilnius-day-aware on the test side. Operator picks at Pause A. Worker / engine code untouched.

- **Phase 18.2.1 reaffirmed as next a11y stop** (note 2026-05-18) — The 21 axe-violation remainder after 19.2.1 is now entirely accent + bg-color failures (`--teal` / `--rose` / `--amber` / `--accent-amber` on cream + 2 pipeline-on-colored-bg labels). Phase 18.2.1 (Baltic palette retune) covers this surface comprehensively. When 18.2.1 ships, expect axe-core baseline to land at ~2 (the documented exceptions: white-on-mint chip + 3:1-boundary decorator).
