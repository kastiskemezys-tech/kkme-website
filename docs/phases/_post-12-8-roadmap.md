...
- **Phase 31.A.3 — `lt_hourly_24` downsampler robustness fix** (NEW 2026-05-17 from 31.A.2 Pause A discovery; ~30-60 min CC, worker-only) — `workers/fetch-s1.js:3367` integer-perHour gate (`Number.isInteger(perHour)`) makes `lt_hourly_24` return null on any N not divisible by 24 (observed live on N=95 today; would also fail on DST-trim 92/100 cases). Same resolution-bug family as Phase 31.A and 31.A.2. Fix shape: generalize the downsampler to hour-bucket aggregation matching `priceShapeMetrics`'s `floor(i / intervalsPerHour)` pattern (or the symmetric `Math.round(h * N / 24)` arithmetic 31.A.2 just shipped); avoid integer-divisibility precondition entirely. Verify by curl on next non-96-entry day. Frontend consumer check needed at Pause A — `lt_hourly_24` is registered in `metricRegistry.ts` from 31.A and consumed by at least the SpreadCapture sparkline.
