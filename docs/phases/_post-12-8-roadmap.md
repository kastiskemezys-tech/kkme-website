...
- **Phase 31.A.1 — Tier-3 editorial-copy sweep per rule #6** (NEW 2026-05-13 from 31.A discovery; ~1-2h CC) — `interpretation()` band-derived editorial copy still surfaces in `RenewableMixCard.tsx:23-27` + `ResidualLoadCard.tsx:26-31` + `SpreadCaptureCard.tsx:35-40` + `PeakForecastCard.tsx:41-47`. Examples: "High renewable penetration — midday surplus likely", "Tight system — thermal at high utilization, elevated evening prices likely", "Wide envelope — supports 2+ profitable cycles", "Above-average spread — favorable for 2-cycle dispatch". Apply the S7/S9 data-derived rewrite pattern shipped in 31.A: replace band-keyed prose with computed strings using ratio + threshold + observed value. Rule #6 mass-violation; tier-3 cards are the highest-density display surface so the cleanup pays double. Prompt to be authored when picked.

- **Phase 31.A.2 — Evening-premium resolution-aware slicing fix** (NEW 2026-05-13 from 31.A discovery; ~30-60 min CC, worker-only) — `workers/fetch-s1.js:3346-3349` computes `lt_evening_premium` via `ltPrices.slice(17, 22)` ("h17-21 evening") and `ltPrices.slice(10, 15)` ("h10-14 shoulder"). These slices assume hourly indexing but on 96-entry 15-min ISP arrays they pick up quarter-hours 17-21 = local 04:15-05:30 and 10-14 = local 02:30-03:30. Same root-cause family as the PeakForecast/SpreadCapture bugs closed by 31.A; pre-existing tech debt CC surfaced during 31.A discovery and explicitly out-of-scope. Fix shape: `const perHour = ltPrices.length === 96 ? 4 : 1; const ltEvening = ltPrices.slice(17*perHour, 22*perHour); const ltShoulder = ltPrices.slice(10*perHour, 15*perHour);`. Verify with live curl on 15-min ISP day post-fix. Same `lt_hourly_24` resolution-aware downsample pattern from 31.A could be applied if a third consumer surfaces.

- **Phase 31 — Tier-3 card structural normalization (formerly 31.B)** (filed 2026-05-08; UNBLOCKED 2026-05-13 by 31.A ship; ~3-4h CC tight / ~6-8h wide) — Visible inconsistencies from operator screenshot: (a) two distinct templates in the grid — cards 1-4 follow canonical anatomy; S7 TTF + S9 EU ETS use a different template (threshold-pill at top + observed badge + hero number BELOW the pill + horizontal scale bar at bottom); (b) three status indicator patterns (filled dot, hollow circle, observed pill) for the same freshness concept; (c) hero metric type-size drift — TTF/Carbon numbers visually larger than rest, possibly a token-application slip; (d) PeakForecast has no viz where every other card has chart/bar/scale; (e) SpreadCapture/PeakTrough has TWO charts stacked while every other card has one or zero; (f) footer affordance gap — TTF/Carbon offer "▸ View [thing] detail" drilldown links, others don't; (g) card heights misalign within rows; (h) color usage incoherent across cards (green progress / teal sparklines / multi-color scale gradient / triangles). **Tight scope:** pick canonical template per CLAUDE.md card anatomy ("header → hero metric → status → interpretation → viz → impact line → source footer → drawer"), apply to all 6, drop the second template, single status pattern, hero type token-aligned, single chart-or-no-chart rule, one footer affordance. **Wide scope:** same plus extract a `<MetricCard>` primitive so all 6 share the same JSX skeleton, configurable via props (closes part of the Phase 7.7g-b primitives gap). **Discipline rule #1 caveat:** visual-inference audit. TTF/Carbon may legitimately deserve a distinct subtype (commodity overlays vs market-state cards have different content); design call to make at Pause A. **Sequence note:** 31.A.1 (rule-#6 sweep) is independent of 31; could ship before or after. 31.A.2 (worker-only) is independent of both.

- **/api/dispatch 404 worker fix** (NEW 2026-05-13 from 31.A DOM probe; ~30 min worker debug) — Two console errors surfaced during Phase 31.A's playwright DOM probe: `/api/dispatch?dur=2h&mode=forecast` and `/api/dispatch?dur=4h&mode=forecast` both return 404 from the worker. Pre-existing (not 31.A-introduced; not visible in card data because of error boundaries) but worth scoping. Either the route exists under a different path and the frontend is mis-routing, or the route was deleted and frontend consumers weren't cleaned up. Code-grep starting points: frontend `useDispatch` hook or equivalent + worker `/dispatch` handler.
...
- **Phase 4G.2 candidate** [NEW 2026-05-08] — entity verification at `POST /curate` per discipline rule #3 (named-entity verification) (~1-2h CC, worker-side). Discipline rule #3 says: no named entity in published content without verifiable source URL traceable to a commercial registry (`registrucentras.lt`, Lursoft, `inforegister.ee`), press release with specific path, or regulator decision URL. Phase 4G.1 closed the encoding leak path; 4G.2 closes the entity-hallucination leak path that originated Phase 12.10.0's Saulėtas Pasaulis purge. Implementation: `validateCurationEntity(body)` runs after encoding gate; checks `source_url` host against allowed-list of commercial registries / regulators / Baltic press; if entity-named claim (heuristic: title contains capitalized proper-noun company-pattern like "UAB X" or "AS Y") AND source_url isn't from allowed-list, return 400 with "entity verification failed — provide commercial registry / regulator / press source URL." Reject-on-first-ship; auto-suggest URLs as future enhancement. Operator picks when ready.
...
- **Three small follow-on phases queued post-7.7g-a-2** (operator-surfaced 2026-05-06 audit "mobile is still off, charts aren't professional, lots of old requests pending"):
  ...
  5. **Phase 19 — A11y MVP** (~3-4h CC, NEW 2026-05-06 from spec) — `:focus-visible` ring 2px `--accent`; aria-labels on all 15 SVG charts; role=status on freshness chips; aria-disabled on inert toggles; cream-on-cream form contrast fix. **Spec:** P1-3 (focus-visible) + P2-2 (chart aria) + audit-v2 P5.
