...
- **Next CC pick — Phase 7.7g-a-3 (operator-confirmed 2026-05-07):** typography rationalization + spacing residuals + per-side CI extension. Prompt at `docs/phases/phase-7-7g-a-3-prompt.md`. ~1-2 days CC. Single PR (or split into 7.7g-a-3a + 7.7g-a-3b per Pause A judgment). Addresses operator-surfaced "fonts all over the place, some old some new" concern from earlier sessions. 7 sub-items: (a) 9-step modular type scale per spec CC-1, (b) drop ad-hoc font sizes, (c) Unbounded → wordmark only per spec P3-1 (17 consumer sites migrate to --font-serif or --font-mono), (d) Plex + Newsreader font subsetting (target ≤4 fonts, <120 KB), (e) shorthand spacing migration (~131 sites), (f) off-scale + missed-pages spacing residuals, (g) per-side CI gate extension (paddingLeft/marginInline/etc.).
