...
- ~~**Phases 48-52 — the hardening arc**~~ **[SHIPPED 2026-08-03/04]** — overnight unsupervised run (10 items, PRs #141-#150) plus supervised follow-through (#152-#162). The through-line: **defects that produce a plausible number rather than an error, and gates that pass because they are broken.**
  ...
  **Open with published consequences: B-076 (P1)** — `s2_activation` frozen **105 days**, feeding `deriveCompression`, so the compression trajectory under the revenue projection has been static since April. Direction established (published projections **understated** — revenue/IRR/NPV low, LCOS high); magnitude deliberately NOT given, because the available comparison sits on an unreconciled ~3× basis difference. Also open: B-069, B-072 follow-through, the seven signed read-path removals, `/digest` write-on-GET.
