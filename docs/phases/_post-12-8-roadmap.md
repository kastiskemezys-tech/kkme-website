...
- **In flight:** none — operator at clean main. Phase 18.1 SHIPPED 2026-05-06 on `phase-18-1-mobile-foundation` (3 commits `1d412b2` / `04949fd` / `2423c3a`; viewport meta + hero collapse <900px + KPI ticker reflow + 8-site touch targets 44×44 + smooth scroll; vitest 925 baseline-exact; 5-viewport scrollWidth=clientWidth all PASS; +125 lines globals.css). **Operator-flagged regression on hard-refresh:** desktop dotted-grid map compressed illegibly on mobile + off-map context city labels (eastern Baltic neighbors) became invisible after `overflow: hidden` SVG fix. Phase 18.1.1 fixes both via direction A — simplified mobile-only SVG variant.
- **Next CC pick — Phase 18.1.1 (operator-confirmed 2026-05-06):** mobile map redesign (direction A) + per-component reflow + band-aid removal. Prompt at `docs/phases/phase-18-1-1-prompt.md`. Leapfrogs Phase 18.2/3 because operator-visible regression on mobile.
- **Then operator chooses among:**
  1. **Phase 18.2** (~2-3h CC) — chart polish; spec P2-2
  2. **Phase 18.3** (~2-3h CC) — animation activation; spec P2-1/3/4
  3. **Phase 12.12 #1+#2** (~2-3 days CC) — schema validation + freshness gates
  4. **Phase 7.7g-a-3** (~1-2 days CC) — spacing residuals + type-scale rationalization; spec CC-1 + P3-1
  5. **Phase 7.7g-b reduced** (~3-4 days CC) — primitives + `useScenario()` hook + worker URL centralization; spec CC-2
  6. **Phase 19** (~3-4h CC) — A11y MVP; spec P1-3 + P2-2
  7. **Phase 20** (~1 day CC) — static IA pages + 4-column footer; spec CC-3
...
#### Phase 18.1.1 — Mobile map redesign + per-component reflow [NEW 2026-05-06] (~3-4h)

**Why:** Phase 18.1 mobile foundation pass shipped 2026-05-06 successfully but operator hard-refresh feedback was "map doesn't work on mobile, might need to think of something better." Operator confirmed direction A (simplified mobile SVG variant) at decision point. Phase 18.1.1 also closes the per-component reflow follow-ups deferred from 18.1 Pause B and removes the `html { overflow-x: hidden }` band-aid.

**Scope (~3-4h, single PR, three pause points):**

5 sub-items, frontend-only:

1. **Mobile-simplified SVG variant** (direction A) — below 900px, swap the dotted-grid Baltic map (`HeroBalticMap.tsx`) for a clean country-outline SVG + colored arrow-lines for interconnectors with MW labels. Keep desktop map untouched. Goal: preserve brand visual identity at appropriate density for mobile fold. Uses different viewBox so off-map eastern context labels question dissolves (no labels in scope).

2. **DSCR triple panel reflow** (`RevenueCard.tsx:1069-1115`) — `min-width: 0` + label-wrap pass; ~5 lines.

3. **RTE assumption rows reflow** — per Phase 18.1 Pause B handover; ~10 lines.

4. **cycles_breakdown 4-col mini-bar reflow** — per Phase 18.1 Pause B handover; ~5 lines.
...
**Pause B verification gate:** with sub-items 2-5 in place, attempt to remove `html { overflow-x: hidden }` band-aid from `globals.css`. If 5-viewport scrollWidth=clientWidth still passes, drop the band-aid. If any viewport regresses, file the offending site as 18.1.2 follow-up and keep band-aid in place.

**Pause points:** Pause A (verify each remaining overflow site survived 18.1 + confirm direction A SVG approach), Pause B (foundation gates + band-aid removal verification), Pause C (visual + commits).

**Sequencing:** ships AFTER Phase 18.1 merges (already shipped 2026-05-06).

**Estimate:** ~3-4h CC. Frontend-only, no worker deploy.

**Prompt:** `docs/phases/phase-18-1-1-prompt.md`.

**Discipline rules engaged:** #1 audit-triage (verify each remaining overflow site at Pause A; Phase 18.1's band-aid may have masked some that don't actually exist), #5 roadmap edit-conflict (Cowork-applied delta only; CC reports needed changes via handover).

**Acceptance:** mobile users at 360-414px see a legible Baltic-flexibility map (country outlines + flow arrows + MW labels); desktop unchanged; per-component reflows resolve sub-card overflows; `html { overflow-x: hidden }` band-aid removed if possible.
