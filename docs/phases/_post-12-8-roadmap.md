...
- **In flight:** none — operator at clean main. Phase 18.1.1 SHIPPED 2026-05-06 on `phase-18-1-1-mobile-map-redesign` (3 commits `3d8b48e` / `1839328` / `fe346c7`; mobile-simplified SVG variant <900px direction A + 5 per-component reflows + BOTH `html` and `body` overflow-x: hidden band-aids removed; 5-viewport scrollWidth=clientWidth verified clean; vitest 925 baseline-exact; +1 lint warning acceptable per Phase 18.1 transition baseline; +1,049 B CSS chunk).
- **Next CC pick — operator chooses among:**
  1. **Phase 18.1.2** (~30-60min Cowork OR CC) — mobile map threshold tuning if 4-amber/2-brick split visually noisy on hard-refresh (one-line edit to `MOBILE_HIGH_FLOW_THRESHOLD_MW` constant; e.g., 500 → 600 to put only Fenno-Skan 2 in brick). Trivial.
