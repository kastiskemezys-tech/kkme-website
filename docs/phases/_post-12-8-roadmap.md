...
- **Phase 4G.1 candidate (NEW 2026-05-07):** `POST /curate` ingest validation gate (~30-60 min CC, worker-side). Encoding-validation (detect cp1257-misdecoded-UTF-8 patterns; auto-correct or reject) + optional entity verification. Origin: pattern of two `cur_*` deletions for content quality (Phase 12.10.0 hallucinated Saulėtas Pasaulis entity + Phase 18.1.3-era mojibake "plÄŠtoti" item `cur_motnhb6l-o8mv3k` deleted 2026-05-07). Memory: `project_curate_ingest_quality_pattern.md`. Operator picks when ready. If a 3rd `cur_*` deletion happens, escalate priority.
