...
- **Phase 18.2.2 — Pipeline-bar background colors + on-colored-bg label contrast** (NEW 2026-05-18 from 18.2.1 OOS pick #1; ~1-2h CC) — Phase 18.2.1 left 4 axe violations on text-rendered ATOP colored pipeline-bar backgrounds: `text-primary` on `#298672` mint chip (3.61 light / 3.02 dark) + `text-primary` on `#947434` amber chip (3.65 light / 2.79 dark) in S4Card hero pipeline bar. Solve at bar-bg-color layer (darken the strong-color background hex to lift contrast against white-ish text) OR swap label text to a darker token on those specific chips. Operator design call at Pause A. Cross-card consistency: any other on-colored-bg label sites (HeroBalticMap pipeline rows, RevenueCard accent chips, intel feed chip backgrounds) should be audited at Pause A for the same pattern. Closes axe-core baseline from 10 → 6 (4 doc'd exceptions + 2 deferred-Financing).

- **Phase 19.2.3 — Parent-opacity residual fix (faded-teal Financing arrow)** (NEW 2026-05-18 from 18.2.1 Pause A deferral; ~30-60 min CC) — 2 axe violations on the S4Card "↓ Financing" arrow at light ratio 1.58 (composited fg `#a1c3b8` on `#edeae5`) / dark ratio 1.86 (composited fg `#0a4944` on `#0f0f12`). Mechanism: parent element opacity (not inline opacity) compounds with `--teal` token alpha. Phase 19.2.1's inline-opacity audit didn't capture this pattern; Phase 18.2.1 (accent-token-only) couldn't either. Component-level fix: refactor the wrapping `style={{ opacity: X }}` to a darker token consumed at full opacity, OR remove the parent opacity and apply opacity per-child instead. Closes axe-core baseline from 6 → 4 (4 doc'd exceptions).
