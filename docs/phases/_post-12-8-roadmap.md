...
- **Worker 404-as-empty status code rationalization** (NEW 2026-05-17 from /api/dispatch Pause A inventory; ~1-1.5h CC, worker-only) — Phase /api/dispatch Pause A discovery: 12 sibling worker routes return HTTP 404 when the actual condition is "KV data not yet computed / populated" rather than "endpoint not found". Inventoried sites in `workers/fetch-s1.js`: line 6901 (feed_index empty), 7052 (fleet data), 7109 (capacity/s1 history), 7137 (activation data), 8760+8764 (`/api/dispatch?mode=realised`), 8829+8831+8840+8842 (`/api/trading/*`), 9124 (capture not yet computed), 9232 (not yet populated). Apply same 200-with-null-envelope pattern Phase /api/dispatch shipped on the forecast route: `return jsonResp({ <fieldname>: null, reason: '...' }, 200);`. Frontend consumers likely already handle null payloads via similar `?.meta` / `??` guards (per Pause A's spot-check on TradingEngineCard) — but each route's caller should be grep-confirmed at Pause A before applying the fix. Genuinely "not found" 404s (line 6908 feed entry with specific id; line 8821 `/api/trading?date=<user-supplied>` for an unknown date) STAY as 404 — those are correct semantics. Cross-card consistency rule #4: data-not-ready empty-state should look the same wire-shape everywhere. Reduces browser console noise + downstream observability false positives + axe-core noise.
...
- **Phase 19.2 — WCAG 2.1 AA color-contrast pass** (NEW 2026-05-17 from Phase 19 axe-core scan; ~2-3h Cowork + CC) — Phase 19's axe-core scan surfaced 7 color-contrast violations that pre-date Phase 19 (verified via git-stash diff). Mostly text-on-light-bg pairs in light mode (token-level issue). Per `feedback_pr_workflow_minimal.md` style: this is brand-discipline work touching `app/globals.css` tokens. Inventory pass: which tokens fail WCAG AA (4.5:1 body / 3:1 large text)? Pick: adjust foreground tokens, adjust backgrounds, or annotate exceptions with operator approval. Risk: token shifts can cascade across surfaces; need visual audit at multiple viewports. Independent of Phase 19.1.
