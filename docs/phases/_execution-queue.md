...
- **38.4 — the arbitrage reconciliation (NEW, signed 2026-08-03; blocks the cutover).** 38.3 measured the cutover and parked it: it moves project IRR **+0.07 to +0.15 pp** (median +0.14 pp), nothing at all in a third of the matrix, and it would publish 222 EFC/yr into a wear model that clamps at 1.0 c/d. Both candidate implementations were rejected — (i) wear-only buys 0.14 pp by re-opening the wear/revenue throughput split 36.B5 just closed; (ii) adopting the hourly dispatch wholesale is picking a number, not a model. **The blocking fact: the shipped engine books arbitrage at 27 % of Y1 gross, the hourly run at 10.7 % — two internal representations of the same asset disagreeing by ~2.5× on its largest merchant line, and both "working".** Neither can be adopted until that is reconciled, and the direction a choice moves the headline is the worst possible basis for making it.
  - **Q1 — is the hourly engine's reserve commitment physical or over-conservative?** It leaves ~27.5 % of nameplate free to trade (cross-checked: DA achieves 28.9 % of anchor — two independent ratios agreeing). B1 measured simultaneity at **75.2-85.5 %** of the unconstrained stack, and that is the independent check that distinguishes the two readings: a commitment rule that is physically right should reconcile with the simultaneity measurement, one that is merely conservative should not.
  - **Q2 — the degradation model gets characterised below 1.0 c/d, or its validity floor becomes an explicit published limit.** 38.3 shipped the limit as drawer disclosure; that is a disclosure, not a fix. A wear model valid only at ≥1.0 c/d cannot price a 0.6 c/d asset regardless of which dispatch wins, so this is a precondition for the cutover, not a follow-up to it.
  - Carries the three routed card defects (`capture_eur_mwh` theoretical spread on losing days, SoC resetting daily, `annual_eur = daily × 365`), which 38.3 did not touch.
...
### 2.97 · Phase 38.4 — dispatch reconciliation (the question that must precede any cutover)
**The two internal representations of the same asset disagree by ~2.5× on arbitrage share: the shipped engine books DA at 27 % of gross, the hourly run at 10.7 %.** Both are ours; both are "working". Scope: (1) reconcile that gap against **B1's measured simultaneity (75.2-85.5 % of unconstrained stack)** — if reserve commitment in the hourly dispatch leaves only ~25 % of nameplate free to trade, that is either the physical truth or an over-conservative commitment rule, and B1's number is the independent evidence that can tell them apart; (2) decide whether the degradation model gets characterised BELOW 1.0 c/d or whether its validity floor becomes an explicit published limit (a wear model valid only ≥1.0 c/d cannot price a 0.6 c/d asset, whichever dispatch wins); (3) only then re-open the cutover, with the branch chosen on evidence rather than on which direction the headline moves. **Do not adopt either representation before this** — picking one now is picking a number, not a model.
...
- **AST replied 2026-08-03** (Ēriks Biņkovskis, IT Project Manager, IS Development Dept; Litgrid + Elering copied). **Policy: none** — the API is open, its documentation public, no User-Agent or registration expectation, and 4-hourly polling of a rolling 9-day window "does not raise any concerns". **SSL: confirmed on their side** — Cloudflare implementation and configuration work in progress, and *intermittent SSL issues may occur during the transition*. **Operational consequence: a future worker-direct 526 is EXPECTED, not a new incident** — do not re-investigate it as one; keep VPS-primary / worker-secondary redundancy and the cert tripwire armed regardless of the path looking healthy. Follow-up sent asking (a) whether a direction (up/down) split exists or is planned — that would convert B-036's transferred split from German data into a measurement and collapse the €3,025-5,714/MW/yr band, and (b) where BTD's settled-vs-provisional boundary sits, which would let the trailing-edge exclusion window be set rather than inferred.
