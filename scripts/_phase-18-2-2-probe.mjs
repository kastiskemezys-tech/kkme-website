#!/usr/bin/env node
// Phase 18.2.2 — Pause A audit + Pause B verify probe.
// 6-config (dark/light × 1440/1024/414) axe-core color-contrast scan + screenshots.
// Cloned from scripts/_phase-18-2-1-probe.mjs; only OUT_VIS / OUT_DAT differ.
//
// Usage:
//   node scripts/_phase-18-2-2-probe.mjs            # audit-only (no screenshots)
//   SHOTS=1 node scripts/_phase-18-2-2-probe.mjs    # full Pause B run (6 PNGs)
import { chromium } from 'playwright';
import { mkdirSync, writeFileSync } from 'node:fs';

const URL = process.env.URL || 'http://localhost:3100';
const OUT_VIS = 'docs/visual-audit/phase-18-2-2';
const OUT_DAT = '/tmp/axe-phase18-2-2';
