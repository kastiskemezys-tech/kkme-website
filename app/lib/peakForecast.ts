...
//
// Phase 31.A: the prior `computePeakTrough` heuristic (resolution-detection
// via adjacent-bar variance) misclassified 96-entry 15-min ISP arrays as
// 24-entry hourly when adjacent variance exceeded €2 (normal Baltic
// volatility). The frontend now reads worker-emitted peak/trough fields
// derived from the same array as `lt_daily_swing_eur_mwh`.
